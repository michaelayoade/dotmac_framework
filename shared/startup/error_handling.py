...
class StartupResult:
    ...
    def has_critical_errors(self) -> bool:
        ...
        return any(
            error.severity == StartupErrorSeverity.CRITICAL 
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                return StartupResult(
                    success=True,
                    metadata={"attempts": attempt + 1, "result": result}
                )
    ...
    def should_continue_startup(self) -> bool:
        ...
        if self.environment == "production":
            high_severity_count = sum(
                1 for e in self.startup_errors 
...
def create_startup_manager(service_name: str, environment: str = None) -> StartupManager:
