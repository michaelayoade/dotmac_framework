...
"""
        ...
        if sys.prefix == sys.base_prefix:
            ...
            if not venv_path.exists():
                ...
                if self.run_command([sys.executable, "-m", "venv", "venv"]):
                    ...
                    self.log_warning("Please activate it with: source venv/bin/activate (Linux/Mac) or venv\\Scripts\\activate (Windows)")
                else:
    ...
    def setup_docker_environment(self):
        ...
        # Build test images
        if self.run_command(["docker", "build", "-f", "Dockerfile.test", "-t", "dotmac-framework:test", "."]):
    ...
    def initialize_databases(self):
        ...
        if self.run_command([
            ...
            "up", "-d", "postgres-test", "redis-test", "timescaledb-test"
        ]):
        ...
        if self.run_command([
            "docker-compose", "-f", "docker-compose.test.yml",
            "exec", "-T", "postgres-test", "psql", "-U", "dotmac_test", "-d", "dotmac_test",
            "-c", "SELECT 1;"
        ]):
