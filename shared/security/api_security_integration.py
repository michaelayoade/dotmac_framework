...
try:
    ...
    from .api_auth_middleware import JWTTokenValidator, RoleBasedAccessControl, create_api_auth_middleware
    from .api_security_headers import setup_api_security
    from .request_validation import RequestValidationMiddleware, create_request_validation_middleware
    from .api_threat_detector import APIThreatDetector, create_threat_detection_middleware
except ImportError:
    ...
    from security.api_auth_middleware import JWTTokenValidator, RoleBasedAccessControl, create_api_auth_middleware
    from security.api_security_headers import setup_api_security
    from security.request_validation import RequestValidationMiddleware, create_request_validation_middleware
    from security.api_threat_detector import APIThreatDetector, create_threat_detection_middleware
...
class APISecuritySuite:
        ...
        try:
            ...
            if self.jwt_validator and self.rbac and self.security_config["authentication"]["enabled"]:
                auth_middleware = create_api_auth_middleware(
                    ...
                    rbac=self.rbac,
                    require_auth=True,
                    exempt_paths=['/docs', '/redoc', '/openapi.json', '/health', '/api/auth/login']
                )
    ...
    async def get_security_health_report(self) -> Dict[str, Any]:
        ...
        if self.rate_limiter:
            try:
                health_report["components_status"]["rate_limiter"] = "healthy" if await self.rate_limiter.redis.ping() else "unhealthy"
            except:
