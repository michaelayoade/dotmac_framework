...
class RedisRateLimiter:
    ...
    def __init__(
        ...
        self.tenant_quotas = {
            'basic': TenantQuota(
                tenant_id='', tier='basic',
                daily_requests=10000, hourly_requests=1000,
                minute_requests=50, concurrent_connections=10,
                burst_allowance=100, priority_weight=1.0
            ),
            'premium': TenantQuota(
                tenant_id='', tier='premium', 
                daily_requests=100000, hourly_requests=10000,
                minute_requests=200, concurrent_connections=50,
                burst_allowance=500, priority_weight=2.0
            ),
            'enterprise': TenantQuota(
                tenant_id='', tier='enterprise',
                daily_requests=1000000, hourly_requests=100000,
                minute_requests=1000, concurrent_connections=200,
                burst_allowance=2000, priority_weight=3.0
            )
        }
    ...
    async def record_metrics(self, result: RateLimitResult, limit_type: RateLimitType, identifier: str):
        ...
        try:
            ...
            if not result.allowed:
                ...
                pipe.hincrby(metrics_key, "quota_exceeded" if result.quota_exceeded else "rate_limited", 1)
                ...
                if checks:
                    ...
                    if failed_check:
                        ...
                        await self.rate_limiter.record_metrics(
                            failed_check, RateLimitType.PER_TENANT, tenant_id or client_ip
                        )
