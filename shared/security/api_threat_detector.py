...
class BruteForceDetector:
    ...
    async def track_failed_login(self, ip_address: str, user_id: Optional[str] = None) -> bool:
        ...
        if recent_ip_attempts >= self.failed_attempts_threshold or recent_user_attempts >= self.failed_attempts_threshold:
            # Lock the IP/user
    ...
    async def check_geographic_anomaly(self, user_id: str, ip_address: str) -> Tuple[bool, str]:
        ...
        if previous_data:
            ...
            if prev_country != current_country and time_diff < 3600:  # Country change in < 1 hour
                return True, f"Impossible travel: {prev_country} to {current_country} in {time_diff/60:.1f} minutes"
...
class APIThreatDetector:
    ...
    def __init__(self, redis_client: Optional[redis.Redis] = None, redis_url: str = "redis://localhost:6379/2"):
        ...
        self.threat_patterns = [
            ThreatPattern("sql_injection", 1, 3600, ThreatType.SQL_INJECTION, ThreatLevel.HIGH, "block"),
            ThreatPattern("xss_attempt", 1, 3600, ThreatType.XSS_ATTEMPT, ThreatLevel.HIGH, "block"),
            ThreatPattern("path_traversal", 1, 3600, ThreatType.PATH_TRAVERSAL, ThreatLevel.HIGH, "block"),
            ThreatPattern("brute_force", 5, 900, ThreatType.BRUTE_FORCE, ThreatLevel.CRITICAL, "block"),
            ThreatPattern("api_scraping", 100, 300, ThreatType.API_SCRAPING, ThreatLevel.MEDIUM, "log"),
        ]
    ...
    async def analyze_request(self, request: Request, user_id: Optional[str] = None, tenant_id: Optional[str] = None) -> List[SecurityEvent]:
    ...
    async def analyze_response(self, request: Request, response: Response, user_id: Optional[str] = None) -> List[SecurityEvent]:
    ...
    def _detect_malicious_patterns(self, url: str, headers: Dict[str, str], user_agent: str) -> List[Tuple[ThreatType, Dict[str, str]]]:
        ...
        for pattern in sql_patterns:
            ...
            if re.search(pattern, combined_text, re.IGNORECASE):
                threats.append((ThreatType.SQL_INJECTION, {"pattern": pattern, "text": combined_text[:200]}))
                break
        ...
        for pattern in xss_patterns:
            import re
            if re.search(pattern, combined_text, re.IGNORECASE):
                threats.append((ThreatType.XSS_ATTEMPT, {"pattern": pattern, "text": combined_text[:200]}))
                break
    ...
    async def get_threat_summary(self) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                ...
                "active_detectors": ["brute_force", "anomaly", "geographic", "pattern_matching"],
...
class APIThreatDetectionMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            user_id = getattr(request.state, 'auth_user', {}).get('user_id') if hasattr(request.state, 'auth_user') else None
...
# Factory functions
def create_threat_detector(redis_url: str = "redis://localhost:6379/2") -> APIThreatDetector:
