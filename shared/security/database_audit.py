...
from sqlalchemy import Column, String, Text, DateTime, Boolean, Integer, JSON, Index, event
...
class DatabaseAuditLogger:
        ...
        try:
            with self.engine.begin() as conn:
                ...
                if event_data and 'statement_preview' in event_data:
                    import hashlib
                    query_hash = hashlib.md5(event_data['statement_preview'].encode()).hexdigest()[:32]
    ...
    async def log_cross_tenant_attempt(
        ...
        return await self.log_event(
            ...
            success=False,
            compliance_tags=['security_incident', 'tenant_isolation', 'access_violation'],
            risk_level="critical"
        )
