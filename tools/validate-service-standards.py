...
class ServiceStandardsValidator:
    ...
    def generate_report(self) -> str:
        ...
        sorted_issues = sorted(
            self.issues, key=lambda x: (x.severity, x.file_path, x.line_number)
        )
        ...
        # Migration guide
        if any(
            i.rule in ["deprecated_pattern", "incorrect_inheritance"]
...
def main():
    ...
    parser.add_argument(
        "--standards", default=".service-standards.json", help="Standards file"
    )
    parser.add_argument(
        "--fail-on-warnings", action="store_true", help="Fail on warnings too"
    )
