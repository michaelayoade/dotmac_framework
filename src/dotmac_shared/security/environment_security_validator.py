...
from .secrets_policy import Environment, HardenedSecretsManager, SecretType, SecretsEnvironmentError
from .unified_csrf_strategy import CSRFMode, CSRFConfig
...
class EnvironmentSecurityValidator:
    ...
    async def _validate_secrets_management(
        self, 
        secrets_manager: Optional[HardenedSecretsManager]
    ) -> Dict[str, List]:
            ...
            try:
                ...
                if not compliance["compliant"]:
                    for violation_msg in compliance["violations"]:
                        severity = SecuritySeverity.CRITICAL if requirements["vault_required"] else SecuritySeverity.HIGH
                        violations.append(SecurityViolation(
                            severity=severity,
        ...
        for check_name, check_config in additional_checks.items():
            try:
                ...
                violations.append(SecurityViolation(
                    ...
                    category=f"additional_check_error",
