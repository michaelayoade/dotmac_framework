...
class UnifiedAuditMonitor(StatefulService):
    ...
    async def _initialize_stateful_service(self) -> bool:
        ...
        try:
            ...
            tenant_id = None
            if (self.audit_config.deployment_context and 
                hasattr(self.audit_config.deployment_context, 'tenant_id')):
    ...
    async def get_security_dashboard(
        ...
        filtered_events = [
            ...
            if (event.timestamp >= period_start and
        ...
        dashboard = {
            ...
            "summary": {
                ...
                "unauthorized_access": len([e for e in filtered_events if e.event_type == SecurityEventType.ACCESS_DENIED]),
                "active_alerts": len([a for a in self._security_alerts if a.status == "open"]),
            },
            "by_threat_level": {},
            "by_event_type": {},
        ...
        # Recent alerts
        recent_alerts = [
            alert for alert in self._security_alerts
    ...
    async def get_active_alerts(
        ...
        alerts = [
            alert for alert in self._security_alerts
            if (alert.status == "open" and
                (not threat_level or alert.threat_level == threat_level) and
                (not tenant_id or alert.tenant_id == tenant_id))
        ]
    ...
    async def _generate_security_alert(
        ...
        alert.source_ips = list(set(e.source_ip for e in events))
        alert.users_affected = list(set(e.username for e in events if e.username))
        alert.affected_resources = list(set(e.resource_id for e in events if e.resource_id))
