...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    def _build_csp_policy(self, request: Request) -> str:
        ...
        if cors_origins:
            ...
            policy_parts = [p if not p.startswith('connect-src') else f"connect-src {' '.join(connect_sources)}" for p in policy_parts]
        
        # In development, be more permissive
        if not self.is_production:
            # Allow localhost and development servers
            dev_sources = ['http://localhost:*', 'http://127.0.0.1:*', 'ws://localhost:*']
            policy_parts = [
                p.replace('\'self\'', '\'self\' ' + ' '.join(dev_sources)) 
                if 'connect-src' in p else p 
