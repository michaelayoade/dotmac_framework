...
from dotmac_shared.application.config import PlatformConfig, DeploymentContext, DeploymentMode
from dotmac_shared.security.tenant_security_enforcer import TenantSecurityEnforcer
from dotmac_shared.middleware.api_versioning import APIVersioningMiddleware, APIVersionInfo, VersionStatus
from dotmac_shared.middleware.background_operations import BackgroundOperationsMiddleware, BackgroundOperationsManager
...
def create_enhanced_app() -> FastAPI:
    ...
    print(f"Applied middleware: {applied_middleware}")
...
async def provision_service_operation(parameters: dict) -> dict:
    ...
    print(f"Provisioning service: {service_id} ({service_type})")
...
async def compensate_create_customer(parameters: dict) -> dict:
    ...
    print(f"Compensating customer creation: {customer_id}")
...
async def compensate_provision_service(parameters: dict) -> dict:
    ...
    print(f"Compensating service provisioning: {service_id}")
...
if __name__ == "__main__":
    ...
    print()
    print("# Health check (no tenant required)")
    print("curl http://localhost:8000/api/v1/health")
    print()
    print("# Get tenant info (requires X-Tenant-ID header)")
    print("curl -H 'X-Tenant-ID: example-tenant-123' http://localhost:8000/api/v1/tenant-info")
    print()
    print("# Create customer with idempotency")
    print("curl -X POST -H 'X-Tenant-ID: example-tenant-123' -H 'Idempotency-Key: unique-key-1' \\")
    print("     -H 'Content-Type: application/json' \\")
    print("     -d '{\"name\": \"John Doe\", \"email\": \"john@example.com\"}' \\")
    print("     http://localhost:8000/api/v1/customers")
    print()
    print("# Start saga workflow")
    print("curl -X POST -H 'X-Tenant-ID: example-tenant-123' \\")
    print("     -H 'Content-Type: application/json' \\")
    print("     -d '{\"customer\": {\"name\": \"Jane Doe\"}, \"service_type\": \"premium\"}' \\")
    print("     http://localhost:8000/api/v1/workflows/customer-onboarding")