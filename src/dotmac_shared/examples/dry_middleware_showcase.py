...
from ..utils.datetime_utils import utc_now, TimeDeltas
from ..middleware.factory import MiddlewareResult
from ..middleware.headers import ExtractedHeaders, extract_headers
from ..middleware.state import RequestStateManager, get_tenant_id, get_api_version
from ..middleware.response import create_success_response, create_error_response
...
class CustomBusinessLogicMiddleware(TenantAwareMiddleware):
    ...
    def __init__(self):
        config = MiddlewareConfig(
            name="BusinessLogic",
            exempt_paths={"/docs", "/health", "/api/public/*"}
        )
...
def add_example_routes(app: FastAPI):
    ...
    async def get_tenant_info(request: Request):
        ...
        return create_success_response({
            ...
            "rate_limit_tier": getattr(state.operation_context, "rate_limit_tier", None) if state.operation_context else None
        }, request)
...
def demonstrate_dry_benefits():
    ...
    print("🎯 DRY Middleware Benefits Demonstration")
    print("=" * 50)
    
    # Code reduction
    print("\n📊 Code Reduction:")
    print(f"Original middleware implementations: ~1,300 lines")
    print(f"DRY implementations: ~1,030 lines") 
    print(f"Reduction: 270 lines (20% less code)")
    print(f"Shared components: 800 lines (one-time investment)")
...
if __name__ == "__main__":
    ...
    print(f"Run with: uvicorn {__name__}:app --reload")