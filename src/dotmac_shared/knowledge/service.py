...
class KnowledgeService:
    ...
    def __init__(self, db_session_factory=None, config: Optional[dict[str, Any]] = None):
        ...
        self.search_config = {
            ...
            "search_boost_factors": {"title": 3.0, "summary": 2.0, "content": 1.0, "tags": 1.5},
        }

    async def create_article(
        self, db: AsyncSession, tenant_id: str, article_data: ArticleCreate, author_id: str, author_name: str
    ) -> ArticleResponse:
    ...
    async def get_article(
        self, db: AsyncSession, tenant_id: str, article_id: str, increment_view: bool = True
    ) -> Optional[ArticleResponse]:
        """Get article by ID."""
        query = (
            select(KnowledgeArticle)
            .where(and_(KnowledgeArticle.id == article_id, KnowledgeArticle.tenant_id == tenant_id))
    ...
    async def get_article_by_slug(
        ...
        query = (
            select(KnowledgeArticle)
            .where(and_(KnowledgeArticle.slug == slug, KnowledgeArticle.tenant_id == tenant_id))
            .options(selectinload(KnowledgeArticle.comments))
        )
    ...
    async def vote_on_article(
        self, db: AsyncSession, tenant_id: str, article_id: str, is_helpful: bool, user_id: Optional[str] = None
    ) -> bool:
    ...
    async def get_popular_articles(
        ...
        try:
            ...
            query = (
                ...
                .order_by(desc(KnowledgeArticle.view_count), desc(KnowledgeArticle.helpful_votes))
                .limit(limit)
            )
    ...
    async def get_customer_portal_settings(
        ...
        try:
            query = select(CustomerPortalSettings).where(
                and_(CustomerPortalSettings.customer_id == customer_id, CustomerPortalSettings.tenant_id == tenant_id)
            )
    ...
    async def update_portal_settings(
        self, db: AsyncSession, tenant_id: str, customer_id: str, settings_data: PortalSettingsUpdate
    ) -> PortalSettingsResponse:
    ...
    async def _get_article_for_update(self, db: AsyncSession, tenant_id: str, article_id: str):
        ...
        query = select(KnowledgeArticle).where(
            and_(KnowledgeArticle.id == article_id, KnowledgeArticle.tenant_id == tenant_id)
        )
        result = await db.execute(query)
    ...
    async def track_article_view(
        ...
        try:
            ...
            query = select(ArticleAnalytics).where(
                and_(ArticleAnalytics.article_id == article_id, func.date(ArticleAnalytics.date) == today)
            )
            ...
            if not analytics:
                analytics = ArticleAnalytics(
                    article_id=article_id, tenant_id=tenant_id, date=datetime.now(timezone.utc)
                )
                db.add(analytics)
