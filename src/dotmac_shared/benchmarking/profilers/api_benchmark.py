    ...
    async def _ensure_session(self):
        ...
        if not self._session:
            ...
            self._session = aiohttp.ClientSession(
                connector=self._connector, timeout=timeout, headers=self.default_headers
            )
...
async def quick_api_benchmark(
    ...
    Returns:
        Benchmark results
    """
    request = ApiRequest(method=method, endpoint=endpoint, headers=headers or {})
