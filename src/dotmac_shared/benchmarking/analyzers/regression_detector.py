...
class RegressionDetector:
    ...
    def detect_regression(
        ...
        return RegressionAnalysis(
            ...
            trend_data=data_points[-self.config.trend_analysis_window :]
            if len(data_points) >= self.config.trend_analysis_window
            else data_points,
        )
    ...
    def _update_baseline(self, key: str):
        ...
        baseline_points = [dp for dp in data_points if dp.timestamp <= baseline_cutoff]
    ...
    def _analyze_trend(self, data_points: list[BenchmarkDataPoint]) -> PerformanceTrend:
        ...
        recent_points = (
            data_points[-self.config.trend_analysis_window :]
            if len(data_points) >= self.config.trend_analysis_window
            else data_points
        )
    ...
    def _calculate_confidence_level(
        self, data_points: list[BenchmarkDataPoint], baseline: float, current: float
    ) -> float:
    ...
    def _generate_regression_recommendations(
        ...
        if trend == PerformanceTrend.VOLATILE:
            recommendations.append(
                "📊 High performance volatility - check for resource contention or environmental issues"
            )
