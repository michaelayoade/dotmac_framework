...
class BenchmarkComparator:
    ...
    def compare_benchmark_sets(
        ...
        degraded_count = sum(1 for d in all_diffs if not d.is_improvement and d.significance != ChangeSignificance.INSIGNIFICANT)
        unchanged_count = len(all_diffs) - improved_count - degraded_count
        
        # Calculate overall performance change
        percentage_changes = [d.percentage_change for d in all_diffs if d.significance != ChangeSignificance.INSIGNIFICANT]
    ...
    def compare_time_series(
        ...
        baseline_end = sorted_timestamps[-comparison_window_days-1] if len(sorted_timestamps) > comparison_window_days else sorted_timestamps[-2]
        baseline_start = baseline_end - timedelta(days=baseline_period_days)
        
        # Define comparison period (recent data)
        comparison_start = sorted_timestamps[-comparison_window_days] if len(sorted_timestamps) > comparison_window_days else sorted_timestamps[-1]
        ...
        if baseline_timestamps:
            for metric_name in time_series_data[baseline_timestamps[0]].keys():
                values = [time_series_data[ts].get(metric_name, 0) for ts in baseline_timestamps if metric_name in time_series_data[ts]]
                if values:
        ...
        if comparison_timestamps:
            for metric_name in time_series_data[comparison_timestamps[0]].keys():
                values = [time_series_data[ts].get(metric_name, 0) for ts in comparison_timestamps if metric_name in time_series_data[ts]]
                if values:
    ...
    def compare_environments(
        ...
        reference_environment: str = None
    ) -> List[ComparisonResult]:
    ...
    def _generate_comparison_summary(self, diffs: List[BenchmarkDiff], overall_change: float) -> str:
        ...
        return f"Performance has {trend} overall ({overall_change:+.1f}%) with {improved} improved and {degraded} degraded metrics out of {len(diffs)} compared."
    ...
    def _generate_comparison_recommendations(self, diffs: List[BenchmarkDiff], overall_change: float) -> List[str]:
        ...
        if critical_diffs:
            ...
            if critical_degraded:
                recommendations.append(f"🚨 CRITICAL: {len(critical_degraded)} metrics show severe performance degradation")
        ...
        if major_diffs:
            ...
            if major_degraded:
                recommendations.append(f"⚠️ MAJOR: {len(major_degraded)} metrics show significant performance regression")
        ...
        slow_queries = [d for d in diffs if "query" in d.metric_name.lower() and not d.is_improvement and d.significance in [ChangeSignificance.MAJOR, ChangeSignificance.CRITICAL]]
        if slow_queries:
            recommendations.append("🔍 Database query performance has degraded - review query optimizations")
        
        high_memory = [d for d in diffs if "memory" in d.metric_name.lower() and not d.is_improvement and d.significance in [ChangeSignificance.MAJOR, ChangeSignificance.CRITICAL]]
        if high_memory:
    ...
    def _generate_report_recommendations(self, comparison_results: List[ComparisonResult]) -> List[str]:
        ...
        critical_count = sum(len([d for d in r.significant_changes if d.significance == ChangeSignificance.CRITICAL]) for r in comparison_results)
        if critical_count > 0:
    ...
    def _create_empty_comparison(self, baseline_name: str, target_name: str, comparison_type: ComparisonType, reason: str) -> ComparisonResult:
