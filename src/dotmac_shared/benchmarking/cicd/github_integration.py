...
class GitHubPerformanceCI:
    ...
    async def _update_status_check(self, state: str, description: str, target_url: str = None):
...
"""
        ...
        if (pipeline_result.regression_detected and 
            self.config.mention_team_on_regression and 
            self.config.team_to_mention):
...
**Stages Completed:** {len([s for s in pipeline_result.stages if s.status.value == 'success'])}/{len(pipeline_result.stages)}  
...
"""
    ...
    async def _upload_artifacts(self, pipeline_result: PipelineResult):
        ...
        if pipeline_result.artifacts:
            print(f"Would upload {len(pipeline_result.artifacts)} artifacts:")
  ...
  performance-tests:
    ...
    - name: Run Performance Benchmarks
      ...
      run: |
        python -c "
        import asyncio
        from dotmac_shared.benchmarking.cicd import GitHubPerformanceCI, GitHubBenchmarkConfig, PipelineConfig
