...
from dotmac_shared.benchmarking.core.benchmark_manager import PerformanceBenchmarkManager, BenchmarkConfig
...
from dotmac_shared.benchmarking.profilers.database_benchmark import DatabaseQueryBenchmarker, DatabaseBenchmarkConfig
from dotmac_shared.benchmarking.analyzers.regression_detector import RegressionDetector, RegressionDetectionConfig
...
class PerformancePipelineRunner:
    ...
    def __init__(self, config: PipelineConfig):
        ...
        ) if config.enable_regression_analysis else None
        self.comparator = BenchmarkComparator(
            BenchmarkComparisonConfig()
        ) if config.enable_comparison else None
        ...
        try:
            ...
            if self.config.enable_database_benchmarks and self.config.database_url:
                ...
                if db_result.status == PipelineStatus.FAILED and self.config.fail_fast:
                    return self._create_failed_pipeline_result(pipeline_start, stages, "Database benchmarks stage failed")
            
            # Regression analysis
            if self.config.enable_regression_analysis:
                regression_result = await self._run_stage(PipelineStage.REGRESSION_ANALYSIS, self._regression_analysis_stage)
    ...
    async def _api_benchmarks_stage(self) -> Dict[str, Any]:
        ...
        with open(api_results_file, 'w') as f:
            json.dump(results.model_dump() if hasattr(results, 'model_dump') else results.__dict__, f, indent=2, default=str)
    ...
    async def _comparison_stage(self) -> Dict[str, Any]:
        ...
        return {
            "status": "completed",
            "message": "Comparison with baseline completed"
        }
    ...
    def _generate_pipeline_summary(self, stages: List[PipelineStageResult], performance_score: float, regression_detected: bool) -> str:
    ...
    def _generate_pipeline_recommendations(self, stages: List[PipelineStageResult], critical_issues: List[Dict[str, Any]]) -> List[str]:
    ...
    def _create_failed_pipeline_result(self, start_time: datetime, stages: List[PipelineStageResult], reason: str) -> PipelineResult:
