...
class TenantIsolationTester:
    ...
    async def test_data_isolation(
        self, model_class: type, tenant_contexts: list[TenantContext], test_data_per_tenant: dict[str, list[dict]]
    ...
    async def _test_tenant_query_isolation(
        self, model_class: type, tenant_context: TenantContext, custom_queries: list[tuple[str, str]]
    ) -> TenantIsolationTestResult:
    ...
    async def test_performance_isolation(
        self, model_class: type, tenant_contexts: list[TenantContext], load_per_tenant: int = 100
    ) -> list[TenantIsolationTestResult]:
    ...
    async def _measure_tenant_performance(
        self, model_class: type, tenant_context: TenantContext, operation_count: int
    ) -> float:
    ...
    async def _measure_concurrent_tenant_performance(
        self, model_class: type, tenant_contexts: list[TenantContext], operation_count: int
    ) -> dict[str, float]:
    ...
    async def test_constraint_isolation(
        self, model_class: type, tenant_contexts: list[TenantContext], constraint_test_data: dict[str, list[dict]]
    ) -> list[TenantIsolationTestResult]:
        ...
        for tenant_context in tenant_contexts:
            result = await self._test_tenant_constraint_isolation(
                model_class, tenant_context, constraint_test_data, tenant_contexts
            )
            results.append(result)
    ...
    async def _test_tenant_constraint_isolation(
        ...
        try:
            ...
            with self.SessionLocal() as session:
                ...
                if tenant_contexts and len(tenant_contexts) > 1:
                    other_tenant = next((tc for tc in tenant_contexts if tc.tenant_id != tenant_id), None)
                        ...
                        try:
                            ...
                            violations.append(
                                IsolationViolation(
                                    ...
                                    description="Constraint prevents same data across different tenants",
                                    severity="medium",
                                )
                            )
                            session.rollback()
    ...
    def get_isolation_summary(self) -> dict[str, Any]:
        ...
        for result in self.test_results:
            ...
            if test_type not in type_breakdown:
                type_breakdown[test_type] = {"total": 0, "passed": 0, "failed": 0, "violations": 0}
        ...
        for result in self.test_results:
            ...
            if tenant_id not in tenant_breakdown:
                tenant_breakdown[tenant_id] = {"total": 0, "passed": 0, "failed": 0, "violations": 0}
