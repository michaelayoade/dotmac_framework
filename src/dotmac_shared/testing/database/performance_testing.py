...
from sqlalchemy import create_engine, text, select, insert, update, delete, func
...
class DatabasePerformanceTester:
    ...
    async def run_load_test(
        ...
        # Start user sessions
        tasks = [
            user_session(i, delay) 
            for i, delay in enumerate(ramp_up_delays)
        ]
    ...
    async def run_stress_test(
        ...
        while current_users <= max_users:
            ...
            try:
                ...
                failure_rate = metrics.failed_operations / metrics.total_operations if metrics.total_operations > 0 else 0
                ...
                if metrics.percentile_95_response_time > 5.0:  # 5 second response time
                    logger.warning(f"High response time ({metrics.percentile_95_response_time:.2f}s) at {current_users} users")
                    break
            ...
            for i in range(iterations):
                ...
                try:
                    with self.SessionLocal() as session:
                        ...
                        rows = result.fetchall()  # Ensure full result retrieval
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        for test_type, data in type_breakdown.items():
