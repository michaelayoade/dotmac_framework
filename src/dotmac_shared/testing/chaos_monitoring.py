...
class MetricsCollector:
    ...
    def get_metric_history(self, name: str, since: datetime = None,
                          experiment_id: str = None) -> List[MetricPoint]:
        """Get metric history"""
    ...
    def get_current_value(self, name: str, experiment_id: str = None) -> Optional[float]:
        """Get the most recent value for a metric"""
        history = self.get_metric_history(name, experiment_id=experiment_id)
        return history[-1].value if history else None
    
    def get_metric_summary(self, name: str, since: datetime = None,
                          experiment_id: str = None) -> Dict[str, float]:
...
class AlertManager:
    ...
    def get_active_alerts(self, experiment_id: str = None) -> List[Alert]:
    ...
    async def _monitor_thresholds(self):
        ...
        while True:
            try:
                ...
                for rule_key, rule in self.threshold_rules.items():
        ...
        if condition_met:
            ...
            if alert_id not in self.active_alerts:
                alert = Alert(
                    ...
                    message=rule.alert_message or f"{rule.metric_name} {rule.condition} {rule.threshold} (current: {current_value})",
...
class ExperimentMonitor:
    ...
    def get_experiment_summary(self) -> Dict[str, Any]:
        ...
        key_metrics = [
            "error_rate", "response_time_avg", "response_time_p99", 
