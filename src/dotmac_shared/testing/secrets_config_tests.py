...
# Note: FieldEncryption from deprecated module, would need dotmac.secrets package
try:
    from dotmac.platform.secrets import FieldEncryption
except ImportError:
    # Fallback for development - remove when dotmac.secrets is available
    ...
    async def test_jwt_secret_rotation(self) -> dict[str, Any]:
        ...
        try:
            ...
            dual_validation_result = await self._test_dual_key_validation(
                initial_tokens["tokens"], initial_secret["secret"], new_secret["secret"]
            )
    ...
    async def test_cookie_secret_rotation(self) -> dict[str, Any]:
        ...
        try:
            ...
            cookie_rotation = await self._initiate_cookie_rotation(
                initial_cookie_secret["secret"], new_cookie_secret["secret"]
            )
    ...
    async def test_config_drift_detection(self) -> dict[str, Any]:
        ...
        try:
            ...
            drift_detection = await self._detect_config_drift(
                baseline_snapshot["config_items"], drift_simulation["modified_config"]
            )
        ...
        try:
            ...
            for token_info in tokens:
                ...
                try:
                    ...
                    self.jwt_service.decode_token(token_info["token"])
...
async def test_jwt_rotation_only():
    ...
    assert result["success"] is True, f"JWT rotation failed: {result}"
...
async def test_config_drift_only():
    ...
    assert result["success"] is True, f"Config drift detection failed: {result}"
