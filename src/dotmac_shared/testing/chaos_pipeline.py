...
class ChaosPipeline:
    ...
    async def _stage_preparation(self):
        ...
        self.current_run.metrics["preparation"] = {
    ...
    async def _stage_baseline(self):
        ...
        self.chaos_monitor.record_metric("pipeline_baseline_start", 1, 
                                       experiment_id=self.current_run.id)
    ...
    async def _stage_recovery(self):
        ...
        while recovery_checks < max_recovery_time / 10:
            ...
            if (current_error_rate is not None and current_error_rate <= self.config.max_error_rate and
                current_availability is not None and current_availability >= self.config.min_availability):
                break
    ...
    async def _stage_cleanup(self):
        ...
        self.current_run.metrics["cleanup"] = {
    ...
    async def _generate_pipeline_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "success_rate": (self.current_run.experiments_completed / 
                               max(1, self.current_run.total_experiments)) * 100
            },
...
class ChaosPipelineScheduler:
    ...
    def _calculate_next_run_time(self, config: PipelineConfig) -> Optional[datetime]:
        ...
        if config.schedule_type == ScheduleType.DAILY:
            next_run = now.replace(hour=config.schedule_time.hour if config.schedule_time else 2,
            ...
            next_run = next_run.replace(hour=config.schedule_time.hour if config.schedule_time else 2,
                                      minute=config.schedule_time.minute if config.schedule_time else 0,
                                      second=0, microsecond=0)
            return next_run
