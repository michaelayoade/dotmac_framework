...
class TestBasicFactoryUsage:
    ...
    def test_create_multiple_tenants(self, tenant_factory):
        ...
        tenants = tenant_factory.create_batch(count=3, plan="professional")
    ...
    def test_build_without_persistence(self, tenant_factory):
        ...
        tenant = tenant_factory.build(name="Test Build", subdomain="testbuild")
...
class TestRelationshipManagement:
    ...
    def test_customer_with_service_relationship(self, customer_factory, service_factory, basic_tenant_setup):
        ...
        customer = customer_factory.create(name="Jane Smith", email="jane@example.com")
    ...
    def test_customer_with_multiple_services(self, customer_factory, service_factory, basic_tenant_setup):
        ...
        tv = service_factory.create(customer_id=customer.id, service_type="tv")

        services = [internet, phone, tv]
        assert len(services) == 3
        assert all(s.customer_id == customer.id for s in services)

    def test_billing_relationship_chain(self, customer_with_service, billing_factory):
...
class TestPerformanceTesting:
    ...
    def test_bulk_customer_creation(self, customer_factory, basic_tenant_setup):
        ...
        customers = customer_factory.create_batch(count=50, type="residential")

        assert len(customers) == 50
        assert all(c.tenant_id == tenant_id for c in customers)
        assert len({c.email for c in customers}) == 50  # All unique emails
...
class TestCustomFactoryCreation:
    ...
    def test_custom_factory_implementation(self, factory_registry):
        ...
        # Use custom factory
        service = custom_factory.create(custom_field="specialized_value")
...
if __name__ == "__main__":
    ...
    logger.info("Test Data Factory Examples")
    logger.info("=" * 40)
    logger.info("Run with: pytest src/dotmac_shared/testing/examples.py -v")
