...
class EventTracer:
    ...
    async def trace_event(self, event_type: str, payload: Dict[str, Any], 
                         journey_id: Optional[str] = None, service_name: Optional[str] = None):
        """Trace an event"""
    ...
    async def get_events_by_journey(self, journey_id: str) -> List[Dict[str, Any]]:
        ...
        journey_events = [
            event for event in self._traced_events 
    ...
    async def wait_for_event(self, event_type: str, journey_id: Optional[str] = None, 
        ...
        while (asyncio.get_event_loop().time() - start_time) < timeout:
            matching_events = [
                ...
                if event["type"] == event_type and (
                    journey_id is None or event.get("journey_id") == journey_id
                )
            ]
    ...
    async def assert_event_sequence(self, expected_sequence: List[str], 
                                  journey_id: str, timeout: int = 30):
