        ...
        for dep_name in self._metadata.dependencies:
            if dep_name not in resolved:
                try:
                    ...
                    if hasattr(self, '_required_dependencies') and dep_name in self._required_dependencies:
                        raise DependencyError(f"Failed to resolve required dependency {dep_name}: {e}")
...
class TenantIsolatedFactory(BaseFactory):
    ...
    def __init__(self, tenant_id: Optional[str] = None, registry: Optional['FactoryRegistry'] = None):
...
class RelationshipManager:
    ...
    def establish_relationship(
        ...
        try:
            ...
            raise RelationshipError(f"Failed to establish relationship {relationship_name}: {e}")
    ...
    def _establish_one_to_one(self, definition: RelationshipDefinition, source: Any, target: Any) -> None:
        ...
        if definition.foreign_key:
            setattr(source, definition.foreign_key, getattr(target, 'id'))
    ...
    def _establish_one_to_many(self, definition: RelationshipDefinition, source: Any, target: Any) -> None:
        ...
        if definition.foreign_key:
            setattr(target, definition.foreign_key, getattr(source, 'id'))
        if definition.back_reference:
...
class FactoryRegistry:
    ...
    def _update_cleanup_order(self) -> None:
        ...
        self._cleanup_order = sorted(
            self.metadata.keys(),
            key=lambda name: self.metadata[name].cleanup_order
        )
