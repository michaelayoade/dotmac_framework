...
from .factory import MiddlewareProcessor, MiddlewareResult, MiddlewareFactory
from .headers import HeaderExtractor, ExtractedHeaders
from .response import ResponseDecorator, ResponseContext, StandardResponseDecorator
from .state import RequestStateManager, RequestState
...
class MiddlewareConfig:
    ...
    def __post_init__(self):
        if self.exempt_paths is None:
            self.exempt_paths = {"/docs", "/redoc", "/openapi.json", "/health", "/metrics"}
...
class UnifiedMiddlewareProcessor(MiddlewareProcessor, ABC):
    ...
    async def process_request(self, request: Request) -> Optional[MiddlewareResult]:
    ...
    async def process_middleware(self, 
                               request: Request, 
                               headers: Optional[ExtractedHeaders] = None) -> Optional[MiddlewareResult]:
    ...
    def get_response_headers(self, request: Request, result: Optional[MiddlewareResult] = None) -> Dict[str, str]:
    ...
    def get_custom_response_headers(self, request: Request, result: Optional[MiddlewareResult] = None) -> Dict[str, str]:
    ...
    def _update_request_state(self, request: Request, headers: Optional[ExtractedHeaders]):
...
class SimpleUnifiedMiddleware(UnifiedMiddlewareProcessor):
    ...
    def __init__(self, 
                 name: str,
                 process_fn: Callable[[Request, Optional[ExtractedHeaders]], Optional[MiddlewareResult]],
...
class TenantAwareMiddleware(UnifiedMiddlewareProcessor):
    ...
    @abstractmethod
    async def process_tenant_middleware(self, 
                                      request: Request, 
                                      headers: Optional[ExtractedHeaders] = None) -> Optional[MiddlewareResult]:
...
class CachingMiddleware(UnifiedMiddlewareProcessor):
    ...
    def __init__(self, 
                 ...
                 cache_get_fn: Callable[[str], Optional[Dict[str, Any]]],
                 cache_set_fn: Callable[[str, Dict[str, Any]], None],
                 cache_key_fn: Optional[Callable[[Request], str]] = None):
...
# Import asyncio for async function checking
import asyncio
...
class ExampleTenantSecurityMiddleware(TenantAwareMiddleware):
    ...
    def __init__(self):
        config = MiddlewareConfig(
            name="TenantSecurity",
            exempt_paths={"/docs", "/health", "/api/auth/login"}
        )
