...
from ..utils.datetime_utils import utc_now, format_iso
...
class MiddlewareResult:
    ...
    @classmethod
    def continue_processing(cls, data: Optional[Dict[str, Any]] = None) -> 'MiddlewareResult':
        """Continue with normal request processing."""
        return cls(action="continue", data=data)
        
    @classmethod
    def early_return(cls, 
                     data: Dict[str, Any], 
                     status: int = 200, 
                     headers: Optional[Dict[str, str]] = None) -> 'MiddlewareResult':
        """Return early with data (e.g., cached result)."""
        return cls(action="early_return", data=data, response_status=status, response_headers=headers)
        
    @classmethod
    def cache_hit(cls, 
                  cached_data: Dict[str, Any], 
                  cache_headers: Optional[Dict[str, str]] = None) -> 'MiddlewareResult':
...
class MiddlewareFactory:
    ...
    def create_simple_middleware(
        process_fn: Callable[[Request], Optional[Dict[str, Any]]],
        name: str,
        exempt_paths: Optional[Set[str]] = None,
        response_headers_fn: Optional[Callable[[Request, Dict], Dict[str, str]]] = None
    ) -> Callable:
    ...
    def create_caching_middleware(
        cache_key_fn: Callable[[Request], Optional[str]],
        ...
        async def caching_middleware(request: Request, call_next):
            ...
            if (hasattr(request.state, "cache_key") and 
...
# Import asyncio for the simple middleware creation
import asyncio