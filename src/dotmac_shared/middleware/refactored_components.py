...
from .unified import UnifiedMiddlewareProcessor, TenantAwareMiddleware, MiddlewareConfig
from .factory import MiddlewareResult
from .headers import ExtractedHeaders
from .state import RequestStateManager, TenantContext
from ..utils.datetime_utils import utc_now, get_common_expiry
...
class DRYAPIVersioningMiddleware(UnifiedMiddlewareProcessor):
    ...
    def __init__(self, default_version: str = "v1", supported_versions: Optional[Set[str]] = None):
        config = MiddlewareConfig(
            name="APIVersioning",
            exempt_paths={"/docs", "/redoc", "/openapi.json", "/health", "/metrics"}
        )
    ...
    async def process_middleware(self, 
        ...
        if existing:
            if existing["status"] == "completed":
                ...
                return MiddlewareResult.early_return(
                    {"status": existing["status"], "operation_id": existing.get("operation_id")},
                    status=202,
...
class DRYStandardMiddlewareStack:
    ...
    def __init__(self, 
                 tenant_enforcer: Optional[DRYTenantSecurityEnforcer] = None,
                 api_versioning: Optional[DRYAPIVersioningMiddleware] = None,
                 background_ops: Optional[DRYBackgroundOperationsMiddleware] = None):
