...
class HeaderConfig:
    ...
    STANDARD_HEADERS = {
        ...
        "api_version": ["X-API-Version", "x-api-version", "API-Version", "Accept-Version"],
...
class ExtractedHeaders:
    ...
    container_tenant: Optional[str] = None
...
class HeaderExtractor:
    ...
    def extract(self, request: Request, header_key: str) -> Optional[str]:
    ...
    def extract_multiple(self, request: Request, header_keys: List[str]) -> Dict[str, Optional[str]]:
    ...
    def extract_tenant_context(self, request: Request) -> Tuple[Optional[str], str, Dict[str, str]]:
    ...
    def extract_versioning_info(self, request: Request) -> Tuple[Optional[str], str]:
    ...
    def _extract_header_with_fallback(self, request: Request, header_variations: List[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    def _extract_tenant_from_subdomain(self, request: Request) -> Optional[str]:
        ...
        try:
            ...
            if len(subdomain) >= 3 and re.match(r"^[a-zA-Z0-9_-]+$", subdomain):
                ...
                excluded_subdomains = {"www", "api", "admin", "app", "cdn", "static", "assets"}
    ...
    def _extract_tenant_from_jwt(self, request: Request) -> Optional[str]:
    ...
    def _normalize_content_type(self, content_type: str) -> Optional[str]:
...
def extract_header(request: Request, header_key: str) -> Optional[str]:
    """Convenience function to extract single header.""" 
    extractor = HeaderExtractor()
    return extractor.extract(request, header_key)

def extract_tenant_info(request: Request) -> Tuple[Optional[str], str, Dict[str, str]]:
