...
from ..database.core import get_database_session
from ..exceptions import ValidationError, NotFoundError, BusinessLogicError
from ..decorators import standard_exception_handler
from ..location.models import Coordinates, Location
...
from .models import (
    ...
    SkillLevel, TechnicianCreate, TechnicianUpdate, TechnicianResponse,
...
class FieldOperationsService:
    ...
    def create_technician(self, tenant_id: str, technician_data: TechnicianCreate) -> TechnicianResponse:
        ...
        existing = self.db.query(Technician).filter(
            and_(
        ...
        technician = Technician(
            tenant_id=tenant_id,
            **technician_data.model_dump(exclude_unset=True)
        )
    ...
    @standard_exception_handler
    def update_technician(self, tenant_id: str, technician_id: UUID, 
                         update_data: TechnicianUpdate) -> TechnicianResponse:
    ...
    @standard_exception_handler
    def get_available_technicians(self, tenant_id: str, 
                                 work_order_type: Optional[WorkOrderType] = None,
                                 location: Optional[Coordinates] = None,
                                 radius_km: float = 50.0) -> List[TechnicianResponse]:
    ...
    def create_work_order(self, tenant_id: str, work_order_data: WorkOrderCreate) -> WorkOrderResponse:
        ...
        work_order = WorkOrder(
            ...
            **work_order_data.model_dump(exclude_unset=True)
        )
    ...
    @standard_exception_handler
    def assign_technician(self, tenant_id: str, work_order_id: UUID, 
                         technician_id: UUID, assigned_by: str) -> WorkOrderResponse:
        ...
        self._create_status_history(
            work_order, old_status, WorkOrderStatus.SCHEDULED,
    ...
    @standard_exception_handler
    def update_work_order_status(self, tenant_id: str, work_order_id: UUID,
                                new_status: WorkOrderStatus, notes: Optional[str] = None,
                                updated_by: Optional[str] = None,
                                location: Optional[Dict[str, float]] = None) -> WorkOrderResponse:
    ...
    @standard_exception_handler
    def get_technician_work_orders(self, tenant_id: str, technician_id: UUID,
                                  status_filter: Optional[List[WorkOrderStatus]] = None,
                                  date_from: Optional[date] = None,
                                  date_to: Optional[date] = None) -> List[WorkOrderResponse]:
    ...
    def _score_technicians_for_work_order(self, work_order: WorkOrder, 
                                         technicians: List[TechnicianResponse],
                                         work_location: Optional[Coordinates]) -> TechnicianResponse:
    ...
    @standard_exception_handler
    def calculate_technician_performance(self, tenant_id: str, technician_id: UUID,
                                       period_start: date, period_end: date) -> PerformanceMetrics:
        ...
        # Time metrics
        time_entries = self.db.query(TechnicianTimeEntry).filter(
            and_(
        ...
        # Quality metrics
        completed_with_rating = [wo for wo in work_orders 
                               if wo.status == WorkOrderStatus.COMPLETED and wo.customer_satisfaction_rating]
        average_customer_rating = None
        if completed_with_rating:
            average_customer_rating = sum(wo.customer_satisfaction_rating for wo in completed_with_rating) / len(completed_with_rating)
    ...
    def _calculate_performance_score(self, completion_rate: float, avg_rating: Optional[float],
                                   sla_met: int, sla_missed: int, total_jobs: int) -> int:
    ...
    @standard_exception_handler
    def optimize_technician_route(self, tenant_id: str, technician_id: UUID,
                                 target_date: date) -> List[WorkOrderResponse]:
    ...
    def _optimize_route_nearest_neighbor(self, work_orders: List[WorkOrder], 
                                       start_location: Dict[str, float]) -> List[WorkOrder]:
    ...
    def _generate_work_order_number(self, tenant_id: str) -> str:
        ...
        count = self.db.query(WorkOrder).filter(
            and_(
                WorkOrder.tenant_id == tenant_id,
                WorkOrder.work_order_number.like(f"{date_prefix}%")
            )
        ).count()
        
        return f"{date_prefix}-{count + 1:04d}"
    
    def _create_status_history(self, work_order: WorkOrder, from_status: Optional[WorkOrderStatus],
                              to_status: WorkOrderStatus, notes: Optional[str] = None,
...
class DispatchService:
    ...
    def emergency_dispatch(self, tenant_id: str, work_order_id: UUID) -> TechnicianResponse:
        ...
        # Assign immediately
        assigned_tech = self.field_service.assign_technician(
            tenant_id=tenant_id,
            work_order_id=work_order_id,
