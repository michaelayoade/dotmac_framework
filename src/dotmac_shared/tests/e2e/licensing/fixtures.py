...
class LicenseTestFixtures:
    ...
    async def create_user_for_tenant(
        self, tenant_id: str, role: str = "user"
    ) -> dict[str, Any]:
        ...
        self.created_resources.append(
            {"type": "user", "id": user_data["user_id"], "tenant_id": tenant_id}
        )

        return user_data

    async def create_feature_flag(
        self, tenant_id: str, feature_name: str, **kwargs
    ) -> dict[str, Any]:
        """Create a feature flag for a tenant."""
        flag = FeatureFlagFactory(
            tenant_id=tenant_id, feature_name=feature_name, **kwargs
        )
    ...
    async def upgrade_license_plan(
        self, contract_id: str, new_plan: str
    ) -> dict[str, Any]:
    ...
    async def simulate_usage_near_limit(
        self, tenant_id: str, feature: str, percentage: float = 90.0
    ):
    ...
    async def wait_for_feature_propagation(
        self, tenant_id: str, feature_name: str, timeout: int = 30
    ):
    ...
    async def _create_license_contract(
        ...
        response = await self.http_client.post(
            f"{self.base_url}/api/v1/licensing/contracts", json=license_data
        )
        response.raise_for_status()
        return response.json()["data"]
    ...
    async def _create_user(self, user_data: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self.http_client.post(
            f"{self.base_url}/api/v1/users", json=user_data
        )
        response.raise_for_status()
        return response.json()["data"]
    ...
    async def _create_feature_flag(self, flag_data: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self.http_client.post(
            f"{self.base_url}/api/v1/feature-flags", json=flag_data
        )
        response.raise_for_status()
        return response.json()["data"]
    ...
    async def _record_usage(self, usage_data: dict[str, Any]):
        """Record usage data."""
        response = await self.http_client.post(
            f"{self.base_url}/api/v1/licensing/usage", json=usage_data
        )
        response.raise_for_status()
