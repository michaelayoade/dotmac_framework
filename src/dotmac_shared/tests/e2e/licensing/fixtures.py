...
from .factories import FeatureFlagFactory, LicenseContractFactory, TestScenarioFactory, UserFactory
...
class LicenseTestFixtures:
    ...
    async def create_tenant_with_license(self, plan: str = "premium") -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        self.created_resources.extend(
            [{"type": "license", "id": license_data["contract_id"]}, {"type": "tenant", "id": tenant_data["tenant_id"]}]
        )
    ...
    async def create_user_for_tenant(self, tenant_id: str, role: str = "user") -> dict[str, Any]:
        ...
        self.created_resources.append({"type": "user", "id": user_data["user_id"], "tenant_id": tenant_id})

        return user_data

    async def create_feature_flag(self, tenant_id: str, feature_name: str, **kwargs) -> dict[str, Any]:
        """Create a feature flag for a tenant."""
        flag = FeatureFlagFactory(tenant_id=tenant_id, feature_name=feature_name, **kwargs)

        flag_data = await self._create_feature_flag(flag.__dict__)

        self.created_resources.append({"type": "feature_flag", "tenant_id": tenant_id, "feature_name": feature_name})

        return flag_data

    async def expire_license(self, contract_id: str) -> dict[str, Any]:
        """Expire a license contract immediately."""
        update_data = {"valid_until": (datetime.now() - timedelta(hours=1)).isoformat(), "status": "expired"}

        response = await self.http_client.patch(
            f"{self.base_url}/api/v1/licensing/contracts/{contract_id}", json=update_data
        )
                ...
                for app in apps_to_check:
                    ...
                    try:
                        response = await self.http_client.get(
                            f"{app_url}/api/v1/features/{feature_name}", headers={"X-Tenant-ID": tenant_id}
                        )
                        ...
                        if response.status_code == 200:
                            ...
                            if feature_data.get("enabled"):
                                propagated_count += 1
                    except (ConnectionError, TimeoutError, KeyError, AttributeError):
                        # App might not be running or feature not implemented
                        pass
    ...
    async def _create_tenant(self, tenant_data: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self.http_client.post(f"{self.base_url}/api/v1/tenants", json=tenant_data)
        response.raise_for_status()
        return response.json()["data"]
    ...
    async def _create_license_contract(self, license_data: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self.http_client.post(f"{self.base_url}/api/v1/licensing/contracts", json=license_data)
        response.raise_for_status()
        return response.json()["data"]

    async def _create_user(self, user_data: dict[str, Any]) -> dict[str, Any]:
        """Create user via management API."""
        response = await self.http_client.post(f"{self.base_url}/api/v1/users", json=user_data)
        response.raise_for_status()
        return response.json()["data"]

    async def _create_feature_flag(self, flag_data: dict[str, Any]) -> dict[str, Any]:
        """Create feature flag via management API."""
        response = await self.http_client.post(f"{self.base_url}/api/v1/feature-flags", json=flag_data)
        response.raise_for_status()
        return response.json()["data"]
    ...
    async def _create_customer(self, customer_data: dict[str, Any]) -> dict[str, Any]:
        ...
        response = await self.http_client.post(
            f"{isp_url}/api/v1/customers", json=customer_data, headers={"X-Tenant-ID": customer_data["tenant_id"]}
        )
        response.raise_for_status()
        return response.json()["data"]
    ...
    async def _record_usage(self, usage_data: dict[str, Any]):
        """Record usage data."""
        response = await self.http_client.post(f"{self.base_url}/api/v1/licensing/usage", json=usage_data)
        response.raise_for_status()

    async def _get_license_by_tenant(self, tenant_id: str) -> dict[str, Any]:
        """Get license contract by tenant ID."""
        response = await self.http_client.get(f"{self.base_url}/api/v1/licensing/contracts/by-tenant/{tenant_id}")
        response.raise_for_status()
        return response.json()["data"]
    ...
    async def _cleanup_resource(self, resource: dict[str, Any]):
        ...
        if resource["type"] == "tenant":
            await self.http_client.delete(f"{self.base_url}/api/v1/tenants/{resource['id']}")
        elif resource["type"] == "license":
            await self.http_client.delete(f"{self.base_url}/api/v1/licensing/contracts/{resource['id']}")
        elif resource["type"] == "user":
            await self.http_client.delete(f"{self.base_url}/api/v1/users/{resource['id']}")
