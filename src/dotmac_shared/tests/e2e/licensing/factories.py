...
class LicenseContractFactory(factory.Factory):
    ...
    enabled_features = factory.LazyFunction(lambda: [
        "basic_analytics", "premium_api", "custom_branding", "email_support", "phone_support"
    ])
    disabled_features = factory.LazyFunction(lambda: [])
    feature_limits = factory.LazyFunction(lambda: {
        "max_integrations": 10,
        "max_webhooks": 5
    })
    ...
    def basic_license(cls, **kwargs):
        ...
        return cls(
            ...
            enabled_features=[
                "basic_analytics", "standard_api", "email_support"
            ],
            feature_limits={
                "max_integrations": 3
            },
...
class FeatureFlagFactory(factory.Factory):
    ...
    feature_name = factory.Faker("random_element", elements=[
        "advanced_analytics", "premium_api", "sso", "custom_branding",
        "white_label", "enterprise_integration", "priority_support"
    ])
    tenant_id = factory.LazyFunction(lambda: f"tenant-{uuid.uuid4().hex[:8]}")
    enabled = True
...
class TestScenarioFactory:
    ...
    def multi_app_tenant():
        ...
        license = LicenseContractFactory.enterprise_license(
            ...
            enabled_features=[
                "basic_analytics", "advanced_analytics", "enterprise_api",
                "sso", "white_label", "enterprise_integration", "priority_support"
            ]
        )
