...
class WebhookManager:
    ...
    async def create(
        self, 
        data: WebhookEndpointCreateSchema, 
        user_id: UUID
    ...
    async def update(
        self, 
        endpoint_id: UUID, 
        data: WebhookEndpointUpdateSchema, 
        user_id: UUID
    ) -> WebhookEndpointResponseSchema:
    ...
    async def trigger_event(
        ...
        matching_endpoints = [
            config for config in self.endpoints.values()
            if config["active"] and (
                not config["events"] or event_type in config["events"]
            )
        ]
