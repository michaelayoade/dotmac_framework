...
class WorkflowAnalyticsService(StatefulService):
    ...
    async def track_workflow_event(
        ...
        # Get tenant context
        tenant_id = None
        if (self.analytics_config.deployment_context and 
            hasattr(self.analytics_config.deployment_context, 'tenant_id')):
        ...
        if (self.analytics_config.batch_processing and 
            len(self._workflow_events) >= self.analytics_config.max_events_per_batch):
            await self._process_batch_analytics()
    ...
    async def get_workflow_metrics(
        ...
        filtered_events = [
            ...
            if (event.workflow_type == workflow_type and
                event.timestamp >= period_start and
                event.timestamp <= period_end and
    ...
    async def _process_real_time_analytics(self, event: WorkflowEvent):
        ...
        if (event.duration_ms and 
            self.analytics_config.performance_alerts):
            await self._check_performance_alerts(event)
        
        # Check for failure alerts
        if (event.status == WorkflowStatus.FAILED and
            self.analytics_config.performance_alerts):
    ...
    async def _calculate_workflow_metrics(
        ...
        if durations:
            ...
            percentile_95_ms = sorted_durations[percentile_95_index] if percentile_95_index < len(sorted_durations) else max_duration_ms
        else:
            percentile_95_ms = 0.0
    ...
    async def _calculate_workflow_trends(
        ...
        while current_date <= end_date:
            ...
            day_events = [
                e for e in self._workflow_events
                if (e.timestamp.date() == current_date and
                    (not tenant_id or e.tenant_id == tenant_id))
            ]
            
            workflows_count = len(set(e.workflow_id for e in day_events))
    ...
    async def _generate_alert(self, alert_type: str, message: str, context: Dict[str, Any]):
        ...
        if self.event_bus:
            await self.event_bus.publish(
                "workflow_analytics.alert_generated",
                alert
            )
