...
from ..core.models import BillingCycle, PricingModel
...
async def example_basic_isp_billing(db: AsyncSession):
    ...
    usage_result = await adapter.record_bandwidth_usage(
...
async def example_saas_platform_billing(db: AsyncSession):
    ...
    usage_result = await adapter.record_api_usage(
        subscription_id=subscription_result["id"],
...
async def example_multi_tenant_billing(db: AsyncSession):
    ...
    tenant_a_customers = await billing_service_a.customer_repo.get_multi(
        tenant_id=tenant_a_id
    )
    tenant_b_customers = await billing_service_b.customer_repo.get_multi(
        tenant_id=tenant_b_id
    )

    print(f"📊 Tenant A has {len(tenant_a_customers)} customers")
    print(f"📊 Tenant B has {len(tenant_b_customers)} customers")

    # Verify cross-tenant isolation
    tenant_a_cannot_see_b = await billing_service_a.customer_repo.get(
        customer_b.id, tenant_a_id
    )
...
if __name__ == "__main__":
    ...
    print("To run these examples, call run_all_examples() with your AsyncSession")
    print(
        """
    from examples.integration_examples import run_all_examples

    async def main():
        async with async_session() as db:
            results = await run_all_examples(db)
            print("Examples completed:", results.keys())
