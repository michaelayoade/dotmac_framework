...
class BusinessPolicy(BaseModel):
    ...
    def evaluate(self, context: PolicyContext, evaluation_data: dict[str, Any]) -> "PolicyEvaluationResult":
        ...
        if self.require_all_rules:
            ...
            final_result = self.default_result if not violated_rules else PolicyResult.DENY
        else:
...
class RuleEvaluator:
    ...
    def test_rule(self, rule: PolicyRule, test_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"rule_name": rule.name, "passed": False, "error": str(e), "test_data": test_data}
...
class PolicyEngine:
    ...
    def evaluate_policy(
        self, policy_name: str, context: PolicyContext, evaluation_data: dict[str, Any], version: Optional[str] = None
