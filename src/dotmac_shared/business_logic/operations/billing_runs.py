...
from ..sagas import SagaStep, SagaDefinition, CompensationHandler, SagaContext
...
class ValidateBillingPeriodStep(SagaStep):
    ...
    async def execute(self, context: SagaContext) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Invalid billing period format: {e}")
...
class ProcessPaymentsStep(SagaStep):
    ...
    async def _process_invoice_payment(self, invoice: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Simulate payment processing with some randomization
        import random
    ...
    async def compensate(self, context: SagaContext) -> None:
        ...
        if payment_processing_result and not payment_processing_result.get("dry_run"):
            successful_payments = [
                p for p in payment_processing_result.get("payment_results", [])
...
class SendNotificationsStep(SagaStep):
    ...
    async def execute(self, context: SagaContext) -> Dict[str, Any]:
        ...
        payment_status_map = {
            p["invoice_id"]: p["status"] for p in payment_results
        }
    ...
    async def execute(self, context: SagaContext) -> Dict[str, Any]:
        ...
        billing_run_summary = {
            ...
            "summary": {
                ...
                "payments_processed": payment_processing_result.get("successful_payments", 0) if payment_processing_result else 0,
                "payment_failures": payment_processing_result.get("failed_payments", 0) if payment_processing_result else 0,
                "total_payment_amount": payment_processing_result.get("total_amount_processed", 0.0) if payment_processing_result else 0.0,
                "notifications_sent": notification_result.get("successful_notifications", 0) if notification_result else 0,
                "notification_failures": notification_result.get("failed_notifications", 0) if notification_result else 0
            },
            "status": "completed",
...
class BillingRunOperation(IdempotentOperation[Dict[str, Any]]):
    ...
    async def execute(
        ...
        saga_context.set_shared_data("operation_context", {
            **context,
