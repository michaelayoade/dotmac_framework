...
from ..auth.edge_validation import EdgeJWTValidator, EdgeAuthMiddleware
from ..auth.service_tokens import configure_service_auth, ServiceAuthMiddleware
from .config import PlatformConfig, DeploymentMode
...
async def setup_observability(
    ...
    try:
        ...
        logger.info(f"   OTEL Enabled: True")
...
def _register_business_slo_metrics(metrics_registry, platform_config: PlatformConfig):
    ...
    from ..observability.metrics_schema import MetricDefinition, MetricType, MetricCategory
...
def _register_service_capabilities(
    ...
    if platform_config.deployment_context:
        ...
        if mode == DeploymentMode.MANAGEMENT_PLATFORM:
            capabilities.extend([
                "tenant_management", "partner_management", "billing", 
                "container_orchestration", "plugin_management"
            ])
