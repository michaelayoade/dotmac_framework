...
class SecureConfigManager:
    ...
    async def get_secret(self, path: str, env_fallback: Optional[str] = None, 
        ...
        if self._adapter and OPENBAO_AVAILABLE:
            try:
                secret_result = await self._adapter.get_secret(
                    path=path,
                    secret_type="application_secret"
                )
    ...
    def get_secret_sync(self, path: str, env_fallback: Optional[str] = None, 
                       required: bool = True) -> str:
        """
    ...
    async def store_secret(self, path: str, value: str) -> bool:
        ...
        try:
            result = await self._adapter.store_secret(
                path=path,
                value=value,
                secret_type="application_secret"
            )
