...
from dotmac_shared.tasks.notifications import NotificationChannelProvider, NotificationChannel
from dotmac_shared.plugins.base import BasePlugin
...
class ChatConnectionManager:
    ...
    async def connect_customer(self, websocket: WebSocket, session_id: str, customer_id: str = None):
    ...
    async def send_message_to_session(self, session_id: str, message: Dict[str, Any], exclude_sender: str = None):
    ...
    def get_available_agents(self, tenant_id: str, skills: List[str] = None) -> List[str]:
...
class LiveChatPlugin(BasePlugin, NotificationChannelProvider):
    ...
    async def handle_customer_websocket(self, websocket: WebSocket, session_id: str, customer_id: str = None):
    ...
    async def _handle_customer_message(self, session_id: str, message_data: Dict[str, Any], customer_id: str = None):
        ...
        if message_type == "message":
            ...
            if content:
                ...
                await self.connection_manager.send_message_to_session(session_id, {
                    "type": "message",
                    "content": content,
                    "sender_type": "customer",
                    "sender_name": message_data.get("sender_name", "Customer"),
    ...
    async def _try_assign_agent(self, session_id: str, queue_id: str = "general"):
        ...
        available_agents = self.connection_manager.get_available_agents("default_tenant")  # In production, get from session
    ...
    async def _session_cleanup(self):
        ...
        while True:
            try:
                ...
                cutoff_time = datetime.now(timezone.utc) - timedelta(minutes=self.auto_close_minutes)
