...
class PluginManager:
    ...
    async def register_plugin(
        self, plugin: BasePlugin, auto_initialize: bool = True
    ) -> None:
        """
        ...
        try:
            ...
            if success:
                ...
                self._logger.warning(
                    f"Plugin was not found during unregistration: {plugin_key}"
                )
    ...
    async def load_plugins_from_config(
        ...
        try:
            ...
            self._logger.info(
                f"Loaded {successful_count}/{len(plugins)} plugins from config"
            )
    ...
    async def load_plugin_from_module(
        ...
        """
        self._logger.info(
            f"Loading plugin from module: {module_path}.{plugin_class_name}"
        )
    ...
    async def execute_plugin(
        ...
        if not plugin:
            available_plugins = [
                f"{p.domain}.{p.name}" for p in await self.list_plugins(domain)
            ]
    ...
    async def initialize_plugins_by_domain(
        ...
        return await self.lifecycle_manager.initialize_plugins(
            plugin_keys, parallel=parallel
        )
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        plugin_health = {
            ...
            "error_plugins": len(
                [p for p in all_plugins if p.status == PluginStatus.ERROR]
            ),
        }
    ...
    def add_lifecycle_event_handler(
    ...
    def remove_lifecycle_event_handler(
        self, event: LifecycleEvent, handler: callable
    ) -> None:
    ...
    def _setup_event_handlers(self) -> None:
        ...
        async def on_status_changed(
            event: LifecycleEvent, plugin: BasePlugin, data: Any
        ) -> None:
            self._logger.info(
        ...
        self.lifecycle_manager.add_event_handler(
            LifecycleEvent.STATUS_CHANGED, on_status_changed
        )
        self.lifecycle_manager.add_event_handler(
            LifecycleEvent.ERROR_OCCURRED, on_error_occurred
        )
