...
class PluginSandbox:
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            if self._monitoring:
                duration = (
                    self._execution_stats["end_time"]
                    - self._execution_stats["start_time"]
                ).total_seconds()
    ...
    def check_permission(self, category: str, operation: str) -> bool:
        ...
        if not has_perm:
            logger.warning(
                f"Permission denied for plugin {self.plugin_id}: {category}:{operation}"
            )
        ...
        try:
            if asyncio.iscoroutinefunction(coro_or_func):
                result = await asyncio.wait_for(
                    coro_or_func(*args, **kwargs), timeout=timeout
                )
            else:
...
class PluginSecurityManager:
    ...
    async def validate_plugin_code(
        self, plugin_code: str, plugin_metadata: Dict[str, Any]
    ) -> bool:
        """
...
def create_secure_plugin_manager() -> PluginSecurityManager:
