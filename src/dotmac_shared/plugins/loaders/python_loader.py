...
class PythonPluginLoader:
    ...
    async def load_plugin_from_module(
        ...
        """
        self._logger.info(
            f"Loading plugin {plugin_class_name} from module {module_path}"
        )
    ...
    async def load_plugin_from_file(
        ...
        try:
            ...
            metadata = await self._create_metadata_from_class(
                plugin_class, str(file_path)
            )
    ...
    async def discover_plugins_in_module(
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module, inspect.isclass):
                if (
                    ...
                    try:
                        ...
                        plugin = await self.load_plugin_from_module(
                            module_path, name, config
                        )
                        plugins.append(plugin)

                    except Exception as e:
                        self._logger.error(
                            f"Failed to load discovered plugin {name}: {e}"
                        )
                        # Continue with other plugins

            self._logger.info(
                f"Discovered {len(plugins)} plugins in module {module_path}"
            )
            return plugins
            ...
            self._logger.error(
                f"Error discovering plugins in module {module_path}: {e}"
            )
            return []
    ...
    async def discover_plugins_in_directory(
        ...
        self._logger.info(
            f"Discovered {len(plugins)} plugins in directory {directory_path}"
        )
        return plugins
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module, inspect.isclass):
                if (
                    ...
                    try:
                        config = plugin_config.get(name, {})
                        plugin = await self.load_plugin_from_file(
                        ...
                        self._logger.error(
                            f"Failed to load plugin {name} from {file_path}: {e}"
                        )
    ...
    async def _create_metadata_from_class(
        ...
        description = getattr(
            plugin_class, "__plugin_description__", plugin_class.__doc__
        )
        ...
        optional_dependencies = getattr(
            plugin_class, "__plugin_optional_dependencies__", []
        )
        python_requires = getattr(plugin_class, "__plugin_python_requires__", None)
        platform_compatibility = getattr(
            plugin_class, "__plugin_platform_compatibility__", ["any"]
        )
        ...
        supports_streaming = getattr(
            plugin_class, "__plugin_supports_streaming__", False
        )
        ...
        required_permissions = set(
            getattr(plugin_class, "__plugin_required_permissions__", [])
        )
...
from dotmac_shared.plugins import BasePlugin, PluginMetadata
