...
class YamlPluginLoader:
    ...
    async def load_plugins_from_file(
        ...
        try:
            ...
            self._logger.info(
                f"Successfully loaded {len(plugins)} plugins from {config_path}"
            )
            return plugins
    ...
    async def load_plugins_from_string(self, yaml_content: str) -> List[BasePlugin]:
        ...
        try:
            ...
            if not config_data:
                raise PluginConfigError(
                    "yaml_loader", config_errors=["Empty YAML content"]
                )

            plugins = await self._parse_and_load_plugins(config_data)

            self._logger.info(
                f"Successfully loaded {len(plugins)} plugins from YAML string"
            )
            return plugins
    ...
    async def _load_single_plugin(self, plugin_config: Dict[str, Any]) -> BasePlugin:
        ...
        missing_fields = [
            field for field in required_fields if field not in plugin_config
        ]
