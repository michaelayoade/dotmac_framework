    ...
    async def send_bulk_messages(self, messages: List[Message]) -> BulkMessageResult:
        ...
        for message in messages:
            try:
                ...
                results.append(
                    MessageResult(
                        success=False, error_message=str(e), status=MessageStatus.FAILED
                    )
                )
...
class CommunicationAdapter:
    ...
    def unregister_plugin(self, plugin_name: str) -> None:
        ...
        if plugin_name in self._plugins:
            ...
            providers_to_remove = [
                k for k, v in self._default_providers.items() if v == plugin_name
            ]
            for provider in providers_to_remove:
                del self._default_providers[provider]

            # Clean up fallback providers
            for message_type, providers in self._fallback_providers.items():
    ...
    def add_fallback_provider(self, message_type: str, plugin_name: str) -> None:
        ...
        if plugin_name not in self._fallback_providers[message_type]:
            ...
            self._logger.info(
                f"Added fallback provider for {message_type}: {plugin_name}"
            )
    ...
    async def send_message(
        ...
        if not providers_to_try:
            raise PluginError(
                f"No providers available for message type: {message.message_type}"
            )
            ...
            try:
                ...
                if not await plugin.validate_recipient(message.recipient):
                    self._logger.warning(
                        f"Invalid recipient for provider {provider_name}: {message.recipient}"
                    )
                    continue
                ...
                if result.success:
                    self._logger.info(
                        f"Message sent successfully using provider: {provider_name}"
                    )
                    return result
                else:
                    self._logger.warning(
                        f"Message failed with provider {provider_name}: {result.error_message}"
                    )
                    last_error = result.error_message

            except Exception as e:
                self._logger.error(
                    f"Error sending message with provider {provider_name}: {e}"
                )
                last_error = str(e)
                continue
    ...
    async def send_bulk_messages(
        ...
        if not messages:
            return BulkMessageResult(
                total_messages=0, successful=0, failed=0, results=[]
            )
        ...
        for provider_name, group_messages in message_groups.items():
            ...
            for i in range(0, len(group_messages), batch_size):
                ...
                try:
                    ...
                    self._logger.error(
                        f"Bulk send failed for provider {provider_name}: {e}"
                    )
    ...
    async def broadcast_message(
        ...
        for provider_name, task in tasks:
            try:
                ...
                results[provider_name] = MessageResult(
                    success=False, error_message=str(e), status=MessageStatus.FAILED
                )
