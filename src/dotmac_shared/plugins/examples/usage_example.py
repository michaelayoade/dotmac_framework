...
class EmailPlugin(CommunicationPlugin):
    ...
    async def _initialize_plugin(self) -> None:
        ...
        self.logger.info(
            f"Initializing SMTP email plugin with host: {self.config.get('smtp_host', 'localhost')}"
        )
...
async def main():
    ...
    async with PluginManager() as manager:
        ...
        rate_limiting.add_plugin_rate_limit(
            "communication.smtp_email", conservative_limit
        )
        ...
        utility_metadata = PluginMetadata(
            name="text_processor", version="1.0.0", domain="utilities"
        )
        ...
        logger.info(
            f"📧 Email result: {email_result.success} - {email_result.message_id}"
        )
        ...
        bulk_messages = [
            Message(f"user{i}@example.com", f"Bulk message {i}", "Bulk Test", "email")
            for i in range(5)
        ]

        bulk_result = await comm_adapter.send_bulk_messages(bulk_messages)
        logger.info(
            f"📦 Bulk result: {bulk_result.successful}/{bulk_result.total_messages} successful"
        )
        ...
        for plugin in all_plugins:
            logger.info(
                f"   {plugin.domain}.{plugin.name} v{plugin.version} - {plugin.status.value}"
            )
...
"""
    ...
    try:
        ...
        async with PluginManager() as manager:
            ...
            logger.info(
                "YAML config structure demonstrated (plugins would load if classes existed)"
            )
