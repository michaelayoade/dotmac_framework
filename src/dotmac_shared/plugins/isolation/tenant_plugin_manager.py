...
from ..security.plugin_sandbox import PluginSandbox, PluginSecurityManager
...
class TenantPluginRegistry(PluginRegistry):
    ...
    async def register_plugin(self, plugin: BasePlugin) -> None:
        ...
        logger.info(
            f"Registered plugin {plugin.domain}.{plugin.name} for tenant {self.tenant_id}"
        )
    ...
    async def _cleanup_tenant_plugin(self, plugin: BasePlugin) -> None:
        ...
        try:
            ...
            logger.info(
                f"Cleaned up tenant data for plugin {plugin.domain}.{plugin.name}"
            )
...
class TenantPluginManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise PluginError(
                f"Tenant plugin manager initialization failed: {e}"
            ) from e
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            self._initialized = False
            self._logger.info(
                f"Tenant plugin manager shutdown complete for {self.tenant_id}"
            )
    ...
    async def install_plugin(
        ...
        try:
            ...
            self._logger.info(
                f"Plugin {plugin_key} installed for tenant {self.tenant_id}"
            )
            return plugin_key
    ...
    async def uninstall_plugin(self, plugin_key: str) -> bool:
        ...
        """
        self._logger.info(
            f"Uninstalling plugin {plugin_key} for tenant {self.tenant_id}"
        )
        ...
        try:
            ...
            self._logger.info(
                f"Plugin {plugin_key} uninstalled for tenant {self.tenant_id}"
            )
            return True
    ...
    async def execute_plugin(
        self, plugin_key: str, method: str, *args, **kwargs
    ) -> Any:
        """
        Execute plugin method in isolated sandbox.
        """
        self._logger.debug(
            f"Executing {plugin_key}.{method} for tenant {self.tenant_id}"
        )
        ...
        try:
            ...
            self._logger.error(
                f"Plugin execution failed for {plugin_key}.{method}: {e}"
            )
    ...
    async def get_tenant_health(self) -> Dict[str, Any]:
        ...
        health_data = {
            ...
            "error_plugins": len(
                [p for p in plugins if p.status == PluginStatus.ERROR]
            ),
    ...
    async def _cleanup_all_sandboxes(self) -> None:
        ...
        for plugin_key, sandbox in self._active_sandboxes.items():
...
class TenantPluginOrchestrator:
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        for tenant_id, manager in self._tenant_managers.items():
            health_data["tenant_health"][
                str(tenant_id)
            ] = await manager.get_tenant_health()
