...
class RateLimiter(ABC):
    ...
    def __init__(self, config: RateLimit):
        ...
        self._logger = logging.getLogger(
            f"plugins.rate_limiter.{self.__class__.__name__}"
        )
...
class TokenBucketLimiter(RateLimiter):
    ...
    async def is_allowed(self, key: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        bucket["tokens"] = min(
            self.config.max_requests, bucket["tokens"] + tokens_to_add
        )
        ...
        reset_time = (
            current_time
            + (self.config.max_requests - bucket["tokens"]) / self._refill_rate
        )
...
class FixedWindowLimiter(RateLimiter):
    ...
    async def is_allowed(self, key: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        current_time = time.time()
        window_start = (
            int(current_time // self.config.time_window) * self.config.time_window
        )
    ...
    async def check_rate_limit(
        ...
        **kwargs,
        ...
        if method_name in self._global_limiters:
            ...
            key = (
                f"{method_name}:{user_id}"
                if limiter.config.per_user and user_id
                else method_name
            )
