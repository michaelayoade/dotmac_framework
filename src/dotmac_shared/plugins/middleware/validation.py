...
class RangeValidator(BaseValidator):
    ...
    def get_error_message(self, value: Any, rule: ValidationRule) -> str:
        if rule.min_value is not None and rule.max_value is not None:
            return (
                f"Value {value} must be between {rule.min_value} and {rule.max_value}"
            )
...
class ValidationMiddleware:
    ...
    def __init__(self):
        self._schemas: Dict[str, Dict[str, ValidationSchema]] = (
            {}
        )  # plugin_key -> method -> schema
        self._validators = [
            TypeValidator(),
            RangeValidator(),
            LengthValidator(),
    ...
    def add_validation_schema(self, plugin_key: str, schema: ValidationSchema) -> None:
        ...
        self._logger.debug(
            f"Added validation schema for {plugin_key}.{schema.method_name}"
        )
    ...
    def remove_validation_schema(self, plugin_key: str, method_name: str) -> None:
        ...
        if plugin_key in self._schemas and method_name in self._schemas[plugin_key]:
            ...
            self._logger.debug(
                f"Removed validation schema for {plugin_key}.{method_name}"
            )
    ...
    def validate_output(
        self, plugin: BasePlugin, method_name: str, output: Any
    ) -> None:
        """
        ...
        if isinstance(output, dict):
            ...
            for rule in schema.output_rules:
                ...
                if rule.required and field_value is None:
                    errors.append(
                        f"Required output field '{rule.field_name}' is missing"
                    )
                    continue
        ...
        if errors:
            raise PluginValidationError(
                plugin.name, errors, field_name=f"{method_name}_output"
            )

    def _get_schema(
        self, plugin_key: str, method_name: str
    ...
    def create_file_storage_validation_schema() -> ValidationSchema:
        ...
        return ValidationSchema(
            ...
            output_rules=[
                ...
                ValidationRule(
                    field_name="file_size", field_type=int, required=True, min_value=0
                ),
            ],
            validate_output=True,
        )
