...
class OpenBaoConfig:
    ...
    url: str = field(
        default_factory=lambda: os.getenv("OPENBAO_ADDR", "http://localhost:8200")
    )
    token: str = field(
        default_factory=lambda: os.getenv("OPENBAO_TOKEN", os.getenv("BAO_TOKEN", ""))
    )
    namespace: Optional[str] = field(
        default_factory=lambda: os.getenv("OPENBAO_NAMESPACE")
    )
    mount_point: str = field(
        default_factory=lambda: os.getenv("OPENBAO_MOUNT_POINT", "secret")
    )
    kv_version: int = field(
        default_factory=lambda: int(os.getenv("OPENBAO_KV_VERSION", "2"))
    )
    timeout: int = 30
    max_retries: int = 3
    cache_ttl: int = 300
    ...
    auth_method: str = field(
        default_factory=lambda: os.getenv("OPENBAO_AUTH_METHOD", "token")
    )
    role_id: Optional[str] = field(default_factory=lambda: os.getenv("OPENBAO_ROLE_ID"))
    secret_id: Optional[str] = field(
        default_factory=lambda: os.getenv("OPENBAO_SECRET_ID")
    )
    kubernetes_role: Optional[str] = field(
        default_factory=lambda: os.getenv("OPENBAO_KUBERNETES_ROLE")
    )
    aws_role: Optional[str] = field(
        default_factory=lambda: os.getenv("OPENBAO_AWS_ROLE")
    )
...
class OpenBaoClient:
        ...
        for attempt in range(self.config.max_retries):
            try:
                async with self._session.request(
                    method, url, data=json_data, params=params, headers=request_headers
                ) as response:
                    response_data = {}
    ...
    async def _authenticate_kubernetes(self) -> bool:
        ...
        if not self.config.kubernetes_role:
            raise OpenBaoAPIException(
                "kubernetes_role required for Kubernetes authentication"
            )
        ...
        auth_data = {"jwt": k8s_token, "role": self.config.kubernetes_role}

        response = await self._make_request(
            "POST", "/v1/auth/kubernetes/login", auth_data
        )
        ...
        raise OpenBaoAPIException(
            "Kubernetes authentication failed - no token returned"
        )
...
class OpenBaoSecretManager:
    ...
    async def get_database_credentials(self, database_name: str) -> Dict[str, str]:
        ...
        try:
            ...
            raise OpenBaoAPIException(
                f"Database credentials not found: {database_name}"
            )
