...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="DotMac ISP Framework", description="Application name")
    ...
    database_url: str = Field(
        default_factory=lambda: os.getenv("DATABASE_URL", "sqlite+aiosqlite:///./isp_framework.db"),
    ...
    max_tenants_per_instance: int = Field(default=100, description="Maximum tenants per instance")
    ...
    base_domain: str = Field(default="dotmac.io", description="Base domain for tenant subdomains")
    dns_strategy: str = Field(default="auto", description="DNS management strategy")
    dns_health_check_interval: int = Field(default=300, description="DNS health check interval")

    # Rate limiting settings
    rate_limiting_enabled: bool = Field(default=True, description="Enable rate limiting")
    rate_limit_storage_backend: str = Field(default="redis", description="Rate limit storage backend (redis/memory)")
    rate_limit_default_per_minute: int = Field(default=100, description="Default rate limit per minute")
    rate_limit_auth_per_minute: int = Field(default=10, description="Auth endpoint rate limit per minute")
    rate_limit_lockout_threshold: int = Field(default=5, description="Failed attempts before lockout")
    rate_limit_lockout_duration: int = Field(default=900, description="Lockout duration in seconds")
