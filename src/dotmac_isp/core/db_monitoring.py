...
monitoring = get_monitoring("db_monitoring")
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                for row in rows:
                    ...
                    # Update metrics using unified monitoring
                    monitoring.record_database_query(
                        operation=query_type,
                        table=row["database"] or "unknown",
                        duration=slow_query.mean_time_ms / 1000,  # Convert to seconds
                        success=True,
                        tenant_id=tenant_id,
                    )
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                for row in rows:
                    ...
                    # Record connection metrics using unified monitoring
                    # Note: The unified system doesn't have direct gauge support
                    # but we can record this as a custom operation
                    monitoring._record_operation_duration(
                        operation_name="database_connections",
                        duration=row["count"],  # Using count as a gauge value
                        success=True,
                        labels={"database": "main", "state": state},
                    )
        except Exception as e:
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                if row and row["cache_hit_ratio"]:
                    # Record cache hit ratio using unified monitoring
                    monitoring._record_operation_duration(
                        operation_name="database_cache_hit_ratio",
                        duration=row["cache_hit_ratio"],
                        success=True,
