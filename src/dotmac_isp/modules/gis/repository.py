...
from dotmac_shared.database.base import TenantModel
...
class ServiceAreaRepository:
    ...
    async def find_by_service_types(
        ...
        if active_only:
            query = query.where(ServiceArea.is_active == True)
        ...
        if service_types:
            ...
            for service_type in service_types:
                conditions.append(
                    func.json_extract(ServiceArea.service_types, f'$[*]').like(f'%{service_type}%')
                )
    ...
    async def find_areas_needing_analysis(
        ...
        query = select(ServiceArea).where(
            ...
            or_(
                ServiceArea.last_analyzed_at.is_(None),
                ServiceArea.last_analyzed_at < cutoff_date
            )
        )
...
class NetworkNodeRepository:
    ...
    async def find_by_node_type(
        ...
        query = select(NetworkNode).where(
            ...
            NetworkNode.node_type == node_type
        )
        
        if active_only:
            query = query.where(
                NetworkNode.is_active == True,
                NetworkNode.operational_status == 'active'
            )
    ...
    async def find_nodes_in_area(
        ...
        query = select(NetworkNode).where(
            NetworkNode.tenant_id == self.tenant_id,
            NetworkNode.service_area_id == service_area_id
        )
    ...
    async def find_nodes_with_capacity(
        ...
        node_types: Optional[List[NetworkNodeTypeEnum]] = None
    ) -> List[NetworkNode]:
    ...
    async def find_offline_nodes(
        self,
        hours_offline: int = 24
    ) -> List[NetworkNode]:
        ...
        query = select(NetworkNode).where(
            ...
            NetworkNode.is_active == True,
            or_(
                NetworkNode.last_seen_at.is_(None),
                NetworkNode.last_seen_at < cutoff_time
            )
        )
    ...
    async def get_network_topology_summary(self) -> Dict[str, Any]:
        ...
        query = select(
            ...
            func.count(
                func.case(
                    (NetworkNode.operational_status == 'active', 1),
                    else_=None
                )
            ).label('active_count')
        ).where(
...
class CoverageGapRepository:
    ...
    async def find_by_service_area(
        ...
        query = select(CoverageGap).where(
            ...
            CoverageGap.service_area_id == service_area_id
        )
        
        if active_only:
            query = query.where(CoverageGap.is_active == True)
...
class TerritoryRepository:
    ...
    async def find_by_assigned_user(
        ...
        query = select(Territory).where(
            Territory.tenant_id == self.tenant_id,
            Territory.assigned_user_id == user_id
        )
...
class RouteOptimizationRepository:
    ...
    async def find_recent_optimizations(
        ...
        query = select(RouteOptimization).where(
            ...
            RouteOptimization.calculated_at >= cutoff_date
    ...
    async def find_by_optimization_type(
        ...
        query = select(RouteOptimization).where(
            RouteOptimization.tenant_id == self.tenant_id,
            RouteOptimization.optimization_type == optimization_type
        )
        
        if active_only:
            query = query.where(RouteOptimization.is_active == True)
    ...
    async def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        query = select(
            ...
            func.count(
                func.distinct(RouteOptimization.optimization_type)
            ).label('optimization_types')
        ).where(
            RouteOptimization.tenant_id == self.tenant_id,
            RouteOptimization.is_active == True
        )
...
class CoverageRecommendationRepository:
    ...
    async def find_by_priority(
        ...
        query = select(CoverageRecommendation).where(
            ...
            CoverageRecommendation.priority == priority
        )
        
        if active_only:
            query = query.where(CoverageRecommendation.is_active == True)
    ...
    async def find_by_status(
        ...
        query = select(CoverageRecommendation).where(
            CoverageRecommendation.tenant_id == self.tenant_id,
            CoverageRecommendation.status == status
        )
    ...
    async def find_pending_recommendations(
        ...
        query = select(CoverageRecommendation).where(
            CoverageRecommendation.tenant_id == self.tenant_id,
            CoverageRecommendation.is_active == True,
