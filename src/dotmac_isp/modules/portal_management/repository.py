...
from dotmac_shared.database import BaseRepository
from dotmac_shared.core.exceptions import standard_exception_handler
from .models import (
    PortalAccount, PortalSession, PortalLoginAttempt, PortalPreferences,
...
class PortalAccountRepository(BaseRepository[PortalAccount]):
    ...
    async def find_by_portal_id(self, portal_id: str) -> Optional[PortalAccount]:
        ...
        query = select(PortalAccount).options(
            joinedload(PortalAccount.customer),
            joinedload(PortalAccount.user)
        ).where(
            and_(
    ...
    async def find_by_customer_id(self, customer_id: UUID) -> Optional[PortalAccount]:
        ...
        query = select(PortalAccount).options(
            joinedload(PortalAccount.sessions)
        ).where(
            and_(
                PortalAccount.tenant_id == self.tenant_id,
    ...
    async def find_accounts_needing_password_change(self) -> List[PortalAccount]:
        ...
        query = select(PortalAccount).where(
            and_(
                PortalAccount.tenant_id == self.tenant_id,
                PortalAccount.must_change_password == True,
    ...
    async def get_account_statistics(self) -> Dict[str, Any]:
        ...
        accounts_by_type = await self.session.execute(
            select(
                PortalAccount.account_type,
                func.count(PortalAccount.id).label('count')
            ).where(
                PortalAccount.tenant_id == self.tenant_id
...
class PortalSessionRepository(BaseRepository[PortalSession]):
    ...
    @standard_exception_handler
    async def find_sessions_by_ip(self, ip_address: str, 
                                 active_only: bool = True) -> List[PortalSession]:
    ...
    async def find_suspicious_sessions(self) -> List[PortalSession]:
        ...
        query = select(PortalSession).options(
            ...
            and_(
                PortalSession.tenant_id == self.tenant_id,
                PortalSession.suspicious_activity == True,
                PortalSession.is_active == True
            )
        ).order_by(PortalSession.last_activity.desc())
    ...
    @standard_exception_handler
    async def terminate_all_sessions_for_account(self, portal_account_id: UUID, 
    ...
    async def get_session_statistics(self, days_back: int = 30) -> Dict[str, Any]:
        ...
        avg_duration = await self.session.execute(
            select(func.avg(
                func.extract('epoch', PortalSession.logout_at - PortalSession.login_at) / 60
            )).where(
                and_(
                    PortalSession.tenant_id == self.tenant_id,
...
class PortalLoginAttemptRepository(BaseRepository[PortalLoginAttempt]):
    ...
    async def find_by_portal_id(self, portal_id: str, limit: int = 100) -> List[PortalLoginAttempt]:
        ...
        ).order_by(PortalLoginAttempt.created_at.desc()).limit(limit)
    ...
    async def find_high_risk_attempts(self, risk_threshold: int = 75) -> List[PortalLoginAttempt]:
        ...
        query = select(PortalLoginAttempt).options(
            joinedload(PortalLoginAttempt.portal_account)
        ).where(
            and_(
                PortalLoginAttempt.tenant_id == self.tenant_id,
                or_(
    ...
    async def get_attempt_statistics(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        successful_attempts = await self.session.execute(
            select(func.count(PortalLoginAttempt.id)).where(
                and_(
                    PortalLoginAttempt.tenant_id == self.tenant_id,
                    PortalLoginAttempt.success == True,
                    PortalLoginAttempt.created_at >= start_date
                )
            )
        )
...
class PortalPreferencesRepository(BaseRepository[PortalPreferences]):
    ...
    async def find_by_theme(self, theme: str) -> List[PortalPreferences]:
        ...
        query = select(PortalPreferences).options(
            joinedload(PortalPreferences.account)
        ).where(
            and_(
                PortalPreferences.tenant_id == self.tenant_id,
    ...
    async def find_marketing_email_subscribers(self) -> List[PortalPreferences]:
        ...
        query = select(PortalPreferences).options(
            ...
            and_(
                PortalPreferences.tenant_id == self.tenant_id,
                PortalPreferences.marketing_emails == True,
                PortalPreferences.email_notifications == True
            )
        )
        result = await self.session.execute(query)
        return result.scalars().all()
    ...
    async def get_preference_statistics(self) -> Dict[str, Any]:
        ...
        notification_stats = await self.session.execute(
            select(
                func.count(PortalPreferences.id).filter(
                    PortalPreferences.email_notifications == True
                ).label('email_enabled'),
                func.count(PortalPreferences.id).filter(
                    PortalPreferences.sms_notifications == True  
                ).label('sms_enabled'),
                func.count(PortalPreferences.id).filter(
                    PortalPreferences.marketing_emails == True
