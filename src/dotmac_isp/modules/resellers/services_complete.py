...
class ResellerApplicationService:
    ...
    async def reject_application(
        self, application_id: str, reviewer_id: str, rejection_reason: str
    ) -> ResellerApplication:
        """Reject application with notification"""

        application = await self.repo.update_status(
            application_id, ApplicationStatus.REJECTED, reviewer_id, rejection_reason
        )
        if not application:
            raise ValueError(f"Application {application_id} not found")
