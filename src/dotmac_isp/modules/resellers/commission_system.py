...
from .db_models import (
    Reseller, ResellerCommission, ResellerCustomer,
    CommissionStructure
)
from .repositories import (
    ResellerRepository, ResellerCommissionRepository,
    ...
    def calculate_commission(
        ...
        if reseller.commission_structure == CommissionStructure.PERCENTAGE:
            commission = cls.calculate_percentage_commission(
                base_amount, 
                reseller.base_commission_rate or Decimal('0')
            )
        ...
        for commission_id in commission_ids:
            try:
                commission = await self.mark_commission_paid(
                    commission_id,
                    f"{batch_reference}-{commission_id}",
                    payment_method
                )
...
class CommissionReportGenerator:
    ...
    def _generate_performance_recommendations(
        ...
        churn_rate = summary['performance_metrics']['customers_churned'] / max(1, summary['performance_metrics']['customers_added'])
