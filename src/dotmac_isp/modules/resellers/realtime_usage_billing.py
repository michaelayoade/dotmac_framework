...
class UsageMetric:
    ...
    def __init__(self, tenant_id: str, metric_type: str, value: float, timestamp: Optional[datetime] = None):
...
class RealtimeUsageBillingEngine:
    ...
    async def track_usage(
        self, tenant_id: str, metric_type: str, value: float, metadata: Optional[dict] = None
    ) -> dict[str, Any]:
        """
        ...
        self.usage_buffer[tenant_id].append(
            {"metric": usage_metric, "metadata": metadata or {}, "tracked_at": datetime.now(timezone.utc)}
        )
        ...
        try:
            ...
            for item in tenant_buffer:
                ...
                if metric.metric_type not in usage_summary:
                    usage_summary[metric.metric_type] = {"count": 0, "total_value": 0, "total_cost": Decimal("0.00")}
            ...
            commission = await self.commission_service.create_commission_record(
                ...
                additional_data={
                    "usage_summary": {
                        k: {"count": v["count"], "total_value": v["total_value"], "total_cost": float(v["total_cost"])}
        ...
        for commission in usage_commissions:
            ...
            for metric_type, data in usage_data.items():
                if metric_type not in usage_breakdown:
                    usage_breakdown[metric_type] = {"total_cost": 0, "total_usage": 0, "count": 0}
...
class UsageBillingIntegrationService:
    ...
    async def get_realtime_usage_summary(self, tenant_id: str) -> dict[str, Any]:
        ...
        for item in buffered_usage:
            metric = item["metric"]
            if metric.metric_type not in usage_summary:
                usage_summary[metric.metric_type] = {"value": 0, "cost": Decimal("0.00")}
