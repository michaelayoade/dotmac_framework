...
from .db_models import (
    ...
    ResellerOpportunity, ResellerCommission,
    ApplicationStatus, ResellerStatus
)
...
class ResellerApplicationRepository(BaseRepository):
    ...
    async def list_by_status(
        ...
        query = select(ResellerApplication).where(
            ResellerApplication.status == status
        )
        if self.tenant_id:
            query = query.where(ResellerApplication.tenant_id == self.tenant_id)
    ...
    async def get_dashboard_data(self, reseller_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'commission_total_pending': sum(
                float(c.commission_amount) for c in reseller.commissions 
