...
class AdvancedCustomerManager:
    ...
    async def create_customer_action_plan(
        ...
        action_plan = {
            ...
            'current_state': {
                ...
                'monthly_value': float(customer.monthly_recurring_revenue),
                'days_since_last_interaction': lifecycle_summary.get('recent_interactions', {}).get('days_since_last', 0)
            },
            'objectives': [],
            'action_items': [],
            'milestones': [],
        ...
        revenue_analysis = {
            ...
            'customer_segments': {
                'enterprise': {
                    ...
                    'revenue': float(sum(c.monthly_recurring_revenue for c in customers if c.monthly_recurring_revenue >= 1000)),
                    'percentage_of_total': 0
                },
                'mid_market': {
                    'count': len([c for c in customers if 200 <= c.monthly_recurring_revenue < 1000]),
                    'revenue': float(sum(c.monthly_recurring_revenue for c in customers if 200 <= c.monthly_recurring_revenue < 1000)),
                    'percentage_of_total': 0
                },
                'small_business': {
                    'count': len([c for c in customers if c.monthly_recurring_revenue < 200]),
                    'revenue': float(sum(c.monthly_recurring_revenue for c in customers if c.monthly_recurring_revenue < 200)),
                    'percentage_of_total': 0
                }
            },
            'expansion_opportunities': [
                {
                    ...
                    'recommended_actions': ['Usage analysis', 'Needs assessment', 'Custom proposal']
                } for c in customers_by_value[:10] if c.monthly_recurring_revenue >= 200  # Top customers with expansion potential
            ][:5],  # Limit to top 5 opportunities
            'risk_analysis': {
                'revenue_at_risk': float(sum(c.monthly_recurring_revenue for c in customers if self._is_customer_at_risk(c))),
    ...
    def _calculate_mrr_by_segment(self, customers) -> Dict[str, float]:
        ...
        return {
            'enterprise': float(sum(c.monthly_recurring_revenue for c in customers if c.monthly_recurring_revenue >= 1000)),
            'mid_market': float(sum(c.monthly_recurring_revenue for c in customers if 200 <= c.monthly_recurring_revenue < 1000)),
            'small_business': float(sum(c.monthly_recurring_revenue for c in customers if c.monthly_recurring_revenue < 200))
        }
    ...
    async def _generate_portfolio_recommendations(
        ...
        if health_distribution.get('critical', 0) + health_distribution.get('poor', 0) > portfolio_summary['total_customers'] * 0.2:
    ...
    def _simulate_risk_factors(self) -> List[str]:
        ...
        import random
        all_factors = [
            'Low usage', 'Payment delays', 'Support tickets', 'Contract expiring',
            'No recent contact', 'Feature requests ignored', 'Competitor interest',
            'Reduced team size', 'Budget constraints', 'Technical issues'
        ]
