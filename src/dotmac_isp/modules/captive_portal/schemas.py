...
class CaptivePortalAuthRequest(BaseModel):
    """Captive portal authentication request with validation."""
    ...
    password: str = Field(
        ...
        min_length=1, 
        max_length=255, 
        description="Password or access code"
    )
    ...
    location: Optional[str] = Field(
        None, 
        max_length=500, 
        description="Portal location identifier"
    )
    ...
    @classmethod
    def validate_device_info(cls, v: dict[str, Any] | None) -> dict[str, Any] | None:
        if v is None:
            return v
        # Sanitize device info keys and limit size
        if len(v) > 20:
            raise ValueError('Too many device info fields')
        cleaned = {}
        for key, value in v.items():
            if len(str(key)) > 50 or len(str(value)) > 500:
