...
from dotmac_isp.shared.exceptions import ValidationError, EntityNotFoundError
from dotmac_shared.api.exception_handlers import standard_exception_handler

from ..models.alarms import Alarm, AlarmRule, AlarmSeverity, AlarmStatus, AlarmType
...
class AlarmManagementService(BaseTenantService):
    ...
    async def create_alarm(self, alarm_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if existing_alarm:
            ...
            logger.info(f"Updated existing alarm {existing_alarm.alarm_id}, occurrence count: {existing_alarm.occurrence_count}")
    ...
    async def acknowledge_alarm(
        ...
        # Create event
        await self._create_alarm_event(alarm, "alarm_acknowledged", {
    ...
    async def clear_alarm(
        ...
        # Create event
        await self._create_alarm_event(alarm, "alarm_cleared", {
    ...
    async def escalate_alarm(
        ...
        # Create event
        await self._create_alarm_event(alarm, "alarm_escalated", {
            "escalated_by": escalated_by,
            "from_severity": previous_severity,
            "to_severity": new_severity,
    ...
    async def suppress_alarm(
        ...
        logger.info(f"Suppressed alarm {alarm_id} by {suppressed_by} for {suppression_duration_hours or 'indefinite'} hours")
    ...
    def _find_existing_alarm(self, alarm_data: Dict[str, Any]) -> Optional[Alarm]:
        ...
        return self.db.query(Alarm).filter(
            and_(
                Alarm.tenant_id == self.tenant_id,
    ...
    async def _evaluate_rule_against_metrics(
        ...
        description = rule.alarm_description_template or f"{rule.metric_name} is {metric_value}, threshold: {threshold_value}"
