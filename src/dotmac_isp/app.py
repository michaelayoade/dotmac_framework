...
async def create_app(tenant_config: Optional[TenantConfig] = None) -> FastAPI:
    ...
    if tenant_config:
        ...
        service_name = f"isp-{tenant_config.tenant_id}"
    ...
    service_token_manager.register_service(
        ...
        service_info={
            ...
            "environment": environment,
            "capabilities": ["customer_management", "billing", "network_services", "support"],
            "tenant_specific": tenant_specific
        },
        allowed_targets=[
            "dotmac-shared", "dotmac-management", "dotmac-logging", "dotmac-metrics",
            "isp-billing", "isp-network", "isp-support"
        ]
    )
    ...
    logger.info(f"✅ ISP Framework application created with observability integration: {service_name}")
