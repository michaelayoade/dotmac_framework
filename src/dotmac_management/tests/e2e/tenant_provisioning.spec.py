...
class TestTenantProvisioningWorkflow:
    ...
    async def test_tenant_provisioning_migration_failure_recovery(
        ...
        logger.info(f"Migration call count: {migration_call_count}")
...
class TestTenantProvisioningAPI:
    ...
    async def test_get_tenant_provisioning_status_via_api(
        self, http_client, management_db_session: Session, tenant_factory
    ):
        ...
        response = await http_client.get(
            f"{os.getenv('TEST_MANAGEMENT_URL', 'http://localhost:8001')}/api/v1/tenants/{tenant.tenant_id}/status",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
