...
from .cleanup import TestIsolationValidator, E2ETestCleaner
...
Examples:
  ...
  python run_tests.py --coverage         # Run with coverage reporting
    ...
    test_group.add_argument(
        "--provisioning",
        action="store_true",
        help="Run tenant provisioning tests only"
    )
    test_group.add_argument(
        "--lifecycle", 
        action="store_true",
        help="Run container lifecycle tests only"
    )
    test_group.add_argument(
        "--isolation",
        action="store_true", 
        help="Run tenant isolation tests only"
    )
    test_group.add_argument(
        "--cleanup-only",
        action="store_true",
        help="Run cleanup validation tests only"
    )
    ...
    parser.add_argument(
        "--parallel",
        type=int,
        help="Number of parallel test workers (default: auto)"
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument(
        "--junit-xml",
        help="Path for JUnit XML report output"
    )
    parser.add_argument(
        "--report-file",
        help="Path for test execution report"
    )
