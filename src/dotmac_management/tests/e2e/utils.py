            ...
            while time.time() - start_time < timeout:
                try:
                    response = await client.get(
                        f"{container_url}{health_endpoint}",
                        timeout=10
                    )
            ...
            try:
                ...
                if response.status_code != 403:  # Should be forbidden
                    ...
                    results["cross_tenant_access"].append({
                        "attempted_access": f"tenant_a_token -> tenant_b_api",
                ...
                if response.status_code != 403:
                    results["isolated"] = False
                    results["cross_tenant_access"].append({
                        "attempted_access": f"tenant_b_token -> tenant_a_api",
                        "status_code": response.status_code,
                        "response": response.text[:200]
                    })
...
async def performance_monitor(test_name: str):
    ...
    try:
        ...
        logger.info(f"  Memory delta: N/A")  # Would calculate actual memory usage
