...
class TestContainerScaling:
    ...
    async def test_horizontal_container_scaling_up(
        ...
        high_load_scenario = ContainerLifecycleDataFactory.create_scaling_scenario(
            base_container_id
        )[0]  # Get base container with high load
        ...
        async with performance_monitor("container_scaling_up"):
            ...
            await expect(management_page.locator("[data-testid=cpu-usage]")).to_contain_text(
                "85%", timeout=10000
            )
            ...
            await expect(management_page.locator("[data-testid=replica-count]")).to_contain_text(
                "3", timeout=10000  
            )
            await expect(management_page.locator("[data-testid=cpu-usage]")).to_contain_text(
                "30%", timeout=5000
    ...
    async def test_data_archival_before_deprovisioning(
        ...
        tenant = tenant_factory(
            ...
            settings={
                ...
                "archive_before_deprovision": True
            }
        )
...
class TestContainerMonitoringAndAlerts:
    ...
    async def test_container_performance_monitoring(
        ...
        metrics = await ContainerTestUtils.monitor_container_resources(
            container_id, duration=10  # 10 seconds
        )
