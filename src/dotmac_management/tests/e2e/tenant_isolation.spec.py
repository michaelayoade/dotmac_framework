...
from .factories import ApiTestDataFactory
from .utils import (
    ApiTestUtils,
    DatabaseTestUtils,
...
class TestDatabaseIsolation:
        ...
        async with performance_monitor("database_schema_isolation"):
            ...
            if "tenant_a" in tenant_db_sessions and "tenant_b" in tenant_db_sessions:
                ...
                tenant_a_session.execute(
                    text(
                        "CREATE TABLE IF NOT EXISTS customers (id SERIAL PRIMARY KEY, tenant_id TEXT, name TEXT, email TEXT)"
                    )
                )
                tenant_a_session.execute(
                    text(
    ...
    async def test_tenant_connection_pool_isolation(
        ...
        if "tenant_a" in tenant_db_sessions and "tenant_b" in tenant_db_sessions:
            ...
            try:
                ...
                tenant_a_session.execute(
                    text(
                        "CREATE TABLE IF NOT EXISTS connection_test (id SERIAL PRIMARY KEY, tenant_id TEXT, data TEXT)"
                    )
                )
                tenant_a_session.execute(
                    text(
    ...
    async def test_tenant_data_encryption_isolation(
        ...
        if "tenant_a" in tenant_db_sessions and "tenant_b" in tenant_db_sessions:
            ...
            tenant_a_session.execute(
                text(
                    "CREATE TABLE IF NOT EXISTS encrypted_data (id SERIAL PRIMARY KEY, tenant_id TEXT, encrypted_value TEXT)"
                )
            )
            tenant_a_session.execute(
                text(
...
class TestAPIIsolation:
    ...
    async def test_tenant_authentication_boundaries(self, http_client, tenant_factory):
        """Test authentication boundaries between tenants."""

        tenant_a = tenant_factory(company_name="Auth Boundary A", subdomain="auth_a")
    ...
    async def test_tenant_jwt_token_isolation(self, http_client, tenant_factory):
        """Test JWT token isolation between tenants."""

        tenant_a = tenant_factory(company_name="JWT Isolation A", subdomain="jwt_a")

        tenant_b = tenant_factory(company_name="JWT Isolation B", subdomain="jwt_b")
...
class TestUIIsolation:
    ...
    async def test_tenant_ui_access_isolation(
        self, browser_context, tenant_page_factory, tenant_factory, isolation_test_data
    ):
...
class TestResourceIsolation:
    ...
    async def test_tenant_memory_isolation(self, tenant_factory, mock_coolify_client):
    ...
    async def test_tenant_cpu_isolation(self, tenant_factory, mock_coolify_client):
    ...
    async def test_tenant_storage_isolation(self, tenant_factory, mock_coolify_client):
...
class TestSecurityIsolation:
    ...
    async def test_tenant_audit_log_isolation(
        ...
        tenant_b = tenant_factory(company_name="Audit Log B", subdomain="audit_b")
        ...
        if "tenant_a" in tenant_db_sessions and "tenant_b" in tenant_db_sessions:
            ...
            tenant_a_session.execute(
                text(
                    "CREATE TABLE IF NOT EXISTS audit_logs (id SERIAL PRIMARY KEY, tenant_id TEXT, action TEXT, timestamp TIMESTAMP)"
                )
            )
            tenant_a_session.execute(
                text(
