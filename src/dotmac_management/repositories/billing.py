...
class InvoiceRepository(BaseRepository[Invoice]):
    ...
    async def get_unpaid_invoices(
        ...
        query = (
            select(Invoice)
            .join(Subscription)
            .where(Invoice.status != InvoiceStatus.PAID, Invoice.is_deleted == False)
        )
        if tenant_id:
...
class UsageRecordRepository(BaseRepository[UsageRecord]):
    ...
    async def get_unbilled_usage(
        ...
        filters = {"subscription_id": subscription_id, "billed": False}

        query = select(UsageRecord).where(
            UsageRecord.subscription_id == subscription_id, UsageRecord.billed == False
        )
        if end_date:
