...
class FileRepository(BaseRepository):
    ...
    async def create_file_metadata(
        ...
        try:
            ...
            raise ValidationError(f"Failed to create file metadata: {str(e)}")

    async def get_file_by_id(self, file_id: str, tenant_id: str) -> Optional[FileMetadata]:
        """Get file metadata by ID."""
        return await self.session.query(FileMetadata).filter(
            and_(
    ...
    async def update_file_metadata(
        ...
        try:
            ...
            raise ValidationError(f"Failed to update file metadata: {str(e)}")
    ...
    async def delete_file_metadata(
        ...
        try:
            ...
            raise ValidationError(f"Failed to delete file metadata: {str(e)}")
    ...
    async def create_file_permission(
        ...
        try:
            ...
            raise ValidationError(f"Failed to create file permission: {str(e)}")
    ...
    async def get_file_permissions(
        ...
        return await self.session.query(FilePermission).filter(
            and_(
    ...
    async def get_file_access_logs(
        ...
        return await self.session.query(FileAccessLog).filter(
            and_(
                FileAccessLog.file_id == file_id,
                FileAccessLog.tenant_id == tenant_id
            )
        ).order_by(FileAccessLog.access_timestamp.desc()).limit(limit).all()
    ...
    async def create_upload_session(
        ...
        try:
            ...
            raise ValidationError(f"Failed to create upload session: {str(e)}")
    ...
    async def get_upload_session(
        ...
        return await self.session.query(FileUploadSession).filter(
            and_(
    ...
    async def update_upload_progress(
        ...
        try:
            ...
            raise ValidationError(f"Failed to update upload progress: {str(e)}")
    ...
    async def get_tenant_file_stats(self, tenant_id: str) -> Dict:
        ...
        try:
            # Basic counts
            total_files = await self.session.query(func.count(FileMetadata.id)).filter(
                FileMetadata.tenant_id == tenant_id
            ).scalar()
            
            total_size = await self.session.query(func.sum(FileMetadata.file_size)).filter(
                FileMetadata.tenant_id == tenant_id
            ).scalar() or 0
            
            # Files by category
            category_stats = await self.session.query(
            ...
            raise ValidationError(f"Failed to get tenant file stats: {str(e)}")
    ...
    async def cleanup_expired_upload_sessions(self) -> int:
        ...
        try:
            ...
            raise ValidationError(f"Failed to cleanup expired upload sessions: {str(e)}")