...
class DomainRepository(BaseRepository):
    ...
    async def create_domain(
        ...
        try:
            ...
            raise ValidationError(f"Failed to create domain: {str(e)}")

    async def get_domain_by_id(self, domain_id: str, tenant_id: str) -> Optional[Domain]:
        """Get domain by ID."""
        return await self.session.query(Domain).filter(
            and_(
    ...
    async def create_dns_record(
        ...
        try:
            ...
            raise ValidationError(f"Failed to create DNS record: {str(e)}")
    ...
    async def get_dns_record_by_id(
        ...
        return await self.session.query(DNSRecord).filter(
            and_(
                DNSRecord.record_id == record_id,
                DNSRecord.tenant_id == tenant_id
            )
        ).first()
    ...
    async def update_dns_record(
        ...
        try:
            ...
            raise ValidationError(f"Failed to update DNS record: {str(e)}")
    ...
    async def delete_dns_record(
        ...
        try:
            ...
            raise ValidationError(f"Failed to delete DNS record: {str(e)}")
    ...
    async def create_ssl_certificate(
        ...
        try:
            ...
            raise ValidationError(f"Failed to create SSL certificate: {str(e)}")
    ...
    async def get_domain_ssl_certificates(
        ...
        return await self.session.query(SSLCertificate).filter(
            and_(
                SSLCertificate.domain_id == domain_id,
                SSLCertificate.tenant_id == tenant_id
            )
        ).order_by(SSLCertificate.created_at.desc()).all()
    ...
    async def create_domain_verification(
        ...
        try:
            ...
            raise ValidationError(f"Failed to create domain verification: {str(e)}")
    ...
    async def update_verification_status(
        ...
        verification = await self.session.query(DomainVerification).filter(
            and_(
        ...
        try:
            ...
            raise ValidationError(f"Failed to update verification status: {str(e)}")
    ...
    async def log_domain_action(
        ...
        try:
            ...
            raise ValidationError(f"Failed to log domain action: {str(e)}")
    ...
    async def get_domain_logs(
        ...
        return await self.session.query(DomainLog).filter(
            and_(
                DomainLog.domain_id == domain_id,
                DomainLog.tenant_id == tenant_id
            )
        ).order_by(DomainLog.log_timestamp.desc()).limit(limit).all()
    ...
    async def create_dns_zone(
        ...
        try:
            ...
            raise ValidationError(f"Failed to create DNS zone: {str(e)}")
    ...
    async def get_tenant_domain_stats(self, tenant_id: str) -> Dict:
        ...
        try:
            # Total domains
            total_domains = await self.session.query(func.count(Domain.id)).filter(
                Domain.tenant_id == tenant_id
            ).scalar()
            
            # Domains by status
            status_stats = await self.session.query(
                Domain.domain_status,
                func.count(Domain.id)
            ).filter(
                Domain.tenant_id == tenant_id
            ).group_by(Domain.domain_status).all()
            
            # SSL certificates by status
            ssl_stats = await self.session.query(
                SSLCertificate.ssl_status,
                func.count(SSLCertificate.id)
            ).join(Domain).filter(
                Domain.tenant_id == tenant_id
            ...
            expiring_ssl = await self.session.query(SSLCertificate).join(Domain).filter(
                and_(
                    Domain.tenant_id == tenant_id,
            ...
            # DNS providers usage
            provider_stats = await self.session.query(
                Domain.dns_provider,
                func.count(Domain.id)
            ).filter(
                Domain.tenant_id == tenant_id
            ).group_by(Domain.dns_provider).all()
            ...
            raise ValidationError(f"Failed to get tenant domain stats: {str(e)}")
    ...
    async def cleanup_expired_verifications(self) -> int:
        ...
        try:
            ...
            raise ValidationError(f"Failed to cleanup expired verifications: {str(e)}")
