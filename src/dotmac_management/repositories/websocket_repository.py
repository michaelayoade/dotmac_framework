...
class WebSocketEventRepository(BaseRepository):
        ...
        query = self.session.query(WebSocketEvent).filter(
            and_(
                ...
                or_(
                    ...
                    WebSocketEvent.broadcast_to_tenant is True,
                ),
            )
        )
    ...
    async def update_connection_activity(self, connection_id: str, activity_data: dict) -> WebSocketConnection:
        ...
        connection = (
            ...
            .filter(WebSocketConnection.connection_id == connection_id)
            .first()
        )
    ...
    async def disconnect_connection(self, connection_id: str) -> bool:
        ...
        try:
            ...
            raise ValidationError(f"Failed to disconnect connection: {str(e)}") from e
    ...
    async def update_delivery_status(
        ...
        try:
            ...
            raise ValidationError(f"Failed to update delivery status: {str(e)}") from e
    ...
    async def record_metrics(self, tenant_id: str, metric_data: dict) -> WebSocketMetrics:
        ...
        try:
            ...
            raise ValidationError(f"Failed to record metrics: {str(e)}") from e
    ...
    async def get_tenant_event_stats(self, tenant_id: str, days: int = 7) -> dict:
        ...
        try:
            ...
            raise ValidationError(f"Failed to get tenant event stats: {str(e)}") from e
    ...
    async def cleanup_expired_events(self) -> int:
        ...
        try:
            ...
            raise ValidationError(f"Failed to cleanup expired events: {str(e)}") from e
    ...
    async def cleanup_old_connections(self, hours: int = 24) -> int:
        ...
        try:
            ...
            raise ValidationError(f"Failed to cleanup old connections: {str(e)}") from e
    ...
    async def cleanup_old_deliveries(self, days: int = 30) -> int:
        ...
        try:
            ...
            raise ValidationError(f"Failed to cleanup old deliveries: {str(e)}") from e
