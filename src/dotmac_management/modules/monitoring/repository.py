...
class ServiceComponentRepository(BaseRepository[ServiceComponent]):
    ...
    async def find_by_name(self, name: str) -> Optional[ServiceComponent]:
        ...
        return self.db.query(ServiceComponent).filter(
            and_(ServiceComponent.tenant_id == self.tenant_id, ServiceComponent.name == name)
        ).first()
    ...
    async def find_by_type(self, component_type: str) -> List[ServiceComponent]:
        ...
        return self.db.query(ServiceComponent).filter(
            and_(
                ...
                ServiceComponent.is_active == True
            )
        ).all()
    ...
    async def get_active_components(self) -> List[ServiceComponent]:
        ...
        return self.db.query(ServiceComponent).filter(
            and_(ServiceComponent.tenant_id == self.tenant_id, ServiceComponent.is_active == True)
        ).all()
    ...
    async def get_critical_components(self) -> List[ServiceComponent]:
        ...
        return self.db.query(ServiceComponent).filter(
            and_(
                ServiceComponent.tenant_id == self.tenant_id,
                ServiceComponent.is_critical == True,
                ServiceComponent.is_active == True
            )
        ).all()
    ...
    async def get_components_with_health_checks(self, limit: int = 100) -> List[ServiceComponent]:
        ...
        return self.db.query(ServiceComponent).options(
            joinedload(ServiceComponent.health_checks)
        ).filter(
            ServiceComponent.tenant_id == self.tenant_id
        ).limit(limit).all()
...
class HealthCheckRepository(BaseRepository[HealthCheck]):
    ...
    async def create_health_check(
        self, component_id: UUID, status: HealthCheckStatus, response_time_ms: float = None,
        status_code: int = None, error_message: str = None, details: Dict[str, Any] = None,
        metrics: Dict[str, Any] = None, check_duration_ms: float = None
    ) -> HealthCheck:
    ...
    async def get_latest_check_for_component(self, component_id: UUID) -> Optional[HealthCheck]:
        ...
        ).order_by(desc(HealthCheck.check_timestamp)).first()
    ...
    async def get_failed_checks(self, hours: int = 24) -> List[HealthCheck]:
        ...
        ).order_by(desc(HealthCheck.check_timestamp)).all()
    ...
    async def get_component_uptime(self, component_id: UUID, hours: int = 24) -> Dict[str, Any]:
        ...
        healthy_checks = self.db.query(HealthCheck).filter(
            and_(
                HealthCheck.tenant_id == self.tenant_id,
                HealthCheck.component_id == component_id,
                HealthCheck.check_timestamp >= cutoff_time,
...
class SystemMetricRepository(BaseRepository[SystemMetric]):
    ...
    async def create_metric(
        self, metric_name: str, metric_value: float, unit: str = None, source: str = None,
        host: str = None, tags: Dict[str, Any] = None, dimensions: Dict[str, Any] = None,
        context: Dict[str, Any] = None, timestamp: datetime = None
    ) -> SystemMetric:
...
class PerformanceMetricRepository(BaseRepository[PerformanceMetric]):
    ...
    async def create_performance_metric(
        ...
        user_id: UUID = None, session_id: str = None, request_size_bytes: int = None,
        response_size_bytes: int = None, database_query_count: int = None,
        database_query_time_ms: float = None, cache_hits: int = 0, cache_misses: int = 0,
        errors: List[Dict[str, Any]] = None, metadata: Dict[str, Any] = None,
    ...
    async def get_slow_requests(self, threshold_ms: float = 1000, hours: int = 24) -> List[PerformanceMetric]:
        ...
        ).order_by(desc(PerformanceMetric.response_time_ms)).all()
...
class MonitoringAlertRepository(BaseRepository[MonitoringAlert]):
    ...
    async def get_active_alerts(self) -> List[MonitoringAlert]:
        ...
        return self.db.query(MonitoringAlert).filter(
            and_(
                ...
                MonitoringAlert.is_resolved == False
            )
        ).order_by(desc(MonitoringAlert.triggered_at)).all()
    ...
    async def resolve_alert(self, alert_id: UUID, resolution_notes: str = None) -> bool:
        """Mark an alert as resolved."""
        result = self.db.query(MonitoringAlert).filter(
    ...
    async def get_alert_statistics(self, days: int = 7) -> Dict[str, Any]:
        ...
        active_alerts = self.db.query(MonitoringAlert).filter(
            and_(
                MonitoringAlert.tenant_id == self.tenant_id,
                MonitoringAlert.is_active == True,
                MonitoringAlert.is_resolved == False
        ...
        critical_alerts = self.db.query(MonitoringAlert).filter(
            and_(
                MonitoringAlert.tenant_id == self.tenant_id,
                MonitoringAlert.severity == "critical",
                MonitoringAlert.triggered_at >= cutoff_time
        ...
        resolved_alerts = self.db.query(MonitoringAlert).filter(
            and_(
                MonitoringAlert.tenant_id == self.tenant_id,
                MonitoringAlert.is_resolved == True,
...
class AlertEventRepository(BaseRepository[AlertEvent]):
    ...
    async def create_event(
        self, alert_id: UUID, event_type: str, previous_state: str = None, new_state: str = None,
        metric_value: float = None, threshold_value: float = None, message: str = None,
        notification_sent: bool = False, notification_channels: List[str] = None,
    ...
    async def get_events_for_alert(self, alert_id: UUID, limit: int = 100) -> List[AlertEvent]:
        ...
        ).order_by(desc(AlertEvent.event_timestamp)).limit(limit).all()
...
class MonitoringDashboardRepository(BaseRepository[MonitoringDashboard]):
    ...
    async def find_by_name(self, name: str) -> Optional[MonitoringDashboard]:
        ...
        return self.db.query(MonitoringDashboard).filter(
            and_(MonitoringDashboard.tenant_id == self.tenant_id, MonitoringDashboard.name == name)
        ).first()

    async def get_public_dashboards(self) -> List[MonitoringDashboard]:
        """Get all public dashboards."""
        return self.db.query(MonitoringDashboard).filter(
            and_(MonitoringDashboard.tenant_id == self.tenant_id, MonitoringDashboard.is_public == True)
        ).all()
    ...
    async def get_default_dashboard(self) -> Optional[MonitoringDashboard]:
        ...
        return self.db.query(MonitoringDashboard).filter(
            and_(
                ...
                MonitoringDashboard.is_default == True
            )
        ).first()
    ...
    async def set_as_default(self, dashboard_id: UUID) -> bool:
        ...
        # Set the new default
        result = self.db.query(MonitoringDashboard).filter(
            and_(MonitoringDashboard.tenant_id == self.tenant_id, MonitoringDashboard.id == dashboard_id)
...
class MetricThresholdRepository(BaseRepository[MetricThreshold]):
    ...
    async def find_by_metric_name(self, metric_name: str) -> List[MetricThreshold]:
        ...
        return self.db.query(MetricThreshold).filter(
            and_(
    ...
    async def get_active_thresholds(self) -> List[MetricThreshold]:
        ...
        return self.db.query(MetricThreshold).filter(
            and_(MetricThreshold.tenant_id == self.tenant_id, MetricThreshold.is_active == True)
        ).all()
