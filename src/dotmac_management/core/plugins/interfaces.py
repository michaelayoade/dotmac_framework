...
class NotificationChannelPlugin(BasePlugin):
    ...
    async def send_notification(
        self, message: str, recipients: List[str], options: Dict[str, Any] = None
    ) -> bool:
...
class PaymentProviderPlugin(BillablePlugin):
    ...
    async def cancel_subscription(
        self, subscription_id: str, reason: str = None
    ) -> bool:
        """Cancel subscription."""
        pass
    ...
    async def refund_payment(
        self, transaction_id: str, amount: Decimal, reason: str = None
    ) -> Dict[str, Any]:
