...
class PluginHealthMonitor:
    ...
    async def start_monitoring(self, check_interval: int = None) -> bool:
    ...
    def get_unhealthy_plugins(self) -> List[PluginHealthReport]:
        ...
        return [
            report for report in self.health_reports.values()
            if report.overall_status == HealthStatus.UNHEALTHY
        ]
    ...
    @standard_exception_handler
    async def force_health_check(self, plugin_name: str = None) -> Dict[str, PluginHealthReport]:
        """Force an immediate health check for specific plugin or all plugins."""
        if plugin_name:
            # Check specific plugin
            provider = (
                self.infrastructure_manager.get_deployment_provider(plugin_name) or
                self.infrastructure_manager.get_dns_provider(plugin_name)
            )
            
            if not provider:
                raise ValueError(f"Plugin not found: {plugin_name}")
            
            plugin_type = "deployment" if plugin_name in self.infrastructure_manager.list_deployment_providers() else "dns"
