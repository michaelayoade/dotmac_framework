...
class ManagementBootstrap:
    ...
    async def bootstrap_if_needed(self) -> bool:
        ...
        if success:
            logger.warning("🔐 SECURITY NOTICE: Remove AUTH_ADMIN_EMAIL and AUTH_INITIAL_ADMIN_PASSWORD from environment after first login!")
    ...
    async def _perform_bootstrap(self, admin_email: str, admin_password: str) -> bool:
        ...
        try:
            with get_db_session() as db:
                ...
                logger.info(f"✅ Management platform bootstrap completed successfully")
                logger.info(f"🔑 Admin user created: {admin_email}")
                logger.info(f"🌐 Management portal ready")
        ...
        roles_data = [
            {
                ...
                "permissions": ["platform.admin", "tenant.create", "tenant.read", 
                               "tenant.update", "tenant.delete", "tenant.provision",
                               "user.create", "user.read", "user.update", "user.delete",
                               "user.impersonate", "platform.settings", "platform.monitoring",
                               "platform.billing", "system.deploy", "system.backup", "system.logs"]
            },
            {
                "name": "platform_admin", 
                "description": "Platform administrator",
                "permissions": ["tenant.create", "tenant.read", "tenant.update", 
                               "tenant.provision", "user.create", "user.read", "user.update",
                               "platform.monitoring", "platform.billing"]
            },
            {
                "name": "support_admin",
    ...
    async def _create_management_tenant(self, db: Session, admin_user: User) -> ManagementTenant:
        ...
        existing = db.query(ManagementTenant).filter_by(
