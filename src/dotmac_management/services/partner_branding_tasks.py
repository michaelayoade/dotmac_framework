...
from dotmac_shared.tasks.decorators import (
    task, background_task, high_priority_task, TaskExecutionContext
)
...
class PartnerBrandingTaskService:
    ...
    async def generate_comprehensive_brand_assets(
        ...
        async with TaskExecutionContext(
            ...
            try:
                ...
                await ctx.update_progress(100, f"Brand assets generated successfully in {result['processing_time']:.1f}s")
                
                logger.info(f"Brand assets generated successfully", extra={
                    'brand_config_id': brand_config_id,
                ...
                logger.error(f"Brand asset generation failed", extra={
                    'brand_config_id': brand_config_id,
                    'error': str(e),
                    'processing_time': result['processing_time']
                })
    ...
    async def process_uploaded_logo(
        ...
        async with TaskExecutionContext(
            ...
            try:
                ...
                raise ValueError(f"Invalid logo data: {e}")
            ...
            return {
                ...
                'processing_metadata': {
                    ...
                    'formats_generated': list(set(v['format'] for v in processed_variants.values()))
                }
            }
    ...
    async def validate_brand_compliance(
        ...
        async with TaskExecutionContext(
            ...
            await ctx.update_progress(100, f"Brand compliance validation completed - Score: {validation_results['overall_score']:.1f}%")
    ...
    async def _generate_advanced_color_palettes(
        ...
        color_assets = {
            ...
            'secondary_palette': self._generate_color_shades(brand_config.secondary_color) if brand_config.secondary_color else None,
            'accent_palette': self._generate_color_shades(brand_config.accent_color) if brand_config.accent_color else None,
        }
        ...
        try:
            ...
            for level in shade_levels:
                ...
                if level <= 500:
                    ...
                    new_l = min(1.0, l + (1 - l) * lightness_factor * 0.8)
                else:
                    # Darker shades
                    darkness_factor = (level - 500) / 500
                    new_l = max(0.0, l - l * darkness_factor * 0.9)
    ...
    def _generate_complementary_colors(self, primary_color: str) -> Dict[str, str]:
        ...
        try:
            ...
            tri1_r, tri1_g, tri1_b = colorsys.hls_to_rgb(tri1_h, l, s)
            tri2_r, tri2_g, tri2_b = colorsys.hls_to_rgb(tri2_h, l, s)
            ...
            ana1_r, ana1_g, ana1_b = colorsys.hls_to_rgb(ana1_h, l, s)
            ana2_r, ana2_g, ana2_b = colorsys.hls_to_rgb(ana2_h, l, s)
