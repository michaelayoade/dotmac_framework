...
from dotmac_shared.tasks.decorators import (
    background_task, scheduled_task, TaskExecutionContext
)
from dotmac_shared.tasks.engine import TaskConfig, TaskPriority
from dotmac_shared.tasks.workflow import Workflow, WorkflowStep, StepType
from dotmac_shared.database.base import get_db_session
from dotmac_management.models.commission_config import CommissionConfig, CommissionTier
from dotmac_management.models.partner import Partner
from dotmac_shared.billing.core.models import Invoice, InvoiceStatus
...
class CommissionBatchProcessor:
    ...
    async def process_commission_batch(
        ...
        async with TaskExecutionContext(
            ...
            await ctx.update_progress(5, f"Starting commission batch {batch_id} with {len(transaction_data)} records")
            ...
            try:
                ...
                if self.config.enable_validation:
                    ...
                    batch_result.validation_results = await self._validate_batch_results(
                        batch_result, validated_data
                    )
                ...
                logger.info(f"Commission batch processing completed", extra={
                    'batch_id': batch_id,
                ...
                logger.error(f"Commission batch processing failed", extra={
                    'batch_id': batch_id,
                    'error': str(e),
                    'processing_time': batch_result.processing_time_seconds
                })
        ...
        async with TaskExecutionContext(
            ...
            return {
                ...
                'processing_summary': {
                    ...
                    'success_rate': batch_result.successful_calculations / batch_result.total_records if batch_result.total_records > 0 else 0
                }
            }
    ...
    async def auto_process_monthly_commissions(self, task_context: Dict = None) -> Dict[str, Any]:
        """
    ...
    async def _load_commission_configurations(
        ...
        partner_ids = list(set(t['partner_id'] for t in transaction_data))
        commission_configs = {}
        ...
        with get_db_session() as db:
            for partner_id in partner_ids:
                ...
                # Load from database
                config = db.query(CommissionConfig).filter_by(
    ...
    async def _generate_partner_summaries(
        ...
        partner_summaries = defaultdict(lambda: {
    ...
    def _update_processing_stats(self, batch_result: BatchProcessingResult):
        ...
        self._processing_stats['average_processing_time'] = (
            (current_avg * (batch_count - 1) + batch_result.processing_time_seconds) / batch_count
        )
        ...
        if batch_result.total_records > 0:
            ...
            self._processing_stats['error_rate'] = (
                (current_error_rate * (total_processed - batch_result.processed_records) + 
                 batch_error_rate * batch_result.processed_records) / total_processed
            )
