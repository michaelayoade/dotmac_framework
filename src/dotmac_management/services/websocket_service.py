...
class WebSocketEventService:
    ...
    async def register_connection(
        ...
        try:
            ...
            ok = await send_ws(websocket, welcome_message, logger=logger)
            if not ok:
            ...
            logger.exception("Failed to register WebSocket connection")
            if connection_id in self.active_connections:
                del self.active_connections[connection_id]
            raise ValidationError(f"Failed to register connection: {str(e)}") from e
    ...
    async def unregister_connection(self, connection_id: str) -> bool:
        ...
        try:
            ...
            for _user_id, conn_set in self.user_connections.items():
                conn_set.discard(connection_id)

            # Remove from tenant tracking
            for _tenant_id, conn_set in self.tenant_connections.items():
                conn_set.discard(connection_id)
    ...
    async def _deliver_event(self, event: WebSocketEvent) -> bool:
        ...
        try:
            ...
            for connection_id in target_connections:
                ...
                if websocket:
                    ok = await send_ws(websocket, message, logger=logger)
                    if ok:
                        delivery_data = {
                            "event_id": event.event_id,
                            "connection_id": connection_id,
            ...
            if delivered_count > 0:
                await self.repository.update_event_delivery_status(
                    event.event_id, event.tenant_id, DeliveryStatus.DELIVERED
                )
            else:
                await self.repository.update_event_delivery_status(
                    event.event_id,
            ...
            logger.exception("Failed to deliver event %s", event.event_id)
            await self.repository.update_event_delivery_status(
                event.event_id, event.tenant_id, DeliveryStatus.FAILED, str(e)
            )
            return False
    ...
    async def acknowledge_event(self, event_id: str, tenant_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            logger.exception("Failed to acknowledge event %s", event_id)
            raise ValidationError(f"Failed to acknowledge event: {str(e)}") from e
    ...
    async def get_user_events(
        ...
        events = await self.repository.get_events_for_user(
            user_id=user_id, tenant_id=tenant_id, event_types=event_types, limit=limit
        )
    ...
    async def process_pending_events(self, tenant_id: Optional[str] = None) -> int:
        ...
        try:
            ...
            logger.exception("Failed to process pending events")
            return 0
    ...
    async def broadcast_tenant_update(
        self, tenant_id: str, update_type: str, update_data: dict, user_id: str
    ) -> WebSocketEvent:
    ...
    async def broadcast_network_alert(
        self, tenant_id: str, alert_data: dict, user_id: str, severity: str = "medium"
    ) -> WebSocketEvent:
        ...
        return await self.broadcast_to_tenant(
            ...
            event_data={"severity": severity, "alert": alert_data},
            user_id=user_id,
            priority=priority,
        )
