...
class EnhancedTenantProvisioningService(TenantProvisioningService):
    ...
    async def provision_multi_app_tenant(
        ...
        try:
            ...
            if result.success:
                logger.info(
                    f"Multi-app provisioning completed successfully for tenant {config.tenant_id}: {result.get_deployment_summary()}"
                )
            else:
                logger.warning(
                    f"Multi-app provisioning completed with issues for tenant {config.tenant_id}: {result.get_deployment_summary()}"
                )
            ...
            logger.error(
                f"Multi-app provisioning failed for tenant {config.tenant_id}: {e}"
            )
    ...
    def _convert_to_legacy_provisioning_request(
        ...
        legacy_config = config.to_legacy_tenant_config()
