...
class FileService:
    ...
    async def create_file(
        ...
        try:
            ...
            if file_content:
                ...
                # Calculate hashes - MD5 for legacy compatibility only (not for security)
    ...
    async def get_file_content(self, file_id: str, tenant_id: str, user_id: str) -> tuple[bytes, str, str]:
        ...
        try:
            ...
            raise ValidationError(f"Failed to read file content: {str(e)}") from e

    async def update_file(
        self, file_id: str, tenant_id: str, user_id: str, updates: FileMetadataUpdate
    ) -> FileMetadataResponse:
    ...
    async def search_files(
        ...
        files, total = await self.file_repository.search_files(
            tenant_id=tenant_id, filters=filters, skip=skip, limit=size
        )
    ...
    async def get_user_files(
        self, tenant_id: str, user_id: str, page: int = 1, size: int = 50
    ) -> tuple[list[FileMetadataResponse], int]:
        """Get files owned by a user."""
        skip = (page - 1) * size

        files, total = await self.file_repository.get_files_by_owner(
            tenant_id=tenant_id, owner_user_id=user_id, skip=skip, limit=size
        )
    ...
    async def create_upload_session(self, tenant_id: str, user_id: str, session_data: FileUploadSessionCreate):
        ...
        return await self.file_repository.create_upload_session(
            tenant_id=tenant_id, session_data=session_dict, user_id=user_id
        )
