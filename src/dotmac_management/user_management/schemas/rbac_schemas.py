...
from ..models.rbac_models import PermissionType, PermissionScope
...
class RoleCreateSchema(BaseCreateSchema):
    ...
    # Multi-app support
    app_scope: Optional[str] = Field(None, description="Application scope for this role (e.g., 'isp', 'crm', 'ecommerce')")
...
class PermissionCreateSchema(BaseCreateSchema):
    ...
    def validate_name_format(cls, v: str) -> str:
        ...
        if not all(c.isalnum() or c in "_:.-" for c in v):
            raise ValueError("Permission name can only contain letters, numbers, underscores, colons, hyphens, and dots")
        return v.lower()
