...
from dotmac_shared.common.schemas import BaseCreateSchema
from dotmac_shared.database.session import get_db_session
from dotmac_shared.observability.logging import get_logger

from ..models.auth_models import UserPasswordModel, UserSessionModel, UserMFAModel, UserAPIKeyModel, AuthAuditModel
from ..models.user_models import UserModel
from ..repositories.auth_repository import (
    AuthRepository, SessionRepository, MFARepository, 
    ApiKeyRepository, AuthAuditRepository
)
from ..repositories.user_repository import UserRepository
from ..schemas.auth_schemas import (
    LoginRequestSchema, LoginResponseSchema, LoginAttemptResult,
    SessionCreateSchema, SessionResponseSchema, SessionStatus,
    MFASetupRequestSchema, MFASetupResponseSchema, MFAVerifyRequestSchema,
    APIKeyCreateRequestSchema, APIKeyResponseSchema,
    PasswordChangeRequestSchema, AuthAuditEventType, AuthEventSchema
)
...
class AuthService(BaseService):
    ...
    async def authenticate_user(
        ...
        if not user:
            await self._log_auth_event(
                None, AuthAuditEventType.LOGIN_FAILED, 
                {"reason": "user_not_found", "username": username}, client_ip
            )
        ...
        if user.status != "active":
            await self._log_auth_event(
                user.id, AuthAuditEventType.LOGIN_FAILED,
                {"reason": "account_inactive", "status": user.status}, client_ip
            )
            return LoginAttemptResult(
                success=False,
        ...
        if await self._is_account_locked(user.id):
            await self._log_auth_event(
                user.id, AuthAuditEventType.LOGIN_FAILED,
                {"reason": "account_locked"}, client_ip
            )
            return LoginAttemptResult(
                success=False,
        ...
        await self._log_auth_event(
            user.id, AuthAuditEventType.LOGIN_SUCCESS,
            {"session_id": str(session.id)}, client_ip
        )
    ...
    async def verify_mfa_and_complete_login(
        ...
        if not mfa_valid:
            await self._log_auth_event(
                user_id, AuthAuditEventType.MFA_FAILED,
                {"reason": "invalid_code"}, client_ip
            )
            return LoginAttemptResult(
                success=False,
        ...
        await self._log_auth_event(
            user_id, AuthAuditEventType.LOGIN_SUCCESS,
            {"session_id": str(session.id), "mfa_verified": True}, client_ip
        )
    ...
    async def logout_user(self, session_id: UUID, client_ip: Optional[str] = None) -> bool:
        ...
        await self._log_auth_event(
            session.user_id, AuthAuditEventType.LOGOUT,
            {"session_id": str(session_id)}, client_ip
        )
    ...
    @standard_exception_handler
    async def create_session(self, session_data: SessionCreateSchema) -> SessionResponseSchema:
        """
    ...
    async def verify_mfa_setup(self, user_id: UUID, mfa_code: str) -> bool:
        ...
        await self._log_auth_event(
            user_id, AuthAuditEventType.MFA_ENABLED, {}
        )
    ...
    async def change_password(
        ...
        if not current_password or not self.pwd_context.verify(request.current_password, current_password.password_hash):
            await self._log_auth_event(
                user_id, AuthAuditEventType.PASSWORD_CHANGE_FAILED,
                {"reason": "invalid_current_password"}, client_ip
            )
            return False
    ...
    async def create_api_key(
        ...
        await self._log_auth_event(
            user_id, AuthAuditEventType.API_KEY_CREATED,
    ...
    async def _record_failed_attempt(self, user_id: UUID, client_ip: Optional[str] = None):
        ...
        await self._log_auth_event(
            user_id, AuthAuditEventType.LOGIN_FAILED,
            {"reason": "invalid_password"}, client_ip
        )
