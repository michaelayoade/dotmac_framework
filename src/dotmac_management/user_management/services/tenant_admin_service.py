...
from ..models.rbac_models import RoleModel, PermissionModel, UserRoleModel
...
class TenantAdminService(BaseService):
    ...
    async def search_across_apps(
        ...
        if not search_params.resource_types or "users" in search_params.resource_types:
            ...
            if not search_params.include_archived:
                user_query = user_query.where(UserModel.is_active == True)
            ...
            for user in users:
                results.append(CrossAppSearchResultSchema(
    ...
    async def bulk_user_operation(self, operation_data: BulkUserOperationSchema) -> Dict[str, Any]:
        ...
        for user_id in operation_data.user_ids:
            try:
                if operation_data.operation == "assign_role":
                    ...
                    app = operation_data.parameters.get("app")
        ...
        logger.info(f"Bulk operation {operation_data.operation} completed: {results['success_count']} success, {results['error_count']} errors")
        return results
