...
from sqlalchemy import Column, String, Text, Boolean, DateTime, ForeignKey, JSON, UniqueConstraint, Index, Integer
from sqlalchemy.dialects.postgresql import UUID as PostgreSQLUUID, ENUM
...
class RoleModel(BaseModel):
    ...
    cross_app_permissions = Column(JSON, nullable=True)  # Cross-app permissions: {'isp': ['customers:read'], 'crm': ['leads:write']}
...
class PermissionModel(BaseModel):
    ...
    # Relationships
    parent_permission = relationship("PermissionModel", remote_side="PermissionModel.id", back_populates="child_permissions")
    child_permissions = relationship("PermissionModel", back_populates="parent_permission", cascade="all, delete-orphan")
...
class RolePermissionModel(BaseModel):
    ...
    permission_id = Column(PostgreSQLUUID(as_uuid=True), ForeignKey("permissions_v2.id", ondelete="CASCADE"), nullable=False)
...
class PermissionGroupItemModel(BaseModel):
    ...
    # Foreign keys
    group_id = Column(PostgreSQLUUID(as_uuid=True), ForeignKey("permission_groups_v2.id", ondelete="CASCADE"), nullable=False)
    permission_id = Column(PostgreSQLUUID(as_uuid=True), ForeignKey("permissions_v2.id", ondelete="CASCADE"), nullable=False)
