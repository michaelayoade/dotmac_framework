...
class Settings(BaseServiceSettings):
    ...
    app_name: str = Field(default="DotMac Management Platform", description="Application name")
    ...
    max_tenants_per_instance: int = Field(default=1000, description="Maximum tenants per instance")
    ...
    sendgrid_from_email: str = Field(default="noreply@dotmac.app", description="SendGrid from email")
    ...
    kubernetes_config_path: Optional[str] = Field(None, description="Kubernetes config path")
    kubernetes_namespace_prefix: str = Field(default="dotmac-tenant", description="K8s namespace prefix")
    ...
    def validate_k8s_namespace_prefix(cls, v: str) -> str:
        ...
        if not v.replace("-", "").replace("_", "").isalnum():
            raise ValueError("Kubernetes namespace prefix must be alphanumeric with hyphens/underscores")
        return v.lower()
    ...
    def validate_management_platform_config(self) -> list[str]:
        ...
        if self.is_production:
            ...
            if not self.vault_url:
                issues.append("Vault URL should be configured for production secrets management")
    ...
    def get_database_url(self, async_driver: bool = False) -> str:
        ...
        if hasattr(self, "database_url") and self.database_url:
            if async_driver and not self.database_url.startswith(("postgresql+asyncpg", "sqlite+aiosqlite")):
