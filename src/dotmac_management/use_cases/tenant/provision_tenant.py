...
from ..base import TransactionalUseCase, UseCaseResult, UseCaseContext, UseCaseStatus
...
class ProvisionTenantUseCase(TransactionalUseCase[ProvisionTenantInput, ProvisionTenantOutput]):
    ...
    async def _execute_transaction(
        ...
        try:
            ...
            self.logger.info(f"Starting tenant provisioning", extra={
            ...
            self.logger.info(f"Tenant provisioning initiated successfully", extra={
                'tenant_id': input_data.tenant_id,
    ...
    async def _create_tenant_record(self, input_data: ProvisionTenantInput, correlation_id: str) -> int:
        ...
        with get_db_session() as db:
            ...
            self.logger.info(f"Created tenant record", extra={
                'tenant_id': input_data.tenant_id,
    ...
    async def _start_provisioning_workflow(self, tenant_db_id: int, correlation_id: str) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as db:
                ...
                tenant_secrets = await self.provisioning_service.generate_tenant_secrets(
                    tenant, correlation_id
                )
                ...
                health_check_passed = await self.provisioning_service.perform_health_checks(
                    tenant, correlation_id
                )
        ...
        try:
            with get_db_session() as db:
                ...
                if tenant:
                    ...
                    self.logger.info(f"Rolled back tenant record", extra={
                        'tenant_db_id': tenant_db_id
                    })
