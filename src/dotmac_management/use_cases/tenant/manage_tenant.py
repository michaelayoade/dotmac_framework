...
from ..base import TransactionalUseCase, UseCaseResult, UseCaseContext
...
class ManageTenantUseCase(TransactionalUseCase[ManageTenantInput, ManageTenantOutput]):
    ...
    async def _execute_transaction(
        ...
        try:
            ...
            self.logger.info(f"Tenant management operation completed", extra={
                'tenant_id': input_data.tenant_id,
    ...
    async def _get_tenant(self, tenant_id: str) -> Optional[CustomerTenant]:
        """Get tenant from database"""
        ...
        try:
            with get_db_session() as db:
                ...
                if tenant:
                    ...
                    self.logger.info(f"Rolled back tenant status", extra={
                        'tenant_id': tenant_id,
