  ...
  captureError(
    ...
    const errorMetric: ErrorMetrics = {
      ...
      context: {
        ...
        viewport:
          typeof window !== 'undefined' ? `${window.innerWidth}x${window.innerHeight}` : 'server',
      },
      severity,
      portal: context.component?.includes('admin')
        ? 'admin'
        : context.component?.includes('customer')
          ? 'customer'
          : context.component?.includes('reseller')
            ? 'reseller'
            : context.component?.includes('technician')
              ? 'technician'
              : 'unknown',
