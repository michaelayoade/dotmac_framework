...
export function withSentryMonitoring<P extends Record<string, any>>(
  ...
  const MonitoredComponent = React.forwardRef<any, P>((props, ref) => {
      ...
      if (enablePerformanceMonitoring) {
        ...
        return () => {
          try {
            ...
            performance.measure(`${componentName}-lifecycle-${renderCount}`, markName, endMarkName);
          } catch (error) {
    ...
    if (enablePerformanceMonitoring) {
      ...
      return Sentry.withProfiler(React.createElement(WrappedComponent, { ...props, ref }), {
        name: spanName,
        tags: {
          portal: portalType,
...
export function SentryMonitoringProvider({
  ...
  return (
    <SentryMonitoringContext.Provider value={value}>{children}</SentryMonitoringContext.Provider>
  );
}
