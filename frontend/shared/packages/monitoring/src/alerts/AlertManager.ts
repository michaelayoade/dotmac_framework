...
export class AlertManager {
  ...
  recordMetric(metric: string, value: number, tags: Record<string, string> = {}): void {
    ...
    this.metricStore.set(
      metric,
      values.filter((v) => v.timestamp > cutoffTime)
    );
    ...
    // Check for recovery
      ...
      if (
        this.evaluateCondition(
          value,
          this.getInverseOperator(rule.condition.operator),
          rule.threshold.recovery
        )
      ) {
    ...
    this.addRule({
      ...
      escalation: {
        levels: [
          {
            level: 0,
            channels: [{ type: 'slack', config: { channel: '#alerts' }, enabled: true }],
    ...
    this.addRule({
      ...
      escalation: {
        levels: [
          {
            level: 0,
            channels: [{ type: 'slack', config: { channel: '#performance' }, enabled: true }],
            acknowledgmentTimeout: 30,
          },
        ],
        maxEscalations: 1,
