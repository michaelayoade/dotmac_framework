...
export class EnvironmentManager {
  ...
  validateConfiguration(): { isValid: boolean; errors: string[] } {
    ...
    if (
      this.config.monitoring.errorReporting.enabled &&
      !this.config.monitoring.errorReporting.dsn
    ) {
    ...
    const configs: Record<string, EnvironmentConfig> = {
      ...
      staging: {
        ...
        monitoring: {
          ...
          metrics: {
            enabled: true,
            endpoint: 'https://metrics-staging.dotmac.com',
            interval: 60000,
          },
  ...
  private setupValidators(): void {
    this.configValidators.set(
      'api.timeout',
      (value) => typeof value === 'number' && value > 0 && value <= 60000
    );
