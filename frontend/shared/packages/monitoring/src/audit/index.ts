...
export const audit = {
  ...
  async login(
    userId: string,
    success: boolean,
    context: Partial<import('./AuditLogger').AuditContext> = {}
  ) {
    const logger = getAuditLogger();
    await logger.logAuthentication(success ? 'login' : 'failed_login', { userId, ...context });
  },
  ...
  async security(
    ...
    const logger = getAuditLogger();
    await logger.logSecurity(event, context, success ? 'success' : 'denied', severity);
  },
