...
export const createMockMonitoringConfig = (portal: string, overrides = {}) => ({
      ...
      portal === 'management-admin'
        ...
        : portal === 'management-reseller'
          ...
          : portal === 'tenant-portal'
            ? 60000
            : 30000,
    timeout: portal === 'technician' ? 8000 : portal === 'management-reseller' ? 7000 : 5000,
    retries: 3,
    endpoints: [
      `/api/${
        portal === 'admin'
  ...
  validation: {
    enabled: true,
    strictMode:
      portal === 'management-admin' || portal === 'reseller' || portal === 'management-reseller',
...
expect.extend({
  toHaveBeenCapturedBySentry(received: any) {
    const wasCaptured = mockCaptureException.mock.calls.some(
      (call) => call[0] === received || call[0].message === received?.message
    );
