...
export class OperationalTransform {
  ...
  private static transformDelete(
    ...
    switch (otherOp.type) {
      ...
      case OperationType.DELETE:
        ...
        if (this.rangesOverlap(deleteData.position, ourEnd, otherOp.data.position, otherEnd)) {
          conflict = true;
      ...
      case OperationType.REPLACE:
        ...
        if (
          this.rangesOverlap(
            deleteData.position,
            deleteData.position + (deleteData.length || 0),
            otherOp.data.position,
            replaceEnd
          )
        ) {
   ...
   */
  private static transformReplace(
    replaceData: OperationData,
    otherOp: Operation
  ): {
    data: OperationData;
    conflict: boolean;
    ...
    switch (otherOp.type) {
      ...
      case OperationType.DELETE:
        ...
        if (
          this.rangesOverlap(replaceData.position, replaceEnd, otherOp.data.position, deleteEnd)
        ) {
          conflict = true;
      ...
      case OperationType.REPLACE:
        ...
        if (this.rangesOverlap(replaceData.position, ourEnd, otherOp.data.position, theirEnd)) {
          conflict = true;
        }
        break;
    }
  ...
  private static transformFormat(
    ...
    if (
      otherOp.type === OperationType.FORMAT &&
  ...
  static resolveConflict(
    ...
    switch (strategy) {
      case ConflictResolutionStrategy.LAST_WRITER_WINS:
        ...
        return [
          conflictingOps.reduce((latest, op) =>
      ...
      case ConflictResolutionStrategy.FIRST_WRITER_WINS:
        ...
        return [
          conflictingOps.reduce((earliest, op) =>
