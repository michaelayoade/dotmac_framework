...
export function useCollaboration(options: UseCollaborationOptions): UseCollaborationResult {
  ...
  useEffect(() => {
    const initializeCollaboration = async () => {
      try {
        ...
        const wsProvider = new WebsocketProvider(websocket_url, `doc-${document_id}`, yDoc, {
          params: {
            userId: user.id,
  ...
  const handleCommentEvent = useCallback(
    (commentData: { action: string; comment: Comment }) => {
      if (!enable_comments) return;
      ...
      switch (commentData.action) {
        ...
        case 'update':
          setComments((prev) =>
            prev.map((c) => (c.id === commentData.comment.id ? commentData.comment : c))
          );
          break;
        case 'delete':
  ...
  const handleSuggestionEvent = useCallback(
    (suggestionData: { action: string; suggestion: Suggestion }) => {
      if (!enable_suggestions) return;
      ...
      switch (suggestionData.action) {
        ...
        case 'withdraw':
          setSuggestions((prev) =>
            prev.map((s) => (s.id === suggestionData.suggestion.id ? suggestionData.suggestion : s))
          );
          break;
      }
    },
  ...
  const addComment = useCallback(
    async (commentData: Omit<Comment, 'id' | 'created_at' | 'author'>): Promise<Comment> => {
      if (!enable_comments) {
        throw new CollaborationError(
  ...
  const resolveComment = useCallback(
    async (comment_id: string): Promise<void> => {
      try {
  ...
  const acceptSuggestion = useCallback(
    async (suggestion_id: string): Promise<void> => {
      try {
  ...
  const updateSelection = useCallback(
    (selection: TextSelection | null): void => {
      if (!enable_presence || !socketRef.current) return;
  ...
  const resolveConflict = useCallback(
    async (conflict_id: string, resolution: ConflictResolutionStrategy): Promise<void> => {
      try {
