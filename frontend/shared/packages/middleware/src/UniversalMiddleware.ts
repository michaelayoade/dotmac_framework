...
export function createUniversalMiddleware(config: UniversalMiddlewareConfig) {
  ...
  if (config.enableRateLimit) {
    middlewares.push(
      createRateLimitMiddleware(portalConfig.rateLimiting.general, portalConfig.rateLimiting.auth)
    );
  }
  ...
  if (config.enableSecurity) {
    middlewares.push(
      createSecurityMiddleware(portalConfig.securityLevel, development, portalConfig.customHeaders)
    );
  }
