...
export class TenantProvisioningService {
  ...
  async provisionTenant(request: TenantProvisioningRequest): Promise<string> {
    try {
      const response = await this.apiClient.request<ProvisioningResponse>(
  ...
  async getTenant(tenantId: string): Promise<TenantConfig> {
    try {
      const response = await this.apiClient.request<TenantConfig>(`/api/v1/tenants/${tenantId}`, {
        method: 'GET',
      });
  ...
  async suspendTenant(tenantId: string, reason?: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to suspend tenant: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async resumeTenant(tenantId: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to resume tenant: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async terminateTenant(tenantId: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to terminate tenant: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async updateResourceLimits(tenantId: string, limits: Partial<ResourceLimits>): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to update resource limits: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getPortalConfig(tenantId: string): Promise<TenantPortalConfig> {
    try {
      ...
      throw new Error(
        `Failed to get portal config: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
   ...
   */
  async updatePortalConfig(tenantId: string, config: Partial<TenantPortalConfig>): Promise<void> {
    try {
  ...
  async getTenantEvents(
    ...
    try {
      ...
      throw new Error(
        `Failed to get tenant events: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async executeManagementAction(
    action: Omit<TenantManagementAction, 'executedAt' | 'status' | 'result' | 'error'>
  ): Promise<string> {
    try {
      ...
      throw new Error(
        `Failed to execute management action: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getManagementActionStatus(actionId: string): Promise<TenantManagementAction> {
    try {
      ...
      throw new Error(
        `Failed to get action status: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async validateTenantConfig(
    ...
    try {
      ...
      throw new Error(
        `Failed to validate tenant config: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
