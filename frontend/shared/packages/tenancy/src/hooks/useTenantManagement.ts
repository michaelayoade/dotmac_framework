...
export function useTenantManagement(
  ...
  const provisionTenant = useCallback(
    async (request: TenantProvisioningRequest): Promise<string> => {
      setIsLoading(true);
      setError(null);
  ...
  const checkProvisioningStatus = useCallback(
  ...
  const updateResourceLimits = useCallback(
    async (limits: Partial<ResourceLimits>) => {
      if (!tenant) return;
  ...
  const updatePortalConfig = useCallback(
    async (config: Partial<TenantPortalConfig>) => {
      if (!tenant) return;
  ...
  const loadEvents = useCallback(
    async (filter?: Partial<TenantEvent>) => {
      if (!tenant) return;
