...
import type { PluginHealth, PluginConfigValidation, UsePluginLifecycleResult } from '../types';
...
export function usePluginLifecycle(): UsePluginLifecycleResult {
  ...
  const initializePlugin = useCallback(
    async (pluginKey: string): Promise<boolean> => {
      try {
  ...
  const shutdownPlugin = useCallback(
    async (pluginKey: string): Promise<boolean> => {
      try {
  ...
  const updatePluginConfig = useCallback(
    async (pluginKey: string, config: Record<string, any>) => {
      try {
  ...
  const validatePluginConfig = useCallback(
    async (pluginKey: string, config: Record<string, any>): Promise<PluginConfigValidation> => {
      try {
  ...
  const initializePluginsByDomain = useCallback(
    async (domain: string): Promise<Record<string, boolean>> => {
      try {
  ...
  const shutdownPluginsByDomain = useCallback(
    async (domain: string): Promise<Record<string, boolean>> => {
      try {
        const response = await apiClient.post<Record<string, boolean>>(
