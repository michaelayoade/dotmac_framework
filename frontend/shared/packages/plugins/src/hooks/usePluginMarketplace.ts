...
export function usePluginMarketplace(): UsePluginMarketplaceResult {
  ...
  const fetchMarketplaceItems = useCallback(
    async (filters?: PluginSearchFilters) => {
      try {
        setLoading(true);
        setError(null);

        const response = await apiClient.get<PluginMarketplaceItem[]>(API_ENDPOINTS.MARKETPLACE, {
          params: filters || {},
        });
  ...
  const installFromMarketplace = useCallback(
    async (item: PluginMarketplaceItem) => {
      try {
        setError(null);
        ...
        setItems((prev) =>
          prev.map((prevItem) =>
            prevItem.id === item.id ? { ...prevItem, installed: true } : prevItem
          )
        );
      } catch (err) {
        const message =
          err instanceof Error ? err.message : 'Failed to install plugin from marketplace';
        setError(message);
  ...
  const getPluginDetails = useCallback(
    async (pluginId: string): Promise<PluginMarketplaceItem | null> => {
      try {
        const response = await apiClient.get<PluginMarketplaceItem>(
  ...
  const checkForUpdates = useCallback(async () => {
    try {
      ...
      const response = await apiClient.get<PluginMarketplaceItem[]>(
        API_ENDPOINTS.MARKETPLACE_UPDATES
      );
