...
export function useMobileCache(options: MobileCacheOptions = {}) {
  ...
  const cacheGet = useCallback(
    async <T>(key: string, params?: Record<string, unknown>): Promise<T | null> => {
    ...
    async <T>(
      ...
      if (useIndexedDB) {
        ...
        try {
          ...
          localStorage.setItem(`dotmac-mobile-cache:${cacheKey}`, JSON.stringify(record));
        } catch (error) {
  ...
  const cacheDelete = useCallback(
    async (key: string, params?: Record<string, unknown>): Promise<void> => {
      if (!currentTenant?.tenant?.id) return;
  ...
  const cacheAsset = useCallback(
    async (url: string, force: boolean = false): Promise<string | null> => {
    ...
    async (
      ...
      const limit =
        prefetchStrategy === 'aggressive'
          ...
          : Math.min(5, sortedResources.length);
  ...
  const clearCache = useCallback(
    async (tenantOnly: boolean = true): Promise<void> => {
      try {
