  ...
  async createProvider(
    ...
    try {
      ...
      if (
        ...
        console.warn(
          `Failed to create ${providerConfig.type} provider, falling back to ${this.config.fallbackProvider}:`,
          error
        );
      ...
      throw new Error(
        `Failed to create map provider: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  clearCache(): void {
    ...
    this.providerCache.forEach((provider) => {
      if (!provider.isDestroyed) {
        provider.destroy().catch((err) => console.warn('Error destroying cached provider:', err));
      }
    });
