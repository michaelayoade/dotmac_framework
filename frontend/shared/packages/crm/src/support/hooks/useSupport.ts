...
interface UseSupportReturn {
  ...
  // Ticket Management
  createTicket: (
    ticket: Omit<SupportTicket, 'id' | 'ticketNumber' | 'createdAt' | 'updatedAt'>
  ...
  getResolutionTimes: () => Promise<{
    avg: number;
    min: number;
    max: number;
    byCategory: Record<string, number>;
  }>;
  getCustomerSatisfaction: () => Promise<{
    rating: number;
    responses: number;
    byCategory: Record<string, number>;
  }>;
...
export function useSupport(options: UseSupportOptions = {}): UseSupportReturn {
    ...
    try {
      ...
      if (customerId) {
        ...
        ticketsList = results.filter((ticket) => {
          ...
          if (currentFilter.priority && !currentFilter.priority.includes(ticket.priority))
            return false;
          if (currentFilter.category && !currentFilter.category.includes(ticket.category))
            return false;
          if (
            currentFilter.assignedTo &&
            ticket.assignedTo &&
            !currentFilter.assignedTo.includes(ticket.assignedTo)
          )
            return false;
          if (currentFilter.customerId && ticket.customerId !== currentFilter.customerId)
            return false;

          if (currentFilter.createdAfter && ticket.createdAt < currentFilter.createdAfter)
            return false;
          if (currentFilter.createdBefore && ticket.createdAt > currentFilter.createdBefore)
            return false;

          if (
            currentFilter.slaBreached !== undefined &&
            ticket.slaBreached !== currentFilter.slaBreached
          )
            return false;
  ...
  const updateTicketStatus = useCallback(
    async (ticketId: string, status: TicketStatus, resolution?: string) => {
      ...
      await addTicketComment(
        ticketId,
        `Status changed to: ${status}${resolution ? ` - ${resolution}` : ''}`,
        true
      );
    },
  ...
  const escalateTicket = useCallback(
    async (ticketId: string, reason: string) => {
      ...
      const newPriority =
        currentIndex < priorityLevels.length - 1
  ...
  const addTicketComment = useCallback(
    async (ticketId: string, content: string, isInternal = false): Promise<Communication> => {
  ...
  // Add customer reply
  const addCustomerReply = useCallback(
    async (ticketId: string, content: string, attachments?: File[]): Promise<Communication> => {
  ...
  const getTicketCommunications = useCallback(
    async (ticketId: string): Promise<Communication[]> => {
      if (!tenantId) return [];
  ...
  const searchTickets = useCallback(
    async (query: string): Promise<SupportTicket[]> => {
      if (!tenantId) return [];
  ...
  const filterTickets = useCallback(
    async (filter: SupportTicketFilter) => {
  ...
  const getTicketsByStatus = useCallback(
    async (status: TicketStatus): Promise<SupportTicket[]> => {
      if (!tenantId) return [];

      return crmDb.supportTickets.where('[tenantId+status]').equals([tenantId, status]).toArray();
    },
    [tenantId]
  );
  ...
  // Bulk operations
  const bulkAssignTickets = useCallback(
    async (ticketIds: string[], userId: string) => {
      try {
  ...
  const bulkUpdatePriority = useCallback(
    async (ticketIds: string[], priority: TicketPriority) => {
      try {
  ...
  const bulkCloseTickets = useCallback(
