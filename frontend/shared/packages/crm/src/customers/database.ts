...
import type { CustomerAccount, Lead, Communication, SupportTicket, CustomerFilter } from '../types';
...
export class CRMDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      customers:
        'id, tenantId, accountNumber, status, type, segment, displayName, createdAt, updatedAt, lastContactDate, syncStatus',
      leads:
        'id, tenantId, status, source, priority, assignedTo, score, createdAt, updatedAt, lastContactDate, syncStatus',
      communications:
        'id, tenantId, customerId, leadId, type, direction, timestamp, userId, syncStatus',
      supportTickets:
        'id, tenantId, ticketNumber, customerId, status, priority, category, assignedTo, createdAt, updatedAt, syncStatus',
    });
  ...
  async getCustomersBySegment(
    segment: CustomerAccount['segment'],
    tenantId: string
  ): Promise<CustomerAccount[]> {
    return this.customers
  ...
  async getCustomersByAccountManager(
    ...
    return this.customers
      ...
      .filter(
        (customer) =>
          ...
          customer.contactMethods.some((method) => method.value.toLowerCase().includes(searchTerm))
      )
      .toArray();
  }
  ...
  async filterCustomers(filter: CustomerFilter, tenantId: string): Promise<CustomerAccount[]> {
    ...
    return results.filter((customer) => {
      ...
      if (
        ...
        !filter.accountManager.includes(customer.accountManagerId)
      )
        return false;
      if (filter.salesRep && customer.salesRepId && !filter.salesRep.includes(customer.salesRepId))
        return false;
      ...
      if (
        filter.lastContactAfter &&
        customer.lastContactDate &&
        customer.lastContactDate < filter.lastContactAfter
      )
        return false;
      if (
        filter.lastContactBefore &&
        customer.lastContactDate &&
        customer.lastContactDate > filter.lastContactBefore
      )
        return false;
  ...
  async getCommunicationsByCustomer(
    customerId: string,
    tenantId: string
  ...
  async getCustomerMetrics(tenantId: string): Promise<{
    ...
    const customers = await this.customers.where('tenantId').equals(tenantId).toArray();
  ...
  async getLeadMetrics(tenantId: string): Promise<{
    ...
    const leads = await this.leads.where('tenantId').equals(tenantId).toArray();
