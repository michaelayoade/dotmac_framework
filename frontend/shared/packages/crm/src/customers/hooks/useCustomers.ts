...
export function useCustomers(options: UseCustomersOptions = {}): UseCustomersReturn {
  const { autoSync = true, syncInterval = 30000, filter: initialFilter, pageSize = 20 } = options;
  ...
  const createCustomer = useCallback(
    async (
      customerData: Omit<CustomerAccount, 'id' | 'createdAt' | 'updatedAt'>
    ): Promise<CustomerAccount> => {
  ...
  const updateCustomer = useCallback(
    async (id: string, updates: Partial<CustomerAccount>) => {
      try {
        const updatedData = {
          ...updates,
          updatedAt: new Date().toISOString(),
  ...
  const cancelCustomer = useCallback(
    async (id: string, reason?: string) => {
      await updateCustomer(id, {
        status: 'cancelled',
        updatedAt: new Date().toISOString(),
  ...
  const addAddress = useCallback(
    async (customerId: string, address: Omit<Address, 'id'>) => {
  ...
  const updateAddress = useCallback(
    async (customerId: string, addressId: string, updates: Partial<Address>) => {
      const customer = await crmDb.customers.get(customerId);
      if (!customer) throw new Error('Customer not found');
  ...
  const addContactMethod = useCallback(
    async (customerId: string, method: Omit<ContactMethod, 'id'>) => {
      const customer = await crmDb.customers.get(customerId);
      if (!customer) throw new Error('Customer not found');
  ...
  const updateContactMethod = useCallback(
    async (customerId: string, methodId: string, updates: Partial<ContactMethod>) => {
      const customer = await crmDb.customers.get(customerId);
      if (!customer) throw new Error('Customer not found');
  ...
  const removeContactMethod = useCallback(
    async (customerId: string, methodId: string) => {
      const customer = await crmDb.customers.get(customerId);
      if (!customer) throw new Error('Customer not found');
  ...
  const setPrimaryContactMethod = useCallback(
    async (customerId: string, methodId: string) => {
      const customer = await crmDb.customers.get(customerId);
      if (!customer) throw new Error('Customer not found');
  ...
  const addCustomField = useCallback(
    async (customerId: string, field: Omit<CustomField, 'id'>) => {
      const customer = await crmDb.customers.get(customerId);
      if (!customer) throw new Error('Customer not found');
  ...
  const updateCustomField = useCallback(
    async (customerId: string, fieldId: string, value: any) => {
      const customer = await crmDb.customers.get(customerId);
      if (!customer) throw new Error('Customer not found');
  ...
  const removeCustomField = useCallback(
    async (customerId: string, fieldId: string) => {
      const customer = await crmDb.customers.get(customerId);
      if (!customer) throw new Error('Customer not found');
  ...
  const searchCustomers = useCallback(
    async (query: string): Promise<CustomerAccount[]> => {
      if (!tenantId) return [];
  ...
  const bulkUpdateCustomers = useCallback(
    async (customerIds: string[], updates: Partial<CustomerAccount>) => {
      try {
  ...
  const exportCustomers = useCallback(
    async (format: 'csv' | 'json'): Promise<string> => {
      ...
      const allCustomers = await crmDb.customers.where('tenantId').equals(tenantId).toArray();
