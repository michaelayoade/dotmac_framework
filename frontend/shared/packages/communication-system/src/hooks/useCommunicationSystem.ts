...
export function useCommunicationSystem(options: UseCommunicationSystemOptions = {}) {
    ...
    switch (data.type) {
      ...
      case 'conversation_update':
        setConversations((prev) =>
          prev.map((conv) =>
            conv.id === data.conversationId ? { ...conv, ...data.updates } : conv
          )
        );
        break;
      ...
      case 'bulk_job_progress':
        setBulkJobs((prev) =>
          prev.map((job) =>
            job.id === data.jobId ? { ...job, progress: data.progress, status: data.status } : job
          )
        );
        break;
    }
  ...
  const sendMessage = useCallback(
    async (messageData: Omit<CommunicationMessage, 'id' | 'status' | 'sentAt'>) => {
      setIsLoading(true);
      setError(null);
  ...
  const sendBulkMessages = useCallback(
    async (messagesData: Array<Omit<CommunicationMessage, 'id' | 'status' | 'sentAt'>>) => {
      setIsLoading(true);
      setError(null);
