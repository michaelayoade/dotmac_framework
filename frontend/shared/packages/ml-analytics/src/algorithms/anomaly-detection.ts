...
export function iqrAnomalyDetection(data: number[], multiplier: number = 1.5): AnomalyPoint[] {
  const anomalies: AnomalyPoint[] = [];
  ...
  return new Promise((resolve) => {
    ...
    resolve({
      ...
      metrics: {
        ...
        severityDistribution: anomalies.reduce(
          (counts, a) => {
    ...
    case 'isolation_forest': {
      ...
      return {
        ...
        metrics: {
          ...
          avgConfidence:
            anomalies.reduce((sum, a) => sum + a.confidence, 0) / Math.max(anomalies.length, 1),
