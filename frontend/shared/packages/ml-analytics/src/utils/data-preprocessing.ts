...
export function preprocessTimeSeriesData(
  ...
  const { data: cleanedData, missingCount } = handleMissingValues(
    processedData,
  ...
  if (missingCount > 0) {
    transformations.push(
      `handled_${missingCount}_missing_values_with_${config.handleMissingValues}`
    );
  }
...
function assessDataQuality(
  ...
  const outliers = Math.max(
    0,
    originalData.length - processedData.length - missingValues - duplicates
  );
...
export function createLagFeatures(data: TimeSeriesData[], lags: number[]): ModelDataset {
  ...
  return {
    ...
    targets,
    timestamps,
    featureNames: [...lags.map((lag) => `lag_${lag}`), 'hour_of_day', 'day_of_week', 'month'],
  };
}
