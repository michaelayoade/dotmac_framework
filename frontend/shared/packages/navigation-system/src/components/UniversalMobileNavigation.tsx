...
export const UniversalMobileNavigation = forwardRef<HTMLDivElement, UniversalMobileNavigationProps>(
  ({ items, activeItem, variant = 'drawer', onNavigate, className, ...props }, ref) => {
        ...
        <li key={item.id}>
          ...
          >
            <div className='flex items-center min-w-0 flex-1'>
              {Icon && <Icon className='mr-3 h-5 w-5 flex-shrink-0 text-gray-400' />}
            ...
            {hasChildren && (
              <ChevronRight
                className={cn('ml-2 h-4 w-4 flex-shrink-0 transition-transform', {
      ...
      <>
        ...
        >
          ...
          <nav className='flex-1 overflow-y-auto'>
            <ul className='py-2'>{items.map((item) => renderNavigationItem(item))}</ul>
          </nav>

          {/* Footer */}
          <div className='border-t border-gray-200 p-4'>
            <p className='text-center text-xs text-gray-500'>Tap outside to close</p>
          </div>
        </div>
      </>
    );
