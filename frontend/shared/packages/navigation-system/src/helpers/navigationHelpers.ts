...
export const NavigationPresets = {
  admin: () =>
    NavigationBuilder.create()
      ...
      .addGroup(
        ...
        [
          { id: 'billing-overview', label: 'Overview', href: '/billing' },
          {
            ...
            href: '/billing/currency-management',
          },
        ],
      ...
      .addItem('monitoring', 'Monitoring', '/monitoring', {
      ...
      .addItem('business-rules', 'Business Rules', '/business-rules', {
        icon: require('lucide-react').Settings2,
      })
      .addItem('configuration', 'Configuration', '/configuration', {
        icon: require('lucide-react').Cog,
      })
  ...
  reseller: () =>
    NavigationBuilder.create()
      .addItem('dashboard', 'Dashboard', '/dashboard', { icon: require('lucide-react').Home })
      .addItem('customers', 'Customers', '/customers', { icon: require('lucide-react').Users })
      .addItem('territories', 'Territories', '/territories', {
        icon: require('lucide-react').MapPin,
      })
      .addItem('sales', 'Sales', '/sales', { icon: require('lucide-react').TrendingUp })
      .addItem('commissions', 'Commissions', '/commissions', {
        icon: require('lucide-react').CreditCard,
      })
      .addItem('opportunities', 'Opportunities', '/opportunities', {
  ...
  technician: () =>
    NavigationBuilder.create()
      .addItem('dashboard', 'Dashboard', '/dashboard', { icon: require('lucide-react').Home })
      .addItem('work-orders', 'Work Orders', '/work-orders', {
        icon: require('lucide-react').Clipboard,
      })
      .addItem('camera-demo', 'Camera & Scanner', '/camera-demo', {
...
export class BreadcrumbBuilder {
  ...
  static fromPath(path: string, labels?: Record<string, string>): BreadcrumbBuilder {
    ...
    segments.forEach((segment, index) => {
      ...
      const label =
        labels?.[segment] || segment.replace(/-/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
...
export const NavigationHookHelpers = {
  ...
  createLogoutHandler:
    (authService: any, redirectPath = '/login') =>
    async () => {
      try {
    ...
    return legacyItems.map((item) => ({
      ...
      children:
        item.children || item.submenu
          ? MigrationHelper.convertLegacyNavigation(item.children || item.submenu)
          : undefined,
    }));
  },
