...
export const UniversalReportGenerator: React.FC<UniversalReportGeneratorProps> = ({
  ...
  const { data, loading, error, refresh, executionTime } = useReportData(report.config.dataSource);

  // Leverage existing @dotmac/data-tables export functionality
  const { exportReport, exporting, exportFormats } = useReportExport();
  ...
  const processedData = useMemo(() => {
    ...
    if (report.config.aggregation?.length) {
      ...
      if (report.config.grouping) {
        const grouped = processed.reduce(
          (acc, row) => {
            const key = report.config.grouping!.fields.map((field) => row[field]).join('|');
        ...
        processed = Object.entries(grouped).map(([key, group]) => {
          ...
          report.config.aggregation!.forEach((agg) => {
            const values = (group as any[])
              .map((row: any) => row[agg.field])
              .filter((v: any) => v != null);

            switch (agg.operation) {
              case 'sum':
                result[agg.label || `${agg.field}_sum`] = values.reduce(
                  (sum: number, val: any) => sum + Number(val),
                  0
                );
                break;
              case 'avg':
                result[agg.label || `${agg.field}_avg`] =
      ...
      try {
        ...
        await exportData(
          ...
          processedData,
          {
            filename,
            includeHeaders: true,
            formats: [format === 'html' || format === 'png' ? 'pdf' : format] as (
              | 'pdf'
              | 'csv'
              | 'xlsx'
              | 'json'
        ...
        <div className='border-b border-gray-200 bg-gray-50 px-6 py-4'>
          <div className='flex items-center justify-between'>
            ...
            <div className='flex items-center space-x-2'>
              ...
              <Button variant='ghost' size='sm' className='text-gray-500' title='Share Report'>
