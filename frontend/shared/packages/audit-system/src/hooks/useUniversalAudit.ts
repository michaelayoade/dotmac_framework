...
const portalActionCategories: Record<PortalType, ActionCategory[]> = {
  ...
  customer: ['authentication', 'billing_operations', 'service_management', 'communication'],
  technician: ['authentication', 'service_management', 'network_operations', 'communication'],
};
...
export function useUniversalAudit(options: UseAuditSystemOptions) {
  ...
  const logUserAction = useCallback(
    async (action: string, details: Partial<AuditEvent> = {}) => {
      try {
  ...
  const logSystemEvent = useCallback(
    async (event: string, details: Partial<AuditEvent> = {}) => {
  ...
  const logComplianceEvent = useCallback(
    async (type: ComplianceType, details: Partial<AuditEvent> = {}) => {
  ...
  const generateComplianceReport = useCallback(
    async (type: ComplianceType, period: { start: Date; end: Date }) => {
      setState((prev) => ({ ...prev, isLoading: true }));
  ...
  const exportAuditTrail = useCallback(
    async (filters: AuditFilters, format: 'csv' | 'json' | 'pdf') => {
      setState((prev) => ({ ...prev, isLoading: true }));
    ...
    if (trackUserActions && typeof window !== 'undefined') {
      const handleClick = (event: MouseEvent) => {
        ...
        if (target.tagName === 'BUTTON' || target.closest('button')) {
          ...
          const action =
            button?.getAttribute('data-audit-action') ||
