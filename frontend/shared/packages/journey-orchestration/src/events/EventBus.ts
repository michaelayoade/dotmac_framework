...
export class JourneyEventBus extends EventEmitter {
   ...
   */
  async emitSupportTicketCreated(
    ticketId: string,
    customerId: string,
    journeyId?: string
  ): Promise<void> {
  ...
  async emitHandoffCompleted(handoff: HandoffRecord): Promise<void> {
    await this.emitJourneyEvent({
      ...
      data: {
        ...
        duration:
          handoff.completedAt && handoff.startedAt
            ? new Date(handoff.completedAt).getTime() - new Date(handoff.startedAt).getTime()
            : null,
      },
    });
  }
