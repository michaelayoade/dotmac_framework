...
export function useHandoffSystem(): UseHandoffSystemReturn {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribeHandoffCompleted = eventBusRef.current.onEventType(
        'handoff:completed',
        () => {
          refreshHandoffs();
        }
  ...
  const createHandoff = useCallback(
    async (handoffData: Omit<HandoffRecord, 'id'>): Promise<HandoffRecord> => {
  ...
  const approveHandoff = useCallback(
    async (handoffId: string, notes?: string): Promise<void> => {
      if (!handoffSystemRef.current) {
        throw new Error('Handoff system not initialized');
      }
  ...
  const rejectHandoff = useCallback(
    async (handoffId: string, reason: string): Promise<void> => {
      if (!handoffSystemRef.current) {
        throw new Error('Handoff system not initialized');
      }
  ...
  const bulkProcessHandoffs = useCallback(
    async (handoffIds: string[]): Promise<void> => {
      if (!handoffSystemRef.current) {
        throw new Error('Handoff system not initialized');
      }
  ...
  // Get handoff status
  const getHandoffStatus = useCallback(
    async (handoffId: string): Promise<HandoffRecord> => {
      if (!handoffSystemRef.current) {
        throw new Error('Handoff system not initialized');
      }
  ...
  const retryFailedHandoffs = useCallback(
    async (handoffIds: string[]): Promise<void> => {
      if (!handoffSystemRef.current) {
        throw new Error('Handoff system not initialized');
      }
