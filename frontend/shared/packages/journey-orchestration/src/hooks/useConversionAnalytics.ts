...
export function useConversionAnalytics(): UseConversionAnalyticsReturn {
  ...
  const refreshMetrics = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const acquisitionFunnel = await analyticsRef.current.generateConversionFunnel(
        journeys,
        'acquisition'
      );
      const onboardingFunnel = await analyticsRef.current.generateConversionFunnel(
        journeys,
        'onboarding'
      );
      const retentionFunnel = await analyticsRef.current.generateConversionFunnel(
        journeys,
        'retention'
      );
  ...
  const generateReport = useCallback(
    async (type: string, params?: Record<string, any>): Promise<any> => {
      if (!analyticsRef.current || !orchestratorRef.current) {
        throw new Error('Analytics not initialized');
      }
        ...
        switch (type) {
          ...
          case 'drop_off_analysis':
            ...
            const dropOffData = await Promise.all(
              stages.map((stage) => analyticsRef.current!.analyzeDropOffs(journeys, stage as any))
            );
            return dropOffData;
