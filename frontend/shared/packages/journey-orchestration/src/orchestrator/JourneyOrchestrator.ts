...
export class JourneyOrchestrator {
  ...
  async startJourney(
    ...
    const activeJourneys = Array.from(this.journeys.values()).filter(
      (j) => j.status === 'active'
    ).length;
  ...
  filterJourneys(filters: {
    ...
    return Array.from(this.journeys.values()).filter((journey) => {
      ...
      if (
        filters.assignedTo &&
        journey.assignedTo &&
        !filters.assignedTo.includes(journey.assignedTo)
      )
        return false;
      return true;
    });
  ...
  private setupEventHandlers(): void {
    ...
    this.eventBus.onEventType('crm:lead_converted', async (event) => {
      const trigger = Array.from(this.triggers.values()).find(
        (t) => t.event === 'lead_converted' && t.isActive
      );
    ...
    this.stateChanges
      .pipe(
        ...
        distinctUntilChanged(
          (prev, curr) => prev.type === curr.type && prev.journeyId === curr.journeyId
        )
      )
      .subscribe(() => {
