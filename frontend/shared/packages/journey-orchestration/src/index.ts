    ...
    steps: [
      {
        ...
        entryConditions: [{ field: 'leadScore', operator: 'greater_than', value: 50 }],
      },
      {
...
export const JourneyUtils = {
  ...
  createTemplate: (config: any) => ({
    ...
    isActive: true,
    estimatedDuration:
      config.steps?.reduce((sum: number, step: any) => sum + (step.estimatedDuration || 0), 0) || 0,
    usageCount: 0,
  }),
  ...
  validateTemplate: (template: any): { isValid: boolean; errors: string[] } => {
    ...
    if (!template.steps || !Array.isArray(template.steps))
      errors.push('Template steps array is required');
