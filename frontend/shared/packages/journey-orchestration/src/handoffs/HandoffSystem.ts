...
export class HandoffSystem {
   ...
   */
  async createHandoff(
    handoffData: Omit<HandoffRecord, 'id' | 'startedAt'>
  ...
  getPendingApprovals(): HandoffRecord[] {
    return Array.from(this.pendingHandoffs.values()).filter(
      (h) => h.handoffType === 'approval_required' && h.status === 'pending'
    );
  }
  ...
  getFailedHandoffs(): HandoffRecord[] {
    return Array.from(this.pendingHandoffs.values()).filter((h) => h.status === 'failed');
  }
  ...
  private validateHandoffData(handoff: HandoffRecord): { isValid: boolean; errors: string[] } {
    ...
    const packageConfig =
      this.packageConfigs[handoff.toPackage as keyof typeof this.packageConfigs];
    if (!packageConfig) {
