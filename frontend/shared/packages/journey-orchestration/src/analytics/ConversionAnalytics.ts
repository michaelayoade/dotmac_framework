...
export class ConversionAnalytics {
  ...
  async generateConversionFunnel(
    ...
    stages.forEach((stage) => {
      stageData.set(
        stage,
        journeys.filter((j) => j.stage === stage)
      );
    });
    ...
    const overallConversionRate =
      journeys.length > 0 ? (completedJourneys.length / journeys.length) * 100 : 0;
    ...
    journeys.forEach((journey) => {
      ...
      if (journey.touchpoints && journey.touchpoints.length > 0) {
        journey.touchpoints.forEach((touchpoint) => {
          const existing = touchpointInfluence.get(touchpoint.type) || {
            influence: 0,
            conversions: 0,
          };
  ...
  async analyzeDropOffs(
    ...
    const droppedOffJourneys = journeys.filter((j) => {
      ...
      const targetStageIndex = stages.indexOf(stage);
      return (
        currentStageIndex < targetStageIndex && (j.status === 'abandoned' || j.status === 'failed')
      );
    });

    const dropOffRate =
      journeysAtStage.length > 0 ? (droppedOffJourneys.length / journeysAtStage.length) * 100 : 0;
    ...
    droppedOffJourneys.forEach((journey) => {
      ...
      if (journey.startedAt && journey.completedAt) {
        const duration =
          new Date(journey.completedAt).getTime() - new Date(journey.startedAt).getTime();
    ...
    return {
      ...
      commonReasons,
      timeToDropOff:
        droppedOffJourneys.length > 0 ? totalTimeToDropOff / droppedOffJourneys.length : 0,
      recoveryRate:
        droppedOffJourneys.length > 0 ? (recoveredCount / droppedOffJourneys.length) * 100 : 0,
    };
  }
   ...
   */
  async exportAnalytics(journeys: CustomerJourney[], format: 'csv' | 'json'): Promise<string> {
  ...
  private async generateStageMetrics(journeys: CustomerJourney[]): Promise<
    Record<
      JourneyStage,
      {
        count: number;
  ...
  > {
    ...
    const stageMetrics: Record<
      JourneyStage,
      { count: number; averageDuration: number; completionRate: number }
    > = {} as any;
      ...
      stageMetrics[stage] = {
        ...
        completionRate:
          stageJourneys.length > 0
            ? (completedStageJourneys.length / stageJourneys.length) * 100
            : 0,
      };
    });
  ...
  private async calculateHandoffSuccessRate(journeys: CustomerJourney[]): Promise<number> {
    ...
    const successfulHandoffs = allHandoffs.filter(
      (h) => h.status === 'completed' && h.result === 'success'
    );
  ...
  private calculateTouchpointInfluence(
