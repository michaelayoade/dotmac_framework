...
import type { Item, ItemCreate, ItemUpdate, ItemFilter, ItemStockSummary } from '../types';
...
export function useInventory(): UseInventoryResult {
  ...
  const getItemWithStock = async (id: string) => {
    try {
      setError(null);
      const response = await apiClient.get<{ item: Item; stock_summary: ItemStockSummary }>(
        `/api/inventory/items/${id}/stock-summary`
      );
      return response;
    } catch (err) {
  ...
  const listItems = async (
    ...
    try {
      ...
      if (filters.is_discontinued !== undefined)
        params.append('is_discontinued', filters.is_discontinued.toString());
