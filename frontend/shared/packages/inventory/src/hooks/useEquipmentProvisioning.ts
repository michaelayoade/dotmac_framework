...
import type { Item, AssetDetails, ItemType, MovementType } from '../types';
...
export function useEquipmentProvisioning(): UseEquipmentProvisioningResult {
  ...
  const createProvisioningRequest = async (
    ...
    try {
      ...
      const response = await apiClient.post<ProvisioningRequest>(
        '/api/inventory/provisioning/requests',
        {
          ...request,
          status: 'pending',
        }
  ...
  const updateProvisioningRequest = async (
    ...
    try {
      ...
      const response = await apiClient.put<ProvisioningRequest>(
        `/api/inventory/provisioning/requests/${id}`,
        updates
      );
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to update provisioning request';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setLoading(false);
  ...
  const allocateEquipment = async (
    ...
    try {
      ...
      setProvisioningRequests((prev) =>
        prev.map((request) =>
          request.id === requestId ? { ...request, status: 'allocated' as const } : request
        )
      );
    } catch (err) {
  ...
  const confirmInstallation = async (
    ...
    try {
      ...
      setProvisioningRequests((prev) =>
        prev.map((request) =>
          request.id === requestId ? { ...request, status: 'installed' as const } : request
        )
      );
    } catch (err) {
  ...
  const createProvisioningKit = async (
    ...
    try {
      ...
      const response = await apiClient.post<ProvisioningKit>(
        '/api/inventory/provisioning/kits',
        kit
      );
  ...
  const updateProvisioningKit = async (
    ...
    try {
      ...
      const response = await apiClient.put<ProvisioningKit>(
        `/api/inventory/provisioning/kits/${id}`,
        updates
      );

      setProvisioningKits((prev) => prev.map((kit) => (kit.id === id ? response : kit)));
  ...
  const getProvisioningAnalytics = async () => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to get provisioning analytics';
      setError(errorMessage);
      return {
        total_requests: 0,
        pending_requests: 0,
  ...
  const refreshRequests = async (): Promise<void> => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to refresh provisioning data';
      setError(errorMessage);
    } finally {
      setLoading(false);
    }
