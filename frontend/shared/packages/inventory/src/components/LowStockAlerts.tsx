...
import { AlertTriangle, Package, TrendingDown, ShoppingCart, RefreshCw } from 'lucide-react';
...
export function LowStockAlerts({
  ...
  const loadAlerts = async () => {
    try {
      ...
      const transformedAlerts: LowStockAlert[] = lowStockItems
        .map((item) => {
    ...
    <div className={clsx('space-y-3', className)}>
      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <h3 className='font-medium text-gray-900'>Low Stock Alerts ({alerts.length})</h3>
        </div>
      ...
      <div className='space-y-2'>
        {alerts.map((alert) => (
          ...
          >
            <div className='flex-shrink-0'>{getCriticalityIcon(alert.criticality)}</div>

            <div className='flex-1 min-w-0'>
              <div className='flex items-center justify-between'>
                <div className='min-w-0 flex-1'>
                  <p className='font-medium text-gray-900 truncate'>{alert.item.name}</p>
                  <p className='text-sm text-gray-600'>{alert.item.item_code}</p>
                </div>
                ...
                <div className='flex items-center gap-3 ml-4'>
                  <div className='text-right'>
                    ...
                    <p className='text-xs text-gray-500'>Need: {alert.reorder_quantity}</p>
                  </div>
