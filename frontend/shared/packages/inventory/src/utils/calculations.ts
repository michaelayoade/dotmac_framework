...
export function calculateCarryingCost(
  ...
  carryingCostRate: number = 0.2 // 20% default
): number {
 ...
 */
export function calculateServiceLevel(demandMet: number, totalDemand: number): number {
 ...
 * Calculate fill rate
 */
export function calculateFillRate(ordersFilled: number, totalOrders: number): number {
...
export function calculatePOTotal(
  ...
  const subtotal = lineItems.reduce((total, line) => {
    return (
      total + calculatePOLineTotal(line.quantity_ordered, line.unit_price, line.discount_percent)
    );
  }, 0);
 ...
 */
export function calculateMovingAverage(values: number[], periods: number = 3): number {
 ...
 */
export function calculateMAPE(actualValues: number[], forecastValues: number[]): number {
