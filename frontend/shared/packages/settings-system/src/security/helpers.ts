...
export const isPasswordExpired = (lastChangeDate: string, expiryDays: number): boolean => {
...
export const getDaysUntilPasswordExpiry = (lastChangeDate: string, expiryDays: number): number => {
  const lastChange = new Date(lastChangeDate);
  const now = new Date();
  const daysDiff = Math.floor((now.getTime() - lastChange.getTime()) / (1000 * 60 * 60 * 24));
...
export const validateSecuritySettings = (
  ...
  if (settings.sessions) {
    if (
      settings.sessions.maxConcurrent &&
      (settings.sessions.maxConcurrent < 1 || settings.sessions.maxConcurrent > 10)
    ) {
      errors['sessions.maxConcurrent'] = 'Maximum concurrent sessions must be between 1 and 10';
    }

    if (
      settings.sessions.timeoutMinutes &&
      (settings.sessions.timeoutMinutes < 15 || settings.sessions.timeoutMinutes > 1440)
    ) {
...
export const getSecurityScore = (
  ...
  if (enabledMethods.length >= 2) score += 1;
    ...
    recommendations.push('Enable a second 2FA method for backup');
...
export const detectSuspiciousActivity = (events: SecurityEvent[]): SecurityEvent[] => {
  ...
  const uniqueLocations = new Set(
    events.filter((e) => e.type === 'login' && e.success).map((e) => e.location)
  );
  ...
  recentLogins.forEach((login) => {
    const locationCount = events.filter(
      (e) => e.type === 'login' && e.success && e.location === login.location
    ).length;
