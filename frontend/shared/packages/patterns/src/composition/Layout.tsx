...
function LayoutRootImpl({ children, className, direction = 'horizontal' }: LayoutRootProps) {
  return (
    <div
      className={clsx(
    ...
    <>
      ...
      <aside
        className={clsx(
          ...
          // Mobile styles
          isMobile && ['fixed left-0 top-0 z-50 h-full', sidebarOpen ? width : '-translate-x-full'],
          // Desktop styles
          !isMobile && [
...
function LayoutMainImpl({ children, className, padding = true }: LayoutMainProps) {
  return (
    <main className={clsx('flex-1 overflow-y-auto', padding && 'p-6', className)}>{children}</main>
  );
}
...
function LayoutFooterImpl({ children, className, sticky = false }: LayoutFooterProps) {
  return (
    <footer className={clsx('border-t bg-background', sticky && 'sticky bottom-0', className)}>
  ...
  return (
    <LayoutProvider>
      <LayoutRoot className={className}>
        {sidebar && <LayoutSidebar {...sidebarProps}>{sidebar}</LayoutSidebar>}
        ...
        <div className='flex flex-1 flex-col'>
          ...
          {footer && <LayoutFooter {...footerProps}>{footer}</LayoutFooter>}
        </div>
      </LayoutRoot>
    </LayoutProvider>
  );
