...
import { Card, Button, Badge, Skeleton, Select, RefreshCw } from '@dotmac/primitives';
...
export function DashboardTemplate({ config, className = '' }: DashboardTemplateProps) {
  ...
  useEffect(() => {
    ...
    try {
      trackPageView(`dashboard-${config.portal}`, { filters, timeRange });
    } catch {}
  ...
  const handleExport = async () => {
    ...
    window.dispatchEvent(event);
    try {
      trackAction('export', 'dashboard', { format: 'json' });
    } catch {}
