        ...
        <div className='space-y-4'>
          {step.fields.map((field) => (
            <div key={field.key} className='space-y-1'>
              <label htmlFor={`${stepIndex}-${field.key}`} className='block text-sm font-medium'>
                {field.label}
              ...
              {field.helpText && (
                <p id={`${stepIndex}-${field.key}-help`} className='text-xs text-gray-500'>
                  {field.helpText}
                </p>
              )}
            </div>
...
function WorkflowTemplateImpl({
  ...
  useEffect(() => {
    ...
    try {
      trackPageView(`workflow-${validatedConfig.title}`, {
        totalSteps: validatedConfig.steps.length,
      });
    } catch {}
  ...
  const goToStep = useCallback(
    (step: number) => {
      if (step >= 0 && step < validatedConfig.steps.length) {
        ...
        window.dispatchEvent(event);
        try {
          trackAction('workflow_step', 'navigation', { step: step + 1 });
        } catch {}
      }
    },
  ...
  const nextStep = useCallback(async () => {
    ...
    if (step.condition) {
      ...
      if (conditionMet && step.condition.nextStep) {
        const conditionalStepIndex = validatedConfig.steps.findIndex(
          (s) => s.id === step.condition!.nextStep
        );
  ...
  const saveProgress = useCallback(async () => {
    ...
    try {
      ...
      window.dispatchEvent(event);
      try {
        trackAction('workflow_save', 'progress', { step: currentStep + 1 });
      } catch {}
    } catch (error) {
  ...
  const resetWorkflow = useCallback(() => {
    ...
    window.dispatchEvent(event);
    try {
      trackAction('workflow_reset', 'action');
    } catch {}
  ...
  const submitWorkflow = useCallback(async () => {
    try {
      ...
      window.dispatchEvent(event);
      try {
        trackAction('workflow_complete', 'success', { totalSteps: validatedConfig.steps.length });
      } catch {}
      ...
      <div className={clsx('max-w-4xl mx-auto space-y-8', className)}>
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center space-x-4'>
            ...
            {/* Reset */}
            <Button variant='ghost' onClick={resetWorkflow} className='flex items-center space-x-2'>
            ...
            <Button
              ...
              className='flex items-center space-x-2'
            >
              <span>{currentStep === validatedConfig.steps.length - 1 ? 'Complete' : 'Next'}</span>
