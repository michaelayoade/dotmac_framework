...
export function getDensitySpacing(density: 'compact' | 'comfortable' | 'spacious') {
  ...
  return Object.fromEntries(
    Object.entries(baseSpacing).map(([key, value]) => [key, Math.round(value * multiplier)])
  );
}
...
export function generateThemeCSS(portal: PortalType, theme?: Partial<PortalTheme>): string {
  ...
  return `
    :root[data-portal="${portal}"] {
      ...
      /* Border Radius */
      --border-radius: ${
        portalTheme.borderRadius === 'none'
          ? '0'
          : portalTheme.borderRadius === 'sm'
            ? '2px'
            : portalTheme.borderRadius === 'md'
              ? '6px'
              : portalTheme.borderRadius === 'lg'
                ? '8px'
                : '6px'
      };
