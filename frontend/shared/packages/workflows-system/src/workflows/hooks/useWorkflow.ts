...
export function useWorkflow(options: UseWorkflowOptions = {}): UseWorkflowReturn {
  ...
  const filteredInstances = useMemo(() => {
    ...
    if (filters.dateRange) {
      filtered = filtered.filter(
        (i) => i.startTime >= filters.dateRange!.start && i.startTime <= filters.dateRange!.end
      );
    }
  ...
  const createInstance = useCallback(
    async (definitionId: string, context: Record<string, unknown> = {}) => {
      try {
  ...
  const cancelInstance = useCallback(
    (instanceId: string, reason = 'Cancelled by user') => {
  ...
  const completeStep = useCallback(
    async (instanceId: string, stepId: string, output?: unknown) => {
      try {
  ...
  const skipStep = useCallback(
    (instanceId: string, stepId: string, reason = 'Skipped by user') => {
  ...
  const refreshData = useCallback(async () => {
    try {
      await Promise.all([engine.loadDefinitions(), engine.loadInstances()]);
    } catch (error) {
  ...
  const on = useCallback(
    (event: string, handler: (event: WorkflowEvent) => void) => {
