...
export function Stepper({
  ...
  const canGoNext =
    currentStep < steps.length - 1 && (steps[currentStep]?.isValid || !config.validateOnNext);
  ...
  const setStepData = useCallback((step: number, data: unknown, isValid = true) => {
    setSteps((prevSteps) =>
      prevSteps.map((s, index) => (index === step ? { ...s, data, isValid, error: undefined } : s))
    );
  ...
  return (
    <StepperContext.Provider value={contextValue}>
      <div
        className={clsx('dotmac-stepper', className, {
          ...
          <div className='step-marker'>
            ...
            {index < steps.length - 1 && (
              <div
                ...
                style={{
                  backgroundColor:
                    index < currentStep ? config.colors?.completed : config.colors?.pending,
                }}
              />
            )}
          </div>
    ...
    <div className={clsx('stepper-content', className)}>
      ...
      {showNavigation && (
        <div className='stepper-navigation'>
          ...
          <div className='nav-center'>{customActions}</div>
...
export function Step({ children, stepId, onValidate, onStepComplete, className }: StepProps) {
  ...
  return <div className={clsx('step-wrapper', className, `step-${stepId}`)}>{children}</div>;
}
