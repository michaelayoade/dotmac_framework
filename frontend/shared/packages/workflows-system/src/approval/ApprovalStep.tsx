...
import type { WorkflowStep, ApprovalStepConfig, WorkflowInstance } from '../types';
...
export function ApprovalStep({
  ...
  const approvalConfig = useMemo((): ApprovalStepConfig => {
    ...
    return {
      approvers: [{ type: 'role', identifier: 'admin', required: true }],
      policy: 'any',
    };
  }, [step.input]);
  ...
  const userHasActed = useMemo(() => {
    return currentApprovals.some(
      (approval) => approval.approver === currentUserId && approval.status !== 'pending'
    );
  ...
  const approvalStatus = useMemo(() => {
    ...
    const requiredApprovedCount = currentApprovals.filter(
  ...
  return (
    <div className={clsx('approval-step', className)}>
        ...
        <div className='policy-details'>
          ...
          {approvalConfig.escalation && (
            <div className='escalation-info'>
              <small>
                Escalates to {approvalConfig.escalation.to} after {approvalConfig.escalation.delay}{' '}
                minutes
              </small>
            </div>
          )}
      ...
      <div className='approvers-list'>
        <h4>
          Approvers ({currentApprovals.filter((a) => a.status !== 'pending').length}/
                ...
                >
                  <div className='approver-header'>
                    <div className='approver-info'>
                      ...
                      {approval.required && <span className='required-badge'>Required</span>}
                    </div>
