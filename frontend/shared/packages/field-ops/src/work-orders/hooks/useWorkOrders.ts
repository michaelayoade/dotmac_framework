...
interface UseWorkOrdersReturn {
  ...
  // Actions
  createWorkOrder: (
    workOrder: Omit<WorkOrder, 'id' | 'createdAt' | 'lastModified'>
  ) => Promise<void>;
...
export function useWorkOrders(options: UseWorkOrdersOptions = {}): UseWorkOrdersReturn {
  ...
  const loadLocalWorkOrders = useCallback(async () => {
    ...
    try {
      ...
      if (technicianId || user?.id) {
        orders = await workOrderDb.getWorkOrdersByTechnician(technicianId || user!.id, tenantId);
      } else {
          ...
          switch (item.syncType) {
            ...
            case 'complete':
              await apiClient.post(
                `/field-ops/work-orders/${item.workOrder.id}/complete`,
                item.workOrder
              );
              break;
          }
  ...
  const createWorkOrder = useCallback(
    async (workOrderData: Omit<WorkOrder, 'id' | 'createdAt' | 'lastModified'>) => {
      if (!tenantId) return;
  ...
  const filterWorkOrders = useCallback(
    async (filter: WorkOrderFilter): Promise<WorkOrder[]> => {
      if (!tenantId) return [];
      ...
      return results.filter((wo) => {
        ...
        if (filter.dateRange) {
          if (
            wo.scheduledDate < filter.dateRange.start ||
            wo.scheduledDate > filter.dateRange.end
          ) {
            return false;
          }
        }
  ...
  const searchWorkOrders = useCallback(
    async (query: string): Promise<WorkOrder[]> => {
      if (!tenantId) return [];
  ...
  const refreshMetrics = useCallback(async () => {
    ...
    try {
      const dbMetrics = await workOrderDb.getWorkOrderMetrics(tenantId, technicianId || user?.id);
