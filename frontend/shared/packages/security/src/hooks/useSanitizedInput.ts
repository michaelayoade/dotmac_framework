...
export function useSanitizedInput(
  ...
  const performSanitization = useCallback(
    (inputValue: string): SanitizationResult => {
...
export function useSanitizedForm<T extends Record<string, any>>(
  ...
  const sanitizeAll = useCallback(() => {
    const { sanitized, violations: allViolations } = sanitizer.sanitizeObject(
      values,
  ...
  const isValid = useMemo(() => {
    return Object.values(violations).every(
      (fieldViolations) =>
        fieldViolations.length === 0 ||
        fieldViolations.every((v) => v.includes('Applied') || v.includes('sanitized'))
    );
  }, [violations]);
