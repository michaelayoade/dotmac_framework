...
export class RevenueEngine {
   ...
   */
  async calculateCustomerRevenue(params: RevenueCalculationParams): Promise<number> {
    try {
      ...
      throw new Error(
        `Failed to calculate customer revenue: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
   ...
   */
  async calculatePartnerCommissions(params: CommissionCalculationParams): Promise<Commission[]> {
    try {
      ...
      for (const customerRevenue of customerRevenues) {
        ...
        if (
          !this.isCommissionEligible(customerType, {
            includeNewCustomers,
            includeRenewals,
            includeUpgrades,
          })
        ...
        const commissionRate = this.getCommissionRate(
          commissionRates,
          customerRevenue.serviceType,
          customerType
        );
        const commissionAmount = new Decimal(customerRevenue.revenue.amount)
      ...
      throw new Error(
        `Failed to calculate partner commissions: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
   ...
   */
  async calculatePlatformRevenue(params: PlatformRevenueParams): Promise<PlatformRevenue> {
    try {
      ...
      // Build metrics
      const metrics =
        includeMetrics && customerMetrics
          ? {
              ...
              averageRevenuePerCustomer: {
                amount:
                  customerMetrics.totalCustomers > 0
                    ? totalCustomerRevenue / customerMetrics.totalCustomers
                    : 0,
                currency: 'USD',
              },
      ...
      throw new Error(
        `Failed to calculate platform revenue: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private validateRevenueAccess(customerId: string): void {
    ...
    const hasAccess =
      this.context.permissions.includes('revenue:read') ||
  ...
  private validateCommissionAccess(partnerId: string): void {
    const hasAccess =
      this.context.permissions.includes('commission:read') ||
  ...
  private validatePlatformRevenueAccess(tenantId: string): void {
    const hasAccess =
      this.context.permissions.includes('platform:revenue:read') ||
      (this.context.tenantId === tenantId &&
        this.context.permissions.includes('tenant:analytics:read')) ||
      this.context.portalType === 'management-admin';
  ...
  private shouldApplyTaxes(): boolean {
    ...
    return (
      this.context.portalType !== 'management-admin' && this.config.features.revenueCalculation
    );
  }
