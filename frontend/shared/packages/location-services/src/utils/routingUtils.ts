 ...
 */
export function estimateTravelTime(distanceKm: number, travelMode: TravelMode = 'driving'): number {
...
export function estimateRouteCost(
  ...
  if (travelMode === 'driving') {
    const fuelPricePerLiter = options?.fuelPricePerLiter || 1.5; // Default price
    ...
    totalCost = distanceKm * 0.2; // $0.20 per km
...
export function validateRoute(waypoints: LocationData[]): {
  ...
  for (let i = 0; i < waypoints.length - 1; i++) {
    for (let j = i + 1; j < waypoints.length; j++) {
      if (calculateDistance(waypoints[i], waypoints[j]) < 10) {
        // 10 meters
