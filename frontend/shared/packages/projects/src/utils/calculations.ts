...
export const taskCalculations = {
  ...
  calculateTaskVelocity(
    ...
    const completionTimes = recentlyCompleted
      ...
      .map((task) => differenceInDays(new Date(task.updated_at!), new Date(task.created_at!)));
...
export const timeCalculations = {
  ...
  calculateTimeByUser(timeEntries: TimeEntry[]): Record<
    string,
    {
      totalMinutes: number;
      billableMinutes: number;
...
export const milestoneCalculations = {
  ...
  calculateMilestoneProgress(
    ...
    return {
      ...
      completedTasks,
      percentageComplete:
        milestoneTasks.length > 0 ? Math.round((completedTasks / milestoneTasks.length) * 100) : 0,
    };
  },
};
