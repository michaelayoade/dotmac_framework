...
export const TimeTracker: React.FC<TimeTrackerProps> = ({
  ...
  const handleManualEntry = async (e: React.FormEvent) => {
    ...
    if (
      ...
      !manualEntry.start_time ||
      !manualEntry.end_time
    ) {
      return;
    }
  ...
  const filteredEntries = timeEntries.filter(
    (entry) =>
      (!projectId || entry.project_id === projectId) && (!taskId || entry.task_id === taskId)
  );

  const todayEntries = filteredEntries.filter(
    (entry) => format(new Date(entry.start_time), 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd')
  );
    ...
    <div className='time-tracker space-y-6'>
      ...
      <Card className='p-6'>
        ...
        <div className='flex items-center gap-2'>
          ...
          <Button variant='outline' size='sm' onClick={refreshTimeEntries}>
            Refresh
          </Button>
        </div>
      </Card>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setManualEntry((prev) => ({ ...prev, start_time: e.target.value }))
                  }
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setManualEntry((prev) => ({ ...prev, end_time: e.target.value }))
                  }
                />
              </div>
            </div>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setManualEntry((prev) => ({ ...prev, activity_type: e.target.value as any }))
                  }
              ...
              <div>
                <label className='flex items-center mt-6'>
                  <input
                    ...
                    onChange={(e) =>
                      setManualEntry((prev) => ({ ...prev, billable: e.target.checked }))
                    }
                    className='mr-2'
                  />
                  Billable
          ...
          {loading ? (
            <div className='space-y-3'>
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className='animate-pulse flex items-center gap-3 p-3 border rounded-lg'
                >
            ...
            <div className='text-center py-8 text-gray-500'>
              ...
              <p className='text-sm'>
                Start the timer or add a manual entry to begin tracking time
              </p>
            </div>
          )}
        </Card>
