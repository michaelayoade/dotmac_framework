    ...
    for (const [dep, version] of Object.entries(packageJson.dependencies || {})) {
      if (version.startsWith('workspace:')) {
        ...
        workspaceDependencies
          .get(dep)
          .push({ location, version, file: path.relative(frontendPath, file) });
      }
    }
  }
...
async function calculateReusabilityScore(frontendPath) {
  ...
  const sharedComponents = packages.packages.filter(
    (p) => p.name.includes('ui') || p.name.includes('components')
  ).length;
...
function displayArchitecturalReport(analysis) {
  ...
  console.log(
    `  • Reusability: ${getScoreColor(analysis.reusability.score)}${analysis.reusability.score}%${chalk.reset()}`
  );
  console.log(
    `  • Consistency: ${getScoreColor(analysis.consistency.score)}${analysis.consistency.score}%${chalk.reset()}`
  );
  ...
  console.log(
    `  • Custom provider compositions: ${chalk.red(analysis.providers.customCompositions)}`
  );
  console.log(
    `  • Dependency version conflicts: ${chalk.red(analysis.dependencies.versionConflicts)}`
  );
