...
import type { ChatSession, ChatMessage, ChatParticipant, PortalType, ChatStatus } from '../types';
...
function ChatHeader({
  ...
  const activeAgent = session?.participants.find(
    (p) => p.role === 'agent' && p.status === 'active'
  );
...
function ChatMessageComponent({
  ...
  return (
    <div className={`flex ${isOwnMessage ? 'justify-end' : 'justify-start'} mb-3`}>
      <div
        className={`flex ${isOwnMessage ? 'flex-row-reverse' : 'flex-row'} items-start space-x-2 max-w-[80%]`}
      >
...
export function UniversalChatWidget({
  ...
  const showActions = useMemo(() => portalConfig.type !== 'customer', [portalConfig.type]);
          ...
          <div className='flex-1 overflow-y-auto p-4 space-y-4'>
            {!currentSession && (
              <div className='text-center py-8'>
                ...
                <p className='text-gray-600 mb-4'>{isOffline ? offlineMessage : welcomeMessage}</p>
                <button
  ...
  return (
    <div className={`z-50 ${positionClasses}`}>
      ...
      {isOpen && (
        <div
          className={`w-96 h-96 bg-white rounded-lg shadow-xl border transition-all ${isMinimized ? 'h-auto' : ''}`}
        >
          <ChatHeader
            session={currentSession}
