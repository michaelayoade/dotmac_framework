...
function CategoryTree({
  ...
  const renderCategory = useCallback(
    (category: KnowledgeBaseCategory, level = 0) => {
  ...
  return <div className='space-y-1'>{categories.map((category) => renderCategory(category))}</div>;
}
...
function ArticleCard({
  ...
  if (viewMode === 'list') {
    return (
      <div className='flex items-start space-x-4 p-4 border-b hover:bg-gray-50 transition-colors'>
        <div className='flex-shrink-0 mt-1'>{getTypeIcon(article.type)}</div>

        <div className='flex-1 min-w-0'>
          <div className='flex items-start justify-between mb-2'>
            <div className='flex-1'>
              <button onClick={() => onView(article)} className='text-left w-full'>
  ...
  return (
    <div className='bg-white border rounded-lg p-4 hover:shadow-md transition-shadow'>
      ...
      <button onClick={() => onView(article)} className='text-left w-full mb-3'>
    ...
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        ...
        >
          ...
          <span>Filters</span>
          <ChevronRight
            className={`w-4 h-4 transform transition-transform ${showFilters ? 'rotate-90' : ''}`}
          />
        </button>
...
export function UniversalKnowledgeBase({
  ...
  const canRate = useMemo(
    () =>
      allowRating &&
      (portalConfig.type === 'customer' || portalConfig.permissions.includes('create')),
  ...
  const showInternal = useMemo(
    () =>
      showInternalContent &&
      (portalConfig.type === 'admin' ||
        portalConfig.type === 'agent' ||
        portalConfig.type === 'management'),
  ...
  const handleBookmark = useCallback(
    async (articleId: string, bookmarked: boolean) => {
      try {
                ...
                >
                  {filteredArticles.map((article) => (
                    <ArticleCard
                      ...
                      onBookmark={
                        canBookmark
                          ? (bookmarked) => handleBookmark(article.id, bookmarked)
                          : undefined
                      }
