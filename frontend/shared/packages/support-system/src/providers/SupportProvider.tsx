...
import {
  ...
  type SupportOperationsConfig,
  type UseSupportOperationsReturn,
...
function SupportProviderInternal({
  ...
  const features = useMemo(() => getPortalFeatures(portalType), [portalType]);

  const portalConfig = useMemo(() => getPortalConfig(portalType), [portalType]);
  ...
  const contextValue = useMemo<SupportContextType>(
    ...
    [operations, preferences, updatePreferences, ui, features, portalConfig]
  );
...
export function SupportProvider({
  ...
  const content = (
    <QueryClientProvider client={client}>
      <SupportProviderInternal {...props}>{children}</SupportProviderInternal>
    </QueryClientProvider>
  );

  if (enableErrorBoundary) {
    return <SupportErrorBoundary>{content}</SupportErrorBoundary>;
  }
