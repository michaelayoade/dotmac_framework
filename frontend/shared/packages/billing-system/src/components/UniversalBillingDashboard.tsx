...
export function UniversalBillingDashboard({
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const totalRevenue = billing.invoices.reduce(
      (sum, inv) => sum + (inv.status === 'paid' ? inv.totalAmount : 0),
      0
    );
    const outstanding = billing.invoices.reduce(
      (sum, inv) => sum + (inv.status !== 'paid' ? inv.amountDue : 0),
      0
    );
    ...
    return {
      ...
      paymentSuccessRate:
        billing.payments.length > 0
          ? (billing.payments.filter((p) => p.status === 'completed').length /
  ...
  const availableTabs = useMemo(() => {
    const tabs = [{ id: 'overview', label: 'Overview', icon: TrendingUp, count: null }];
    ...
    if (portalFeatures.invoiceGeneration || billing.invoices.length > 0) {
      tabs.push({
        ...
        label: 'Invoices',
        icon: FileText,
        count: billing.invoices.length > 0 ? billing.invoices.length : null,
      });
    }
    ...
    if (portalFeatures.paymentProcessing || billing.payments.length > 0) {
      tabs.push({
        ...
        label: 'Payments',
        icon: CreditCard,
        count: billing.payments.length > 0 ? billing.payments.length : null,
      });
    }
    ...
    if (billing.paymentMethods.length > 0 || portalType !== 'admin') {
      tabs.push({
        ...
        icon: Settings,
        count: billing.paymentMethods.length > 0 ? billing.paymentMethods.length : null,
      });
    }
    ...
    tabs.push({
      ...
      icon: History,
      count:
        billing.invoices.length + billing.payments.length > 0
          ? billing.invoices.length + billing.payments.length
          : null,
    });
    ...
    if (billing.failedPayments.length > 0 && portalFeatures.paymentProcessing) {
      tabs.push({
        ...
        icon: AlertTriangle,
        count: billing.failedPayments.length > 0 ? billing.failedPayments.length : null,
      });
    }
