...
export function UniversalInvoiceGenerator({
    ...
    (index: number, field: keyof InvoiceLineItem, value: any) => {
      ...
      if (field === 'quantity' || field === 'unitPrice') {
        newLineItems[index].amount =
          (newLineItems[index].quantity || 0) * (newLineItems[index].unitPrice || 0);
      }
  ...
  const handleGenerate = useCallback(
    async (action: 'generate' | 'save' | 'send') => {
    ...
    <div className='space-y-4'>
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            setFormData({ ...formData, taxRate: parseFloat(e.target.value) / 100 || 0 })
          }
          className='w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent'
          placeholder='8.00'
        />
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
          ...
          <div className='text-right'>
            ...
            <div className='text-sm text-gray-600'>
              ...
              {formData.accountId && (
                <div className='text-xs text-gray-500 mt-1'>Account: {formData.accountId}</div>
              )}
            </div>
          </div>
        </div>
