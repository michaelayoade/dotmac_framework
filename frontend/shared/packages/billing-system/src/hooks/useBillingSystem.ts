...
export function useBillingSystem(
  options: UseBillingSystemOptions = {}
): BillingSystemState & BillingSystemActions {
  const {
    portalType = 'customer',
    customerId,
    accountId,
  ...
  const sendInvoice = useCallback(
    async (invoiceId: string, email?: string): Promise<void> => {
  ...
  const updateInvoiceStatus = useCallback(
    async (invoiceId: string, status: string): Promise<Invoice> => {
    ...
    async (paymentData: PaymentRequest): Promise<PaymentResponse> => {
      ...
      if (response.payment) {
        setState((prev) => ({
          ...
          failedPayments: prev.failedPayments.filter(
            (p) => p.invoiceId !== response.payment.invoiceId
          ),
        }));
      }
  ...
  const retryPayment = useCallback(
    async (paymentId: string, paymentMethodId?: string): Promise<PaymentResponse> => {
  ...
  const refundPayment = useCallback(
    async (refundData: RefundRequest): Promise<void> => {
  ...
  const updatePaymentMethod = useCallback(
    async (methodId: string, updates: Partial<PaymentMethod>): Promise<PaymentMethod> => {
      ...
      setState((prev) => ({
        ...prev,
        paymentMethods: prev.paymentMethods.map((m) => (m.id === methodId ? updatedMethod : m)),
      }));
