...
export function PortalSwitcher({
  ...
  const accessiblePortals = availablePortals.filter(
    (portal) => portal !== currentPortal && canAccessPortal(portal)
  );
  ...
  return (
    <div className={`portal-switcher ${className}`}>
      <label className='block text-sm font-medium text-gray-700'>Switch Portal</label>
      <select
        className='mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500'
        onChange={(e) => onPortalSwitch(e.target.value as PortalVariant)}
        value={currentPortal}
      >
        <option value={currentPortal}>{getPortalConfig(currentPortal).name} (Current)</option>
