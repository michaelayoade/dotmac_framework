...
export class SecurityService {
  ...
  async getUserEvents(userId: string, limit: number = 50): Promise<AuthEvent[]> {
    ...
    return userEvents.slice(-limit).map((event) => ({
      id: event.id!,
  ...
  private getRecentFailuresByIP(ipAddress: string): number {
    ...
    for (const userEvents of this.events.values()) {
      count += userEvents.filter(
        (event) =>
          event.ipAddress === ipAddress && !event.success && event.timestamp.getTime() > oneHourAgo
      ).length;
    }
  ...
  private cleanupOldEvents(): void {
    ...
    for (const [userId, events] of this.events.entries()) {
      const filteredEvents = events.filter((event) => event.timestamp.getTime() > thirtyDaysAgo);
