...
export function useNetworkMonitoring(
  ...
  const { tenant_id, real_time = true, alert_thresholds = {} } = options;
  ...
  const initializeSocket = useCallback(() => {
    ...
    socket.on(
      'node_status_changed',
      (data: { node_id: string; status: string; timestamp: string }) => {
    ...
    socket.on(
      'link_utilization_updated',
      (data: { link_id: string; utilization: number; timestamp: string }) => {
