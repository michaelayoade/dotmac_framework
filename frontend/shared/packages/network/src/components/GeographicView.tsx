  ...
  useEffect(() => {
    if (network_nodes.length > 0) {
      ...
      if (validNodes.length > 0) {
        const avgLat =
          validNodes.reduce((sum, node) => sum + node.latitude!, 0) / validNodes.length;
        const avgLng =
          validNodes.reduce((sum, node) => sum + node.longitude!, 0) / validNodes.length;
  ...
  return (
    <Card className={`geographic-view ${className} relative w-full h-full`}>
                ...
                <Popup>
                  <div className='p-2'>
                    ...
                    <div className='text-sm mt-1'>
                      ...
                      <div className='mt-2'>Services: {area.service_types.join(', ')}</div>
                    </div>
                  </div>
                </Popup>
              </Polygon>
              ...
              <Popup>
                <div className='p-2 min-w-64'>
                  ...
                  <div className='text-sm mt-1'>
                    ...
                    <div>
                      Status:{' '}
                      <span className={`font-medium ${getStatusTextColor(node.status)}`}>
                        {node.status}
                      </span>
                    </div>
      ...
      <div className='map-status absolute bottom-4 left-4 z-10 bg-white/95 px-3 py-2 rounded text-xs'>
        <span>
          Showing {filteredNodes.length} of {network_nodes.length} nodes
        </span>
...
function getNodeIconSize(nodeType: NodeType): number {
  switch (nodeType) {
    case NodeType.CORE_ROUTER:
      return 30;
    case NodeType.DISTRIBUTION_ROUTER:
      return 24;
    case NodeType.POP:
      return 28;
    case NodeType.DATA_CENTER:
      return 32;
    case NodeType.CELL_TOWER:
      return 22;
    case NodeType.ACCESS_SWITCH:
      return 18;
    case NodeType.WIFI_AP:
      return 16;
    case NodeType.CUSTOMER_PREMISES:
      return 14;
    default:
      return 20;
  }
