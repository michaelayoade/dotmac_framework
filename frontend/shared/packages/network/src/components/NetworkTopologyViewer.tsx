...
export const NetworkTopologyViewer: React.FC<NetworkTopologyProps> = ({
  ...
  const renderWithD3 = useCallback(() => {
    ...
    const simulation = d3
      ...
      .force(
        'link',
        d3
          .forceLink(filteredData.links as any)
          .id((d: any) => d.node_id)
          .distance((d) =>
            (d as NetworkLink).length_km ? (d as NetworkLink).length_km! * 10 : 100
          )
          .strength(0.5)
      )
    ...
    nodeElements
      ...
      .attr('stroke', (d) =>
        currentSelection.selected_nodes.includes(d.node_id) ? '#007bff' : '#fff'
      )
      ...
      style: [
        {
          ...
          style: {
            'background-color': (ele: any) =>
              ele.data('color') || getNodeTypeColor(ele.data('type')),
        ...
        },
        {
          ...
          style: {
            ...
            'target-arrow-color': (ele: any) =>
              ele.data('color') || getStatusColor(ele.data('status')),
  ...
  const handleZoomOut = useCallback(() => {
    if (currentEngine === 'd3' && svgRef.current) {
      const svg = d3.select(svgRef.current);
      svg.transition().call(d3.zoom<SVGSVGElement, unknown>().scaleBy as any, 0.75);
    } else if (cytoscapeInstance.current) {
  ...
  const handleFitView = useCallback(() => {
    if (currentEngine === 'd3' && svgRef.current) {
      ...
      if (bounds) {
        ...
        svg.transition().call(d3.zoom<SVGSVGElement, unknown>().transform as any, transform);
      }
    } else if (cytoscapeInstance.current) {
    ...
    <Card className={`network-topology-viewer ${className}`} style={style}>
      ...
      {showControls && (
        <div className='topology-controls absolute top-4 left-4 z-10 flex gap-2'>
          ...
          <Button variant='outline' size='sm' onClick={handleFitView} title='Fit to View'>
      ...
      <div className='topology-canvas relative w-full h-full'>
        {currentEngine === 'd3' ? (
          ...
          <div ref={cytoscapeRef} className='w-full h-full' style={{ minHeight: '400px' }} />
        )}
      </div>
