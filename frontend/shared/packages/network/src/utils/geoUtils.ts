...
export const geoUtils = {
  ...
  calculateCoverageArea: (
    ...
    for (let i = 0; i < segments; i++) {
      ...
      const lonOffset =
        (radiusKm / (111.32 * Math.cos(geoUtils.toRadians(node.latitude)))) * Math.sin(angle);
  ...
  convertCoordinates: {
    ...
    webMercatorToWGS84: (x: number, y: number): { latitude: number; longitude: number } => {
      ...
      latitude =
        (180 / Math.PI) * (2 * Math.atan(Math.exp((latitude * Math.PI) / 180)) - Math.PI / 2);
  ...
  calculateSignalStrength: (
    ...
    const signalPercentage = Math.max(
      0,
      Math.min(100, ((receivedPower - minSignal) / (maxSignal - minSignal)) * 100)
    );
