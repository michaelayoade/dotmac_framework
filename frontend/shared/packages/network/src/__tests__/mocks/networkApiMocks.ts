...
export const mockCriticalNodes: CriticalNode[] = [
  {
    ...
    impact_analysis:
      'Primary core router serving downtown area. Failure would affect 3 distribution nodes and 12,000 customers.',
    redundancy_paths: 2,
  ...
  },
  {
    ...
    impact_analysis:
      'Key distribution switch for midtown residential area. Serves 4,500 customers.',
    redundancy_paths: 1,
...
export const networkApiMocks = {
    ...
    Promise.resolve({
      data: {
        ...
        buildout_recommendations: [
          'Deploy 3 additional access points',
          'Extend fiber to rural areas',
        ],
      },
    }),
  ...
  getEntityMetrics: (entityId: string, entityType: string, metricName?: string) =>
    Promise.resolve({
      data: mockPerformanceMetrics.filter(
        (m) =>
