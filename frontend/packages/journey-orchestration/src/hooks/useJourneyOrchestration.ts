...
export function useJourneyOrchestration(): UseJourneyOrchestrationReturn {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const journeySubscription = orchestratorRef.current.subscribeToJourneys()
        .subscribe(updatedJourneys => {
          setJourneys(updatedJourneys);
          setLoading(false);
        });

      const activeJourneySubscription = orchestratorRef.current.subscribeToActiveJourney()
        .subscribe(journey => {
          setActiveJourney(journey);
        });

      const stateSubscription = orchestratorRef.current.subscribeToStateChanges()
        .subscribe(change => {
  ...
  // Skip step
  const skipStep = useCallback(async (journeyId: string, stepId: string, reason: string): Promise<void> => {
    if (!orchestratorRef.current) {
      throw new Error('Orchestrator not initialized');
    }
  ...
  // Retry step
  const retryStep = useCallback(async (journeyId: string, stepId: string): Promise<void> => {
    // This would retry a failed step by advancing again
    await advanceStep(journeyId, stepId);
  }, [advanceStep]);

  // Update context
  const updateContext = useCallback(async (journeyId: string, updates: Record<string, any>): Promise<void> => {
    if (!orchestratorRef.current) {
      throw new Error('Orchestrator not initialized');
    }
  ...
  // Complete handoff
  const completeHandoff = useCallback(async (handoffId: string, result: Record<string, any>): Promise<void> => {
    if (!handoffSystemRef.current) {
      throw new Error('Handoff system not initialized');
    }
  ...
  const subscribeToJourney = useCallback((
    journeyId: string,
    callback: (journey: CustomerJourney) => void
  ): (() => void) => {
    if (!orchestratorRef.current) {
      return () => {};
    }

    const subscription = orchestratorRef.current.subscribeToStateChanges()
      .subscribe(change => {
