...
const meta: Meta<typeof ProtectedComponent> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'Component that conditionally renders children based on user permissions and roles. Supports fine-grained access control with fallback options.'
      }
    }
  },
  argTypes: {
      ...
      >
        <div className="p-4 space-y-4">
          <div className="bg-blue-50 p-3 rounded">
            ...
            <br />Permissions: {(context.globals.userPermissions || ['users:read', 'billing:read']).join(', ')}
            <br />Roles: {(context.globals.userRoles || ['admin']).join(', ')}
          </div>
          <Story />
        </div>
      </AccessControlProvider>
...
export const WithPermission: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Component protected by a single permission. Content shows when user has "users:read" permission.'
      }
    }
  }
};
...
export const WithMultiplePermissions: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Component requiring ANY of multiple permissions. Content shows when user has at least one permission.'
      }
    }
  }
};
...
export const WithAllPermissionsRequired: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Component requiring ALL permissions. Content only shows when user has both permissions.'
      }
    }
  }
};
  ...
  args: {
    ...
    fallback: (
      <div className="bg-yellow-100 border border-yellow-400 p-4 rounded">
        ...
        <p className="text-yellow-700">Access attempt will trigger callback (check browser alert).</p>
      </div>
    ),
    children: <ProtectedContent />
  },
