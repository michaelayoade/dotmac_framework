...
export const useMetrics = (options: UseMetricsOptions = {}) => {
  ...
  const getMetricsByCategory = useCallback((targetCategory: string) => {
    return metrics.filter(metric => metric.category === targetCategory);
  }, [metrics]);

  const getMetricsByStatus = useCallback((status: KPIMetric['status']) => {
  ...
  const summary = {
    ...
    lastUpdated: metrics.length > 0
      ? new Date(Math.max(...metrics.map(m => m.updatedAt.getTime())))
      : null,
  };
