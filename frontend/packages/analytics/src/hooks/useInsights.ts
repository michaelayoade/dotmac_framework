...
export const useInsights = (options: UseInsightsOptions = {}) => {
  ...
  const getInsightsByType = useCallback((type: BusinessIntelligenceInsight['type']) => {
    return insights.filter(insight => insight.type === type);
  }, [insights]);

  const getInsightsBySeverity = useCallback((targetSeverity: BusinessIntelligenceInsight['severity']) => {
    return insights.filter(insight => insight.severity === targetSeverity);
  }, [insights]);

  const getInsightsByStatus = useCallback((targetStatus: BusinessIntelligenceInsight['status']) => {
