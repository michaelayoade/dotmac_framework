...
export const DataTable: React.FC<DataTableProps> = ({
  ...
  const isAllSelected = selection && selection.selectedRows.length === sortedData.length && sortedData.length > 0;
  const isPartiallySelected = selection && selection.selectedRows.length > 0 && selection.selectedRows.length < sortedData.length;
            ...
            {loading ? (
              Array.from({ length: 5 }).map((_, index) => (
                <tr key={index} className="animate-pulse">
                  {selection && <td className="px-6 py-4"><div className="w-4 h-4 bg-gray-200 rounded"></div></td>}
                  {columns.map(column => (
