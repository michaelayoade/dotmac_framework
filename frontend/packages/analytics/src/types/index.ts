...
export interface AnalyticsContextValue {
  ...
  actions: {
    ...
    createDashboard: (dashboard: Omit<AnalyticsDashboard, 'id' | 'createdAt' | 'updatedAt'>) => Promise<string>;
    ...
    // Insights
    getInsights: (filters?: { category?: string; severity?: string }) => Promise<BusinessIntelligenceInsight[]>;
...
export type ChartType = 'line' | 'area' | 'bar' | 'pie' | 'donut' | 'scatter' | 'heatmap' | 'funnel';
export type AggregationType = 'sum' | 'avg' | 'min' | 'max' | 'count' | 'distinct' | 'median' | 'percentile';
export type ComparisonType = 'previous_period' | 'same_period_last_year' | 'target' | 'baseline';
export type AlertCondition = 'greater_than' | 'less_than' | 'equals' | 'percentage_change' | 'anomaly';
