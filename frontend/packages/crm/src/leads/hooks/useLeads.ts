...
interface UseLeadsReturn {
  ...
  convertToCustomer: (leadId: string, customerData: Partial<CustomerAccount>) => Promise<CustomerAccount>;
...
export function useLeads(options: UseLeadsOptions = {}): UseLeadsReturn {
    ...
    try {
      ...
      if (currentFilter) {
        ...
        leadsList = results.filter(lead => {
          ...
          if (currentFilter.priority && !currentFilter.priority.includes(lead.priority)) return false;
          if (currentFilter.assignedTo && lead.assignedTo && !currentFilter.assignedTo.includes(lead.assignedTo)) return false;

          if (currentFilter.createdAfter && lead.createdAt < currentFilter.createdAfter) return false;
          if (currentFilter.createdBefore && lead.createdAt > currentFilter.createdBefore) return false;
  ...
  // Convert to customer
  const convertToCustomer = useCallback(async (leadId: string, customerData: Partial<CustomerAccount>): Promise<CustomerAccount> => {
    if (!tenantId || !user?.id) throw new Error('Missing required information');
  ...
  const getLeadScoreFactors = useCallback((lead: Lead) => {
    ...
    const sourceScores: Record<string, number> = {
      website: 20, referral: 30, advertising: 15, social_media: 10,
      cold_call: 5, trade_show: 25, partner: 35
    };
  ...
  // Schedule follow up
  const scheduleFollowUp = useCallback(async (leadId: string, date: string, notes: string) => {
    await updateLead(leadId, {
  ...
  // Set next follow up
  const setNextFollowUp = useCallback(async (leadId: string, date: string) => {
  ...
  // Filter leads
  const filterLeads = useCallback(async (filter: LeadFilter) => {
  ...
  const getHighScoreLeads = useCallback(async (minScore = 80): Promise<Lead[]> => {
    if (!tenantId) return [];
  ...
  const getLeadsBySource = useCallback(async (): Promise<Record<string, number>> => {
    ...
    const allLeads = await crmDb.leads
      .where('tenantId')
      .equals(tenantId)
      .toArray();
  ...
  // Bulk operations
  const bulkAssignLeads = useCallback(async (leadIds: string[], userId: string) => {
    try {
  ...
  const bulkUpdateStatus = useCallback(async (leadIds: string[], status: Lead['status']) => {
    try {
  ...
  const exportLeads = useCallback(async (format: 'csv' | 'json'): Promise<string> => {
    ...
    if (format === 'json') {
      ...
      const headers = [
        'Name', 'Company', 'Email', 'Phone', 'Status', 'Source', 'Priority',
        'Score', 'Budget', 'Interested Services', 'Assigned To', 'Created Date'
      ];
