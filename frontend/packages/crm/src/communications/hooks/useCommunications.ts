...
interface UseCommunicationsReturn {
  ...
  forwardCommunication: (communicationId: string, to: string[], note?: string) => Promise<Communication>;
  ...
  getCommunicationTrends: (days?: number) => Promise<{ date: string; count: number; type: CommunicationType }[]>;

  // Templates
  getEmailTemplates: () => Promise<{ id: string; name: string; subject: string; content: string }[]>;
  renderTemplate: (templateId: string, variables: Record<string, string>) => Promise<{ subject: string; content: string }>;
...
export function useCommunications(options: UseCommunicationsOptions = {}): UseCommunicationsReturn {
    ...
    try {
      ...
      if (customerId) {
        ...
        communicationsList = results.filter(comm => {
          ...
          if (currentFilter.direction && !currentFilter.direction.includes(comm.direction)) return false;
          if (currentFilter.customerId && comm.customerId !== currentFilter.customerId) return false;
          ...
          if (currentFilter.sentiment && !currentFilter.sentiment.includes(comm.sentiment!)) return false;
  ...
  // Send SMS
  const sendSMS = useCallback(async (params: {
    phoneNumber: string;
    message: string;
  }): Promise<Communication> => {
    if (!user?.id || !tenantId) throw new Error('Missing required information');
  ...
  const addNote = useCallback(async (params: {
    ...
    const communication: Communication = {
      ...
      subject: 'Note',
      content: params.content,
      summary: params.summary || (params.content.substring(0, 100) + (params.content.length > 100 ? '...' : '')),
      fromAddress: user.id,
      toAddresses: [],
      timestamp: new Date().toISOString(),
      status: 'sent',
  ...
  // Add reply
  const addReply = useCallback(async (communicationId: string, content: string): Promise<Communication> => {
  ...
  // Upload attachment
  const uploadAttachment = useCallback(async (communicationId: string, file: File): Promise<CommunicationAttachment> => {
  ...
  // Download attachment
  const downloadAttachment = useCallback(async (attachmentId: string): Promise<Blob> => {
    try {
  ...
  const filterCommunications = useCallback(async (filter: CommunicationFilter) => {
  ...
  const getCommunicationsByType = useCallback(async (type: CommunicationType): Promise<Communication[]> => {
    if (!tenantId) return [];
  ...
  const getRecentCommunications = useCallback(async (days = 7): Promise<Communication[]> => {
    if (!tenantId) return [];
  ...
  // Bulk operations
  const bulkArchive = useCallback(async (communicationIds: string[]) => {
    try {
  ...
  const bulkMarkAsRead = useCallback(async (communicationIds: string[]) => {
    try {
      const updatePromises = communicationIds.map(id =>
        crmDb.communications.update(id, {
          status: 'read',
  ...
  const exportCommunications = useCallback(async (format: 'csv' | 'json'): Promise<string> => {
    ...
    if (format === 'json') {
      ...
      const headers = [
        'Date', 'Type', 'Direction', 'Subject', 'From', 'To', 'Status',
        'Duration', 'User', 'Customer ID', 'Lead ID'
      ];
  ...
  return {
    ...
    markAsRead,
    addReply,
    forwardCommunication: async () => { throw new Error('Not implemented') }, // Simplified for brevity
    archiveCommunication: async (id) => bulkArchive([id]),

    // Attachments
    uploadAttachment,
    downloadAttachment,
    removeAttachment: async () => { throw new Error('Not implemented') }, // Simplified for brevity
    ...
    getSentimentAnalysis: async () => metrics?.sentimentAnalysis || { positive: 0, neutral: 0, negative: 0 },
    ...
    bulkArchive,
    bulkMarkAsRead,
    bulkDelete: async () => { throw new Error('Not implemented') }, // Simplified for brevity
