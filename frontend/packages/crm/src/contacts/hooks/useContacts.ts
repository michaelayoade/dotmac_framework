...
export function useContacts(options: UseContactsOptions = {}): UseContactsReturn {
  ...
  const updateContactMethod = useCallback(async (methodId: string, updates: Partial<ContactMethod>) => {
    if (!customerId) throw new Error('No customer selected');
  ...
  const removeContactMethod = useCallback(async (methodId: string) => {
    if (!customerId) throw new Error('No customer selected');
  ...
  // Set primary contact
  const setPrimaryContact = useCallback(async (methodId: string, type: 'email' | 'phone') => {
    if (!customerId) throw new Error('No customer selected');
    ...
    const updatedMethods = customer.contactMethods.map(method => ({
      ...method,
      isPrimary: method.id === methodId && (method.type === type ||
        (type === 'phone' && ['phone', 'mobile'].includes(method.type)))
    }));
  ...
  // Send email
  const sendEmail = useCallback(async (to: string, subject: string, content: string) => {
    ...
    try {
      ...
      await crmDb.communications.add({ ...communication, id: commId, createdAt: communication.timestamp });
  ...
  // Make call
  const makeCall = useCallback(async (phone: string, notes?: string) => {
    if (!customerId || !user?.id) throw new Error('Missing required information');
  ...
  // Update preferences
  const updatePreferences = useCallback(async (methodId: string, preferences: ContactMethod['preferences']) => {
    await updateContactMethod(methodId, { preferences });
  }, [updateContactMethod]);

  // Check do not contact
  const checkDoNotContact = useCallback((type: 'email' | 'phone' | 'sms'): boolean => {
  ...
  // Validate phone
  const validatePhone = useCallback(async (phone: string): Promise<boolean> => {
  ...
  // Quick email
  const sendQuickEmail = useCallback(async (templateId: string, variables: Record<string, string> = {}) => {
    if (!customerId) throw new Error('No customer selected');
  ...
  // Schedule follow up
  const scheduleFollowUp = useCallback(async (date: string, notes: string) => {
    if (!customerId || !user?.id) throw new Error('Missing required information');
  ...
  const primaryPhone = contactMethods.find(m => ['phone', 'mobile'].includes(m.type) && m.isPrimary) || null;
