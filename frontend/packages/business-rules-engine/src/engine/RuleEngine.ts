...
export class RuleEngine {
	...
	async executeRules(
		...
		const activeRules = rules.filter(
			(rule) =>
				rule.status === "active" &&
				(!rule.portalScope.length ||
					rule.portalScope.includes(context.portal!)),
		);
	...
	private async executeRule(
		...
		const allConditionsMet = this.checkConditionLogic(
			rule,
			conditionsEvaluated,
		);
		...
		try {
			switch (condition.operator) {
				...
				case "not_in":
					...
					break;
				case "matches":
					result = new RegExp(expectedValue).test(
	...
	private async executeWebhook(
		...
		const webhookUrl =
			this.config.webhookEndpoints?.[action.parameters?.endpoint || ""];
		if (!webhookUrl) {
