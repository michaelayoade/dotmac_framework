	...
	getRulesByPortal(portal: string): BusinessRule[] {
		return Array.from(this.rules.values()).filter(
			(rule) =>
				...
				rule.portalScope.includes(portal as any),
		);
	}
	...
	async executeRules(
		...
		if (filters?.category) {
			rulesToExecute = rulesToExecute.filter(
				(rule) => rule.category === filters.category,
			);
		}
		...
		if (filters?.portal) {
			rulesToExecute = rulesToExecute.filter(
				(rule) =>
					rule.portalScope.length === 0 ||
					rule.portalScope.includes(filters.portal as any),
			);
		}
		...
		rule.actions.forEach((action, index) => {
			...
			switch (action.type) {
				case "set_value":
					if (!action.target)
						errors.push(
							`Action ${index + 1}: target is required for set_value`,
						);
					break;
				case "send_notification":
					if (!action.parameters?.message)
						errors.push(
							`Action ${index + 1}: message parameter is required for send_notification`,
						);
					break;
				case "apply_discount":
					if (!action.value && !action.parameters?.percent) {
						errors.push(
							`Action ${index + 1}: discount value or percent parameter is required`,
						);
					}
					break;
			}
	...
	importRules(
		...
		try {
			...
			rules.forEach((rule, index) => {
				...
				if (!validation.valid) {
					result.errors.push(
						`Rule ${index + 1}: ${validation.errors.join(", ")}`,
					);
					result.skipped++;
					return;
				}
