...
export const ISPRuleTemplates: RuleTemplate[] = [
	// Billing & Payment Rules
	{
		...
		description:
			"Automatically suspend service when payment is overdue by specified days",
		category: "billing",
		template: {
			name: "Auto Suspend Service for Non-Payment",
			description:
	...
	// Service Provisioning Rules
	{
		...
		description:
			"Automatically approve fiber installation requests in covered areas",
			...
			actions: [
				...
				},
				{
					...
					parameters: {
						...
						description:
							"Customer {{customer.id}} approved for fiber installation",
			...
			actions: [
				...
				},
				{
					...
					parameters: {
						...
						description:
							"Customer {{customer.id}} upgraded to enterprise - assign account manager",
			...
			actions: [
				...
				},
				{
					...
					parameters: {
						type: "in_app",
						message:
							"VIP customer {{customer.id}} has an open ticket - priority handling required",
	...
	// Compliance Rules
	{
		...
		template: {
			name: "GDPR Data Retention Compliance",
			description:
				"Archive or delete customer data based on retention policies",
			category: "compliance",
			conditions: [
				{
					id: "condition_1",
			...
			actions: [
				{
					...
					parameters: {
						title:
							"GDPR compliance - review customer data for archival/deletion",
						description:
				...
				},
				{
					...
					parameters: {
						...
						message:
							"GDPR data retention review triggered for customer {{customer.id}}",
					},
				},
				{
					id: "action_3",
...
export function instantiateRuleTemplate(
	...
	const processedRuleString = ruleString.replace(
		/\{\{(\w+)\}\}/g,
		(match, varName) => {
...
export function getRuleTemplateCategories(): string[] {
	return Array.from(
		new Set(ISPRuleTemplates.map((template) => template.category)),
	);
}
