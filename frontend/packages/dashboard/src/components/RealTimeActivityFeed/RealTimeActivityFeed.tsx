...
import { ActivityFeed, type ActivityFeedProps, ActivityFeedPresets } from '../ActivityFeed/ActivityFeed';
...
export interface RealTimeActivityFeedProps extends Omit<ActivityFeedProps, 'activities' | 'onRefresh'> {
...
export const RealTimeActivityFeed: React.FC<RealTimeActivityFeedProps> = ({
  ...
  const defaultTransformActivity = useCallback((eventType: string, rawData: any): Activity | null => {
    ...
    switch (variant) {
      case 'admin':
        ...
        if (eventType === 'network_alert' && rawData.severity === 'error') {
          return ActivityFeedPresets.admin.networkOutage(
            rawData.location || 'Unknown Location'
          );
        }
        break;
        ...
        try {
          ...
          if (newActivity) {
            ...
            if (notificationsEnabled && isConnected) {
              showToast({
                type: newActivity.type === 'error' ? 'error' :
                      newActivity.type === 'warning' ? 'warning' :
                      newActivity.type === 'success' ? 'success' : 'info',
                title: 'New Activity',
  ...
  }, [eventTypes, subscribe, transformActivity, defaultTransformActivity, maxActivities, onError, notificationsEnabled, showToast, isConnected]);
  ...
  const ConnectionStatus = () => {
    ...
    const getStatusConfig = () => {
      ...
      if (isConnected) {
        ...
        const qualityColor = qualityColors[connectionQuality] || qualityColors.offline || 'text-gray-500 bg-gray-100';
