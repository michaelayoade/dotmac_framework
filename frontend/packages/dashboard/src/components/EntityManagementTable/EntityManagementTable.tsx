              ...
              <div className="flex items-center gap-2 mr-3">
                ...
                {bulkActions.map((action) => (
                  <Button
                    ...
                    className="gap-1"
                  >
                    {action.icon && React.createElement(action.icon as any, { className: "w-3.5 h-3.5" })}
                    {action.label}
                  </Button>
                ))}
              </div>
        ...
        <table className={cn(tableVariants({ variant }))}>
          ...
          <thead>
            <tr>
              ...
              {actions.length > 0 && (
                <th className={cn(headerVariants({ variant }), 'w-20 text-center')}>
                  Actions
                </th>
              )}
            </tr>
                    ...
                    {actions.length > 0 && (
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          {actions.filter(action => action.isVisible?.(item) !== false).length > 2 && (
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-8 w-8 p-0"
        ...
        <div className="px-4 py-3 border-t border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <select
              ...
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => pagination.onPageSizeChange(Number(e.target.value))}
...
export const TABLE_CONFIGS = {
  tenants: {
    ...
    actions: [
      ...
      createEntityAction('delete', 'Delete', Trash2, (tenant) => console.log('Delete', tenant), 'danger')
    ]
  },
  customers: {
    columns: [
