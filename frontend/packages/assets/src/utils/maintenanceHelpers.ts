...
export const maintenanceHelpers = {
  ...
  getMaintenanceTypeColor: (type: MaintenanceType): string => {
    switch (type) {
      ...
      case 'corrective': return '#ef4444';
      case 'predictive': return '#3b82f6';
      case 'condition_based': return '#8b5cf6';
      case 'calibration': return '#f59e0b';
      case 'inspection': return '#06b6d4';
      case 'upgrade': return '#10b981';
      case 'replacement': return '#f97316';
  ...
  getMaintenanceUrgency: (schedule: MaintenanceSchedule): 'overdue' | 'due_today' | 'due_soon' | 'scheduled' => {
  ...
  calculateMaintenanceCost: (records: MaintenanceRecord[]): {
    totalCost: number;
    averageCost: number;
    ...
    const costByType = records.reduce((acc, record) => {
      acc[record.maintenance_type] = (acc[record.maintenance_type] || 0) + (record.actual_cost || 0);
      return acc;
  ...
  getMaintenanceEfficiency: (records: MaintenanceRecord[]): {
    ...
    const assetMaintenanceCount = records.reduce((acc, record) => {
  ...
  generateMaintenanceReport: (
    ...
    const overdueSchedules = assetSchedules.filter(s => maintenanceHelpers.isMaintenanceOverdue(s));
    ...
    const lastMaintenance = assetRecords
    ...
    return {
      ...
      costAnalysis,
      efficiency,
      recommendations: maintenanceHelpers.generateMaintenanceRecommendations(asset, assetRecords, overdueSchedules)
    };
  },
  ...
  generateMaintenanceRecommendations: (
    ...
    if (overdueSchedules.length > 0) {
      recommendations.push(`${overdueSchedules.length} maintenance task(s) are overdue. Schedule immediately to prevent downtime.`);
    }
    ...
    if (correctiveRecords.length > 2) {
      recommendations.push('High frequency of corrective maintenance detected. Consider increasing preventive maintenance intervals.');
    }
    ...
    if (asset.purchase_date) {
      ...
      const recentRecords = records.filter(r =>
        (Date.now() - new Date(r.performed_date).getTime()) / (1000 * 60 * 60 * 24) <= 365
      );

      if (ageInYears > 5 && recentRecords.length < 2) {
        recommendations.push('Asset is aging and requires more frequent maintenance. Consider quarterly inspections.');
      }
    }
    ...
    switch (asset.condition) {
      case 'poor':
        recommendations.push('Asset condition is poor. Schedule comprehensive inspection and consider replacement planning.');
        break;
      case 'needs_repair':
        recommendations.push('Asset needs repair. Address immediately to prevent further deterioration.');
        break;
      case 'beyond_repair':
    ...
    if (asset.purchase_price && costAnalysis.totalCost > asset.purchase_price * 0.5) {
      recommendations.push('Total maintenance costs exceed 50% of asset value. Evaluate replacement options.');
    }
