import type { AssetStatus, AssetCondition, Asset, MaintenanceStatus, MaintenancePriority } from '../types';
...
export const assetStatusHelpers = {
  ...
  getConditionColor: (condition: AssetCondition): string => {
    switch (condition) {
      ...
      case 'beyond_repair': return '#dc2626';
      default: return '#9ca3af';
    }
  },
  ...
  needsAttention: (asset: Asset): boolean => {
    return asset.condition === 'needs_repair' ||
           asset.status === 'in_maintenance' ||
           (asset.warranty_expiry && new Date(asset.warranty_expiry) < new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)); // 30 days
  },
  ...
  getWarrantyStatus: (asset: Asset): 'active' | 'expiring_soon' | 'expired' | 'unknown' => {
    ...
    const daysUntilExpiry = Math.ceil((warrantyExpiry.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
