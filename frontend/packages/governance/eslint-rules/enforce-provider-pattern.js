...
module.exports = createRule({
  ...
  meta: {
    ...
    hasSuggestions: true,
    messages: {
      forbiddenProvider: 'Direct use of {{providerName}} is forbidden. Use UniversalProviders instead.',
      nestedProviders: 'Multiple provider nesting detected. Use UniversalProviders for standardized composition.',
  ...
  create(context, [options = {}]) {
    ...
    const forbiddenProviders = new Set([...FORBIDDEN_DIRECT_PROVIDERS, ...(options.forbiddenProviders || [])]);
...
function isAppRootFile(filename) {
  return filename.includes('app/providers.tsx') ||
...
function hasUniversalProviderImport(sourceCode) {
  ...
  return imports.some(importNode => {
    return importNode.source.value === '@dotmac/providers' &&
