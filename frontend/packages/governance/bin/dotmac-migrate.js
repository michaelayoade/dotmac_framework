...
async function analyzeProject(frontendPath) {
  ...
  console.log(`  • Reusability score: ${getScoreColor(analysis.reusabilityScore)}${analysis.reusabilityScore}%${chalk.reset()}`);
  ...
  console.log(`  • Consistency score: ${getScoreColor(analysis.consistencyScore)}${analysis.consistencyScore}%${chalk.reset()}`);
...
async function findProviderFiles(frontendPath) {
  ...
  for (const file of files) {
    ...
    const providerCount = (content.match(/(QueryClientProvider|ThemeProvider|AuthProvider|TenantProvider)/g) || []).length;
...
async function migrateProviderFile(file) {
  ...
  const migratedContent = content
    ...
    .replace(/export function Providers\(\{[^}]*\}\) \{[\s\S]*?\}/g,
...
async function performArchitecturalAnalysis(frontendPath) {
  ...
  return {
    ...
    reusabilityScore: Math.max(0, Math.round(((UNIFIED_COMPONENTS.length - duplicates.length) / UNIFIED_COMPONENTS.length) * 100)),
    customProviders: providerFiles.length,
    universalProviders: PORTALS.length - providerFiles.length,
    consistencyScore: Math.max(0, Math.round(((PORTALS.length - providerFiles.length) / PORTALS.length) * 100)),
