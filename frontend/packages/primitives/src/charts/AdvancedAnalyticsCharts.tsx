...
export const AdvancedAnalyticsCharts: React.FC<AdvancedAnalyticsChartsProps> = ({
  ...
  const customerAnalysis = useMemo(() => {
    ...
    const avgChurn = timeSeriesData.reduce((sum, data) => sum + data.churn, 0) / timeSeriesData.length;
    const avgARPU = timeSeriesData.reduce((sum, data) => sum + data.arpu, 0) / timeSeriesData.length;
            ...
            <div key={metric.id} className="metric-card">
              ...
              <div className="metric-value">
                {metric.unit === 'currency' ? formatCurrency(metric.value) : 
                 metric.unit === 'percent' ? formatPercent(metric.value / 100) :
                 formatNumber(metric.value)}
              </div>
              {metric.changePercent && (
                <div className={`metric-change ${metric.changePercent > 0 ? 'positive' : 'negative'}`}>
            ...
            <div className="chart-section">
              ...
              <ResponsiveContainer width="100%" height={400}>
                <ComposedChart data={timeSeriesData}>
                  ...
                  <Line yAxisId="right" type="monotone" dataKey="costs" stroke={CHART_COLORS.danger} name="Costs" />
            ...
            <div className="chart-section">
              ...
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={serviceMetrics}>
                  ...
                  <Bar yAxisId="left" dataKey="subscribers" fill={CHART_COLORS.primary} name="Subscribers" />
                  <Bar yAxisId="left" dataKey="revenue" fill={CHART_COLORS.secondary} name="Revenue" />
                  <Line yAxisId="right" type="monotone" dataKey="satisfaction" stroke={CHART_COLORS.accent} name="Satisfaction" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
                ...
                <ScatterChart data={geographicData}>
                  ...
                  <Tooltip 
                    formatter={(value, name, props) => [
                      name === 'customers' ? formatNumber(value as number) : 
