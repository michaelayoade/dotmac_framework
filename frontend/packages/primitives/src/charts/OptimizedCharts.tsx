...
const createDataSelector = <T extends any[]>(
  validator: any,
  fallback: T
) => createMemoizedSelector(
  (data: unknown) => {
...
const OptimizedTooltip: React.FC<CustomTooltipProps> = memo(({ 
  ...
  const safeLabel = useMemo(() => 
    label ? sanitizeText(String(label)) : '', [label]
  );
...
export const OptimizedRevenueChart: React.FC<RevenueChartProps> = memo(({ 
  ...
  const [activeIndex, setActiveIndex, throttledActiveIndex] = useThrottledState<number | null>(null, 16);
  ...
  const dataSelector = useMemo(() => 
    createDataSelector(revenueDataSchema, []), []
  );
  
  const validatedData = useMemo(() => 
    dataSelector(data), [dataSelector, data]
  );

  const safeClassName = useMemo(() => 
    validateClassName(className), [className]
  );
