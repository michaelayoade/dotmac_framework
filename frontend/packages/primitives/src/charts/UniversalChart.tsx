...
export function UniversalChart({
  ...
  // Format functions
  const formatXAxis = useCallback((value: any) => {
    if (xAxis?.format) return xAxis.format(value);
    if (value instanceof Date) return value.toLocaleDateString();
    return value;
  }, [xAxis]);

  const formatYAxis = useCallback((value: any, axis: 'left' | 'right' = 'left') => {
      ...
      case 'area':
        return (
          <AreaChart {...commonProps}>
            ...
            {!yAxis?.left?.hide && (
              <YAxis tickFormatter={(value) => formatYAxis(value, 'left')} />
            )}
            {showTooltip && <Tooltip content={<CustomTooltip />} />}
            {showLegend && <Legend content={<CustomLegend onLegendClick={onLegendClick} />} />}
            {seriesWithColors.map((s) => (
  ...
  if (loading) {
    return (
      <div className={cn('bg-white rounded-lg border border-gray-200', cardWrapper && 'p-6', className)}>
