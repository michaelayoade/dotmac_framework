  ...
  const handleClick = useCallback(() => {
    try {
      if (onClick) {
        setIsPressed(true);
        onClick();
        announceToScreenReader(`Status changed to ${computedValues.accessibleStatusText}`, 'polite');
...
export const OptimizedUptimeIndicator: React.FC<UptimeIndicatorProps> = memo(({
  ...
  const computedValues = useMemo(() => {
    ...
    if (validatedUptime >= 99.9) {
      uptimeStatus = { status: 'excellent', color: 'text-green-600', bg: 'bg-green-500', label: 'Excellent' };
      ...
      uptimeStatus = { status: 'fair', color: 'text-yellow-600', bg: 'bg-yellow-500', label: 'Fair' };
    } else {
