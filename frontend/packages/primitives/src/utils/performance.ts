...
export const useRenderProfiler = (componentName: string, props?: any) => {
  ...
  useEffect(() => {
    ...
    const propsChanges = props && lastProps.current 
...
export const generatePerformanceReport = (): {
  ...
  const slowest = profiles.reduce((prev, curr) => curr.avgRenderTime > prev.avgRenderTime ? curr : prev);
  const fastest = profiles.reduce((prev, curr) => curr.avgRenderTime < prev.avgRenderTime ? curr : prev);
