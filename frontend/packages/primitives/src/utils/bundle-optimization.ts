...
export const LazyStatusIndicators = {
  StatusBadge: createDynamicImport(
    () => import('../indicators/OptimizedStatusIndicators').then(m => ({ default: m.OptimizedStatusBadge })),
    'StatusBadge'
  ),
  
  UptimeIndicator: createDynamicImport(
    () => import('../indicators/OptimizedStatusIndicators').then(m => ({ default: m.OptimizedUptimeIndicator })),
    'UptimeIndicator'
  ),
  
  NetworkPerformanceIndicator: createDynamicImport(
    () => import('../indicators/StatusIndicators').then(m => ({ default: m.NetworkPerformanceIndicator })),
  ...
  try {
    ...
    components.forEach(component => {
      ...
      if (component.size > SplitPoints.CHARTS.threshold) {
        analysis.recommendations.push(`Consider code splitting for ${component.name} (${(component.size / 1000).toFixed(1)}KB)`);
        analysis.splitPoints.push(component.name);
      } else if (component.size > 20000) {
        analysis.recommendations.push(`Monitor bundle size for ${component.name} (${(component.size / 1000).toFixed(1)}KB)`);
      }
    });

    // Overall bundle recommendations
    if (analysis.totalSize > 200000) {
      analysis.recommendations.push('Bundle size exceeds 200KB - implement aggressive code splitting');
    } else if (analysis.totalSize > 100000) {
      analysis.recommendations.push('Bundle size is large - consider lazy loading non-critical components');
    }
