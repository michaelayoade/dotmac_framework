...
import { UniversalMap, UniversalMapProps, MapMarker, ServiceArea, NetworkNode, Route } from './UniversalMap';
...
export function TechnicianRouteMap({
  ...
  const technicianMarkers: MapMarker[] = technicians.map(tech => ({
    ...
    status: tech.status === 'available' ? 'active' as const :
           tech.status === 'busy' ? 'maintenance' as const : 'inactive' as const,
    title: tech.name,
  ...
  const workOrderMarkers: MapMarker[] = workOrders.map(order => ({
    ...
    status: order.priority === 'urgent' ? 'error' as const :
           order.priority === 'high' ? 'maintenance' as const : 'active' as const,
...
export function NetworkOutageMap({
  ...
  onOutageSelect,
  ...props
}: NetworkOutageMapProps) {
  const outageMarkers: MapMarker[] = outages.map(outage => ({
    id: outage.id,
    position: outage.location,
    type: 'issue' as const,
    status: outage.severity === 'critical' ? 'error' as const :
           outage.severity === 'major' ? 'maintenance' as const : 'inactive' as const,
...
export function SignalStrengthMap({
  ...
  const markers: MapMarker[] = filteredData.map((data, index) => {
    const strengthLevel = data.strength > -70 ? 'active' :
                         data.strength > -85 ? 'maintenance' : 'error';
