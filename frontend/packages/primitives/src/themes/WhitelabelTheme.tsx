...
export const WhitelabelThemeProvider: React.FC<WhitelabelThemeProviderProps> = ({
  ...
  return (
    <WhitelabelContext.Provider value={contextValue}>
      <ISPBrandTheme theme={enhancedTheme}>
        {children}
      </ISPBrandTheme>
...
export const WhitelabelLogo: React.FC<{
  ...
  const logoUrl = variant === 'dark' && config.brand.logo_dark 
...
export const WhitelabelContact: React.FC<{
  ...
  if (type === 'email') {
    return <a href={`mailto:${value}`} className={className}>{value}</a>;
  }
  
  if (type === 'phone') {
    return <a href={`tel:${value}`} className={className}>{value}</a>;
  }
  
  if (type === 'support') {
    return <a href={value} className={className} target="_blank" rel="noopener noreferrer">Support</a>;
  }
