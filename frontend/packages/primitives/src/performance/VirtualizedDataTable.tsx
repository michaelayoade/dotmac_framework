...
export function VirtualizedDataTable({
  ...
  const handleScroll = useCallback((scrollOffset: number, scrollDirection: string) => {
    if (hasNextPage && loadMore && scrollDirection === 'forward') {
      const scrollPercentage = scrollOffset / (data.length * (typeof rowHeight === 'number' ? rowHeight : 48));
