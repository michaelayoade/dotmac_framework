...
interface AuditHandler {
  system: (type: string, context: any, success: boolean, severity: SecurityLevel, metadata?: any) => Promise<void>;
  security: (type: string, context: any, severity: SecurityLevel, success: boolean, metadata?: any) => Promise<void>;
}
    ...
    if (process.env.NODE_ENV === 'production') {
      try {
        await fetch('/api/audit/system', {
          ...
          body: JSON.stringify({ type, context, success, severity, metadata, timestamp: new Date().toISOString() })
        });
      } catch (error) {
    ...
    if (process.env.NODE_ENV === 'production') {
      try {
        await fetch('/api/audit/security', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ type, context, severity, success, metadata, timestamp: new Date().toISOString() })
        });
      } catch (error) {
        // Fail silently to avoid breaking the app
