...
function AutomationCenterImpl({ 
  ...
  // Rule operations
  const createRule = useCallback((template?: keyof typeof RULE_TEMPLATES) => {
    ...
    const newRule: BusinessRule = {
      ...
      portalScope: ['admin'],
      conditionLogic: 'all',
      conditions: templateData?.conditions?.map((cond, index) => ({
        id: `condition_${index}`,
        ...cond
      })) || [],
      actions: templateData?.actions?.map((action, index) => ({
        id: `action_${index}`,
        name: action.type,
        ...action
      })) || [],
  ...
  const saveRule = useCallback(async (rule: BusinessRule) => {
    try {
                ...
                <td>
                  <Badge
                    variant={
                      ...
                      rule.status === 'draft' ? 'secondary' : 'destructive'
                    }
          ...
          {results.map((result, index) => (
            <div key={index} className="p-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="font-medium">Conditions:</span> {result.conditionsEvaluated.length}
...
// Analytics Tab
function AnalyticsTab({ rules, results }: { rules: BusinessRule[]; results: RuleExecutionResult[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <Card className="p-4">
          <div className="text-2xl font-bold text-indigo-600">{analytics.successRate.toFixed(1)}%</div>
