...
function ServiceIntegrationDashboardImpl({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'integrations' | 'monitoring' | 'analytics'>('overview');
  ...
  const overallMetrics = useMemo(() => {
    ...
    const avgResponseTime = Object.values(metrics).reduce((sum, m) => sum + m.averageResponseTime, 0) / Object.values(metrics).length;
    const avgUptime = Object.values(metrics).reduce((sum, m) => sum + m.uptime, 0) / Object.values(metrics).length;
...
function OverviewTab({ metrics, integrations }: { metrics: any; integrations: ServiceIntegration[] }) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-orange-600">{Math.round(metrics.avgResponseTime)}ms</div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-purple-600">{metrics.avgUptime.toFixed(1)}%</div>
              ...
              <div key={integration.id} className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className={clsx(
                    ...
                    integration.status === 'inactive' ? 'bg-gray-400' : 'bg-yellow-500'
                  )} />
...
function MonitoringTab({ integrations, metrics }: { 
  integrations: ServiceIntegration[]; 
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-medium">{integrationMetrics.totalRequests.toLocaleString()}</div>
                  </div>
                  <div>
                    <span className="text-gray-600">Errors</span>
                    <div className="font-medium text-red-600">{integrationMetrics.failedRequests}</div>
                  </div>
                </div>
...
function AnalyticsTab({ integrations, metrics }: { 
  integrations: ServiceIntegration[]; 
  metrics: Record<string, IntegrationMetrics>; 
}) {
  return (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value: any) => setEditedIntegration(prev => ({ ...prev, type: value }))}
            >
          ...
          <div className="flex justify-end space-x-3">
            ...
            <Button onClick={() => onSave(editedIntegration)}>
              Save Integration
            </Button>
          </div>
        </div>
