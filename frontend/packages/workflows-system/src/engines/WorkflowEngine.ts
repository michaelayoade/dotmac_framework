...
class WorkflowEngine {
  ...
  private stepHandlers: Map<string, (step: WorkflowStep, context: ExecutionContext) => Promise<unknown>> = new Map();
  ...
  private initializeBuiltInHandlers() {
    ...
    this.registerStepHandler('conditional', async (step: WorkflowStep, context: ExecutionContext) => {
    ...
    this.registerStepHandler('notification', async (step: WorkflowStep, context: ExecutionContext) => {
  ...
  registerStepHandler(type: string, handler: (step: WorkflowStep, context: ExecutionContext) => Promise<unknown>) {
  ...
  private isWorkflowComplete(instance: WorkflowInstance): boolean {
    return instance.steps.every(step =>
      ...
      instances: state.instances.map(instance =>
        instance.id === instanceId
          ? {
              ...instance,
              status,
              endTime: ['completed', 'failed', 'cancelled'].includes(status) ? Date.now() : instance.endTime,
  ...
  private updateStepStatus(instanceId: string, stepId: string, status: WorkflowStep['status'], output?: unknown) {
    const now = Date.now();
        ...
        instance.id === instanceId
          ? {
              ...
              steps: instance.steps.map(step =>
                step.id === stepId
                  ? {
                      ...step,
                      status,
                      endTime: ['completed', 'failed', 'skipped'].includes(status) ? now : step.endTime,
              ...
              completedSteps: status === 'completed'
                ? [...instance.completedSteps, stepId].filter((id, index, arr) => arr.indexOf(id) === index)
                : instance.completedSteps.filter(id => id !== stepId),
              failedSteps: status === 'failed'
                ? [...instance.failedSteps, stepId].filter((id, index, arr) => arr.indexOf(id) === index)
                : instance.failedSteps.filter(id => id !== stepId),
              skippedSteps: status === 'skipped'
                ? [...instance.skippedSteps, stepId].filter((id, index, arr) => arr.indexOf(id) === index)
  ...
  private evaluateCondition(value: unknown, operator: string, expected: unknown): boolean {
    switch (operator) {
      ...
      case 'not_exists': return value === null || value === undefined;
