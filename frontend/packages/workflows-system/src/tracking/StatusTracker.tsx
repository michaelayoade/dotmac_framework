...
export function StatusTracker({
  ...
  function calculateAverageStepTime(): number {
    const completedWithDuration = instance.steps.filter(s =>
      s.status === 'completed' && s.actualDuration
    );

    if (completedWithDuration.length === 0) return 0;

    const totalDuration = completedWithDuration.reduce((total, step) =>
      total + (step.actualDuration || 0), 0
    );
                ...
                >
                  <div className="timeline-marker">
                    ...
                    {index < instance.steps.length - 1 && (
                      <div className={clsx('timeline-connector', {
                        completed: instance.completedSteps.includes(step.id) ||
