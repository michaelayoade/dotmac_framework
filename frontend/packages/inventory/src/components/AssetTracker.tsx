...
export function AssetTracker({
  ...
  useEffect(() => {
    ...
    if (filters.location !== 'all') {
      filtered = filtered.filter(asset =>
        asset.current_location?.warehouse_id === filters.location
      );
    }
    ...
    if (filters.technician !== 'all') {
      filtered = filtered.filter(asset =>
        asset.assigned_to?.technician_id === filters.technician
      );
    }
            ...
            {showFilters && (
              <div className="flex items-center gap-2">
                <select
                  ...
                  onChange={(e) => setFilters(prev => ({ ...prev, status: e.target.value as ItemStatus | 'all' }))}
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {asset.customer_deployment && (
                    <div className="flex items-center text-gray-600">
                      ...
                      <span className="truncate">
                        {asset.customer_deployment.customer_name}
                      </span>
                    </div>
                  )}
                  ...
                  {asset.next_maintenance_due && (
                    <div className="flex items-center text-orange-600">
                      ...
                      <span>Maintenance: {new Date(asset.next_maintenance_due).toLocaleDateString()}</span>
                    </div>
                  )}
                </div>
              </div>
