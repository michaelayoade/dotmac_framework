...
export interface UseStockResult {
  ...
  updateStockLocation: (stockItemId: string, location: {
    bin_location?: string;
    zone?: string;
    aisle?: string;
    shelf?: string;
...
export function useStock(): UseStockResult {
  ...
  const performStockCount = async (params: {
    ...
    try {
      ...
      const response = await apiClient.post<{ count_id: string }>('/api/inventory/stock/count/create', params);
  ...
  const updateStockLocation = async (stockItemId: string, location: {
    bin_location?: string;
    zone?: string;
    aisle?: string;
    shelf?: string;
