...
export interface UseAssetTrackingResult {
  ...
  trackAssetMovement: (assetId: string, newLocation: AssetLocation, reason?: string) => Promise<void>;
  assignAssetToTechnician: (assetId: string, technicianId: string, technicianName: string) => Promise<void>;
  deployAssetToCustomer: (assetId: string, customerInfo: {
    customer_id: string;
    customer_name: string;
    service_address: string;
  }) => Promise<void>;
  markAssetForMaintenance: (assetId: string, maintenanceType: string, notes?: string) => Promise<void>;
...
export function useAssetTracking(): UseAssetTrackingResult {
  ...
  const getAssetHistory = async (assetId: string): Promise<AssetHistory[]> => {
    try {
      setError(null);
      const response = await apiClient.get<AssetHistory[]>(`/api/inventory/assets/${assetId}/history`);
      return response;
    } catch (err) {
  ...
  const trackAssetMovement = async (
    ...
    try {
      ...
      setAssets(prev => prev.map(asset =>
        asset.id === assetId
          ? { ...asset, current_location: newLocation }
          : asset
      ));
    } catch (err) {
  ...
  const assignAssetToTechnician = async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to assign asset to technician';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setLoading(false);
  ...
  const deployAssetToCustomer = async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to deploy asset to customer';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setLoading(false);
  ...
  const markAssetForMaintenance = async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to mark asset for maintenance';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setLoading(false);
  ...
  const searchAssetsBySerial = async (serialNumber: string): Promise<AssetDetails[]> => {
    try {
      setError(null);
      const response = await apiClient.get<AssetDetails[]>(`/api/inventory/assets/search?serial_number=${encodeURIComponent(serialNumber)}`);
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to search assets by serial number';
      setError(errorMessage);
      return [];
    }
  };

  const getAssetsByLocation = async (warehouseId: string, zone?: string): Promise<AssetDetails[]> => {
    try {
      setError(null);
      const params = new URLSearchParams({ warehouse_id: warehouseId });
      if (zone) params.append('zone', zone);

      const response = await apiClient.get<AssetDetails[]>(`/api/inventory/assets/by-location?${params.toString()}`);
      return response;
    } catch (err) {
  ...
  const getAssetsByTechnician = async (technicianId: string): Promise<AssetDetails[]> => {
    try {
      setError(null);
      const response = await apiClient.get<AssetDetails[]>(`/api/inventory/assets/by-technician/${technicianId}`);
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get assets by technician';
      setError(errorMessage);
      return [];
    }
  };
  ...
  const getCustomerAssets = async (customerId: string): Promise<AssetDetails[]> => {
    try {
      setError(null);
      const response = await apiClient.get<AssetDetails[]>(`/api/inventory/assets/by-customer/${customerId}`);
      return response;
    } catch (err) {
  ...
  const getMaintenanceDue = async (daysAhead: number = 30): Promise<AssetDetails[]> => {
    try {
      setError(null);
      const response = await apiClient.get<AssetDetails[]>(`/api/inventory/assets/maintenance-due?days_ahead=${daysAhead}`);
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get maintenance due assets';
      setError(errorMessage);
      return [];
    }
  };
