...
export function useMovements(): UseMovementsResult {
  ...
  const listMovements = async (filters: MovementFilter = {}, page: number = 1, size: number = 50): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (filters.movement_type) {
        const types = Array.isArray(filters.movement_type) ? filters.movement_type : [filters.movement_type];
  ...
  const getMovementsByItem = async (itemId: string, limit: number = 100): Promise<StockMovement[]> => {
    try {
      setError(null);
      const response = await apiClient.get<StockMovement[]>(`/api/inventory/movements/by-item/${itemId}?limit=${limit}`);
      return response;
    } catch (err) {
  ...
  const getMovementsByWarehouse = async (warehouseId: string, limit: number = 100): Promise<StockMovement[]> => {
    try {
      setError(null);
      const response = await apiClient.get<StockMovement[]>(`/api/inventory/movements/by-warehouse/${warehouseId}?limit=${limit}`);
      return response;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get movements by warehouse';
      setError(errorMessage);
      return [];
    }
  };
  ...
  const getRecentMovements = async (limit: number = 50): Promise<StockMovement[]> => {
    try {
      setError(null);
      const response = await apiClient.get<StockMovement[]>(`/api/inventory/movements/recent?limit=${limit}`);
      return response;
    } catch (err) {
