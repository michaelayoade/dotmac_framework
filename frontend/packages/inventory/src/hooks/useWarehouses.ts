...
export function useWarehouses(): UseWarehousesResult {
  ...
  const listWarehouses = async (filters: WarehouseFilter = {}, page: number = 1, size: number = 50): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (filters.warehouse_type) {
        const types = Array.isArray(filters.warehouse_type) ? filters.warehouse_type : [filters.warehouse_type];
  ...
  const setupStandardWarehouses = async (): Promise<Warehouse[]> => {
    try {
      ...
      const response = await apiClient.post<Warehouse[]>('/api/inventory/warehouses/setup-standard');
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to setup standard warehouses';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setLoading(false);
