...
export interface UsePurchaseOrdersResult {
  ...
  currentPage: number;
  pageSize: number;
  createPurchaseOrder: (data: PurchaseOrderCreate, lineItems: PurchaseOrderLineCreate[]) => Promise<PurchaseOrder>;
  ...
  listPurchaseOrders: (filters?: PurchaseOrderFilter, page?: number, size?: number) => Promise<void>;
  approvePurchaseOrder: (id: string) => Promise<PurchaseOrder>;
  sendToVendor: (id: string, vendorEmail?: string) => Promise<void>;
  receiveItems: (poId: string, receipts: {
    line_id: string;
...
export function usePurchaseOrders(): UsePurchaseOrdersResult {
  ...
  const updatePurchaseOrder = async (id: string, data: PurchaseOrderUpdate): Promise<PurchaseOrder> => {
    try {
      setLoading(true);
      setError(null);
      ...
      setPurchaseOrders(prev =>
        prev.map(po => po.id === id ? response : po)
      );
  ...
  const listPurchaseOrders = async (filters: PurchaseOrderFilter = {}, page: number = 1, size: number = 50): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (filters.required_date_from) params.append('required_date_from', filters.required_date_from);
  ...
  const approvePurchaseOrder = async (id: string): Promise<PurchaseOrder> => {
    try {
      ...
      const response = await apiClient.post<PurchaseOrder>(`/api/inventory/purchase-orders/${id}/approve`, {
  ...
  const sendToVendor = async (id: string, vendorEmail?: string): Promise<void> => {
    try {
      ...
      setPurchaseOrders(prev =>
        prev.map(po =>
          po.id === id
            ? { ...po, po_status: PurchaseOrderStatus.SENT_TO_VENDOR }
            : po
        )
      );
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to send purchase order to vendor';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setLoading(false);
  ...
  const receiveItems = async (poId: string, receipts: {
    line_id: string;
    quantity_received: number;
    unit_cost?: number;
    condition?: string;
  ...
  const createReorderPurchaseOrders = async (defaultWarehouseId: string): Promise<PurchaseOrder[]> => {
    try {
      setLoading(true);
      setError(null);

      const response = await apiClient.post<PurchaseOrder[]>('/api/inventory/purchase-orders/create-reorders', {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to create reorder purchase orders';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setLoading(false);
