...
import { Button, DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, DropdownMenuSeparator, Badge } from '@dotmac/primitives';
...
const toolbarVariants = cva(
  'flex flex-col gap-4 p-4 bg-white border-b',
  {
    variants: {
      portal: {
...
export const TableToolbar = <TData extends any>({
  ...
  const handleExport = async (format: 'csv' | 'xlsx' | 'json' | 'pdf') => {
    ...
    setIsExporting(true);
    try {
      const dataToExport = exportConfig.selectedOnly && selectedData?.length
        ? selectedData
        : data;
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" disabled={isExporting}>
                    ...
                    Export
                    {isExporting && <div className="ml-2 w-3 h-3 border border-gray-400 border-t-transparent rounded-full animate-spin" />}
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end">
                  {exportConfig.formats.map((format) => {
                    const isRecommended = format === getRecommendedExportFormat(data, exportConfig.formats);
                    return (
                      <DropdownMenuItem
                        key={format}
