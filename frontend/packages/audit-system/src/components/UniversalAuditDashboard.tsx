...
export function UniversalAuditDashboard({
  ...
  const activityItems = useMemo(() => {
    return activities.map(activity => ({
      ...activity,
      type: activity.type as 'user_action' | 'system_event' | 'error' | 'success' | 'info' | 'warning',
  ...
  const renderMetricsOverview = () => {
    ...
    const cards = [
      ...
      },
      {
        ...
        value: Object.values(metrics.complianceEvents).reduce((a, b) => a + b, 0).toString(),
        ...
        {cards.map((card, index) => (
          <div key={index} className={`bg-white rounded-lg p-4 border border-${card.color}-100`}>
            <div className="flex items-center justify-between">
              ...
              <div className={`text-sm ${card.trend === 'up' ? 'text-green-600' : card.trend === 'down' ? 'text-red-600' : 'text-gray-500'}`}>
                {card.change}
              </div>
            </div>
          </div>
              ...
              >
                <div className="flex items-start space-x-3">
                  <div className={`p-1 rounded-full ${
                    ...
                    event.severity === 'high' ? 'bg-yellow-100' :
                    event.severity === 'medium' ? 'bg-blue-100' :
                    'bg-gray-100'
                  }`}>
                    <div className={`w-2 h-2 rounded-full ${
                      event.severity === 'critical' ? 'bg-red-600' :
                      event.severity === 'high' ? 'bg-yellow-600' :
                      event.severity === 'medium' ? 'bg-blue-600' :
                      'bg-gray-600'
                    }`} />
                  </div>
