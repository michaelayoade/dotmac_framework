...
class UniversalAuditService {
  ...
  constructor(config: Partial<UniversalAuditConfig> = {}) {
    this.config = {
      ...
      portals: {
        admin: {
          portalType: 'admin',
          enabledCategories: ['authentication', 'customer_management', 'billing_operations', 'system_admin', 'configuration'],
          requiredCompliance: ['audit_trail', 'data_retention', 'sox'],
          retentionPolicy: { defaultRetention: 2555 }, // 7 years for admin
          sensitiveDataHandling: { autoRedact: true, redactionFields: ['password', 'ssn', 'credit_card'], requiresApproval: true },
          realTimeAlerts: { criticalActions: ['user_delete', 'data_export'], securityEvents: ['failed_login_attempts'], complianceViolations: ['gdpr_violation'] }
        },
        customer: {
          portalType: 'customer',
          ...
          sensitiveDataHandling: { autoRedact: true, redactionFields: ['credit_card', 'bank_account'], requiresApproval: false },
          realTimeAlerts: { criticalActions: ['payment_failure'], securityEvents: ['suspicious_login'], complianceViolations: ['gdpr_violation'] }
        },
        reseller: {
          portalType: 'reseller',
          enabledCategories: ['authentication', 'customer_management', 'billing_operations', 'reporting'],
          requiredCompliance: ['audit_trail', 'financial', 'data_retention'],
          retentionPolicy: { defaultRetention: 1825 }, // 5 years
          sensitiveDataHandling: { autoRedact: true, redactionFields: ['commission_rates', 'financial_data'], requiresApproval: true },
          realTimeAlerts: { criticalActions: ['commission_change'], securityEvents: ['data_breach'], complianceViolations: ['financial_irregularity'] }
        },
        management: {
          portalType: 'management',
          enabledCategories: ['authentication', 'customer_management', 'billing_operations', 'network_operations', 'reporting'],
          requiredCompliance: ['audit_trail', 'sox', 'iso27001', 'financial'],
          retentionPolicy: { defaultRetention: 2555 }, // 7 years
          sensitiveDataHandling: { autoRedact: true, redactionFields: ['financial_data', 'strategic_data'], requiresApproval: true },
          realTimeAlerts: { criticalActions: ['financial_report'], securityEvents: ['privilege_escalation'], complianceViolations: ['sox_violation'] }
        },
        technician: {
          ...
          sensitiveDataHandling: { autoRedact: false, redactionFields: [], requiresApproval: false },
          realTimeAlerts: { criticalActions: ['network_change'], securityEvents: ['unauthorized_access'], complianceViolations: [] }
        }
      },
  ...
  async logUserAction(
    ...
    const event: AuditEvent = {
      ...
      actionDescription: details.actionDescription || this.generateActionDescription(action, details.actionCategory),
  ...
  async getMetrics(period?: { start: Date; end: Date }): Promise<AuditMetrics> {
    ...
    // Calculate metrics
    const eventsByCategory = this.groupBy(events, 'actionCategory') as Record<ActionCategory, number>;
  ...
  private determineComplianceTypes(portalType: PortalType, category: ActionCategory): ComplianceType[] {
  ...
  private getRetentionPeriod(portalType: PortalType, category: ActionCategory): number {
    const portalConfig = this.config.portals[portalType];
    return portalConfig.retentionPolicy.categoryRetention?.[category] ||
  ...
  private calculateFailureRateChange(events: AuditEvent[], period?: { start: Date; end: Date }): number {
    // Implementation for failure rate change calculation
    return 0; // Placeholder
  }

  private calculateComplianceEventChange(events: AuditEvent[], period?: { start: Date; end: Date }): number {
  ...
  private generateExecutiveSummary(type: ComplianceType, events: AuditEvent[], violations: AuditEvent[]): string {
