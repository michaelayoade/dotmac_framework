...
export function useKeyboardNavigation({
  ...
  const currentIndex = activeItem
    ? flatItems.findIndex(item => item.id === activeItem)
    : -1;
  ...
  const focusItem = useCallback((index: number) => {
    ...
    const element = containerRef.current.querySelector(`[data-nav-item="${item.id}"]`) as HTMLElement;
    element?.focus();
  }, [flatItems]);
