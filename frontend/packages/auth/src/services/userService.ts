...
export class UserService {
  ...
  static async getUserById(userId: string, tenantId: string): Promise<User | null> {
    try {
      ...
      const dbUser = mockUsers.find(u => u.id === userId && u.tenant_id === tenantId && u.is_active);
   ...
   */
  static async getUserByEmail(email: string, portalType: PortalType, tenantId?: string): Promise<User | null> {
    try {
      ...
      if (tenantId) {
        dbUser = mockUsers.find(u => u.email === email && u.tenant_id === tenantId && u.is_active);
      }
   ...
   */
  static async verifyCredentials(email: string, password: string, portalType: PortalType): Promise<User | null> {
    try {
