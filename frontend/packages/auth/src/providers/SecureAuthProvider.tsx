...
import type { AuthConfig, AuthContextValue, LoginCredentials, User, Permission, UserRole, AuthTokens, PortalType } from '../types';
...
export const SecureAuthProvider: React.FC<SecureAuthProviderProps> = ({
  ...
  const verifyMFA = React.useCallback(async (code: string): Promise<boolean> => {
    const token = localStorage.getItem(`secure_auth_${portal}_token`);
  ...
  const disableMFA = React.useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (response.ok && state.user) {
        setState(prev => ({
          ...prev,
          user: prev.user ? { ...prev.user, metadata: { ...prev.user.metadata, mfaEnabled: false } } : null
        }));
      }
    } catch (error) {
