...
export const EnterpriseAuthProvider: React.FC<EnterpriseAuthProviderProps> = ({
        ...
        if (storedToken && storedUser) {
          ...
          if (isValid) {
            ...
            setState(prev => ({
              ...
              deviceTrusted: localStorage.getItem(`enterprise_auth_${portal}_device_trusted`) === 'true',
            }));
  ...
  const handleSuccessfulLogin = async (data: any) => {
    ...
    localStorage.setItem(`enterprise_auth_${portal}_device_trusted`, data.deviceTrusted?.toString() || 'false');
  ...
  const hasPermission = React.useCallback((permission: Permission | Permission[]): boolean => {
    const result = !state.user || !config.enablePermissions ||
      (Array.isArray(permission) ? permission : [permission])
        .some(p => state.user!.permissions.includes(p));
