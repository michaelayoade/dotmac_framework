...
export const usePerformanceMonitor = (componentName: string) => {
  ...
  const measureInteraction = useCallback(<T>(interactionName: string, interactionFn: () => T): T => {
  ...
  const recordError = useCallback((operation: string, error: Error) => {
    if (globalMonitor) {
...
export const useInteractionMetrics = () => {
  ...
  const measureAsyncInteraction = useCallback(async <T>(name: string, fn: () => Promise<T>): Promise<T> => {
    const start = performance.now();
          ...
          if (resource.name.includes('.js') || resource.name.includes('.css') || resource.name.includes('.woff')) {
            ...
            assets.push({
              ...
              type: resource.name.includes('.js') ? 'javascript' : 
                    resource.name.includes('.css') ? 'stylesheet' : 
