...
export function useServicePlans(props: UseServicePlansProps) {
  ...
  const getAvailablePlans = useCallback(
    async (customerType: 'residential' | 'business' | 'enterprise', filters?: PlanFilterCriteria): Promise<ServicePlan[]> => {
  ...
  const calculateUpgradeImpact = useCallback(
    async (currentPlanId: string, targetPlanId: string, effectiveDate?: Date): Promise<UpgradeImpact> => {
  ...
  const applyPlanChange = useCallback(
    async (customerId: string, newPlanId: string, effectiveDate: Date, options?: {
