...
export class ServicePlanEngine {
  ...
  async getAvailablePlans(
    ...
    try {
      ...
      throw new Error(`Failed to retrieve service plans: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async calculateUpgradeImpact(
    ...
    try {
      ...
      throw new Error(`Failed to calculate upgrade impact: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async applyPlanChange(
    ...
    try {
      ...
      const planChange: PlanChange = {
        ...
        costs: {
          prorationCredit: impact.pricing.currentMonthlyPrice.amount > impact.pricing.newMonthlyPrice.amount
            ? {
                amount: impact.pricing.currentMonthlyPrice.amount - impact.pricing.newMonthlyPrice.amount,
      ...
      throw new Error(`Failed to apply plan change: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
    ...
    try {
      ...
      if (currentPlan && this.hasContractRestrictions(currentPlan)) {
        result.restrictions = {
          ...
          earlyTerminationPenalty: currentPlan.contractTerms.earlyTerminationFee || { amount: 0, currency: 'USD' },
      ...
      throw new Error(`Failed to validate plan eligibility: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async getCustomerPlanHistory(customerId: string): Promise<CustomerPlanHistory> {
    try {
      ...
      throw new Error(`Failed to retrieve customer plan history: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  // Private helper methods
  private validatePlanAccess(): void {
    const hasAccess = this.context.permissions.includes('plans:read') ||
  ...
  private validatePlanChangeAccess(customerId: string): void {
    const hasAccess = this.context.permissions.includes('plans:write') ||
  ...
  private validateCustomerAccess(customerId: string): void {
    const hasAccess = this.context.permissions.includes('customer:read') ||
                     (this.context.portalType === 'customer' && this.context.userId === customerId) ||
                     this.context.portalType === 'admin';
  ...
  private applyPlanFilters(plans: ServicePlan[], filters: PlanFilterCriteria): ServicePlan[] {
    return plans.filter(plan => {
      ...
      // Service type filter
      if (filters.serviceTypes && !filters.serviceTypes.some(type =>
        plan.availability.serviceTypes.includes(type)
      )) {
        return false;
      }

      // Region filter
      if (filters.regions && !filters.regions.some(region =>
        plan.availability.regions.includes(region)
      )) {
        return false;
      }

      // Features filter
      if (filters.features && !filters.features.every(feature =>
      ...
      if (filters.contractTerms) {
        if (filters.contractTerms.maxTermMonths &&
            plan.contractTerms.minimumTermMonths > filters.contractTerms.maxTermMonths) {
          return false;
        }

        if (filters.contractTerms.allowMonthToMonth &&
            plan.contractTerms.minimumTermMonths > 1) {
          return false;
        }
      }
  ...
  private sortPlansByRelevance(plans: ServicePlan[], customerType: string): ServicePlan[] {
    return plans.sort((a, b) => {
      ...
      const priceSort = customerType === 'residential'
  ...
  private calculateServiceChanges(currentPlan: ServicePlan, targetPlan: ServicePlan) {
    const speedIncrease = {
      downloadMbps: targetPlan.technical.downloadSpeedMbps - currentPlan.technical.downloadSpeedMbps,
  ...
  private calculatePricingImpact(currentPlan: ServicePlan, targetPlan: ServicePlan, effectiveDate: Date) {
    ...
    const proratedCharge = monthlyDifference.amount > 0 ? {
    ...
    const totalUpfrontCost = {
      amount: (proratedCharge?.amount || 0) +
  ...
  private async calculateChangeTimeline(currentPlan: ServicePlan, targetPlan: ServicePlan, effectiveDate: Date) {
    ...
    const estimatedInstallationDays = requiresInstallation ?
      await this.getEstimatedInstallationTime(targetPlan) : 0;

    const estimatedActivationDate = addMonths(effectiveDate, 0);
    if (requiresInstallation) {
      estimatedActivationDate.setDate(estimatedActivationDate.getDate() + estimatedInstallationDays);
    }
  ...
  private requiresInstallation(currentPlan: ServicePlan, targetPlan: ServicePlan): boolean {
    ...
    return targetPlan.availability.requiresInstallation ||
  ...
  private calculateEquipmentUpgradeFee(currentPlan: ServicePlan, targetPlan: ServicePlan): Money | undefined {
  ...
  private async scheduleServiceProvisioning(customerId: string, planChange: PlanChange): Promise<void> {
    throw new Error('Method not implemented - requires API integration');
  }
  ...
  private async sendPlanChangeNotifications(customerId: string, planChange: PlanChange): Promise<void> {
    throw new Error('Method not implemented - requires API integration');
  }
  ...
  private async checkTechnicalEligibility(customer: any, plan: ServicePlan): Promise<{ feasible: boolean; reasons: string[] }> {
    // Technical feasibility checks
    return { feasible: true, reasons: [] };
  }

  private checkContractRestrictions(currentPlan: ServicePlan, targetPlan: ServicePlan): { restricted: boolean; reasons: string[] } {
  ...
  private analyzePlanHistory(changes: PlanChange[]): { averageMonthsOnPlan: number; preferredFeatures: string[] } {
