...
const CommentItem: React.FC<CommentItemProps> = ({
  ...
  return (
    <div className={`comment-item p-4 border-l-4 ${
      comment.resolved
      ...
      {comment.replies.length > 0 && (
        <div className="comment-replies ml-4 border-l-2 border-gray-200 pl-3">
          {comment.replies.map((reply) => (
            <div key={reply.id} className="reply-item py-2 border-b border-gray-100 last:border-b-0">
...
export const CommentsPanel: React.FC<CommentsPanelProps> = ({
  ...
  const handleReplyToComment = useCallback(async (comment_id: string, content: string) => {
