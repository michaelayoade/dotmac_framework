...
const ConflictItem: React.FC<ConflictItemProps> = ({
  ...
  const [selectedResolution, setSelectedResolution] = useState<ConflictResolutionStrategy | null>(null);
  const [resolving, setResolving] = useState(false);

  const handleResolve = useCallback(async (strategy: ConflictResolutionStrategy) => {
    if (resolving) return;
          ...
          <div>
            ...
            <div className="mt-1 space-y-1">
              {conflict.conflicting_operations.slice(0, 2).map((operation, index) => (
                <div key={index} className="text-xs bg-white p-2 rounded border-l-2 border-orange-400">
...
export const ConflictResolver: React.FC<ConflictResolverProps> = ({
  ...
  const handleResolveAll = useCallback(async () => {
    if (!confirm(`Are you sure you want to resolve all ${conflicts.length} conflicts using the "Merge Changes" strategy?`)) {
      return;
    }
      ...
      <div className="panel-header p-4 border-b border-red-200 bg-red-50">
        ...
        <div className="flex items-center justify-between">
          <div className="text-sm text-red-700">
            ...
            <p>
              Choose a resolution strategy to continue collaborative editing.
            </p>
          </div>
        ...
        {sortedConflicts.length > 0 ? (
          ...
          <div className="empty-state flex flex-col items-center justify-center h-full text-gray-500 p-8">
            ...
            <p className="text-sm text-center text-green-600">
              All editing conflicts have been resolved.
              Collaborative editing can continue normally.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="panel-footer p-4 border-t bg-gray-50">
        <div className="text-xs text-gray-600">
          ...
          <ul className="space-y-1 list-disc list-inside">
            <li><strong>Last Writer Wins:</strong> Keeps the most recent changes</li>
            <li><strong>First Writer Wins:</strong> Preserves the original changes</li>
            <li><strong>Merge Changes:</strong> Combines compatible changes automatically</li>
            <li><strong>Manual Resolution:</strong> Review and choose specific changes</li>
          </ul>
        </div>
      </div>
    </div>
