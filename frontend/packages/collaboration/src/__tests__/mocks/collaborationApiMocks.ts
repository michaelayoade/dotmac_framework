...
export const mockSuggestions: DocumentSuggestion[] = [
  {
    ...
    suggested_change: {
      ...
      suggested_content: "- Expand fiber coverage to underserved rural areas with priority on high-density zones"
    },
...
export const collaborationApiMocks = {
  ...
  unlockDocument: (documentId: string) =>
    Promise.resolve({ data: { unlocked: true } }),
  ...
  getDocumentVersion: (documentId: string, version: number) =>
    Promise.resolve({
      data: mockDocumentVersions.find(v => 
        v.document_id === documentId && v.version_number === version
      )
    }),
