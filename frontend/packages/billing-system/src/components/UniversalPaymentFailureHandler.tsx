...
export function UniversalPaymentFailureHandler({
  ...
  const getRecommendedActions = (payment: FailedPayment) => {
    ...
    if (category === 'insufficient_funds' && retryCount < 2) {
      ...
      actions.push({ type: 'contact', priority: 'medium', text: 'Contact customer about low balance' });
    } else if (category === 'invalid_card') {
      actions.push({ type: 'update_method', priority: 'high', text: 'Request payment method update' });
      ...
      actions.push({ type: 'manual_retry', priority: 'medium', text: 'Manual retry after verification' });
  ...
  const handleRetryPayment = useCallback(async (paymentId: string, strategy: string, paymentMethodId?: string) => {
  ...
  const handleContactCustomer = useCallback(async (paymentId: string, method: 'email' | 'phone') => {
  ...
  const groupedPayments = failedPayments.reduce((groups, payment) => {
    const severity = getFailureSeverity(payment);
    const category = getFailureCategory(payment.failureReason || '');
  ...
  const avgRetryCount = failedPayments.length > 0
                      ...
                      <tr>
                        <th className="px-4 py-3 text-left">
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedPayments(prev => new Set([...prev, ...payments.map(p => p.id)]));
                              } else {
                                setSelectedPayments(prev => {
                                  const newSet = new Set(prev);
