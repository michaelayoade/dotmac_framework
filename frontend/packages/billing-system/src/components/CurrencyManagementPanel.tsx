...
export const CurrencyManagementPanel: React.FC<CurrencyManagementPanelProps> = ({
  ...
  const handleSetBaseCurrency = async (currencyId: string) => {
    try {
      const response = await fetch(`/api/customers/${customerId}/currencies/${currencyId}/set-base`, {
        method: 'POST',
      });
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 text-sm">Manage supported currencies for multi-currency payments</p>
        </div>
        <button
      ...
      {currencies.length > 0 ? (
        ...
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-400">
            Add currencies to enable multi-currency payments
          </p>
        </div>
      )}
      ...
      <div className="mt-6 p-4 bg-gray-50 border border-gray-200 rounded-md">
        ...
        <ul className="text-sm text-gray-600 space-y-1">
          <li>• <strong>Base Currency:</strong> Your primary accounting currency</li>
          <li>• <strong>Payment Currencies:</strong> Currencies customers can pay in</li>
          <li>• <strong>Exchange Rates:</strong> Set manually for each transaction</li>
          <li>• <strong>Conversion:</strong> Payments converted to base currency for accounting</li>
        </ul>
      </div>
    </div>
  );
