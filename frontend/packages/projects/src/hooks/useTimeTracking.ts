...
export function useTimeTracking(): UseTimeTrackingResult {
  ...
  const createTimeEntry = useCallback(async (data: TimeEntryCreate): Promise<TimeEntry> => {
    try {
      setError(null);
  ...
  const getTimeTrackingAnalytics = useCallback(async (filters?: {
    projectId?: string;
    userId?: string;
    ...
    try {
      ...
      const response = await apiClient.get<TimeTrackingAnalytics>(
        API_ENDPOINTS.TIME_ANALYTICS,
        { params }
      );

      return response.data;
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to fetch time tracking analytics';
      throw new Error(message);
    }
  }, [apiClient]);
  ...
  const refreshTimeEntries = useCallback(async (): Promise<void> => {
    try {
      ...
      const response = await apiClient.get<{ time_entries: TimeEntry[] }>(API_ENDPOINTS.TIME_ENTRIES);
  ...
  useEffect(() => {
    ...
    const checkActiveTimer = async () => {
      try {
        const response = await apiClient.get<{ active_timer: TimeEntry | null }>('/api/projects/timer/status');
