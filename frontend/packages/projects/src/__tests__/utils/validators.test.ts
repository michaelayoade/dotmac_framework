import { projectValidators, taskValidators, timeEntryValidators, generalValidators } from '../../utils/validators';
...
describe('taskValidators', () => {
  ...
  describe('validateTaskDependencies', () => {
    ...
    it('should fail validation when too many dependencies', () => {
      const dependencies = Array(21).fill(0).map((_, i) => `task${i}`);
  ...
  describe('validateChecklistItems', () => {
    ...
    it('should fail validation when too many items', () => {
      const items = Array(51).fill(0).map((_, i) => ({
        id: i.toString(),
        text: `Item ${i}`,
        completed: false
      }));
