...
const collaborationHandlers = [
  ...
  http.get('/api/collaboration/documents/:documentId', ({ params }) => {
    ...
    const document = documentId === collaborationScenario.document.id 
...
const networkHandlers = [
  ...
  http.get('/api/gis/service-areas/:serviceAreaId', ({ params }) => {
    ...
    return HttpResponse.json({ 
      data: networkScenario.serviceAreas.find(area => area.id === serviceAreaId) || networkScenario.serviceAreas[0]
    });
  }),
  ...
  http.get('/api/network/performance/entity/:entityId', ({ params, request }) => {
    ...
    return networkApiMocks.getEntityMetrics(entityId, entityType || 'node', metricName || undefined);
  })
];
