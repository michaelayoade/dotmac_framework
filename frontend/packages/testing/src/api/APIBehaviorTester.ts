...
export class APIBehaviorTester {
  ...
  setupResellerMocks(): void {
    ...
    this.mockAndLog('GET /api/reseller/territory/dashboard', {
      ...
      data: {
        ...
        leads: [
          { id: 'lead-1', company: 'TechCorp', location: 'North', score: 85, priority: 'high', estimatedValue: 25000 },
          { id: 'lead-2', company: 'SmallBiz', location: 'South', score: 65, priority: 'medium', estimatedValue: 12000 }
        ],
        metrics: {
    ...
    this.mockAndLog('GET /api/reseller/sales/deals', {
      ...
      data: {
        ...
        pipeline: {
          leads: [
            { id: 'lead-1', title: 'TechCorp ISP Setup', value: 25000, priority: 'high' }
          ],
          qualified: [
  ...
  getRequestStats(): {
    ...
    const successful = this.requestLog.filter(r => 
      r.response?.status && r.response.status < 400
    ).length;
