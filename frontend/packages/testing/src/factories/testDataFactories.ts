...
export const UserFactory = {
  build: (overrides?: Partial<User>): User => ({
    ...
    permissions: faker.helpers.arrayElements(['read', 'write', 'comment', 'suggest', 'resolve_conflicts'], { min: 1, max: 5 }),
    ...overrides
  }),
...
export const DocumentFactory = {
  build: (overrides?: Partial<CollaborativeDocument>): CollaborativeDocument => {
    ...
    return {
      ...
      permissions: {
        read: Array.from({ length: faker.number.int({ min: 1, max: 5 }) }, () => faker.string.uuid()),
        write: Array.from({ length: faker.number.int({ min: 1, max: 3 }) }, () => faker.string.uuid()),
        comment: Array.from({ length: faker.number.int({ min: 1, max: 4 }) }, () => faker.string.uuid()),
        admin: [userId]
      },
      metadata: {
        category: faker.helpers.arrayElement(['planning', 'procedures', 'documentation', 'specifications']),
        tags: faker.helpers.arrayElements(['network', 'infrastructure', 'planning', 'support', 'billing'], { min: 1, max: 4 }),
...
export const ConflictFactory = {
  build: (overrides?: Partial<DocumentConflict>): DocumentConflict => ({
    ...
    resolution_strategy: faker.helpers.maybe(() => faker.helpers.arrayElement(['manual_merge', 'accept_local', 'accept_remote', 'custom'])) || null,
    ...overrides
  }),
...
export const NetworkNodeFactory = {
  build: (overrides?: Partial<NetworkNode>): NetworkNode => ({
    ...
    latitude: faker.helpers.maybe(() => faker.location.latitude({ min: 35, max: 45, precision: 6 })),
    longitude: faker.helpers.maybe(() => faker.location.longitude({ min: -125, max: -65, precision: 6 })),
    ...
    hostname: faker.helpers.maybe(() => `${faker.string.alphanumeric(8)}.${faker.internet.domainName()}`),
    manufacturer: faker.helpers.maybe(() => faker.helpers.arrayElement(['Cisco', 'Juniper', 'Arista', 'HP', 'Dell', 'Ubiquiti'])),
    ...
    coverage_radius_km: faker.helpers.maybe(() => faker.number.float({ min: 0.1, max: 50, precision: 0.1 })),
    ...
    uptime_percentage: faker.helpers.maybe(() => faker.number.float({ min: 85, max: 100, precision: 0.1 })),
    ...
    power_consumption: faker.helpers.maybe(() => faker.number.float({ min: 5, max: 1000, precision: 0.1 })),
    connected_links: Array.from({ length: faker.number.int({ min: 0, max: 5 }) }, () => `link_${faker.string.alphanumeric(6)}`),
...
export const NetworkLinkFactory = {
  build: (overrides?: Partial<NetworkLink>): NetworkLink => ({
    ...
    latency_ms: faker.helpers.maybe(() => faker.number.float({ min: 0.1, max: 100, precision: 0.1 })),
    length_km: faker.helpers.maybe(() => faker.number.float({ min: 0.1, max: 100, precision: 0.1 })),
    cost: faker.number.int({ min: 1, max: 100 }),
    utilization_percentage: faker.helpers.maybe(() => faker.number.int({ min: 0, max: 100 })),
    packet_loss: faker.helpers.maybe(() => faker.number.float({ min: 0, max: 5, precision: 0.001 })),
    error_rate: faker.helpers.maybe(() => faker.number.float({ min: 0, max: 1, precision: 0.0001 })),
    status: faker.helpers.enumValue(NodeStatus),
    operational_status: faker.helpers.maybe(() => faker.helpers.arrayElement(['up/up', 'up/down', 'down/down', 'admin-down'])),
    ...overrides
  }),
...
export const ServiceAreaFactory = {
  build: (overrides?: Partial<ServiceArea>): ServiceArea => {
    ...
    return {
      ...
      service_types: faker.helpers.arrayElements(['fiber', 'wireless', 'dsl', 'cable', 'satellite'], { min: 1, max: 4 }),
...
export const CoverageGapFactory = {
  build: (overrides?: Partial<CoverageGap>): CoverageGap => {
    ...
    return {
      ...
      gap_type: faker.helpers.arrayElement(['rural_underserved', 'urban_dead_zone', 'signal_weak_area', 'infrastructure_gap']),
      ...
      status: faker.helpers.arrayElement(['identified', 'analysis_complete', 'pending_approval', 'approved', 'in_progress', 'completed']),
      created_at: faker.date.past().toISOString(),
      ...overrides
    };
  },
...
export const PerformanceMetricFactory = {
  build: (overrides?: Partial<PerformanceMetric>): PerformanceMetric => ({
    timestamp: faker.date.recent().toISOString(),
    metric_name: faker.helpers.arrayElement(['cpu_usage', 'memory_usage', 'disk_usage', 'network_utilization', 'link_utilization', 'temperature', 'power_consumption']),
...
export const NetworkAlertFactory = {
  build: (overrides?: Partial<NetworkAlert>): NetworkAlert => ({
    ...
    type: faker.helpers.arrayElement(['high_cpu', 'high_memory', 'link_down', 'high_temperature', 'power_failure', 'authentication_failed']),
