...
export class GestureManager {
  ...
  private handleGestureEnd(): void {
    ...
    if (distance > this.options.swipe.threshold && velocity > this.options.swipe.velocity) {
      ...
      if (absAngle <= this.options.swipe.maxAngle || absAngle >= (180 - this.options.swipe.maxAngle)) {
        // Horizontal swipe
        if (deltaX > 0) {
      ...
      } else if (absAngle >= (90 - this.options.swipe.maxAngle) && absAngle <= (90 + this.options.swipe.maxAngle)) {
        // Vertical swipe
        if (deltaY > 0) {
