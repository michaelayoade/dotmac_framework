...
import { TechnicianWorkflowScanner, WorkflowType, WorkflowScanResult } from '../utils/TechnicianWorkflowScanner';
...
export function TechnicianScanner({
  ...
  const handleBarcodeResult = useCallback(async (result: BarcodeResult) => {
    ...
    try {
      ...
      if (workflowType) {
        const workflowSpecific = TechnicianWorkflowScanner.filterByWorkflow(analyses, [workflowType]);
      ...
      <div className="technician-scanner__overlay">
        ...
        {lastScanResult && !isAnalyzing && (
          <div className="scanner-results">
            ...
            {bestResult ? (
              <div className={`scanner-result-item ${bestResult.isValid ? 'scanner-result-item--valid' : 'scanner-result-item--invalid'}`}>
                ...
                <div className="scanner-result-item__data">
                  {bestResult.isValid ? (
                    ...
                    <div className="scanner-result-item__raw">
                      Raw: {bestResult.data}
                    </div>
                  )}
                </div>
