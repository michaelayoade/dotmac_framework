...
import { DeviceManager, LocationPosition, CameraOptions, BiometricOptions, DeviceCapabilities } from './DeviceManager';
...
export function useCamera() {
  ...
  const initializeCamera = useCallback(async (options: CameraOptions = {}) => {
...
export function useVibration() {
  ...
  const vibratePattern = useCallback((type: 'success' | 'error' | 'warning' | 'info') => {
    const patterns = {
      success: [100, 50, 100],
