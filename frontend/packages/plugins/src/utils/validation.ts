...
export function validatePluginDomain(domain: string): ValidationResult {
  ...
  if (!domain || domain.trim().length === 0) {
    ...
    if (!/^[a-zA-Z0-9_.-]+$/.test(domain)) {
      errors.push('Plugin domain can only contain letters, numbers, dots, hyphens, and underscores');
    }
...
export function validatePluginDescription(description?: string): ValidationResult {
  ...
  if (description) {
    ...
    if (description.trim().length < 10) {
      warnings.push('Plugin description should be at least 10 characters for better discoverability');
    }
...
export function validatePluginTags(tags: string[]): ValidationResult {
  ...
  for (const tag of tags) {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(tag)) {
      errors.push(`Invalid tag format: ${tag}. Only letters, numbers, hyphens, and underscores allowed`);
    }
  }
...
export function validatePluginCategories(categories: string[]): ValidationResult {
  ...
  const validCategories = [
    'authentication', 'billing', 'communication', 'analytics', 'monitoring',
    'network', 'security', 'integration', 'workflow', 'reporting',
    'data-processing', 'ai-ml', 'automation', 'ui-extension', 'system'
  ];
...
export function validatePluginConfig(config: Record<string, any>, schema?: Record<string, any>): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
    ...
    for (const [key, schemaRule] of Object.entries(schema)) {
      ...
      if (value !== undefined && schemaRule.type) {
        ...
        if (expectedType !== actualType) {
          errors.push(`Configuration field ${key} should be of type ${expectedType}, got ${actualType}`);
        }
      }
      ...
      if (typeof value === 'string' && schemaRule.minLength && value.length < schemaRule.minLength) {
        errors.push(`Configuration field ${key} must be at least ${schemaRule.minLength} characters`);
      }

      if (typeof value === 'string' && schemaRule.maxLength && value.length > schemaRule.maxLength) {
        errors.push(`Configuration field ${key} cannot exceed ${schemaRule.maxLength} characters`);
      }

      if (typeof value === 'number' && schemaRule.minimum !== undefined && value < schemaRule.minimum) {
        errors.push(`Configuration field ${key} must be at least ${schemaRule.minimum}`);
      }

      if (typeof value === 'number' && schemaRule.maximum !== undefined && value > schemaRule.maximum) {
