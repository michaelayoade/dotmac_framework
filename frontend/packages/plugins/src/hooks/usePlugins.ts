...
export function usePlugins(): UsePluginsResult {
  ...
  const fetchPlugins = useCallback(async (domain?: string) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const updatePlugin = useCallback(async (request: PluginUpdateRequest) => {
    try {
      setError(null);
  ...
  const uninstallPlugin = useCallback(async (request: PluginUninstallRequest) => {
    try {
      setError(null);
  ...
  const enablePlugin = useCallback(async (pluginKey: string) => {
    try {
      setError(null);
  ...
  const restartPlugin = useCallback(async (pluginKey: string) => {
    try {
      setError(null);
  ...
  const getPlugin = useCallback((domain: string, name: string): Plugin | null => {
    return plugins.find(plugin =>
      plugin.metadata.domain === domain && plugin.metadata.name === name
    ) || null;
  }, [plugins]);

  const getPluginHealth = useCallback(async (pluginKey: string): Promise<PluginHealth> => {
    try {
      const response = await apiClient.get<PluginHealth>(`${API_ENDPOINTS.PLUGIN_HEALTH}/${pluginKey}`);
  ...
  const findPlugins = useCallback((filters: PluginSearchFilters): Plugin[] => {
    return plugins.filter(plugin => {
      ...
      if (filters.name_pattern) {
        ...
        if (!pattern.test(plugin.metadata.name) &&
  ...
  const enableMultiplePlugins = useCallback(async (pluginKeys: string[]): Promise<Record<string, boolean>> => {
  ...
  const disableMultiplePlugins = useCallback(async (pluginKeys: string[]): Promise<Record<string, boolean>> => {
    const results: Record<string, boolean> = {};
