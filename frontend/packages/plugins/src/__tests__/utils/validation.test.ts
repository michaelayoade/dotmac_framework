...
describe('Plugin Validation Utils', () => {
  describe('validatePluginName', () => {
    ...
    it('should validate character format', () => {
      const result = validatePluginName('my plugin!');
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Plugin name can only contain letters, numbers, hyphens, and underscores');
    });

    it('should warn about leading/trailing special characters', () => {
      const result = validatePluginName('-my-plugin-');
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain('Plugin name should not start or end with hyphens or underscores');
    });
  });
  ...
  describe('validatePluginVersion', () => {
    ...
    it('should validate semantic versioning format', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Plugin version must follow semantic versioning (e.g., 1.0.0, 1.2.3-alpha.1)');
    });
  });
  ...
  describe('validatePluginDescription', () => {
    ...
    it('should warn about short descriptions', () => {
      ...
      expect(result.warnings).toContain('Plugin description should be at least 10 characters for better discoverability');
    });
  ...
  describe('validatePluginDependencies', () => {
    ...
    it('should validate dependency format', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Invalid dependency format: invalid-format. Expected format: domain.name');
    });
  ...
  describe('validatePluginTags', () => {
    ...
    it('should warn about too many tags', () => {
      ...
      expect(result.warnings).toContain('Too many tags, consider using only the most relevant ones');
    });
  });
  ...
  describe('validatePluginCategories', () => {
    ...
    it('should warn about unknown categories', () => {
      ...
      expect(result.warnings).toContain('Unknown category: unknown-category. Consider using standard categories');
    });
  });
