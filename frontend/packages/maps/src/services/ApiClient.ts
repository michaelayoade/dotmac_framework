...
export class ApiClient {
  ...
  async getServiceAreas(territoryId?: string): Promise<ApiResponse<ServiceArea[]>> {
    const endpoint = territoryId
      ? `/service-areas?territoryId=${territoryId}`
      : '/service-areas';
  ...
  async updateNetworkNode(id: string, updates: Partial<NetworkNode>): Promise<ApiResponse<NetworkNode>> {
  ...
  async calculateRoute(waypoints: Coordinates[]): Promise<ApiResponse<{
    route: Coordinates[];
    distance: number;
    duration: number;
    instructions: string[];
  ...
  async reverseGeocode(coordinates: Coordinates): Promise<ApiResponse<{
    address: string;
    components: {
      street?: string;
      city?: string;
  ...
  async getCompetitorData(territoryId: string): Promise<ApiResponse<Array<{
    name: string;
    serviceTypes: string[];
    coverage: number;
    marketShare: number;
