...
export class LeafletProvider extends MapProvider {
  ...
  addServiceArea(area: ServiceArea): void {
    try {
      const coordinates = area.polygon.coordinates.map(coord => [coord.lat, coord.lng] as [number, number]);
  ...
  addRoute(route: Route): void {
    try {
      const coordinates = route.waypoints.map(coord => [coord.lat, coord.lng] as [number, number]);
  ...
  async exportPDF(options: { pageSize: string; orientation: string } = { pageSize: 'A4', orientation: 'landscape' }): Promise<Blob> {
