...
const partnerCodeSchema = z.string()
...
function createPasswordSchema(portalVariant: PortalVariant) {
  ...
  let schema = z.string()
    ...
    .max(requirements.maxLength, `Password must be no more than ${requirements.maxLength} characters`);
 ...
 */
export const passwordChangeSchema = (portalVariant: PortalVariant) => z.object({
 ...
 */
export const passwordResetConfirmSchema = (portalVariant: PortalVariant) => z.object({
...
): { success: true; data: LoginCredentials } | { success: false; errors: Record<string, string[]> } {
  try {
...
export function validatePasswordStrength(
  ...
  return {
    ...
    feedback: feedback.length > 0 ? feedback :
             score >= 90 ? ['Excellent password strength!'] :
             score >= 70 ? ['Good password strength'] :
             score >= 50 ? ['Fair password strength'] :
