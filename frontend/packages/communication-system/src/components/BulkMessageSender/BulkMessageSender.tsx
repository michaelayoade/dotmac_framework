...
export function BulkMessageSender({
  ...
  const parseCsvData = useCallback((csvText: string): RecipientData[] => {
  ...
  const generatePreview = useCallback((recipient: RecipientData): string => {
    ...
    selectedTemplate.variables.forEach(variable => {
      const value = recipient.variables?.[variable] || recipient[variable as keyof RecipientData] || `[${variable}]`;
  ...
  }, [validateStep, selectedTemplate, processedRecipients, priority, scheduledAt, jobName, communication, onJobCreated]);
            ...
            {selectedTemplate && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-sm text-blue-600 mt-1">
                  Channel: {selectedTemplate.channel} | Variables: {selectedTemplate.variables.join(', ')}
                </p>
              </div>
            )}
                              ...
                              <div key={variable}>
                                ...
                                <input
                                  ...
                                  onChange={(e) => updateRecipient(index, {
                                    variables: { ...recipient.variables, [variable]: e.target.value }
                                  })}
                                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                                />
                              </div>
                  ...
                  >
                    ...
                    {processedRecipients.slice(0, 10).map((recipient, index) => (
                      <option key={index} value={index}>
                        {recipient.name || recipient.email || recipient.phone || `Recipient ${index + 1}`}
                      </option>
                    ))}
                  </select>
                </div>
