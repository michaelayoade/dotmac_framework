...
export function TemplateManager({
  ...
  }, [communication.templates, searchTerm, selectedCategory, selectedChannel, selectedPriority, selectedStatus]);
  ...
  const handleCreateTemplate = useCallback(async (templateData: Omit<CommunicationTemplate, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
  ...
  const handleUpdateTemplate = useCallback(async (templateId: string, updates: Partial<CommunicationTemplate>) => {
    try {
  ...
  const handleDuplicateTemplate = useCallback(async (template: CommunicationTemplate) => {
    const duplicateData = {
      ...template,
  ...
  const TemplateCard = ({ template }: { template: CommunicationTemplate }) => {
    const ChannelIcon = channelIcons[template.channel as keyof typeof channelIcons] || MessageSquare;
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className={`px-2 py-1 text-xs rounded-full ${statusColors[template.isActive.toString() as keyof typeof statusColors]}`}>
