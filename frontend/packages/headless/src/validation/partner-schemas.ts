...
export const PartnerSchema = z.object({
  ...
  partnerCode: z.string().min(3).max(10).regex(/^[A-Z0-9]+$/),
...
export const PartnerLoginSchema = z.object({
  ...
  mfaCode: z.string().regex(/^\d{6}$/).optional(), // 6-digit MFA code
});
...
export const CommissionQueryParamsSchema = z.object({
  page: z.number().min(1).default(1),
  limit: z.number().min(1).max(100).default(10),
  period: z.string().regex(/^\d{4}(-\d{2})?$/).optional(), // YYYY or YYYY-MM
...
export const DashboardDataSchema = z.object({
  ...
  recentCustomers: z.array(CustomerSchema.extend({
    service: z.string().min(1).max(50),
    commission: z.number().min(0),
  })).max(10),
  salesGoals: z.array(z.object({
    id: z.string().min(1),
