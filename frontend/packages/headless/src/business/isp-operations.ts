...
export interface ISPBusinessOperations {
  ...
  customerService: {
    ...
    getBillingHistory(customerId: string, filters?: { limit?: number; status?: string }): Promise<Invoice[]>;
...
export class ISPBusinessService implements ISPBusinessOperations {
  ...
  customerService = {
    async getCustomerProfile(customerId: string): Promise<CustomerProfile> {
      try {
        const response = await this.apiClient.request<{ data: CustomerProfile }>(`/customers/${customerId}/profile`);
        if (!response.data) {
          throw new ISPError({
    ...
    async getUsageHistory(customerId: string, period: DateRange): Promise<UsageData[]> {
      try {
        const response = await this.apiClient.request<{ data: UsageData[] }>(`/customers/${customerId}/usage`, {
          params: {
    ...
    async getBillingHistory(customerId: string, filters: { limit?: number; status?: string } = {}): Promise<Invoice[]> {
      try {
        const response = await this.apiClient.request<{ data: Invoice[] }>(`/customers/${customerId}/invoices`, {
          params: filters
        });
        return response.data || [];
      } catch (error) {
    ...
    async calculateUsageCost(customerId: string, period: DateRange): Promise<number> {
      try {
        const response = await this.apiClient.request<{ data: { cost: number } }>(`/customers/${customerId}/usage/cost`, {
          params: {
            startDate: period.startDate.toISOString(),
            endDate: period.endDate.toISOString()
          }
  ...
  serviceOperations = {
    async getServiceStatus(customerId: string): Promise<ServiceStatus> {
      try {
        const response = await this.apiClient.request<{ data: ServiceStatus }>(`/customers/${customerId}/service/status`);
        if (!response.data) {
          throw new ISPError({
    ...
    async scheduleMaintenanceWindow(params: MaintenanceRequest): Promise<MaintenanceWindow> {
      try {
        const response = await this.apiClient.request<{ data: MaintenanceWindow }>('/maintenance/schedule', {
          method: 'POST',
    ...
    async troubleshootConnection(customerId: string): Promise<DiagnosticsResult> {
      try {
        const response = await this.apiClient.request<{ data: DiagnosticsResult }>(`/customers/${customerId}/diagnostics`, {
          method: 'POST'
        });
        if (!response.data) {
          throw new ISPError({
    ...
    async applyAutomatedFix(customerId: string, fixId: string): Promise<boolean> {
      try {
        const response = await this.apiClient.request<{ data: { success: boolean } }>(`/customers/${customerId}/diagnostics/fix/${fixId}`, {
          method: 'POST'
        });
    ...
    async getMaintenanceHistory(customerId: string): Promise<MaintenanceWindow[]> {
      try {
        const response = await this.apiClient.request<{ data: MaintenanceWindow[] }>(`/customers/${customerId}/maintenance/history`);
        return response.data || [];
      } catch (error) {
        throw new ISPError({
    ...
    async getServicePlans(filters: { category?: string; active?: boolean } = {}): Promise<ServicePlan[]> {
      try {
  ...
  networkOperations = {
    ...
    async getRegionStatus(regionId: string): Promise<RegionStatus> {
      try {
        const response = await this.apiClient.request<{ data: RegionStatus }>(`/network/regions/${regionId}/status`);
        if (!response.data) {
          throw new ISPError({
    ...
    async getDeviceStatus(deviceId: string): Promise<DeviceStatus> {
      try {
        const response = await this.apiClient.request<{ data: DeviceStatus }>(`/network/devices/${deviceId}/status`);
        if (!response.data) {
          throw new ISPError({
    ...
    async restartDevice(deviceId: string): Promise<boolean> {
      try {
        const response = await this.apiClient.request<{ data: { success: boolean } }>(`/network/devices/${deviceId}/restart`, {
          method: 'POST'
        });
        return response.data?.success || false;
      } catch (error) {
    ...
    async getNetworkAlerts(filters: { severity?: string; resolved?: boolean } = {}): Promise<NetworkAlert[]> {
      try {
    ...
    async getNetworkMetrics(period: DateRange): Promise<Record<string, any>> {
      try {
        const response = await this.apiClient.request<{ data: Record<string, any> }>('/network/metrics', {
          params: {
            startDate: period.startDate.toISOString(),
            endDate: period.endDate.toISOString()
          }
  ...
  billingOperations = {
    async calculateRevenue(params: RevenueParams): Promise<RevenueData> {
      try {
        const response = await this.apiClient.request<{ data: RevenueData }>('/billing/revenue/calculate', {
          method: 'POST',
          body: JSON.stringify(params)
        });
        if (!response.data) {
    ...
    async processPayment(paymentRequest: PaymentRequest): Promise<PaymentResult> {
      try {
        const response = await this.apiClient.request<{ data: PaymentResult }>('/billing/payments/process', {
          method: 'POST',
        ...
        throw new ISPError({
          ...
          category: 'business',
          severity: 'high',
          technicalDetails: { originalError: error, paymentRequest: { ...paymentRequest, paymentMethodId: '[REDACTED]' } }
        });
      }
    },
    ...
    async generateCommissions(resellerId: string, period: DateRange): Promise<Commission[]> {
      try {
        const response = await this.apiClient.request<{ data: Commission[] }>('/billing/commissions/generate', {
          method: 'POST',
          body: JSON.stringify({
            resellerId,
            startDate: period.startDate.toISOString(),
    ...
    async createInvoice(customerId: string, lineItems: Omit<InvoiceLineItem, 'id'>[]): Promise<Invoice> {
      try {
        const response = await this.apiClient.request<{ data: Invoice }>('/billing/invoices/create', {
          method: 'POST',
    ...
    async generateStatement(customerId: string, period: DateRange): Promise<any> {
      try {
        const response = await this.apiClient.request<{ data: any }>(`/billing/customers/${customerId}/statement`, {
          params: {
            startDate: period.startDate.toISOString(),
            endDate: period.endDate.toISOString()
          }
    ...
    async calculateCommissionTiers(resellerId: string): Promise<any> {
      try {
        const response = await this.apiClient.request<{ data: any }>(`/billing/resellers/${resellerId}/commission-tiers`);
        return response.data || {};
      } catch (error) {
        throw new ISPError({
