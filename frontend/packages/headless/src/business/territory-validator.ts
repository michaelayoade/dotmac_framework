...
const ValidationResultSchema = z.object({
  ...
  conflictingTerritories: z.array(z.object({
...
export class TerritoryValidator {
  ...
  async validateAddress(
    ...
    if (matchingTerritories.length === 0) {
      for (const territory of activeTerritories) {
        if (territory.boundaries.cities?.some(city => 
          city.toLowerCase() === validatedAddress.city.toLowerCase()
        )) {
          if (!this.isExcluded(validatedAddress, territory)) {
            matchingTerritories.push({
              territory,
    ...
    return ValidationResultSchema.parse({
      ...
      conflictingTerritories: conflictingTerritories.length > 0 ? conflictingTerritories : undefined,
      ...
      if (territory.boundaries.cities) {
        for (const otherTerritory of this.territories) {
          ...
          const overlap = territory.boundaries.cities.filter(city =>
            otherTerritory.boundaries.cities?.some(otherCity =>
              city.toLowerCase() === otherCity.toLowerCase()
            )
          );
    ...
    for (const address of addresses) {
      try {
        ...
        results.push({
          ...
          confidence: 0,
          warnings: [`Invalid address format: ${error instanceof Error ? error.message : 'Unknown error'}`],
        });
      }
    }
