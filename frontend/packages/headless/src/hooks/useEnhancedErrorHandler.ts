...
import { EnhancedISPError, EnhancedErrorFactory, ErrorCode, type ErrorContext } from '../utils/enhancedErrorHandling';
  ...
  const handleError = useCallback((
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('fetch')) {
        enhancedError = EnhancedErrorFactory.network(
          error.message,
          context?.operation
        );
      }
    ...
    // HTTP response errors
      ...
      if (status === 401) {
        ...
        enhancedError = EnhancedErrorFactory.system(
          message,
          context?.operation
        );
      } else {
