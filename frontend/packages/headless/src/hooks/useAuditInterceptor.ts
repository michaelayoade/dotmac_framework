...
import { AuditEventType, AuditOutcome, FrontendAuditEventType, AuditSeverity } from '../api/types/audit';
...
export function useAuditInterceptor(config: AuditInterceptorConfig = {}) {
  ...
  const shouldExcludeUrl = useCallback((url: string): boolean => {
    return excludeUrls.some(pattern => pattern.test(url));
  }, [excludeUrls]);

  // Check if element should be excluded from auditing
  const shouldExcludeElement = useCallback((element: Element): boolean => {
  ...
  useEffect(() => {
    ...
    const handleClick = async (event: MouseEvent) => {
      ...
      await logUIEvent(
        FrontendAuditEventType.UI_BUTTON_CLICK,
        elementInfo.tag,
        {
          element: elementInfo,
  ...
  useEffect(() => {
    ...
    const handlePopState = async () => {
      await logUIEvent(
        FrontendAuditEventType.UI_PAGE_VIEW,
        'navigation',
        {
  ...
  return {
    ...
    manualLog: {
      logAPICall: useCallback(async (method: string, url: string, outcome: AuditOutcome, metadata?: any) => {
        await logDataAccess(method.toLowerCase(), 'api_endpoint', url, outcome, metadata);
      }, [logDataAccess]),

      logUserAction: useCallback(async (action: string, element: string, metadata?: any) => {
        await logUIEvent(FrontendAuditEventType.UI_FEATURE_USED, element, { action, ...metadata });
      }, [logUIEvent]),

      logBusinessProcess: useCallback(async (process: string, outcome: AuditOutcome, metadata?: any) => {
        await logEvent({
