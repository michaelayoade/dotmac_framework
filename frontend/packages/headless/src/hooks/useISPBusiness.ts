...
export interface UseISPBusinessReturn extends ISPBusinessOperations {
  ...
  customerOperations: {
    ...
    getResellerMetrics: (resellerId: string, period: DateRange) => Promise<{
  ...
  adminOperations: {
    ...
    bulkCustomerOperation: (customerIds: string[], operation: 'suspend' | 'reactivate') => Promise<void>;
  };
  ...
  managementOperations: {
    ...
    generateReports: (type: 'financial' | 'operational' | 'customer', period: DateRange) => Promise<any>;
  };
}
...
export function useISPBusiness(options: UseISPBusinessOptions = {}): UseISPBusinessReturn {
  ...
  const customerOperations = useMemo(() => ({
    ...
    getMyCustomers: async (resellerId: string) => {
      ...
      const response = await apiClient.request<{ data: CustomerProfile[] }>(`/resellers/${resellerId}/customers`);
  ...
  const technicianOperations = useMemo(() => ({
    getWorkOrders: async (technicianId: string) => {
      const response = await apiClient.request<{ data: MaintenanceWindow[] }>(`/technicians/${technicianId}/work-orders`);
      return response.data || [];
    },
    ...
    getDeviceList: async (regionId?: string) => {
      ...
      const response = await apiClient.request<{ data: DeviceStatus[] }>('/network/devices', { params });
      return response.data || [];
    }
  }), [businessService, apiClient]);
  ...
  const managementOperations = useMemo(() => ({
    ...
    generateReports: async (type: 'financial' | 'operational' | 'customer', period: DateRange) => {
...
export function useCustomerBusiness(customerId: string) {
  ...
  return {
    ...
    getMyProfile: useCallback(() => business.customerOperations.getMyProfile(customerId), [business, customerId]),
    getMyUsage: useCallback((period: DateRange) => business.customerOperations.getMyUsage(customerId, period), [business, customerId]),
    getMyBills: useCallback(() => business.customerOperations.getMyBills(customerId), [business, customerId]),
    getMyServiceStatus: useCallback(() => business.customerOperations.getMyServiceStatus(customerId), [business, customerId]),
    upgradeMyService: useCallback((planId: string) => business.serviceOperations.upgradeService(customerId, planId), [business, customerId]),
    payBill: useCallback((paymentRequest: PaymentRequest) => business.billingOperations.processPayment(paymentRequest), [business])
  };
}
...
export function useResellerBusiness(resellerId: string) {
  ...
  return {
    ...
    getMyCustomers: useCallback(() => business.customerOperations.getMyCustomers(resellerId), [business, resellerId]),
    getMyCommissions: useCallback((period: DateRange) => business.billingOperations.generateCommissions(resellerId, period), [business, resellerId]),
    getMyMetrics: useCallback((period: DateRange) => business.customerOperations.getResellerMetrics(resellerId, period), [business, resellerId]),
    addNewCustomer: useCallback(async (customerData: Partial<CustomerProfile>) => {
...
export function useTechnicianBusiness(technicianId: string) {
  ...
  return {
    ...
    getMyWorkOrders: useCallback(() => business.technicianOperations.getWorkOrders(technicianId), [business, technicianId]),
    completeWorkOrder: useCallback((workOrderId: string, notes: string) => business.technicianOperations.completeWorkOrder(workOrderId, notes), [business]),
    diagnoseCustomer: useCallback((customerId: string) => business.technicianOperations.runDiagnostics(customerId), [business]),
    getMyDevices: useCallback((regionId?: string) => business.technicianOperations.getDeviceList(regionId), [business]),
    scheduleJob: useCallback((request: MaintenanceRequest) => business.serviceOperations.scheduleMaintenanceWindow(request), [business])
  };
}
...
export function useAdminBusiness() {
  ...
  return {
    ...
    bulkSuspendCustomers: useCallback((customerIds: string[]) => business.adminOperations.bulkCustomerOperation(customerIds, 'suspend'), [business]),
    bulkReactivateCustomers: useCallback((customerIds: string[]) => business.adminOperations.bulkCustomerOperation(customerIds, 'reactivate'), [business]),
...
export function useManagementBusiness() {
  ...
  return {
    ...
    getDashboard: useCallback((period: DateRange) => business.managementOperations.getDashboardData(period), [business]),
    getFinancialReport: useCallback((period: DateRange) => business.managementOperations.generateReports('financial', period), [business]),
    getOperationalReport: useCallback((period: DateRange) => business.managementOperations.generateReports('operational', period), [business]),
    getCustomerReport: useCallback((period: DateRange) => business.managementOperations.generateReports('customer', period), [business])
  };
}
