...
describe('useBilling', () => {
  ...
  describe('Payment Processing', () => {
    ...
    it('processes refunds', async () => {
      const refundedPayment = mockPayment({ id: 'pay_123', refundedAmount: 50.00 });
      (fetch as jest.Mock).mockResolvedValueOnce({
        ok: true,
      ...
      const completedPayment = mockPayment({ id: 'pay_123', status: 'completed', amount: 100.00 });
      act(() => {
        result.current.payments.push(completedPayment);
      });

      await act(async () => {
        await result.current.refundPayment('pay_123', 50.00, 'Customer request');
      });

      expect(result.current.payments.find(p => p.id === 'pay_123')?.refundedAmount).toBe(50.00);
    });
  });
