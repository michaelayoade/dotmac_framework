...
export function useBilling(options: UseBillingOptions = {}) {
  ...
  // Load Statistics
  const loadStats = useCallback(async (timeRange: '30d' | '90d' | '1y' = '30d') => {
    try {
  ...
  }, [loadAccounts, loadInvoices, loadPayments, loadStats, connectWebSocket, enableRealtime, pollInterval]);
