...
export interface ProvisioningRequest {
  ...
  status: 'pending' | 'approved' | 'provisioning' | 'installing' | 'active' | 'failed' | 'cancelled';
...
export function useProvisioning(options: UseProvisioningOptions = {}) {
                  ...
                  req.id === data.requestId
                    ? {
                        ...
                        tasks: req.tasks.map(task =>
                          task.id === data.taskId
                            ? { ...task, status: 'completed', completedAt: new Date(data.completedAt) }
                            : task
                        ),
                      }
                    : req
  ...
  }, [websocketEndpoint, enableRealtime, apiKey, tenantId, resellerId, maxRetries, addNotification]);
  ...
  // Load Statistics
  const loadStats = useCallback(async (timeRange: '24h' | '7d' | '30d' = '24h') => {
    try {
  ...
  const scheduleInstallation = useCallback(async (
    requestId: string,
    scheduledAt: Date,
    technicianId?: string
  ) => {
  ...
  const cancelRequest = useCallback(async (requestId: string, reason: string) => {
    try {
      ...
      setState(prev => ({
        ...
        requests: prev.requests.map(req =>
          req.id === requestId
            ? { ...req, status: 'cancelled', notes: reason }
            : req
        ),
      }));
  ...
  const bulkUpdateStatus = useCallback(async (
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to bulk update requests';
  ...
  return {
    ...
    todayInstallations: state.requests.filter(req =>
      req.scheduledAt && 
      new Date(req.scheduledAt).toDateString() === new Date().toDateString()
    ),
  };
}
