...
// API functions
const fetchTerritories = async (resellerId: string, filters?: TerritoryFilters): Promise<Territory[]> => {
...
const createTerritory = async (territory: Omit<Territory, 'id' | 'createdAt' | 'updatedAt'>): Promise<Territory> => {
...
const updateTerritory = async (territoryId: string, updates: Partial<Territory>): Promise<Territory> => {
...
const isPointInTerritory = (point: [number, number], boundaries: number[][]): boolean => {
  ...
  for (let i = 0, j = boundaries.length - 1; i < boundaries.length; j = i++) {
    if (
      ...
      (point[0] < ((boundaries[j][0] - boundaries[i][0]) * (point[1] - boundaries[i][1])) / 
        (boundaries[j][1] - boundaries[i][1]) + boundaries[i][0])
    ) {
      inside = !inside;
    }
  }
...
export const useTerritory = (options: UseTerritoryOptions) => {
  ...
  const optimizeTerritoryMutation = useMutation({
    mutationFn: ({ territoryId, criteria }: {
      territoryId: string;
  ...
  const territoryStats = useMemo(() => {
    ...
    const priorityDistribution = territories.reduce((acc, t) => {
