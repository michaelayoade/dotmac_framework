...
export const PortalWebSockets: Story = {
  ...
  render: () => {
    ...
    const {
      ...
      connect,
      disconnect
    } = useWebSocket(currentConfig.url);
    ...
    useEffect(() => {
      if (isConnected) {
        ...
        const interval = setInterval(() => {
          ...
          setSimulatedEvents(prev => [...prev.slice(-4), `${randomEvent}: Event at ${timestamp}`]);
        }, 2000);
    ...
    return (
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center space-x-4">
          <div className={`px-3 py-1 rounded-full text-sm font-medium bg-${currentConfig.color}-100 text-${currentConfig.color}-800`}>
            {connectionState.toUpperCase()}
          </div>
          <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              {currentConfig.features.map((feature) => (
                <div key={feature} className={`p-2 rounded text-sm bg-${currentConfig.color}-50 text-${currentConfig.color}-800`}>
                  • {feature}
                </div>
              ))}
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className={`h-32 border rounded-lg p-3 bg-${currentConfig.color}-50 overflow-y-auto`}>
              {simulatedEvents.length === 0 ? (
                ...
                <div className="space-y-1">
                  {simulatedEvents.map((event, index) => (
                    <div key={index} className={`text-sm p-1 rounded bg-${currentConfig.color}-100`}>
                      {event}
                    </div>
                  ))}
                </div>
...
export const RealTimeNotifications: Story = {
  ...
  render: () => {
    ...
    const {
      ...
      connect,
      disconnect
    } = useWebSocket('wss://api.dotmac.com/ws/notifications', {
      onConnect: () => {
        addNotification('success', 'WebSocket Connected', 'Successfully connected to notification service');
      },
      onDisconnect: () => {
        addNotification('warning', 'WebSocket Disconnected', 'Connection to notification service lost');
      }
    });
    ...
    const simulateNotification = (type: 'info' | 'success' | 'warning' | 'error') => {
      const messages = {
        ...
        success: { title: 'Operation Successful', message: 'Your request has been processed successfully' },
...
export const ConnectionRecovery: Story = {
  ...
  render: () => {
    ...
    const {
      ...
      disconnect,
      reconnect
    } = useWebSocket('wss://api.dotmac.com/ws/recovery-demo', {
      reconnectAttempts,
      reconnectInterval,
    ...
    return (
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center space-x-2">
          <Button onClick={connectAll}>Connect All ({connectedCount}/{activeConnections})</Button>
          <Button onClick={disconnectAll} variant="secondary">Disconnect All</Button>
        </div>
            ...
            {connections.map((connection, index) => (
              <div
                ...
                className={`p-2 rounded text-sm ${
                  ...
                  connection.isConnecting ? 'bg-yellow-100 text-yellow-800' :
