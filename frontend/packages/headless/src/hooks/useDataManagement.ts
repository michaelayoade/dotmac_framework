...
export function useDataManager<T = any>(
  ...
  const load = useCallback(async (params?: PaginationParams) => {
  ...
  }, [apiClient, appStore, contextId, endpoint, cacheTimeout, transform, onSuccess, onError, contextState?.filters, contextState?.pagination]);
  ...
  // Bulk operations
  const bulkCreate = useCallback(async (items: Omit<T, 'id'>[]): Promise<T[]> => {
  ...
  const bulkUpdate = useCallback(async (updates: Array<{ id: string; data: Partial<T> }>): Promise<T[]> => {
  ...
  return {
    ...
    selectItems: (items: T[]) => items.forEach(item => appStore.selectItem(contextId, item, true)),
