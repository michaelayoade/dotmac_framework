...
interface UseAuditLoggerReturn {
  ...
  logAuthEvent: (type: AuditEventType, outcome: AuditOutcome, message: string, metadata?: Record<string, any>) => Promise<void>;
  logDataAccess: (operation: string, resourceType: string, resourceId: string, outcome: AuditOutcome, metadata?: Record<string, any>) => Promise<void>;
  logUIEvent: (type: FrontendAuditEventType, element: string, metadata?: Record<string, any>) => Promise<void>;
  logError: (error: Error, context: string, metadata?: Record<string, any>) => Promise<void>;
  logBusinessEvent: (type: AuditEventType, workflow: string, outcome: AuditOutcome, metadata?: Record<string, any>) => Promise<void>;
...
export function useAuditLogger(config: AuditLoggerConfig): UseAuditLoggerReturn {
  const { serviceName, batchSize = 10, batchTimeout = 5000, enableLocalStorage = true, enableConsoleLogging = false } = config;
  ...
  const storeLocally = useCallback((event: CreateFrontendAuditEvent) => {
  ...
  const consoleLog = useCallback((event: CreateFrontendAuditEvent) => {
    if (!enableConsoleLogging) return;

    const logLevel = event.severity === AuditSeverity.CRITICAL || event.severity === AuditSeverity.HIGH ? 'error' :
                    event.severity === AuditSeverity.MEDIUM ? 'warn' : 'info';
  ...
  const logEvent = useCallback(async (event: CreateFrontendAuditEvent) => {
    try {
  ...
  // Batch logging
  const logBatch = useCallback(async (events: CreateFrontendAuditEvent[]) => {
  ...
  const logDataAccess = useCallback(async (
    ...
    const eventType = operation === 'create' ? AuditEventType.DATA_CREATE :
                     operation === 'read' ? AuditEventType.DATA_READ :
                     operation === 'update' ? AuditEventType.DATA_UPDATE :
                     operation === 'delete' ? AuditEventType.DATA_DELETE : AuditEventType.DATA_READ;

    await logEvent({
      event_type: eventType,
      message: `${operation} operation on ${resourceType} ${resourceId}`,
      outcome,
      severity: operation === 'delete' && outcome === AuditOutcome.SUCCESS ? AuditSeverity.MEDIUM : AuditSeverity.LOW,
      actor: createActor(),
      resource: {
        id: resourceId,
        type: resourceType
  ...
  const logUIEvent = useCallback(async (
