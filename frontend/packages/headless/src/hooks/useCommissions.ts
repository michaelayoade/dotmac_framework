...
const calculateCommissionTier = (
  ...
  return tiers.find(tier => 
    monthlyRevenue >= tier.minRevenue && 
    (!tier.maxRevenue || monthlyRevenue <= tier.maxRevenue)
  );
};
...
export const useCommissions = (options: UseCommissionsOptions) => {
  ...
  const metrics = useMemo(() => {
    ...
    const currentMonthAmount = currentMonthTransactions.reduce((sum, t) => sum + t.commissionAmount, 0);
    ...
    // Service performance
    const servicePerformance = transactions.reduce((acc, t) => {
      if (!acc[t.serviceId]) {
    ...
    const topService = Object.entries(servicePerformance)
...
export const usePayoutHistory = (resellerId: string) => {
  ...
  const payoutStats = useMemo(() => {
    ...
    const lastPayout = payouts.sort((a, b) => 
