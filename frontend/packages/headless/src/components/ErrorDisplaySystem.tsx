...
import { EnhancedISPError, type EnhancedErrorResponse, ErrorCode } from '../utils/enhancedErrorHandling';
...
export const EnhancedErrorDisplay: React.FC<EnhancedErrorDisplayProps> = ({
  ...
  return (
    <div className={`rounded-lg border-2 p-4 ${severityColors[errorData.error.severity]} ${className}`}>
      {/* Error Header */}
      ...
      {errorData.context.customerImpact && errorData.context.customerImpact !== 'none' && (
        <div className="mt-4 p-3 bg-white bg-opacity-50 rounded border border-current border-opacity-25">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="font-medium text-sm">Customer Impact: {errorData.context.customerImpact}</p>
              {errorData.context.customerImpact === 'high' || errorData.context.customerImpact === 'critical' && (
                <p className="text-xs mt-1">This error may significantly impact customer experience.</p>
              )}
            </div>
          </div>
        </div>
      ...
      {showTechnicalDetails && (errorData.details.technicalMessage || errorData.details.debugInfo) && (
        <details className="mt-4">
          <summary className="cursor-pointer text-sm font-medium text-gray-600 hover:text-gray-800">
            Technical Details
          </summary>
          <div className="mt-2 p-3 bg-gray-100 rounded text-xs font-mono">
            <p><strong>Error ID:</strong> {errorData.error.id}</p>
            <p><strong>Timestamp:</strong> {errorData.error.timestamp}</p>
            {errorData.details.requestId && (
              <p><strong>Request ID:</strong> {errorData.details.requestId}</p>
            )}
            {errorData.context.correlationId && (
              <p><strong>Correlation ID:</strong> {errorData.context.correlationId}</p>
            )}
            {errorData.details.technicalMessage && (
              <p className="mt-2"><strong>Technical Message:</strong> {errorData.details.technicalMessage}</p>
            )}
...
export const CompactErrorDisplay: React.FC<CompactErrorDisplayProps> = ({
  ...
  return (
    <div className={`flex items-center justify-between p-3 border rounded-md ${severityColors[errorData.error.severity]} ${className}`}>
      <div className="flex items-center space-x-2">
        <span className="text-lg">❌</span>
        <div>
