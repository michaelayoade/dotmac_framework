...
export interface UseManagementOperationsReturn {
  ...
  // Entity Operations
  listEntities: <T extends BaseEntity>(entityType: string, filters?: EntityFilters) => Promise<EntityListResponse<T>>;
  getEntity: <T extends BaseEntity>(entityType: string, entityId: string) => Promise<T>;
  createEntity: <T extends BaseEntity>(request: CreateEntityRequest) => Promise<EntityOperationResult<T>>;
  updateEntity: <T extends BaseEntity>(entityType: string, entityId: string, request: UpdateEntityRequest) => Promise<EntityOperationResult<T>>;
  deleteEntity: (entityType: string, entityId: string) => Promise<EntityOperationResult<void>>;

  // Billing Operations
  getBillingData: (entityId: string, period: { start_date: string; end_date: string }) => Promise<BillingData>;
  ...
  // Batch Operations
  batchOperation: <T>(operations: Array<{ method: string; endpoint: string; data?: any }>) => Promise<EntityOperationResult<T>[]>;
...
export function useManagementOperations(
  ...
  function setGlobalState(updates: Partial<Pick<ManagementOperationsState, 'globalLoading' | 'globalError'>>): void {
  ...
  const listEntities = useCallback(async <T extends BaseEntity>(
    entityType: string,
  ...
  const getEntity = useCallback(async <T extends BaseEntity>(
    entityType: string,
    entityId: string
  ): Promise<T> => {
    if (!apiClientRef.current) throw new Error('API client not initialized');
  ...
  const batchOperation = useCallback(async <T>(
  ...
  const refreshData = useCallback(async (entityType?: string, entityId?: string): Promise<void> => {
  ...
  const subscribeToEntityList = useCallback((entityType: string, filters?: EntityFilters): (() => void) => {
  ...
  const getError = useCallback((operation?: string): string | null => {
    if (!operation) {
      return state.globalError;
    }
...
export function useEntityManagement<T extends BaseEntity>(entityType: string, config: UseManagementOperationsConfig) {
  ...
  return {
    ...
    createEntity: (data: any) => operations.createEntity({
      entity_type: entityType as EntityType,
      data
    }),
    updateEntity: (entityId: string, data: any) => operations.updateEntity<T>(entityType, entityId, { data }),
    deleteEntity: (entityId: string) => operations.deleteEntity(entityType, entityId),
    isLoading: (operation?: string) => operations.isLoading(`entities.${entityType}.${operation || 'entities'}`),
    hasError: (operation?: string) => operations.hasError(`entities.${entityType}.${operation || 'entities'}`),
    getError: (operation?: string) => operations.getError(`entities.${entityType}.${operation || 'entities'}`)
  };
}
...
export function useBillingOperations(config: UseManagementOperationsConfig) {
  ...
  return {
    ...
    isLoading: (entityId: string, operation = 'billingData') => operations.isLoading(`billing.${entityId}.${operation}`),
    hasError: (entityId: string, operation = 'billingData') => operations.hasError(`billing.${entityId}.${operation}`),
    getError: (entityId: string, operation = 'billingData') => operations.getError(`billing.${entityId}.${operation}`)
  };
}
