...
describe('secureStorage', () => {
  ...
  describe('Security Controls', () => {
    it('should block storage of tokens', async () => {
      await expect(
        secureStorage.setItem('auth_token', 'secret-token-value')
      ).rejects.toThrow('Attempted to store sensitive data in insecure storage');
    });

    it('should block storage of passwords', async () => {
      await expect(
        secureStorage.setItem('user_password', 'super-secret')
      ).rejects.toThrow('Attempted to store sensitive data in insecure storage');
    });

    it('should block storage of secrets', async () => {
      await expect(
        secureStorage.setItem('api_secret', 'confidential')
      ).rejects.toThrow('Attempted to store sensitive data in insecure storage');
    });

    it('should block storage with AUTH in key name', async () => {
      await expect(
        secureStorage.setItem('authentication_data', 'some-data')
      ).rejects.toThrow('Attempted to store sensitive data in insecure storage');
    });
  ...
  describe('Error Handling', () => {
    it('should handle storage quota exceeded', async () => {
      ...
      await expect(
        secureStorage.setItem('large_data', 'x'.repeat(10000))
      ).resolves.toBeUndefined();
    });
  ...
  describe('SSR Safety', () => {
    it('should handle server-side rendering', async () => {
      ...
      await expect(
        secureStorage.setItem('ssr_data', 'value')
      ).resolves.toBeUndefined();
