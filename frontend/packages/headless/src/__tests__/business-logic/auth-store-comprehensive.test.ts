      ...
      beforeEach(() => {
        ...
        mockFetch = BusinessLogicTestFactory.createMockFetch({
          'POST /api/auth/login': {
            status: 200,
            data: {
              user: BusinessLogicTestFactory.createUser(portalType === 'admin' ? 'admin' : portalType),
          ...
          businessRules: [
            ...
            },
            {
              ...
              setup: async (auth: AuthStore) => {
                ...
                expect(authConfig.portal.security.requireDeviceVerification).toBe(requiresDeviceVerification);
              },
              test: (auth: AuthStore) => {
            ...
            },
            {
              ...
              setup: async (auth: AuthStore) => {
                ...
                expect(authConfig.portal.security.maxSessionDuration)
                  .toBe(sessionTimeouts[portalType as keyof typeof sessionTimeouts]);
              },
              test: (auth: AuthStore) => {
                expect(true).toBe(true);
              },
