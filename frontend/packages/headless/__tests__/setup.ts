...
export const createMockPortalConfig = (portal: string, overrides = {}) => ({
  ...
  theme: portal === 'admin' ? 'professional' :
         ...
         portal === 'reseller' ? 'business' :
         portal === 'management-admin' ? 'enterprise' :
         portal === 'management-reseller' ? 'corporate' :
         portal === 'tenant-portal' ? 'minimal' : 'default',
  features: {
    notifications: true,
    realtime: portal !== 'customer',
    analytics: portal === 'admin' || portal === 'management-admin' || portal === 'management-reseller',
    ...
    return {
      message: () =>
        isCached
          ...
          : `Expected response to be cached`,
      pass: isCached,
    };
  }
});
