...
export class InputSanitizer {
  ...
  sanitizeText(input: string, config?: Partial<SanitizationConfig>): SanitizationResult {
    ...
    workingConfig.customValidators.forEach((validator, index) => {
      try {
        ...
        violations.push(`Custom validator ${index + 1} failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    });
  ...
  sanitizeEmail(email: string): SanitizationResult {
    ...
    return {
      ...
      violations,
      isValid: violations.filter(v => !v.includes('Applied') && !v.includes('sanitized')).length === 0,
    };
  }
   ...
   */
  sanitizeNumber(input: string | number, options?: { min?: number; max?: number; decimals?: number }): SanitizationResult {
    const violations: string[] = [];
...
export const sanitizeEmail = (email: string) => 
  defaultSanitizer.sanitizeEmail(email);

export const sanitizePhone = (phone: string) => 
  defaultSanitizer.sanitizePhone(phone);

export const sanitizeNumber = (input: string | number, options?: { min?: number; max?: number; decimals?: number }) => 
  defaultSanitizer.sanitizeNumber(input, options);

export const sanitizePath = (path: string) => 
  defaultSanitizer.sanitizePath(path);
