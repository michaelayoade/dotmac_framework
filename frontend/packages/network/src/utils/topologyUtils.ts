...
export const topologyUtils = {
  ...
  calculateStatistics: (nodes: NetworkNode[], links: NetworkLink[]) => {
    ...
    return {
      ...
      network_density: nodes.length > 1
   ...
   */
  validateTopology: (nodes: NetworkNode[], links: NetworkLink[]): {
    valid: boolean;
    errors: string[];
  } => {
    const errors: string[] = [];
    ...
    links.forEach(link => {
      if (!nodeIds.has(link.source_node_id)) {
        errors.push(`Link ${link.link_id} references non-existent source node ${link.source_node_id}`);
      }
      if (!nodeIds.has(link.target_node_id)) {
        errors.push(`Link ${link.link_id} references non-existent target node ${link.target_node_id}`);
      }
    });
