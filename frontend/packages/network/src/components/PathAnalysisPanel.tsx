...
export const PathAnalysisPanel: React.FC<PathAnalysisPanelProps> = ({
  ...
  const pathStats = useMemo(() => {
    ...
    const avgPacketLoss = result.hops.reduce((sum, hop) => sum + hop.packetLoss, 0) / result.hops.length;
    ...
    return {
      ...
      qualityScore: Math.max(0, 100 - (avgPacketLoss * 2 + (totalRtt / reachableHops.length || 0) / 10))
    };
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{pathStats.avgPacketLoss.toFixed(1)}%</p>
              </div>
      ...
      {currentResult && (
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge 
              variant={currentResult.status === 'completed' ? 'default' : 'destructive'}
            >
          ...
          <div className="space-y-3">
            {currentResult.hops.map((hop, index) => (
              <div key={hop.id} className="flex items-center space-x-4 p-3 border rounded-lg hover:bg-gray-50">
          ...
          <div className="mt-4 pt-4 border-t border-gray-200 text-sm text-gray-500">
            ...
            {currentResult.mtu && (
              <span className="ml-4">MTU: {currentResult.mtu} bytes</span>
            )}
          </div>
        </Card>
      )}
