...
export function useNetworkTopology(options: UseNetworkTopologyOptions): UseNetworkTopologyResult {
  ...
  const analyzePath = useCallback(async (source: string, target: string): Promise<PathAnalysis> => {
    if (!tenant_id) {
  ...
  const getNodeConnectivity = useCallback(async (nodeId: string) => {
    if (!tenant_id) {
      throw new Error('Tenant ID is required');
    }

    try {
      const response = await apiClient.get(`/api/network/topology/nodes/${nodeId}/connectivity?tenant_id=${tenant_id}`);
      return response.data;
    } catch (err: any) {
...
export function useNetworkPath(sourceNode?: string, targetNode?: string) {
  ...
  const analyzePath = useCallback(async (source: string, target: string) => {
...
export function useNetworkAlerts(tenantId: string) {
  ...
  useEffect(() => {
    ...
    const fetchAlerts = async () => {
      try {
        const response = await apiClient.get(`/api/network/alerts?tenant_id=${tenantId}&status=active`);
