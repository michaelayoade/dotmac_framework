...
export function useGeographicData(options: UseGeographicDataOptions): UseGeographicDataResult {
  ...
  const fetchGeographicData = useCallback(async () => {
    ...
    try {
      ...
      const nodesResponse = await apiClient.get(`/api/network/topology/nodes?tenant_id=${tenant_id}&include_coordinates=true`);
      setNetworkNodes(nodesResponse.data?.filter((node: NetworkNode) =>
        node.latitude && node.longitude
      ) || []);

      // Fetch coverage gaps if requested
      if (include_coverage_analysis) {
        const gapsResponse = await apiClient.get(`/api/gis/coverage-gaps?tenant_id=${tenant_id}&status=active`);
...
export function useServiceArea(serviceAreaId: string) {
  ...
  const updateServiceArea = useCallback(async (updates: Partial<ServiceArea>) => {
    if (!serviceAreaId) {
...
export function useCoverageGaps(tenantId: string) {
  ...
  const resolveGap = useCallback(async (gapId: string, resolution: string) => {
    try {
  ...
  const updateGapPriority = useCallback(async (gapId: string, priorityScore: number) => {
    try {
      const response = await apiClient.patch(`/api/gis/coverage-gaps/${gapId}`, {
        priority_score: priorityScore
      });

      setCoverageGaps(prev => prev.map(gap =>
        gap.id === gapId ? { ...gap, priority_score: priorityScore } : gap
      ));
...
export function useRouteOptimization() {
  ...
  const optimizeRoute = useCallback(async (routeParams: {
