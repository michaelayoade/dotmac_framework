...
export const safeLocalStorage = typeof window !== 'undefined' ?
  new SafeStorage(window.localStorage) : null;
export const safeSessionStorage = typeof window !== 'undefined' ?
  new SafeStorage(window.sessionStorage) : null;
...
export function getStorageKeys(storage: StorageInterface | null): string[] {
  ...
  try {
    ...
    const actualStorage = storage === safeLocalStorage ?
...
export function createStorageCache<T>(
  ...
  return {
    ...
    clear(): void {
      const keys = getStorageKeys(storage);
      keys.filter(key => key.startsWith(`${prefix}_`))
