...
import { MLService, InsightService, recognizePatterns, detectAnomalies } from '@dotmac/ml-analytics';
import type {
  WorkOrder,
  BusinessInsight,
  AnomalyDetection,
...
export function WorkOrderAnalytics({
  ...
  const analyzeWorkOrders = async () => {
    ...
    try {
      ...
      const recentAvg = recentDurations.reduce((sum, d) => sum + d, 0) / recentDurations.length || 0;
      const olderAvg = olderDurations.reduce((sum, d) => sum + d, 0) / olderDurations.length || 0;

      const efficiencyTrend = recentAvg < olderAvg ? 'up' :
                            recentAvg > olderAvg ? 'down' : 'stable';
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-600">
                      ...
                      Duration: {anomaly.value.toFixed(1)}h
                      (Expected: {anomaly.expected?.toFixed(1)}h)
                    </div>
                  </div>
                </div>
