...
export function FieldOpsProvider({ children, config: configOverride = {} }: FieldOpsProviderProps) {
  ...
  useEffect(() => {
    const initializeFieldOps = async () => {
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'Failed to initialize field operations';
  ...
  const updateConfig = React.useCallback((newConfig: Partial<FieldOpsConfig>) => {
    setConfig(prev => {
  ...
  return (
    <FieldOpsContext.Provider value={contextValue}>
      {children}
