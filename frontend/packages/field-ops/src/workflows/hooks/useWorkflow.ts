...
interface UseWorkflowReturn {
  ...
  completeStep: (stepId: string, data: Record<string, any>, evidence: StepEvidence[]) => Promise<void>;
...
export function useWorkflow(options: UseWorkflowOptions): UseWorkflowReturn {
  ...
  const updateWorkflow = useCallback((updates: Partial<WorkflowInstance>) => {
    if (!workflow) return;
  ...
  const pauseStep = useCallback(async (stepId: string) => {
    if (!workflow) return;
  ...
  // Navigation helpers
  const canGoToStep = useCallback((stepId: string): boolean => {
  ...
  const validateWorkflow = useCallback((): WorkflowValidationResult => {
    if (!workflow) {
      return { isValid: false, errors: [{ stepId: '', message: 'No workflow loaded' }], warnings: [] };
    }
  ...
  const addStepEvidence = useCallback((stepId: string, evidence: StepEvidence) => {
    if (!workflow) return;

    const updatedSteps = workflow.steps.map(s =>
      s.id === stepId
        ? { ...s, evidence: [...(s.evidence || []), evidence] }
        : s
    );
  ...
  const removeStepEvidence = useCallback((stepId: string, evidenceId: string) => {
    if (!workflow) return;
  ...
  // Customer interaction
  const setCustomerPresent = useCallback((present: boolean) => {
    updateWorkflow({ customerPresent: present });
  }, [updateWorkflow]);

  const addCustomerSignature = useCallback((signature: StepEvidence) => {
    updateWorkflow({ customerSignature: signature });
  }, [updateWorkflow]);

  const addCustomerFeedback = useCallback((rating: number, comments: string) => {
    updateWorkflow({
  ...
  const estimatedTimeRemaining = workflow?.steps
