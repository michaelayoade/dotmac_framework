...
class MLAnalyticsService {
  ...
  async getModelPerformanceHistory(modelId: string): Promise<ModelPerformance[]> {
    const history = await this.makeRequest<ModelPerformance[]>(`/models/${modelId}/performance-history`);
  ...
  async getAutoMLExperiment(experimentId: string): Promise<AutoMLExperiment> {
    const experiment = await this.makeRequest<AutoMLExperiment>(`/automl/experiments/${experimentId}`);
