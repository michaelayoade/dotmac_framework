...
export class InsightService {
  ...
  private async generateRevenueInsights(
    ...
    if (Math.abs(growthRate) > 0.1) { // 10% threshold
      insights.push({
        ...
        recommendations: growthRate > 0
          ...
          : ['Investigate decline causes', 'Review pricing strategy', 'Focus on customer retention'],
        metrics: {
  ...
  private async analyzeTrend(data: TimeSeriesData[]): Promise<PredictiveInsight | null> {
    ...
    if (strength > 0.6) { // Strong trend threshold
      return {
        id: `trend-${Date.now()}`,
        type: 'predictive',
        category: 'trend',
        ...
        recommendations: trend > 0
          ? ['Capitalize on positive momentum', 'Scale successful initiatives', 'Plan for increased demand']
  ...
  private calculateChurnRate(customerData: any[]): number {
    ...
    const activeCustomers = customerData.filter(c =>
      new Date(c.lastActivity || c.updated_at || c.created_at) > thirtyDaysAgo
    );
  ...
  private calculateEfficiencyMetrics(operationData: any[]) {
    ...
    const actualUsage = operationData.reduce((sum, item) => sum + (item.usage || item.utilized || 0), 0);
  ...
  private simpleSeasonalityDetection(values: number[]): { detected: boolean; period?: number; strength?: number } {
