...
export function calculateClassificationMetrics(
  ...
  const confusionMatrix = Array(classes).fill(null).map(() => Array(classes).fill(0));
  ...
  for (let classIdx = 0; classIdx < classes; classIdx++) {
    ...
    const falsePositive = confusionMatrix.reduce((sum, row, i) =>
      i !== classIdx ? sum + row[classIdx] : sum, 0
    );
    const falseNegative = confusionMatrix[classIdx].reduce((sum, val, j) =>
      j !== classIdx ? sum + val : sum, 0
    );
  ...
  for (let fold = 0; fold < folds; fold++) {
    ...
    switch (metric) {
      ...
      case 'mse':
        ...
        break;
      case 'accuracy':
        score = testTargets.filter((actual, i) => actual === Math.round(predictions[i])).length / testTargets.length;
        break;
      default:
        score = mean(testTargets.map((actual, i) => Math.abs(actual - predictions[i])));
    }
