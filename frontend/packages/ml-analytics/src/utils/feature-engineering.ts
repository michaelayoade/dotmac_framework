...
export function engineerFeatures(
  ...
  if (config.includeLagFeatures) {
    const { features: lagFeatures, featureNames: lagFeatureNames } = createLagFeatures(data, config.lagPeriods);
  ...
  if (config.includeRollingFeatures) {
    const { features: rollingFeatures, featureNames: rollingFeatureNames } = createRollingWindowFeatures(
      data,
      config.rollingWindows
    );
  ...
  if (config.includeSeasonalFeatures) {
    const { features: seasonalFeatures, featureNames: seasonalFeatureNames } = createSeasonalFeatures(
      data,
  ...
  const { features: technicalFeatures, featureNames: technicalFeatureNames } = createTechnicalIndicators(data);
...
export function createTechnicalIndicators(data: TimeSeriesData[]): {
  ...
  for (let i = 20; i < data.length; i++) { // Start from index 20 to ensure all indicators are available
    const featureRow: number[] = [];
...
export function normalizeFeatures(
  ...
  scalers: Array<{ min?: number; max?: number; mean?: number; std?: number; median?: number; iqr?: number }>;
} {
  ...
  features.forEach(row => {
    ...
    row.forEach((value, featureIdx) => {
      ...
      switch (method) {
        case 'minmax':
          normalizedValue = scaler.max !== scaler.min ? (value - scaler.min) / (scaler.max - scaler.min) : 0;
          break;
        case 'zscore':
...
export function selectTopFeatures(
  ...
  const selectedFeatures = features.map(row =>
    topFeatureIndices.map(idx => row[idx])
  );
