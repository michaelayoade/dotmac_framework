...
export const validateAppearanceSettings = (settings: Partial<AppearanceSettings>): Record<string, string> => {
  ...
  if (settings.layout) {
    if (settings.layout.density && !['compact', 'comfortable', 'spacious'].includes(settings.layout.density)) {
...
export const generateColorVariations = (baseColor: string): Record<string, string> => {
  ...
  return {
    ...
    200: rgbToHex(
      Math.min(255, Math.floor(r + (255 - r) * 0.70)),
      Math.min(255, Math.floor(g + (255 - g) * 0.70)),
      Math.min(255, Math.floor(b + (255 - b) * 0.70))
    ),
    300: rgbToHex(
    ...
    700: rgbToHex(Math.floor(r * 0.70), Math.floor(g * 0.70), Math.floor(b * 0.70)),
    800: rgbToHex(Math.floor(r * 0.55), Math.floor(g * 0.55), Math.floor(b * 0.55)),
    900: rgbToHex(Math.floor(r * 0.40), Math.floor(g * 0.40), Math.floor(b * 0.40)),
  };
};
...
export const watchSystemThemeChanges = (callback: (theme: 'light' | 'dark') => void): (() => void) => {
