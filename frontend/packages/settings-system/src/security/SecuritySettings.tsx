      ...
      <div className="p-6 space-y-8">
                ...
                <div key={key} className="flex items-center justify-between">
                  ...
                  {readonly ? (
                    <span className="text-sm text-gray-900">
                      {settings.passwordPolicy[key as keyof typeof settings.passwordPolicy] ? 'Yes' : 'No'}
                    </span>
                  ) : (
                    <button
                      onClick={() => updatePasswordPolicy(key as any, !settings.passwordPolicy[key as keyof typeof settings.passwordPolicy])}
                      disabled={isLoading}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        settings.passwordPolicy[key as keyof typeof settings.passwordPolicy] ? 'bg-blue-600' : 'bg-gray-200'
                      } ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                          settings.passwordPolicy[key as keyof typeof settings.passwordPolicy] ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  )}
        ...
        <div>
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Control how sessions are managed
            </p>
          </div>
            ...
            <div>
              ...
              {readonly ? (
                ...
                <input
                  ...
                  onChange={e => updateSessionSettings('timeoutMinutes', parseInt(e.target.value))}
                  disabled={isLoading}
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 disabled:opacity-50"
                />
              )}
