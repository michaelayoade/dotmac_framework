    ...
    >
      ...
      {metric.change && (
        <div className="mt-2 flex items-center text-xs">
          ...
          >
            <span>{metric.change.value > 0 ? '+' : ''}{metric.change.value}%</span>
...
function FilterToolbarImpl({
  ...
  const activeFiltersCount = useMemo(() => {
    return Object.keys(values).filter(key => values[key] !== undefined && values[key] !== '').length;
  }, [values]);
      ...
      case 'boolean':
        return (
          <Select
            ...
            onValueChange={(val) => onFilterChange(filter.key, val === '' ? undefined : val === 'true')}
          >
      ...
      <div className="flex items-center space-x-2">
        ...
        {actions
          ...
          .map((action) => (
            ...
            >
              {action.loading && <div className="animate-spin h-4 w-4 mr-2 border-2 border-current border-t-transparent rounded-full" />}
              ...
              {config.showFilters && config.filters.length > 0 && (
                <FilterToolbar
                  ...
                  onFilterChange={(values) => {
                    try { trackAction('filter', 'management-filters', { values }); } catch {}
