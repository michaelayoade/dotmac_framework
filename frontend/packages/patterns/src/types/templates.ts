...
export const ActionConfigSchema = z.object({
  ...
  onClick: z.function().args(z.any()).returns(z.void().or(z.promise(z.void()))).optional(),
});
...
export const WorkflowStepConfigSchema = z.object({
  ...
  fields: z.array(z.object({
    key: z.string(),
    label: z.string(),
    type: z.enum(['text', 'email', 'number', 'select', 'multiselect', 'boolean', 'date', 'file', 'textarea']),
    required: z.boolean().default(false),
    validation: z.object({
      min: z.number().optional(),
      max: z.number().optional(),
  ...
  validation: z.object({
    rules: z.array(z.object({
      field: z.string(),
      operator: z.enum(['equals', 'notEquals', 'contains', 'greaterThan', 'lessThan', 'regex']),
      value: z.any(),
      message: z.string(),
    })).default([]),
    onValidate: z.function().args(z.record(z.any())).returns(z.promise(z.object({
      isValid: z.boolean(),
...
export const WorkflowConfigSchema = BaseTemplateConfigSchema.extend({
  ...
  onStepChange: z.function().args(z.number(), z.record(z.any())).returns(z.void().or(z.promise(z.void()))).optional(),
  onComplete: z.function().args(z.record(z.any())).returns(z.promise(z.any())).optional(),
  onCancel: z.function().returns(z.void().or(z.promise(z.void()))).optional(),
});
