...
function getStatusIcon(status: UploadStatus, size = 'w-4 h-4') {
  switch (status) {
    case 'completed': return <Check className={`${size} text-green-600`} />;
    case 'uploading': return <Loader2 className={`${size} text-blue-600 animate-spin`} />;
    case 'paused': return <Pause className={`${size} text-yellow-600`} />;
    case 'failed': return <AlertCircle className={`${size} text-red-600`} />;
    case 'cancelled': return <X className={`${size} text-gray-500`} />;
    default: return <Upload className={`${size} text-gray-600`} />;
  }
}

// ===== SUB-COMPONENTS =====

function Dropzone({ onDrop, accept, multiple = true, disabled, children, className = '' }: DropzoneProps) {
...
export function UniversalFileUpload({
  ...
  const validateFile = useCallback((file: File): string | null => {
    ...
    if (effectiveAccept.length > 0 && !effectiveAccept.includes('*/*')) {
      ...
      const isAccepted = effectiveAccept.some(accept =>
        accept === mimeType ||
        accept === extension ||
    ...
    if (validFiles.length > 0) {
      ...
      if (autoUpload) {
        ...
        setTimeout(() => {
          ...
          if (allCompleted) {
            const completedUploads = validFiles.map(file =>
              uploads.find(u => u.name === file.name)!
            ).filter(Boolean);
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-600">
              ...
              {uploadStats.failed > 0 && <span className="text-red-600">{uploadStats.failed} failed</span>}
