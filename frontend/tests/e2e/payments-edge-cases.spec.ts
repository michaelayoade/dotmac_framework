...
test.describe('Payments Edge Cases (mocked)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/payments/charge', async (route) => {
      ...
      if (scenario === 'declined') {
        await route.fulfill({
          ...
          body: JSON.stringify({ status: 'declined', code: 'card_declined' }),
        });
        ...
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ status: 'succeeded', id: 'pi_123' }),
        });
      }
    });
    ...
    await page.route('**/api/payments/refund', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ status: 'refunded', id: 're_123' }),
      });
    });
    ...
    await page.route('**/api/payments/chargeback', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ status: 'chargeback', id: 'cb_123' }),
      });
    });
  ...
  test('handles declined payment gracefully', async ({ page }) => {
    ...
    await page.evaluate(async () => {
      ...
      el.dataset.testid = 'payment-error';
      el.textContent = 'Card declined';
  ...
  test('performs 3DS authentication and completes payment', async ({ page }) => {
    ...
    await page.evaluate(async () => {
      const res = await fetch('/api/payments/charge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ scenario: '3ds_required' }),
      });
    ...
    await page.evaluate(() => {
      ...
      done.dataset.testid = 'three-ds-complete';
  ...
  test('processes refund successfully', async ({ page }) => {
    await page.goto('http://localhost:3001/customer/billing');
    const res = await page.request.post('/api/payments/refund', {
      data: { paymentId: 'pi_123', amount: 1000 },
    });
  ...
  test('simulates chargeback event', async ({ page }) => {
    await page.goto('http://localhost:3001/customer/billing');
    const res = await page.request.post('/api/payments/chargeback', {
  ...
  test('displays dunning banner and retry schedule', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      b.dataset.testid = 'dunning-banner';
      b.textContent = 'Payment overdue - retry scheduled';
