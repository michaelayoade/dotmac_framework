...
test.describe('Automation Center - ISP Business Rules Management', () => {
  ...
  test('should create and execute service activation automation rule', async ({ page }) => {
    ...
    await page.fill(
      ...
      'Automatically activate internet service when payment is confirmed and credit check passes'
    );
    ...
    await expect(actionResults.nth(2)).toContainText(
      'create_task: Task created for field_technician'
    );
  });
  ...
  test('should handle payment overdue escalation workflow', async ({ page }) => {
    ...
    await page.fill(
      '[data-testid="rule-description"]',
      'Automatically escalate overdue payments and suspend services'
    );

    // Verify and adjust conditions
    await expect(page.locator('[data-testid="condition-field-0"]')).toHaveValue(
  ...
  test('should manage technical support escalation automation', async ({ page }) => {
    ...
    await page.fill(
      '[data-testid="rule-description"]',
      'Auto-escalate critical technical tickets to senior technicians'
    );
    ...
    await page.selectOption('[data-testid="condition-operator-2"]', 'in');
    await page.fill(
      '[data-testid="condition-value-2"]',
      'network_outage,service_down,security_incident'
    );
  ...
  test('should handle rule conflicts and priority resolution', async ({ page }) => {
    ...
    await page.fill(
      '[data-testid="rule-description"]',
      'Medium priority rule for payment processing'
    );
    ...
    await page.fill(
      '[data-testid="rule-description"]',
      'High priority rule for VIP customer payments'
    );
    ...
    await expect(results.nth(0).locator('[data-testid="rule-name"]')).toContainText(
      'High Priority VIP Payment Rule'
    );
    await expect(results.nth(1).locator('[data-testid="rule-name"]')).toContainText(
      'Medium Priority Payment Rule'
    );
    ...
    await expect(standardResults.nth(0).locator('[data-testid="rule-name"]')).toContainText(
      'Medium Priority Payment Rule'
    );
  });
