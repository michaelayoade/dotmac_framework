...
test.describe('Cross-Portal Email Verification with MailHog', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (!isConnected) {
      throw new Error(
        'MailHog is not accessible at http://localhost:8025. Please start MailHog service.'
      );
    }
  ...
  for (const portal of verificationPortals) {
    test(`completes email verification flow for ${portal.name} portal @email-verification @${portal.name.toLowerCase()}`, async ({
      page,
    }) => {
    ...
    test(`handles verification email resend for ${portal.name} portal @email-verification @resend`, async ({
      page,
    }) => {
      const journey = new EmailVerificationJourney(page, mailHogHelper);
    ...
    test(`handles unverified login for ${portal.name} portal @email-verification @unverified`, async ({
      page,
    }) => {
      const journey = new EmailVerificationJourney(page, mailHogHelper);
  ...
  test('email verification accessibility @email-verification @a11y', async ({ page }) => {
    ...
    await expect(page.getByTestId('email-verification-prompt')).toHaveAttribute(
      'aria-live',
      'polite'
    );
  });
