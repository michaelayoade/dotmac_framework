...
import {
  ...
  CurrencyTestData,
  DateTestData,
} from '../testing/e2e/shared-scenarios/i18n-test-helper';
...
class I18nJourney {
  ...
  async testLanguagePersistence(portal: PortalConfig) {
    ...
    const testUrls = [`${portal.url}/dashboard`, `${portal.url}/settings`, `${portal.url}/profile`];
...
test.describe('Cross-Portal Internationalization', () => {
  ...
  for (const portal of portals) {
    test(`supports language switching for ${portal.name} portal @i18n @language-switching @${portal.name.toLowerCase()}`, async ({
      page,
    }) => {
    ...
    test(`maintains RTL layout for ${portal.name} portal @i18n @rtl @${portal.name.toLowerCase()}`, async ({
      page,
    }) => {
      const journey = new I18nJourney(page, i18nHelper);
    ...
    if (portal.hasBilling) {
      test(`formats currency correctly for ${portal.name} portal @i18n @currency @${portal.name.toLowerCase()}`, async ({
        page,
      }) => {
        const journey = new I18nJourney(page, i18nHelper);
        const currencyTests = I18nTestHelper.getCurrencyTestData();
    ...
    test(`formats dates correctly for ${portal.name} portal @i18n @dates @${portal.name.toLowerCase()}`, async ({
      page,
    }) => {
      const journey = new I18nJourney(page, i18nHelper);
      const dateTests = I18nTestHelper.getDateTestData();
