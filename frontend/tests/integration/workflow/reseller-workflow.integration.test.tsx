...
jest.mock('@dotmac/styled-components', () => ({
  card: ({ children }: unknown) => <div className='card'>{children}</div>,
  badge: ({ children }: unknown) => <span className='badge'>{children}</span>,
  button: ({ children, onClick }: unknown) => (
    <button type='button' onClick={onClick} onKeyDown={(e) => e.key === 'Enter' && onClick}>
      {children}
    </button>
  ),
  input: ({ placeholder, onChange, value }: unknown) => (
