...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const now = Date.now();
    const cacheValid =
      healthCheckCache.result && now - healthCheckCache.timestamp < healthCheckCache.ttl;
      ...
      details: {
        ...
        errorRate:
          applicationMetrics.requestCount > 0
            ? applicationMetrics.errorCount / applicationMetrics.requestCount
            : 0,
...
async function checkMemoryUsage(): Promise<HealthCheckResult['checks'][string]> {
  ...
  try {
    ...
    const status = memoryPercentage > 90 ? 'fail' : memoryPercentage > 75 ? 'warn' : 'pass';
...
function getBasicMetrics(): HealthCheckResult['metrics'] {
  ...
  return {
    ...
    requests: {
      ...
      errorRate:
        applicationMetrics.requestCount > 0
          ? Math.round((applicationMetrics.errorCount / applicationMetrics.requestCount) * 100) /
            100
          : 0,
    },
  };
