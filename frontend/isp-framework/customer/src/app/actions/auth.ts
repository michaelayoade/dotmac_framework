...
export async function loginAction(formData: FormData) {
  ...
  const passwordValidation = validateInput(rawPassword, 'password', {
    required: true,
    minLength: 8,
    maxLength: 128,
  });
  ...
  try {
    // Log login attempt
    await audit.logAuthentication('login', { ...context, userId: email }, { portal: 'customer' });
    ...
    // Log system error
    await audit.system('login_system_error', 'auth_action', false, 'high', {
...
export async function logoutAction() {
  try {
    ...
    // Log logout attempt
    await audit.logAuthentication('logout', context, { logoutMethod: 'user_initiated' });
    ...
    await audit.logAuthentication('logout', context, { result: 'success' });
  } catch (error) {
    // Log error but continue with logout
    await audit.system('logout_error', 'auth_action', false, 'medium', {
      error: error instanceof Error ? error.message : 'Unknown error',
    });
  }
