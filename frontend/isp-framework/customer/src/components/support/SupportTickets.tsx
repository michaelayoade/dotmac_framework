...
const mockTickets: SupportTicket[] = [
  {
    ...
    description:
      'My internet connection has been dropping every few hours for the past week. The modem lights show everything is fine, but I lose connectivity completely.',
    category: 'technical',
    priority: 'high',
    status: 'in_progress',
    ...
    responses: [
      ...
      },
      {
        ...
        message:
          'Hi John, thank you for contacting us. I can see you are experiencing intermittent connectivity issues. Let me run some diagnostics on your line. Can you please restart your modem and router and let me know if the issue persists?',
  ...
  const columns = [
    {
      ...
      label: 'Ticket ID',
      render: (value: string) => <div className='font-mono text-sm text-gray-900'>{value}</div>,
    },
    {
      ...
      render: (value: string, item: SupportTicket) => (
        <div>
          ...
          <div className='text-sm text-gray-500 mt-1'>{item.responses.length} responses</div>
        </div>
      ),
    },
    {
      ...
      render: (value: string) => (
        <span
          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${getCategoryColor(value)}`}
        >
          {value.replace('_', ' ')}
        </span>
      ),
    },
    {
      ...
      render: (value: string) => (
        <div className='flex items-center space-x-2'>
          {getStatusIcon(value)}
          <span
            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${getStatusColor(value)}`}
          >
            {value.replace('_', ' ')}
          </span>
        </div>
    ...
    },
    {
      ...
      render: (value: string) => (
        <span
          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${getPriorityColor(value)}`}
        >
          {value}
        </span>
      ),
    ...
    },
    {
      ...
      render: (value: string | undefined) => (
        <div className='text-sm text-gray-900'>{value || 'Unassigned'}</div>
      ),
    },
    {
      key: 'id' as keyof SupportTicket,
