...
test.describe('Security Penetration Testing', () => {
      ...
      await context.addCookies([
        {
          name: 'secure-auth-token',
          value:
            'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyLCJleHAiOjEwMDA5OTk5OTl9.invalid', // Expired token
          domain: 'localhost',
          path: '/',
        },
      ]);
    ...
    test('should prevent SQL injection attempts', async () => {
      ...
      for (const payload of SECURITY_CONFIG.payloads.sqli) {
        ...
        await page.route('**/api/auth/customer/login', (route) => {
          ...
          if (
            body &&
            (body.email?.includes('DROP TABLE') || body.email?.includes('UNION SELECT'))
          ) {
    ...
    test('should prevent path traversal attacks', async () => {
      ...
      for (const payload of SECURITY_CONFIG.payloads.pathTraversal) {
        ...
        const apiResponse = await page.request.get(
          `/api/customer/file/${encodeURIComponent(payload)}`
        );
  ...
  test.describe('Business Logic Security', () => {
    test('should prevent privilege escalation', async () => {
      ...
      const adminEndpoints = ['/admin', '/api/admin/users', '/api/admin/system', '/management'];
