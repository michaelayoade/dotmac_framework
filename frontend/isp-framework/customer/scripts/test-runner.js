...
function generateCoverageReport() {
  ...
  if (fs.existsSync(path.join(coveragePath, 'coverage-summary.json'))) {
    ...
    for (const [metric, threshold] of Object.entries(thresholds)) {
      ...
      console.log(
        `${status} ${metric.padEnd(12)}: ${colors[color]}${actual}%${colors.reset} (target: ${threshold}%)`
      );
    }
...
function checkTestFiles() {
  ...
  const componentsWithoutTests = srcFiles.filter((srcFile) => {
    ...
    return !possibleTestFiles.some((testFile) => fs.existsSync(path.join(process.cwd(), testFile)));
  });
...
async function main() {
  ...
  // 1. Type checking
  results.typecheck = runCommand('pnpm type-check', '🔍 TypeScript Type Checking');

  // 2. Linting
  results.lint = runCommand('pnpm lint', '🧹 ESLint Code Quality Check', {
  ...
  log(
    `  - Components without tests: ${testStats.untested}`,
    testStats.untested > 0 ? 'yellow' : 'green'
  );
  log(
    `  - Test Suites Passed: ${passedTests}/${totalTests}`,
    passedTests === totalTests ? 'green' : 'red'
  );
  log(
    `  - Coverage Target Met: ${coverageResult ? 'Yes' : 'No'}`,
    coverageResult ? 'green' : 'red'
  );
