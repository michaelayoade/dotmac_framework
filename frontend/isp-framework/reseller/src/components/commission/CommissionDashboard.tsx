...
export const CommissionDashboard: React.FC<CommissionDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'transactions' | 'payouts' | 'calculator'
  >('overview');
      ...
      <div className='dashboard-header'>
        <div className='header-content'>
          ...
          <div className='header-actions'>
            ...
            >
              <option
                value={`${new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]}|${new Date().toISOString().split('T')[0]}`}
              >
                Last 30 days
              </option>
              <option
                value={`${new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]}|${new Date().toISOString().split('T')[0]}`}
              >
                Last 90 days
              </option>
              <option
                value={`${new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]}|${new Date().toISOString().split('T')[0]}`}
              >
                Last year
              </option>
            </select>
      ...
      {metrics && (
        <div className='summary-cards'>
          <div className='summary-card'>
            <div className='card-header'>
              ...
              <span className={`trend ${metrics.growthRate >= 0 ? 'positive' : 'negative'}`}>
                ...
                {formatPercent(Math.abs(metrics.growthRate))}
              </span>
            </div>
            <div className='card-value'>{formatCurrency(metrics.totalEarned)}</div>
            <div className='card-subtitle'>{formatCurrency(metrics.currentMonth)} this month</div>
          </div>

          <div className='summary-card'>
            <div className='card-header'>
              <h3>Pending Payout</h3>
              {metrics.isPayoutReady && <span className='payout-ready'>Ready</span>}
            </div>
            <div className='card-value'>{formatCurrency(metrics.pendingAmount)}</div>
            <div className='card-subtitle'>Min: {formatCurrency(metrics.minPayoutAmount)}</div>
          </div>
          ...
          <div className='summary-card'>
            ...
            <div className='card-subtitle'>{metrics.transactionCount} transactions</div>
          </div>
          ...
          <div className='summary-card'>
            ...
            <div className='card-subtitle'>{metrics.topService?.count || 0} sales</div>
          </div>
        </div>
      )}
                  ...
                  <ResponsiveContainer width='100%' height={250}>
                    <PieChart>
                      <Pie
                        ...
                        cy='50%'
                        outerRadius={80}
                        label={({ serviceId, amount }) => `${serviceId}: ${formatCurrency(amount)}`}
                      >
                ...
                <div className='service-list'>
                  {servicePerformanceData.map((service) => (
                    <div key={service.serviceId} className='service-item'>
                      <div className='service-color' style={{ backgroundColor: service.color }} />
            ...
            {metrics?.isPayoutReady && (
              <div className='payout-section'>
                <div className='payout-card'>
                  ...
                  <p>You have {formatCurrency(metrics.pendingAmount)} available for payout.</p>
                  <button onClick={handleRequestPayout} className='payout-button primary'>
                    Request Payout
                  </button>
                </div>
              </div>
              ...
              <div className='table-body'>
                {transactions.map((transaction) => (
                  <div key={transaction.id} className='table-row'>
                    ...
                    <div className='table-cell'>{formatCurrency(transaction.commissionAmount)}</div>
                    <div className='table-cell'>
                  ...
                  <div className='stat-item'>
                    ...
                    <value>
                      {payoutHistory.payoutStats.lastPayout
                        ? new Date(
                            payoutHistory.payoutStats.lastPayout.payoutDate
                          ).toLocaleDateString()
                        : 'None'}
                    </value>
                  </div>
              ...
              <div className='table-body'>
                {payoutHistory.payouts.map((payout) => (
                  <div key={payout.id} className='table-row'>
                    ...
                    <div className='table-cell'>{formatCurrency(payout.totalAmount)}</div>
                    <div className='table-cell'>{payout.transactionCount}</div>
                    <div className='table-cell'>{formatCurrency(payout.fees || 0)}</div>
                    <div className='table-cell'>{formatCurrency(payout.netAmount)}</div>
                    <div className='table-cell'>
