...
import { CardLoading, PageLoading, RefreshLoading } from '../common/LoadingStates';
...
export function ResellerDashboard() {
  ...
  const [salesOpportunities, setSalesOpportunities] = useState<SalesOpportunity[] | null>(null);

  // Get partner ID from user context
  const partnerId = user?.partnerId || user?.id;

  // Use real API data instead of mock data
  const dashboardQuery = usePartnerDataWithErrorBoundary(usePartnerDashboard(partnerId));
      ...
      <ResellerLayout>
        <div className='flex items-center justify-center h-full'>
          <div className='text-center'>
            ...
            <p className='text-gray-600 mb-4'>There was an error loading your partner dashboard.</p>
            <button
                ...
                {commissionData.commission_summary && (
                  <div className='mt-3 pt-3 border-t border-blue-200 flex justify-between text-sm'>
                    ...
                    <span className='font-semibold text-gray-900'>
                      {formatCurrency(commissionData.commission_summary.pending_amount)}
                    </span>
                  </div>
                )}
              </div>
