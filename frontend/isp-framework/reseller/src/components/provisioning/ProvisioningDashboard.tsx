...
import { useNotifications, VirtualizedTable, Modal } from '@dotmac/providers';
...
export function ProvisioningDashboard({ className = '' }: ProvisioningDashboardProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'requests' | 'templates' | 'calendar'>(
    'overview'
  );
  ...
  const filteredRequests = useMemo(() => {
    return provisioning.requests.filter((request) => {
      ...
      if (requestFilters.dateFrom && request.requestedAt < new Date(requestFilters.dateFrom))
        return false;
      if (requestFilters.dateTo && request.requestedAt > new Date(requestFilters.dateTo))
        return false;
      return true;
    });
    ...
    <div className='overview-tab'>
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        ...
        <div className='bg-white p-6 rounded-lg shadow'>
          ...
          <div className='space-y-3'>
            {Object.entries(stats.technicianWorkload)
              .slice(0, 5)
              .map(([technician, workload]) => (
            ...
            <div className='flex items-center justify-between mb-4'>
              ...
              <span
                className={`px-2 py-1 rounded-full text-xs font-medium ${
                  template.category === 'internet'
                    ? 'bg-blue-100 text-blue-800'
                    : template.category === 'phone'
                      ? 'bg-green-100 text-green-800'
                      : template.category === 'tv'
            ...
            <div className='space-y-2 mb-4'>
              {template.speed && (
                <p className='text-sm text-gray-600'>
                  Speed: <span className='font-medium'>{template.speed}</span>
                </p>
              )}
              <p className='text-sm text-gray-600'>
                Setup: <span className='font-medium'>${template.pricing.setup}</span>
              </p>
              <p className='text-sm text-gray-600'>
                Monthly: <span className='font-medium'>${template.pricing.monthly}</span>
              </p>
              <p className='text-sm text-gray-600'>
                SLA: <span className='font-medium'>{template.sla.provisioningTime}h</span>
              </p>
            </div>
...
function CalendarTab({ installations, onReschedule }: CalendarTabProps) {
  ...
  const installationsByDate = installations.reduce(
    (acc, installation) => {
