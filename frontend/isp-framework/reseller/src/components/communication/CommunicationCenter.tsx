...
import { useNotifications, NotificationList, NotificationBadge } from '@dotmac/providers';
...
export function CommunicationCenter({ className = '' }: CommunicationCenterProps) {
  const [activeTab, setActiveTab] = useState<'messages' | 'templates' | 'channels' | 'settings'>(
    'messages'
  );
  ...
  const sendBulkCustomerNotification = useCallback(
    async (templateId: string, recipients: string[]) => {
      try {
  ...
  const filteredMessages = useMemo(() => {
    return communication.messages.filter((message) => {
      ...
      if (
        messageFilters.dateFrom &&
        message.sentAt &&
        message.sentAt < new Date(messageFilters.dateFrom)
      )
        return false;
      if (
        messageFilters.dateTo &&
        message.sentAt &&
        message.sentAt > new Date(messageFilters.dateTo)
      )
        return false;
      return true;
    });
    ...
    <div className={`communication-center ${className}`}>
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-6'>
        ...
        <div className='bg-white p-6 rounded-lg shadow'>
          ...
          <div className='mt-2'>
            <p className='text-xs text-gray-500'>{stats.deliveryRate.toFixed(1)}% delivery rate</p>
          </div>
        </div>
        ...
        <div className='bg-white p-6 rounded-lg shadow'>
          ...
          <div className='mt-2'>
            <p className='text-xs text-gray-500'>{stats.failureRate.toFixed(1)}% failure rate</p>
          </div>
        </div>
          ...
          <div className='flex items-center'>
            <div className='flex-1'>
              ...
              <p className='text-2xl font-bold text-blue-600'>
                {communication.activeChannels.length}
              </p>
            </div>
            <div className='w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center'>
              <span className='text-blue-600'>📡</span>
                ...
                {messages.map((message) => (
                  <tr key={message.id} className='hover:bg-gray-50'>
                    <td className='px-6 py-4 whitespace-nowrap'>
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(message.status)}`}
                      >
        ...
        {channels.map((channel) => (
          <div key={channel.id} className='bg-white p-6 rounded-lg shadow'>
            <div className='flex items-center justify-between mb-4'>
              ...
              <span
                className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(channel.status)}`}
              >
                {channel.status}
              </span>
            </div>
