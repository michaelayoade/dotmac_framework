...
import { Search, X, MapPin, Phone, User, Loader2, History } from 'lucide-react';
...
export function CustomerSearch({
  ...
  const searchCustomers = async (term: string): Promise<Customer[]> => {
    try {
      ...
      return filtered
        .sort((a, b) => {
          const aExactMatch =
            a.name.toLowerCase() === searchLower || a.serviceId.toLowerCase() === searchLower;
          const bExactMatch =
            b.name.toLowerCase() === searchLower || b.serviceId.toLowerCase() === searchLower;
  ...
  const saveRecentSearch = (term: string) => {
    ...
    const updated = [term, ...recentSearches.filter((search) => search !== term)].slice(0, 5); // Keep only 5 recent searches
              ...
              >
                ...
                {recentSearches.length === 0 && recentCustomers.length === 0 && (
                  <div className='text-center py-12'>
                    ...
                    <p className='text-gray-600 text-sm'>
                      Type at least 2 characters to search for customers by name, phone, email, or
                      service ID
                    </p>
                  </div>
                )}
