...
export function PerformanceDashboard({ isOpen, onClose }: PerformanceDashboardProps) {
  const [activeTab, setActiveTab] = useState<
    'overview' | 'vitals' | 'database' | 'sync' | 'network'
  >('overview');
  ...
  const loadPerformanceData = async () => {
    try {
      const [webVitals, dbStats, syncStats, syncMetrics] = await Promise.all([
                  ...
                  <div className='flex items-center justify-between'>
                    <div>
                      ...
                      <p className='text-2xl font-bold text-blue-900'>
                        {performanceData?.webVitals?.webVitals?.find((v: any) => v.name === 'LCP')
                          ?.value
                          ? formatMs(
                              performanceData.webVitals.webVitals.find((v: any) => v.name === 'LCP')
                                .value
                  ...
                  <div className='space-y-2'>
                    ...
                    <div className='flex items-center justify-between'>
                      ...
                      >
                        {performanceData?.syncManager?.pending === 0
                          ? 'Synced'
                          : `${performanceData?.syncManager?.pending || 0} pending`}
                      </span>
                    </div>
                  </div>
                  <div className='space-y-2'>
                    ...
                    <div className='flex items-center justify-between'>
                      ...
                      <span className='text-sm text-gray-900'>
                        {performanceData?.database?.cacheSize
                          ? formatBytes(performanceData.database.cacheSize)
                          : '0 B'}
                      </span>
                    </div>
                  </div>
          ...
          {activeTab === 'database' && (
            <div className='space-y-6'>
              ...
              <div className='bg-white border rounded-lg p-4'>
                ...
                <div className='space-y-2'>
                  {Object.entries(performanceData?.database?.queryStats || {}).map(
                    ([query, stats]: [string, any]) => (
                      <div
                        key={query}
                ...
                <div className='bg-blue-50 border border-blue-200 rounded-lg p-4'>
                  <div className='flex items-start space-x-3'>
                    ...
                    <div>
                      ...
                      {Object.entries(performanceData.database.indexSuggestions).map(
            ...
            <div className='space-y-6'>
              ...
              {performanceData?.webVitals?.resourceCounts && (
                <div className='bg-white border rounded-lg p-4'>
                  ...
                  <div className='space-y-2'>
                    {Object.entries(performanceData.webVitals.resourceCounts).map(
