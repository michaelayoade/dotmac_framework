...
export const InventoryManagement: React.FC = () => {
  ...
  const { isOnline, pendingSync, syncData, addToQueue, retrySync } = useOfflineSync('inventory');
  ...
  const columns = [
    {
      ...
      label: 'Type',
      render: (value: string) => (
        <span className='capitalize bg-gray-100 px-2 py-1 rounded-full text-sm'>{value}</span>
      ),
    },
    ...
    { key: 'serial' as keyof InventoryItem, label: 'Serial Number' },
    {
      ...
      render: (value: string) => {
        ...
        return (
          <span
            className={`px-2 py-1 rounded-full text-sm capitalize ${colors[value as keyof typeof colors]}`}
          >
            {value}
          </span>
        );
  ...
  const handleAddItem = async (
    newItem: Omit<InventoryItem, 'id' | 'lastUpdated' | 'syncStatus'>
  ) => {
  ...
  return (
    ...
    >
      <div className='bg-white rounded-lg p-6 w-full max-w-md'>
        <h2 id='add-item-title' className='text-lg font-semibold mb-4'>
          Add Inventory Item
        </h2>
...
const QRScannerModal: React.FC<{
  ...
  return (
    ...
    >
      <div className='bg-white rounded-lg p-6 w-full max-w-md'>
        <h2 id='qr-scanner-title' className='text-lg font-semibold mb-4'>
          QR Code Scanner
        </h2>
