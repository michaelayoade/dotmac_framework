  ...
  const initializeCamera = async () => {
    try {
      ...
      if (videoDevices.length === 0) {
        throw createAppError(ErrorType.CAMERA, 'No cameras found on this device.');
      }
    ...
    for (let y = Math.floor(height / 2) - 10; y < Math.floor(height / 2) + 10; y++) {
      ...
      if (linePattern.length > 20 && /^[01]+$/.test(linePattern)) {
        ...
        if (binaryGroups.length >= 3) {
          ...
          barcodePattern = binaryGroups
            .slice(0, 12)
            .map((group) => parseInt(group.padEnd(8, '0'), 2) % 10)
            .join('');
          ...
          {scanSuccess && (
            ...
            >
              <div className='text-center'>
                ...
                <p className='text-white text-sm opacity-80'>{lastScanResult}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {error && (
          <div className='absolute inset-0 flex items-center justify-center bg-black'>
            <div className='text-center text-white p-6'>
              ...
              <p className='text-sm opacity-80 mb-6 max-w-sm'>{error}</p>
