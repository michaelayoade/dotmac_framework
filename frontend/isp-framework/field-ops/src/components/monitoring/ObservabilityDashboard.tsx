  ...
  return (
    <div className='py-2 px-3 border-b border-gray-200 text-sm'>
      <div className='flex items-center gap-2'>
        <span
          className={`px-2 py-1 rounded text-xs font-medium ${levelColors[level as keyof typeof levelColors] || levelColors.info}`}
        >
          {level.toUpperCase()}
        </span>
...
export const ObservabilityDashboard: React.FC<DashboardProps> = ({ isVisible, onClose }) => {
  ...
  const [selectedTab, setSelectedTab] = useState<
    'overview' | 'performance' | 'logs' | 'events' | 'errors'
  >('overview');
  ...
  const overviewMetrics = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        unit: 'ms',
        color:
          performanceStats.averageLoadTime > 2000
            ? ('red' as const)
      ...
      <div className='bg-white rounded-lg shadow-2xl w-full max-w-6xl h-full max-h-[90vh] overflow-hidden'>
          ...
          {selectedTab === 'logs' && (
            <div className='space-y-4'>
              ...
              <div className='bg-gray-50 rounded-lg border max-h-96 overflow-auto'>
                {recentLogs.length > 0 ? (
                  recentLogs.map((log, index) => <LogEntry key={index} {...log} />)
                ) : (
        ...
        <div className='bg-gray-100 p-3 border-t'>
          <div className='flex justify-between items-center text-xs text-gray-600'>
            <span>Session: {summary?.sessionId?.slice(-8) || 'Unknown'}</span>
