...
export function usePerformance(options: UsePerformanceOptions = {}) {
  ...
  useEffect(() => {
    ...
    const fpsInterval = setInterval(() => {
      ...
      const lastFPSMetric = performanceMonitor
        .getReport()
        .customMetrics.filter((m) => m.name === 'fps')
        .pop();
  ...
  const measureOperation = useCallback(
    <T>(operationName: string, operation: () => T, metadata?: Record<string, any>): T => {
  ...
  const debouncedCallback = useCallback(
    <T extends (...args: any[]) => any>(callback: T, deps: any[] = []): T => {
      return useCallback(debounce(callback, debounceMs), deps) as T;
    },
    [debounceMs]
  );
  ...
  const throttledCallback = useCallback(
    <T extends (...args: any[]) => any>(callback: T, deps: any[] = []): T => {
      return useCallback(throttle(callback, throttleMs), deps) as T;
    },
    [throttleMs]
  );

  const memoizedValue = useCallback(<T>(computeValue: () => T, deps: any[], ttl?: number): T => {
...
export function useOperationTracking(operationName: string, componentName?: string) {
  ...
  const measureOperation = useCallback(
    <T>(operation: () => T, metadata?: Record<string, any>): T => {
  ...
  const measureAsyncOperation = useCallback(
    <T>(operation: () => Promise<T>, metadata?: Record<string, any>): Promise<T> => {
      startOperation(metadata);
      return operation()
        .then((result) => {
          endOperation({ ...metadata, success: true });
