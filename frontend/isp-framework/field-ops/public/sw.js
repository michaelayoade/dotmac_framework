...
const NO_CACHE_PATTERNS = [/\/api\/auth/, /\/api\/sync/, /\/api\/upload/];
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    event.waitUntil(self.registration.showNotification(data.title || 'Technician App', options));
  } catch (error) {
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(handleNotificationClick(clickAction, notificationData));
});
...
async function handleNavigationRequest(request) {
  try {
    ...
    return (
      offlineResponse ||
      new Response('<h1>App is offline</h1><p>Please check your internet connection.</p>', {
...
async function getStoredUpdates(storeName) {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      ['workOrderUpdates', 'photoUploads', 'locationUpdates', 'inventoryUpdates'].forEach(
        (name) => {
