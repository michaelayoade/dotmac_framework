...
export function DomainManagement({ tenantId }: { tenantId: string }) {
  ...
  const [verificationResult, setVerificationResult] = useState<DomainVerificationResponse | null>(
    null
  );
    ...
    return (
      <div className='space-y-4'>
        <Alert>
          ...
          <AlertDescription>
            Add these DNS records to your domain `{setupResult.domain}` in your domain registrar's
            control panel:
          </AlertDescription>
        </Alert>
        ...
        {setupResult.required_dns_records.map((record, index) => (
          <Card key={index} className='p-4'>
            <div className='flex items-center justify-between'>
              ...
              <Button variant='ghost' size='sm' onClick={() => copyToClipboard(record.content)}>
        ...
        <Button onClick={handleDomainVerification} disabled={isLoading} className='w-full'>
          {isLoading ? (
            <>
  ...
  return (
    <div className='max-w-4xl mx-auto p-6'>
      <div className='mb-6'>
        ...
        <p className='text-gray-600 mt-2'>Configure custom domains for your ISP platform</p>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className='w-full'>
            ...
            <CardContent className='space-y-4'>
              <div className='flex gap-4'>
                <Input
                  ...
                  className='flex-1'
                />
                <Button onClick={handleDomainSetup} disabled={isLoading || !domain.trim()}>
                  {isLoading ? (
                    <>
                      <Loader2 className='mr-2 h-4 w-4 animate-spin' />
                      Setting up...
              ...
              <div className='text-sm text-gray-600'>
                ...
                <ul className='list-disc list-inside mt-1 space-y-1'>
                  <li>
                    <code>portal.acmeisp.com</code> - Customer portal
                  </li>
                  <li>
                    <code>admin.acmeisp.com</code> - Admin interface
                  </li>
                  <li>
                    <code>billing.acmeisp.com</code> - Billing portal
                  </li>
                </ul>
              </div>
            </CardContent>
        ...
        <TabsContent value='dns-records' className='space-y-6'>
          <Card>
            ...
            <CardContent>{renderDNSRecords()}</CardContent>
          </Card>
        </TabsContent>
            ...
            {verificationResult.verification_details && (
              <div className='space-y-2 text-sm'>
                <div>
                  ...
                  {verificationResult.verification_details.verification_method}
                </div>
                {verificationResult.verification_details.records_found.length > 0 && (
                  <div>
                    <strong>Records Found:</strong>
                    <ul className='list-disc list-inside ml-4'>
                      {verificationResult.verification_details.records_found.map(
                        (record, index) => (
                ...
                {verificationResult.verification_details.records_missing && (
                  <div>
                    ...
                    <ul className='list-disc list-inside ml-4 text-red-600'>
                      {verificationResult.verification_details.records_missing.map(
                        (record, index) => (
                          <li key={index} className='font-mono'>
                            {record}
                          </li>
