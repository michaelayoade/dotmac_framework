    ...
    <div className={cn('space-y-2', className)}>
      ...
      <div className='relative'>
        {React.cloneElement(children as React.ReactElement, {
          id,
          'aria-describedby': cn(description && `${id}-description`, hasError && `${id}-error`),
...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    { className, variant = 'default', size = 'base', leftIcon, rightIcon, error, type, ...props },
    ref
  ) => {
        ...
        return (
          <div key={option.value} className='flex items-start space-x-3'>
            ...
            <div className='text-sm'>
              ...
              {option.description && <p className='text-gray-500 mt-1'>{option.description}</p>}
            </div>
          </div>
        );
      })}
...
export function ValidationMessage({ type, message, className }: ValidationMessageProps) {
  ...
  return (
    <div
      className={cn('flex items-center space-x-2 p-3 rounded-lg border', styles[type], className)}
    >
