...
export function useTenantLimits() {
  ...
  const isWithinLimits = useCallback(
    (resource: keyof TenantLimits['currentUsage']): boolean => {
  ...
  const getRemainingQuota = useCallback(
    (resource: keyof TenantLimits['currentUsage']): number => {
...
export function useTenantSettings() {
  ...
  const updateSettings = useCallback(
    async (
      settings: Partial<import('../lib/tenant-manager').TenantSettings>
    ): Promise<TenantInfo> => {
      if (!tenant) {
...
export function useTenantRouting() {
  ...
  const buildTenantUrl = useCallback(
    (path: string): string => {
