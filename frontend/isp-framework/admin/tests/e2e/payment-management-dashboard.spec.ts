...
class PaymentManagementJourney {
  ...
  async testFailedPaymentsManagement() {
    ...
    await this.paymentHelper.createSuccessfulPayment({
      amount: '50.00',
  ...
  async testChargebackDashboard() {
    ...
    await this.page.fill(
      ...
      'Transaction was authorized with valid CVV and 3DS authentication'
    );
    await this.page.fill(
      '[data-testid="dispute-message"]',
      'Customer signature on file, delivery confirmed'
    );
  ...
  async testRefundManagement() {
    ...
    await this.page.fill(
      ...
      'Customer requested partial refund for service issue'
    );
  ...
  async testDunningManagement() {
    ...
    await this.page.fill(
      ...
      'Customer unresponsive to payment reminders'
    );
