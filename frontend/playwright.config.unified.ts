...
const PORTAL_CONFIGS = {
  admin: { port: 3000, path: 'apps/admin' },
  customer: { port: 3001, path: 'apps/customer' },
  technician: { port: 3002, path: 'apps/technician' },
  reseller: { port: 3003, path: 'apps/reseller' },
  'management-admin': { port: 3004, path: 'apps/management-admin' },
  'management-reseller': { port: 3004, path: 'apps/management-reseller' },
  'tenant-portal': { port: 3006, path: 'apps/tenant-portal' }
} as const;
...
export function createUnifiedConfig(options: UnifiedPlaywrightConfig = {}) {
  ...
  const baseURL = portalConfig 
    ? `http://localhost:${portalConfig.port}`
    : 'http://localhost:3000';

  return defineConfig({
    testDir: portal ? `./apps/${portal}/tests/e2e` : './tests/e2e',
    ...
    use: {
      ...
      storageState: portal 
        ? `tests/auth/${portal}-auth.json`
        : 'tests/auth/default-auth.json',
    ...
    webServer: portal ? {
      ...
      {
        command: 'pnpm --filter ./apps/admin dev',
        port: 3000,
        reuseExistingServer: !isCI,
        timeout: 120000
      },
      {
        command: 'pnpm --filter ./apps/customer dev', 
        port: 3001,
        reuseExistingServer: !isCI,
        timeout: 120000
      }
...
export const technicianConfig = () => createUnifiedConfig({ portal: 'technician' });
