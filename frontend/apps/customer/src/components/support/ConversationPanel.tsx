...
interface Message { id: string; from: 'customer' | 'agent'; body: string; createdAt: string }
    ...
    <Card className="p-4 mt-6" data-testid="conversation-panel">
      ...
      {loading ? (
        ...
        <div className="space-y-2 max-h-64 overflow-auto">
          {messages.map((m) => (
            <div key={m.id} className={"text-sm " + (m.from === 'agent' ? 'text-blue-700' : 'text-gray-800')}>
      ...
      <div className="flex items-center gap-2 mt-3">
        ...
        <Button onClick={send} data-testid="send-message">Send</Button>
      </div>
    </Card>
  );
}
