...
class LoadTester {
  ...
  processResults(results, testName) {
    ...
    // Collect errors
    const errors = results.filter(r => !r.success).map(r => r.error || `Status ${r.statusCode}`);
    ...
    this.log(`  ✅ Successful: ${successful}/${results.length} (${successRate.toFixed(2)}%)`, 'success');
    this.log(`  ❌ Failed: ${failed}/${results.length} (${errorRate.toFixed(2)}%)`, failed > 0 ? 'error' : 'info');
    this.log(`  📈 Avg Response: ${avgResponseTime.toFixed(2)}ms`, 'info');
    this.log(`  📊 Min/Max: ${minResponseTime}ms / ${maxResponseTime}ms`, 'info');
    
    if (failed > results.length * 0.05) { // More than 5% failure rate
      this.log(`  🚨 HIGH FAILURE RATE: ${errorRate.toFixed(2)}% - Investigation required`, 'critical');
    }
    
    if (avgResponseTime > 2000) { // Slow response time
      this.log(`  ⚠️  SLOW RESPONSE: ${avgResponseTime.toFixed(2)}ms avg - Optimization needed`, 'warning');
    }
  }
  ...
  generateReport() {
    ...
    const overallAvgResponse = this.results.responseTimes.reduce((a, b) => a + b, 0) / this.results.responseTimes.length;
    ...
    this.log(`⚡ Throughput: ${(this.results.total / (totalTime / 1000)).toFixed(2)} req/sec`, 'info');
    this.log('', 'info');
...
`);
  ...
  try {
    ...
    const avgResponse = loadTester.results.responseTimes.reduce((a, b) => a + b, 0) / loadTester.results.responseTimes.length;
