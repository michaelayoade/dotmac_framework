    ...
    if (fs.existsSync(chunksDir)) {
      ...
      chunks.slice(0, 10).forEach((chunk, index) => {
        const color = chunk.size > 100000 ? colors.red : 
                     chunk.size > 50000 ? colors.yellow : colors.green;
      ...
      if (totalSize > config.bundleSizeLimit) {
        log(`⚠️  Bundle size exceeds limit (${formatBytes(config.bundleSizeLimit)})`, colors.yellow);
      } else {
...
function analyzeDependencies() {
  ...
  try {
    ...
    const heavyPackages = [
      'lodash', 'moment', 'rxjs', '@angular/core', 'bootstrap', 
      'jquery', 'three', 'fabric', 'codemirror'
    ];
    ...
    const duplicates = allDeps.filter((dep, index) => 
      allDeps.indexOf(dep) !== index
    );
...
function generateRecommendations() {
  ...
  recommendations.forEach((rec, index) => {
    const color = rec.impact === 'High' ? colors.red :
                 rec.impact === 'Medium' ? colors.yellow : colors.green;
