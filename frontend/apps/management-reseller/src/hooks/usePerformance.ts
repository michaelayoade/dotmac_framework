...
export function useBundlePerformance(): UseBundlePerformanceReturn {
  ...
  const getMetricsSummary = useCallback(() => {
    ...
    const metrics = [
      ...
      loadingMetrics.firstContentfulPaint && `FCP: ${loadingMetrics.firstContentfulPaint.toFixed(0)}ms`,
      loadingMetrics.largestContentfulPaint && `LCP: ${loadingMetrics.largestContentfulPaint.toFixed(0)}ms`,
  ...
  useEffect(() => {
    const checkBudgets = () => {
      ...
      if (budgets.renderTime) {
        ...
        const recentRenderMeasures = measures.filter(m => 
          m.name.includes('render') && 
          m.startTime > Date.now() - 5000 // Last 5 seconds
        );
