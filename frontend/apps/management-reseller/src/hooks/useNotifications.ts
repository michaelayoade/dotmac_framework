...
export function useNotificationActions() {
  ...
  const showApiSuccess = useCallback((action: string, resource?: string) => {
    showSuccess(
      'Success!',
      `${resource || 'Item'} ${action} successfully.`
    );
  }, [showSuccess]);

  const showApiError = useCallback((action: string, error?: string, resource?: string) => {
    showError(
      'Operation Failed',
