...
async function validateJWTTokenServerSide(token: string): Promise<TokenValidation> {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3004'}/api/auth/validate-token`, {
      method: 'POST',
      headers: {
...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/(authenticated)') || 
    ...
    if (tokenValidation.expiresIn < 300) { // 5 minutes in seconds
