...
interface VirtualizedListProps<T> {
  ...
  renderItem: ({ item, index, style }: { item: T; index: number; style: React.CSSProperties }) => React.ReactNode;
...
export function VirtualizedList<T>({
  ...
  const ListComponent = (typeof itemHeight === 'function' ? VariableSizeList : FixedSizeList) as any;
...
export const VirtualizedPartnerList = memo(({ 
  ...
  const renderPartner = useCallback(({ item: partner, style }: any) => (
...
export const VirtualizedCommissionList = memo(({ 
  ...
  const renderCommission = useCallback(({ item: commission, style }: any) => (
    <div className="border-b border-gray-200 p-4 hover:bg-gray-50">
      <div className="flex items-center justify-between">
        <div>
