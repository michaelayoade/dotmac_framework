...
export const SecureInput = forwardRef<HTMLInputElement, SecureInputProps>(
  ({
    ...
    const validateInput = useCallback((inputValue: string): { isValid: boolean; error?: string; sanitizedValue?: string } => {
      if (!inputValue) {
        return { isValid: true };
      }
    ...
    const handleBlur = useCallback((event: React.FocusEvent<HTMLInputElement>) => {
      ...
      if (sanitizeOnBlur && currentValue) {
        ...
        if (validation.isValid && validation.sanitizedValue && validation.sanitizedValue !== currentValue) {
...
export interface SecureSelectProps extends Omit<React.SelectHTMLAttributes<HTMLSelectElement>, 'onChange'> {
  label?: string;
  error?: string;
