...
async function handleAPIRequest(request) {
  ...
  try {
    ...
    if (url.pathname === '/api/auth/me') {
      return new Response(JSON.stringify({ error: 'offline', message: 'User data unavailable offline' }), {
        status: 503,
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification('Management Admin Portal', options)
  );
});
