...
import { testTenants, testMetrics, testSystemAlerts, testUserActivities, apiResponses } from '../fixtures/test-data';
...
export async function mockAPIError(page: Page, endpoint: string, status = 500, message = 'Internal Server Error'): Promise<void> {
...
export async function mockPagination(page: Page, endpoint: string, data: any[], pageSize = 10): Promise<void> {
  await page.route(endpoint, (route) => {
