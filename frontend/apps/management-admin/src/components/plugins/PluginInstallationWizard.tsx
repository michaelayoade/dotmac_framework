...
export function PluginInstallationWizard({
  ...
  const [installationStatus, setInstallationStatus] = useState<PluginInstallationResponse | null>(null);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          <div className="flex items-center">
            <ShieldCheckIcon className={`h-5 w-5 mr-2 ${plugin.security.signed ? 'text-green-500' : 'text-gray-400'}`} />
            <span className="text-sm">
              {plugin.security.signed ? 'Digitally signed' : 'Not digitally signed'}
            </span>
          </div>
          <div className="flex items-center">
            <CheckIcon className={`h-5 w-5 mr-2 ${plugin.security.verified ? 'text-blue-500' : 'text-gray-400'}`} />
            <span className="text-sm">
              {plugin.security.verified ? 'Verified publisher' : 'Unverified publisher'}
            </span>
          </div>
          <div className="flex items-center">
            <ServerIcon className={`h-5 w-5 mr-2 ${plugin.security.sandboxed ? 'text-green-500' : 'text-yellow-500'}`} />
            <span className="text-sm">
  ...
  const renderPermissionsStep = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {Object.entries(plugin.permissions).map(([category, perms]) => (
          perms.length > 0 && (
            ...
            <>
              ...
              <div className="text-center">
                ...
                {installationStatus.estimated_completion && (
                  <p className="text-xs text-gray-500 mt-2">
                    Estimated completion: {new Date(installationStatus.estimated_completion).toLocaleTimeString()}
                  </p>
                )}
              </div>
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 4: // Installation
        return installationStatus?.status === 'completed' || installationStatus?.status === 'failed';
      default:
        return true;
    }
  };
