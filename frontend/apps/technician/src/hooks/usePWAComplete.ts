...
export function usePWAComplete() {
  ...
  useEffect(() => {
    const updateSyncState = () => {
      ...
      setSyncState({
        ...
        lastSync: status.tasks.length > 0 
  ...
  const share = useCallback(async (data: { title?: string; text?: string; url?: string }): Promise<boolean> => {
  ...
  // Convenience Methods
  const syncWorkOrderUpdate = useCallback(async (workOrderId: string, data: any): Promise<string> => {
  ...
  const syncPhotoUpload = useCallback(async (workOrderId: string, photoData: FormData): Promise<string> => {
