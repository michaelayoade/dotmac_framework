...
export function WorkOrdersVirtualList({ 
          ...
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-2 mb-1">
                ...
                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getPriorityColor(workOrder.priority)}`}>
                  {workOrder.priority}
                </span>
          ...
          {workOrder.checklist.length > 0 && (
            <div className="mt-3 pt-3 border-t border-gray-100">
              <div className="flex items-center justify-between text-xs text-gray-500">
                <span>
                  Checklist: {workOrder.checklist.filter(item => item.completed).length} / {workOrder.checklist.length}
                </span>
  ...
  const virtualListProps = useMemo(() => ({
    ...
    onItemsRendered: (startIndex: number, endIndex: number) => {
      performanceMonitor.recordCustomMetric('work_orders_visible_range', endIndex - startIndex + 1);
    },
    emptyComponent: (
    ...
    <div className="w-full">
        ...
        <div className="max-h-96 overflow-y-auto">
          {filteredOrders.map((workOrder, index) => (
            <div key={workOrder.id}>
              {renderWorkOrderItem(workOrder, index, {})}
            </div>
          ))}
        </div>
      )}
      ...
      {featureFlags.isDebugLoggingEnabled() && (
        <div className="fixed bottom-4 right-4 bg-black bg-opacity-75 text-white p-2 rounded text-xs">
          Items: {filteredOrders.length} | Virtual: {enableVirtualization && filteredOrders.length > 20 ? 'ON' : 'OFF'}
        </div>
      )}
    </div>
  );
