...
export function InventoryVirtualGrid({
  ...
  const itemWidth = Math.floor((containerDimensions.width - 32 - (columnsCount - 1) * 16) / columnsCount);
  ...
  const renderInventoryItem = React.useCallback((
    ...
    const getStockStatus = (item: InventoryItem) => {
      ...
      if (item.quantity <= item.minStock) return { status: 'low', color: 'text-orange-600', bg: 'bg-orange-50' };
          ...
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center space-x-2">
              ...
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${stockStatus.bg} ${stockStatus.color}`}>
                {stockStatus.status === 'out' ? 'Out' : stockStatus.status === 'low' ? 'Low' : 'Available'}
              </span>
            </div>
            {item.barcode && (
    ...
    <div className="w-full">
      ...
      {featureFlags.isDebugLoggingEnabled() && (
        <div className="fixed bottom-4 left-4 bg-black bg-opacity-75 text-white p-2 rounded text-xs">
          ...
          Virtual: {enableVirtualization && filteredInventory.length > 12 ? 'ON' : 'OFF'} | 
          Cols: {columnsCount}
        </div>
      )}
    </div>
