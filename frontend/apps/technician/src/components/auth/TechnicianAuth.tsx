...
import SecureTokenManager, { AuthState, TechnicianProfile } from '../../lib/auth/secure-token-manager';
        ...
        >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className={`flex items-center ${deviceChecks.online ? 'text-green-300' : 'text-red-300'}`}>
                {deviceChecks.online ? <CheckCircle className="w-3 h-3 mr-1" /> : <AlertCircle className="w-3 h-3 mr-1" />}
                {deviceChecks.online ? 'Connected' : 'Offline'}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-primary-100">Location</span>
              <span className={`flex items-center ${deviceChecks.geolocation ? 'text-green-300' : 'text-yellow-300'}`}>
                {deviceChecks.geolocation ? <CheckCircle className="w-3 h-3 mr-1" /> : <AlertCircle className="w-3 h-3 mr-1" />}
                {deviceChecks.geolocation ? 'Available' : 'Permissions needed'}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-primary-100">Camera</span>
              <span className={`flex items-center ${deviceChecks.camera ? 'text-green-300' : 'text-yellow-300'}`}>
                {deviceChecks.camera ? <CheckCircle className="w-3 h-3 mr-1" /> : <AlertCircle className="w-3 h-3 mr-1" />}
