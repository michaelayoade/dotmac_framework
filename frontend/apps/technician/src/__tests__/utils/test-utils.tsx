...
export class MockIndexedDB {
  ...
  async transaction(storeNames: string[], mode: 'readonly' | 'readwrite', callback: () => Promise<any>): Promise<any> {
    return callback();
  }
 ...
 */
export function createMockTechnicianProfile(overrides?: Partial<TechnicianProfile>): TechnicianProfile {
...
export class MockApiClient {
  ...
  constructor() {
    ...
    this.customers = [
      createMockCustomer(),
      createMockCustomer({ status: 'suspended' }),
    ];
...
export class PerformanceTester {
  ...
  async measureRender(component: ReactElement): Promise<{ duration: number; element: HTMLElement }> {
    this.start();
...
export async function expectAccessible(element: HTMLElement): Promise<void> {
  ...
  interactiveElements.forEach(el => {
    ...
    if (!hasAccessibleName) {
      throw new Error(`Interactive element missing accessible name: ${htmlEl.outerHTML.slice(0, 100)}...`);
    }
