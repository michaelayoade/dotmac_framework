...
let useAuthStore: any, useCSRFProtection: any, showErrorNotification: any, showSuccessNotification: any;
...
class BillingApiClient {
  ...
  async getInvoices(filters?: BillingFilters, page = 1, limit = 10): Promise<PaginatedResponse<Invoice>> {
    ...
    const response = await this.makeRequest<PaginatedResponse<Invoice>>(`/billing/invoices?${params}`);
  ...
  async getPayments(page = 1, limit = 10): Promise<PaginatedResponse<Payment>> {
    ...
    const response = await this.makeRequest<PaginatedResponse<Payment>>(`/billing/payments?${params}`);
    return response.data;
  }
