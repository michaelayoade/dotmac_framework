...
export function useGridNavigation<T extends HTMLElement = HTMLElement>(
  ...
  const getCurrentCell = useCallback((): { row: number; col: number } | null => {
    const activeElement = document.activeElement as HTMLElement;
    const row = activeElement?.getAttribute('data-row') || activeElement?.getAttribute('data-grid-row');
    const col = activeElement?.getAttribute('data-col') || activeElement?.getAttribute('data-grid-col');
