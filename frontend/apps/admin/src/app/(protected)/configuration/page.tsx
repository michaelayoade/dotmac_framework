...
export default function SystemConfigurationPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'configured':
        return <Badge variant="secondary" className="text-green-700 bg-green-100">Configured</Badge>;
      case 'needs_attention':
        return <Badge variant="secondary" className="text-yellow-700 bg-yellow-100">Needs Attention</Badge>;
      case 'not_configured':
        return <Badge variant="secondary" className="text-red-700 bg-red-100">Not Configured</Badge>;
      default:
        return null;
    }
  };
  ...
  const configuredCount = categories.reduce((count, cat) => 
    count + cat.items.filter(item => item.status === 'configured').length, 0
  );
                ...
                <div>
                  <div className="text-2xl font-bold text-yellow-600">
                    {categories.reduce((count, cat) => 
                      count + cat.items.filter(item => item.status === 'needs_attention').length, 0
                    )}
                  </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-bold text-red-600">
                    {categories.reduce((count, cat) => 
                      count + cat.items.filter(item => item.status === 'not_configured').length, 0
                    )}
                  </div>
