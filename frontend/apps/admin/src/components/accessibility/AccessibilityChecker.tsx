...
import { useAccessibility, type AccessibilityReport, type AccessibilityViolation } from './AccessibilityProvider';
                      ...
                      {report.violations.map((violation) => (
                        <button
                          ...
                          className={`w-full text-left p-4 hover:bg-gray-50 transition-colors ${
                            selectedViolation?.id === violation.id ? 'bg-blue-50 border-l-4 border-blue-500' : ''
                          }`}
                        >
                          <div className="flex items-start gap-3">
                            <div className={`mt-0.5 ${getImpactColor(violation.impact).split(' ')[0]}`}>
                              {getImpactIcon(violation.impact)}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getImpactColor(violation.impact)}`}>
                                  {violation.impact}
                                </span>
                              </div>
                      ...
                      <div className="p-6">
                        <div className="mb-6">
                          <div className="flex items-center gap-2 mb-2">
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getImpactColor(selectedViolation.impact)}`}>
                            ...
                            <div>
                              ...
                              <div className="bg-gray-100 rounded-lg p-3">
                                <code className="text-sm text-gray-800">
                                  ...
                                  {selectedViolation.element.id && ` id="${selectedViolation.element.id}"`}
                                  {selectedViolation.element.className && ` class="${selectedViolation.element.className}"`}
                                </code>
                              </div>
                              <button
            ...
            <div className="border-t border-gray-200 p-4 bg-gray-50">
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-600">
                  This is a basic accessibility check. For comprehensive testing, use tools like axe-core or WAVE.
                </div>
