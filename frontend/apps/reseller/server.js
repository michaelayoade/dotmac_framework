...
app.prepare().then(() => {
  ...
  server.keepAliveTimeout = 30020; // 30 seconds
  ...
  const gracefulShutdown = (signal) => {
    ...
    const gracePeriod = parseInt(process.env.SHUTDOWN_GRACE_PERIOD || '30020', 10);
    
    setTimeout(() => {
      console.log(`> Grace period (${gracePeriod}ms) expired, forcefully closing ${connections.size} connections`);
