...
function SalesPipelineSection({ data, isLoading }: { data?: any; isLoading: boolean }) {
  ...
  const handleCardMove = useCallback((cardId: string, sourceColumn: string, destColumn: string, destIndex: number) => {
...
const managementConfig: ManagementPageConfig = {
  ...
  tableColumns: [
    ...
    { key: 'value', label: 'Value', sortable: true, formatter: (val) => `$${val?.toLocaleString()}` },
    { key: 'stage', label: 'Stage', component: ({ value }) => <Badge variant="outline">{value}</Badge> },
    { key: 'assignee', label: 'Assignee' },
    { key: 'dueDate', label: 'Due Date', sortable: true },
    { key: 'priority', label: 'Priority', component: ({ value }) => (
