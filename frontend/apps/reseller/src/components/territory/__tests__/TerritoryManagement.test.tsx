...
const mockUseTerritoryData = useTerritoryData as jest.MockedFunction<typeof useTerritoryData>;
...
jest.mock('../components/TerritoryHeader', () => ({
  TerritoryHeader: ({ onViewModeChange, onToggleFilters }: any) => (
    <div data-testid='territory-header'>
      ...
      <button onClick={() => onViewModeChange('analytics')} data-testid='view-analytics'>
        Analytics View
      </button>
...
jest.mock('../components/TerritoryContent', () => ({
  TerritoryContent: ({ viewMode, onTerritorySelect }: any) => (
    <div data-testid='territory-content'>
      ...
      <button
        onClick={() => onTerritorySelect({ id: 'territory_1', name: 'Test Territory' })}
...
describe('TerritoryManagement', () => {
  const mockTerritories: Territory[] = [
    ...
    },
    {
      ...
      opportunities: { newDevelopments: 8, businessParks: 2, competitorWeakness: 15 },
    },
  ];
