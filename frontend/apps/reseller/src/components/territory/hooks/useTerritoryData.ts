...
const fetchTerritories = mockApiWrapper(
	fetchTerritoryDataFromAPI,
	createMockTerritories,
	{
		delay: 1000,
...
export function useTerritoryData(): UseTerritoryDataReturn {
	...
	const filteredTerritories = useMemo(() => {
		...
		if (filters.region) {
			filtered = filtered.filter(
				(territory) => territory.region === filters.region,
			);
		}
		...
		if (filters.competitionLevel) {
			filtered = filtered.filter(
				(territory) => territory.competition === filters.competitionLevel,
			);
		}
