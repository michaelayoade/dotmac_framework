...
export function TerritoryAnalytics({
	...
	const aggregateMetrics = useMemo((): AnalyticsMetric[] => {
		...
		const totalCustomers = territories.reduce(
			(sum, t) => sum + t.totalCustomers,
			0,
		);
		const totalProspects = territories.reduce(
			(sum, t) => sum + t.activeProspects,
			0,
		);
		const avgGrowthRate =
			territories.reduce((sum, t) => sum + t.growthRate, 0) /
			territories.length;
		const avgPenetration =
			territories.reduce((sum, t) => sum + t.marketPenetration, 0) /
			territories.length;
		const avgServiceability =
			territories.reduce((sum, t) => sum + t.serviceability, 0) /
			territories.length;
	...
	const performanceSegments = useMemo((): PerformanceSegment[] => {
		const highPerformers = territories.filter(
			(t) => t.growthRate > 15 && t.marketPenetration > 35,
		);
	...
	const competitionAnalysis = useMemo(() => {
		...
		const mediumCompetition = territories.filter(
			(t) => t.competition === "medium",
		);
	...
	const serviceDistribution = useMemo(() => {
		...
		const fiberAvg =
			territories.reduce((sum, t) => sum + t.services.fiber, 0) /
			totalTerritories;
		const cableAvg =
			territories.reduce((sum, t) => sum + t.services.cable, 0) /
			totalTerritories;
		const dslAvg =
			territories.reduce((sum, t) => sum + t.services.dsl, 0) /
			totalTerritories;
			...
			>
				<div className="flex items-center justify-between">
					<h3 className="text-xl font-bold text-gray-900">
						{selectedTerritory.name} Analytics
					</h3>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
					...
					<div className="bg-white p-6 rounded-lg border border-gray-200">
						...
						<div className="space-y-3">
							{Object.entries(selectedTerritory.demographics).map(
								([type, percentage]) => (
					...
					<div className="bg-white p-6 rounded-lg border border-gray-200">
						...
						<div className="space-y-3">
							{Object.entries(selectedTerritory.services).map(
								([service, percentage]) => (
									<div
										key={service}
