...
export function TerritoryDetailsPanel({ territory, onClose }: TerritoryDetailsPanelProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className='grid grid-cols-3 gap-4 text-sm'>
        <div>
          ...
          <div className='font-semibold'>{territory.population.toLocaleString()}</div>
        </div>
        <div>
          <span className='text-gray-600'>Avg Income:</span>
          <div className='font-semibold'>${territory.averageIncome.toLocaleString()}</div>
        </div>
        <div>
