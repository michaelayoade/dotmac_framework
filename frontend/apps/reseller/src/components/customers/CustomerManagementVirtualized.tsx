...
export interface Customer {
  ...
  state?: string;
  zipCode?: string;
  status: 'active' | 'pending' | 'suspended' | 'cancelled' | 'prospect' | 'qualified' | 'negotiating';
...
export function CustomerManagementVirtualized({
  ...
  const {
    ...
    clearSelection,
    selectAll,
  } = useVirtualizedTable(customersData?.customers || []);
  ...
  const renderStatusBadge = useCallback((status: Customer['status']) => {
    const statusConfig = {
      ...
      negotiating: { color: 'bg-orange-100 text-orange-800', icon: TrendingUp, label: 'Negotiating' },
    };
