...
export function TenantSwitcher() {
  const { currentTenant, accessibleTenants, isMultiTenant, switchTenant, isLoadingTenantSwitch } =
    useTenant();
    ...
    if (tenantId !== currentTenant?.id) {
      try {
        ...
        if (checkRateLimit('tenantSwitch')) {
          ...
          logSecurityEvent(SecurityEventType.RATE_LIMIT_EXCEEDED, SecuritySeverity.MEDIUM, {
        ...
        logSecurityEvent(SecurityEventType.TENANT_SWITCH, SecuritySeverity.MEDIUM, {
          fromTenantId: currentTenant?.id,
          toTenantId: tenantId,
        ...
        logSecurityEvent(SecurityEventType.TENANT_SWITCH, SecuritySeverity.LOW, {
          fromTenantId: currentTenant?.id,
          toTenantId: tenantId,
          success: true,
          timestamp: new Date().toISOString(),
        ...
        logSecurityEvent(SecurityEventType.TENANT_SWITCH, SecuritySeverity.HIGH, {
          fromTenantId: currentTenant?.id,
          toTenantId: tenantId,
          success: false,
