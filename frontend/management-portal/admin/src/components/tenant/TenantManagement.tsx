  ...
  const stats = showStats
    ? [
        ...
        },
        {
          name: 'Active Tenants',
          value:
            tenantsData?.tenants?.filter((t: Tenant) => t.status === TenantStatus.ACTIVE).length ||
            0,
          change: '+5%',
          changeType: 'positive',
        ...
        },
        {
          ...
          icon:
            healthData?.overall_status === 'healthy' ? CheckCircleIcon : ExclamationTriangleIcon,
          href: routes.monitoring,
        },
        {
          name: 'Pending Setup',
          value:
            tenantsData?.tenants?.filter((t: Tenant) => t.status === TenantStatus.PENDING).length ||
            0,
          change: 'Require attention',
          changeType: tenantsData?.tenants?.some((t: Tenant) => t.status === TenantStatus.PENDING)
            ? 'negative'
            : 'neutral',
                      ...
                      <div className='ml-4 flex-1'>
                        <dl>
                          ...
                          <dd className='text-lg font-medium text-gray-900'>
                            {tenantsLoading ? <LoadingSpinner size='small' /> : item.value}
                          </dd>
                        </dl>
                      </div>
                    </div>
              ...
              <tbody className='table-body'>
                {tenantsData?.tenants?.map((tenant: Tenant) => (
                  <tr key={tenant.id} className='hover:bg-gray-50'>
                    ...
                    {!compact && (
                      <td className='table-cell'>
                        <span className='text-sm text-gray-900'>{tenant.plan || 'No Plan'}</span>
                      </td>
                    )}
                    {!compact && (
                      <td className='table-cell'>
