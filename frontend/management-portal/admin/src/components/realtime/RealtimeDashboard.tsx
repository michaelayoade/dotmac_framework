...
export function RealtimeDashboard() {
  ...
  const { data: metrics } = useRealtimeData<RealtimeMetric[]>(
    WebSocketEventType.METRICS_UPDATED,
    []
  );
            ...
            {stats && (
              <div className='text-xs text-gray-500'>
                ...
                {stats.queuedMessages > 0 && (
                  <span className='ml-2 text-yellow-600'>Queued: {stats.queuedMessages}</span>
                )}
              </div>
            )}
