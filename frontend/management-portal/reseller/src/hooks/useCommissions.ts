...
export function useApproveCommission() {
  ...
  const { updateCommissionOptimistically, revertOptimisticUpdate } =
    useOptimisticCommissionUpdate();
  ...
  return useMutation({
    ...
    onSuccess: (data, { id }) => {
      ...
      queryClient.setQueryData(queryKeys.commissionsDetail(id), data);
      ...
      return {
        ...
        averageAmount:
          commissions.length > 0
            ...
            : 0,
        byPaymentMethod: commissions.reduce(
          (acc, commission) => {
...
export function useOptimisticCommissionUpdate() {
  ...
  const updateCommissionOptimistically = (id: string, updates: any) => {
    queryClient.setQueryData(queryKeys.commissionsDetail(id), (oldData: any) => {
