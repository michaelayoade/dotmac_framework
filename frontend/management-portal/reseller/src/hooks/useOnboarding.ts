...
export function useUpdateOnboardingStep() {
  ...
  const { updateOnboardingOptimistically, revertOptimisticUpdate } =
    useOptimisticOnboardingUpdate();
  ...
  return useMutation({
    ...
    onMutate: async ({
      ...
      updateOnboardingOptimistically(id, {
        steps: {
          [stepId]: { ...data, status: 'COMPLETED', completed_at: new Date().toISOString() },
        },
      });
    ...
    onSuccess: (data, variables, context) => {
      ...
      queryClient.setQueryData(queryKeys.onboardingDetail(id), data);
...
export function useApproveOnboardingStep() {
  ...
  return useMutation({
    mutationFn: ({ stepId }: { stepId: string }) => API.onboarding.approveStep(stepId),
    ...
    queryClient.setQueryData(
      ...
      (oldData: { data: OnboardingRequest } | undefined) => {
        ...
        if (updates.steps) {
          updatedData.steps = {
            ...oldData.data.steps,
            ...Object.keys(updates.steps).reduce(
              (acc, stepId) => {
                acc[stepId] = {
