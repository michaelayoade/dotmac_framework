...
export class CommissionService {
   ...
   */
  static async approvePayments(
    request: ApprovalRequest
  ): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      // Sanitize input
      const sanitizedRequest = {
        payment_ids: request.payment_ids.map((id) =>
          InputSanitizer.validate_safe_input(id, 'payment_id')
        ),
        approval_notes: request.approval_notes
          ? InputSanitizer.sanitize_html(request.approval_notes)
          : undefined,
  ...
  static async processPayments(
    ...
    try {
      const sanitizedIds = paymentIds.map((id) =>
        InputSanitizer.validate_safe_input(id, 'payment_id')
      );
   ...
   */
  static async exportCommissions(
    ...
    try {
      ...
      const sanitizedOptions = {
        ...
        payment_ids: options.payment_ids.map((id) =>
          InputSanitizer.validate_safe_input(id, 'payment_id')
        ),
   ...
   */
  private static async executeExportPlugin(
    options: any
  ): Promise<{ success: boolean; downloadUrl?: string; error?: string }> {
    try {
      // Leverage the existing plugin system from management platform
      const { api } = await import('@/lib/http');
      const { data: result } = await api.post('/api/plugins/export/commissions', {
  ...
  static async getPaymentDetails(
    paymentId: string
  ): Promise<{ success: boolean; data?: CommissionPayment; error?: string }> {
    try {
