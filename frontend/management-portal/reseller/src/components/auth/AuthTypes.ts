...
export const mapRoleToManagementRole = (
  apiRole: string
): 'MASTER_ADMIN' | 'CHANNEL_MANAGER' | 'OPERATIONS_MANAGER' => {
  switch (apiRole) {
    case 'master_admin':
    case 'platform_admin':
      return 'MASTER_ADMIN';
...
export const mapRoleToPermissions = (apiRole: string): string[] => {
  ...
  switch (apiRole) {
    ...
    case 'operations_manager':
    case 'tenant_admin':
      return [...basePermissions, 'VIEW_ANALYTICS', 'MANAGE_TERRITORIES', 'VIEW_COMMISSIONS'];
    default:
