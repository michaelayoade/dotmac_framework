...
export function DashboardHeader({ user, onExport, onRefresh, isRefreshing }: DashboardHeaderProps) {
  ...
  return (
    <div className='flex items-center justify-between'>
      <div>
        <h2 className='text-2xl font-bold text-gray-900'>Channel Management Dashboard</h2>
        ...
        <>
          ...
          <div className='absolute right-0 mt-2 w-32 bg-white rounded-lg shadow-lg border border-gray-200 z-20'>
            <div className='py-1' role='menu'>
              ...
              <ExportButton format='XLSX' onClick={() => onExport('XLSX')} label='Excel' />
              <ExportButton format='PDF' onClick={() => onExport('PDF')} />
            </div>
          </div>
        </>
      )}
