  ...
  const loadBillingData = async () => {
    ...
    try {
      ...
      const mockData: BillingData = {
        ...
        usage_charges: {
          'API Requests': 150.0,
          'Storage Overage': 75.0,
          'Bandwidth Overage': 25.0,
        },
        overage_charges: {
          'Additional Users': 50.0,
        },
        payment_methods: [
          {
        ...
        recent_invoices: [
          {
            ...
            line_items: [
              { description: 'Standard Plan', amount: 2500.0, quantity: 1 },
              { description: 'API Overages', amount: 150.0, quantity: 1 },
            ],
          ...
          <div className='flex items-center'>
            ...
            <div>
              ...
              <p className='text-sm text-red-700'>
                Your account is {billingData.days_overdue} days overdue. Please update your payment
                method.
              </p>
            </div>
          </div>
        ...
        <div className='space-y-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            <div className='metric-card'>
              ...
              <div className='metric-trend neutral'>Billing cycle: {billingData.billing_cycle}</div>
            </div>
            ...
            <div className='metric-card'>
              <div className='flex items-center justify-between'>
                <div>
                  <div
                    className={`metric-value ${billingData.account_status === 'active' ? 'text-green-600' : 'text-red-600'}`}
                  >
                    {billingData.account_status.charAt(0).toUpperCase() +
                      billingData.account_status.slice(1)}
                  </div>
                  <div className='metric-label'>Account Status</div>
                </div>
                <CheckCircle
                  className={`h-8 w-8 ${billingData.account_status === 'active' ? 'text-green-500' : 'text-red-500'}`}
                />
              </div>
              <div className='metric-trend neutral'>Plan: {billingData.subscription_tier}</div>
            </div>
          </div>
                ...
                <>
                  ...
                  {Object.entries(billingData.overage_charges).map(([item, amount]) => (
                    <div key={item} className='flex items-center justify-between py-2'>
                      ...
                      <div className='text-lg font-semibold text-red-600'>${amount.toFixed(2)}</div>
                    </div>
                  ))}
                </>
              )}
                  ...
                  {billingData.recent_invoices.map((invoice) => (
                    <tr key={invoice.invoice_id}>
                      ...
                      <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>
                        {invoice.period_start.toLocaleDateString()} -{' '}
                        {invoice.period_end.toLocaleDateString()}
                      </td>
      ...
      {activeTab === 'payment-methods' && (
        <div className='space-y-6'>
          <div className='flex justify-between items-center'>
            ...
            <button className='tenant-button-primary'>Add Payment Method</button>
          </div>
            ...
            {billingData.payment_methods.map((method) => (
              <div key={method.payment_method_id} className='tenant-card p-4'>
                <div className='flex items-center justify-between'>
                  ...
                  {method.is_default && <span className='status-active'>Default</span>}
                </div>
