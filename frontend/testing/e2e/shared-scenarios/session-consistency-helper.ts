  ...
  private async validateSessionConsistency(
    ...
    try {
      ...
      return {
        ...
        details: invalidValidations.length === 0 
    ...
    try {
      ...
      if (localStorageSession) {
        return {
          ...
          permissions: localStorageSession.permissions || localStorageSession.user?.permissions || [],
  ...
  async testRoleBasedAccessConsistency(
    ...
    try {
      ...
      for (const portal of portals) {
        ...
        const currentUrl = this.page.url();
        const accessible = !currentUrl.includes('/unauthorized') && 
      ...
      const details = accessResults
        .map(r => `${r.portal}: ${r.accessible === r.expectedAccess ? '✅' : '❌'} ${r.accessible ? 'accessible' : 'blocked'} (expected: ${r.expectedAccess ? 'accessible' : 'blocked'})`)
        .join('; ');
