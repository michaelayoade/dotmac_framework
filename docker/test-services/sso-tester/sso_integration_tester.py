...
class SSOIntegrationTester:
    ...
    async def test_oidc_token_exchange(self):
        ...
        try:
            ...
            async with self.session.post(
                f"{self.sso_provider_url}/oauth2/authorize/decision", data=decision_data, allow_redirects=False
            ) as resp:
                if resp.status in [302, 303]:
                    ...
                    if "code" in query_params:
                        ...
                        async with self.session.post(
                            f"{self.sso_provider_url}/oauth2/token", data=token_data
                        ) as token_resp:
    ...
    async def test_oidc_userinfo(self):
        ...
        try:
            ...
            async with self.session.get(f"{self.sso_provider_url}/oauth2/userinfo", headers=headers) as resp:
                if resp.status == 200:
    ...
    async def test_oidc_token_refresh(self):
        ...
        try:
            ...
            refresh_data = {"grant_type": "refresh_token", "refresh_token": "mock_refresh_token_for_testing"}

            async with self.session.post(f"{self.sso_provider_url}/oauth2/token", data=refresh_data) as resp:
    ...
    async def test_saml_sso_flow(self):
        ...
        try:
            ...
            async with self.session.post(f"{self.sso_provider_url}/saml/sso", data=saml_data) as resp:
                if resp.status == 200:
                    content = await resp.text()
    ...
    async def test_error_scenarios(self):
        ...
        for error_test in error_tests:
            try:
                ...
                async with self.session.get(auth_url) as resp:
                    if resp.status == error_test["expected_status"] or (
                        resp.status == 200 and "error" in await resp.text()
                    ):
                        passed_tests += 1
    ...
    async def test_token_validation(self):
        ...
        try:
            ...
            async with self.session.get(f"{self.sso_provider_url}/oauth2/userinfo", headers=invalid_headers) as resp:
                if resp.status == 401:
                    error = await resp.json()
    ...
    def print_test_summary(self):
        ...
        print(f"\n{'='*60}")
        print("🧪 SSO Integration Test Summary")
        print(f"{'='*60}")
