...
class ObservabilityIntegrationTester:
    def __init__(self, base_url: str = "http://localhost:3001"):
        ...
        self.test_tenants = ["tenant-alpha-001", "tenant-beta-002", "tenant-gamma-003"]

        # Telemetry endpoints to test
        self.telemetry_endpoints = [
            TelemetryEndpoint(
                "logs", "/api/telemetry/logs", ["timestamp", "level", "message", "tenantId", "traceId"], True
            ),
            TelemetryEndpoint(
                "metrics", "/api/telemetry/metrics", ["name", "value", "timestamp", "tenantId", "tags"], True
            ),
            TelemetryEndpoint("traces", "/api/telemetry/traces", ["traceId", "spanId", "operationName", "tags"], True),
            TelemetryEndpoint(
                "events", "/api/telemetry/events", ["eventType", "timestamp", "tenantId", "payload"], True
            ),
        ]
            ...
            telemetry_entries = [
                ...
                },
                {
                    ...
                    "payload": {
                        ...
                        "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ"),
                        "tenantId": tenant_id,
                        "tags": {"correlationId": correlation_id, "tenantId": tenant_id, "test": "correlation"},
                    },
                },
                {
                    "type": "trace",
        ...
        try:
            ...
            for metric_test in metric_tests:
                try:
                    metric_payload = {
                        ...
                        "tags": {**metric_test["tags"], "tenantId": metric_test["tenant_id"], "test": "tenant_tagging"},
                    }
                    ...
                    async with self.session.post(
                        ...
                        if resp.status in [200, 201, 202]:
                            ...
                            logger.info(
                                f"✅ Tenant-tagged metric sent: {metric_test['metric_name']} for {metric_test['tenant_id']}"
                            )
                        else:
            ...
            self.test_results.append(
                ObservabilityTestResult(
                    ...
                    success=success,
                    duration_ms=int((time.time() - start_time) * 1000),
                    details={"successful_metrics": successful_metrics, "total_metrics_tested": len(metric_tests)},
                )
            )
    ...
    async def test_distributed_tracing(self):
        ...
        try:
            ...
            trace_spans = [
                {
                    ...
                    "startTime": time.strftime("%Y-%m-%dT%H:%M:%SZ"),
                    "duration": 1500,
                    "tags": {"tenantId": tenant_id, "span.kind": "server", "component": "web-server"},
                },
                {
                    "traceId": trace_id,
                    ...
                    "startTime": time.strftime("%Y-%m-%dT%H:%M:%SZ"),
                    "duration": 800,
                    "tags": {"tenantId": tenant_id, "span.kind": "client", "db.type": "postgresql"},
                },
                {
                    "traceId": trace_id,
                    ...
                    "startTime": time.strftime("%Y-%m-%dT%H:%M:%SZ"),
                    "duration": 50,
                    "tags": {"tenantId": tenant_id, "span.kind": "client", "cache.type": "redis"},
                },
            ]
            ...
            for span in trace_spans:
                try:
                    async with self.session.post(
                        f"{self.base_url}/api/telemetry/traces", json=span, headers={"Content-Type": "application/json"}
                    ) as resp:
                        if resp.status in [200, 201, 202]:
                            successful_spans += 1
    ...
    async def test_tenant_data_isolation(self):
        ...
        try:
            ...
            isolation_tests = [
                {"tenant_id": tenant_1, "data_type": "sensitive_metric", "value": "tenant_1_secret_value"},
                {"tenant_id": tenant_2, "data_type": "sensitive_metric", "value": "tenant_2_secret_value"},
            ]
            ...
            for test in isolation_tests:
                try:
                    ...
                    log_payload = {
                        ...
                        "metadata": {"sensitive_data": test["value"], "data_type": test["data_type"]},
                    }
            ...
            try:
                ...
                logger.info(
                    f"✅ Tenant isolation test: {successful_isolations}/{len(isolation_tests)} tenants isolated"
                )
    ...
    async def test_observability_performance(self):
        ...
        try:
            ...
            successful_calls = sum(
                1 for r in responses if not isinstance(r, Exception) and hasattr(r, "status") and r.status < 400
            )
            ...
            logger.info(
                f"✅ Performance test: {successful_calls}/10 calls successful, {performance_overhead:.2f}ms overhead"
            )
        ...
        try:
            ...
            for portal in self.portals[:2]:  # Test first 2 portals
                try:
                    ...
                    log_payload = {
                        ...
                        "metadata": {"portal": portal["name"], "test": "cross_portal_consistency"},
                    }
            ...
            self.test_results.append(
                ObservabilityTestResult(
                    ...
                    duration_ms=int((time.time() - start_time) * 1000),
                    tenant_id=tenant_id,
                    details={"consistent_portals": consistent_portals, "total_portals_tested": len(self.portals[:2])},
                )
            )
    ...
    def generate_test_payload(self, endpoint: TelemetryEndpoint) -> dict:
        ...
        base_payload = {"timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ"), "tenantId": self.test_tenants[0]}
        ...
        if endpoint.name == "logs":
            ...
            return {**base_payload, "name": "test.metric", "value": 123.45, "tags": {"test": "endpoint_availability"}}
            ...
            return {**base_payload, "eventType": "test_event", "payload": {"test": "endpoint_availability"}}
