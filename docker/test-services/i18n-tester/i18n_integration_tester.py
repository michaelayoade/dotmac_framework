...
class I18nIntegrationTester:
                ...
                if resp.status == 200:
                    ...
                    if "locales" in locales_data:
                        ...
                        for required in required_codes:
                            found = (
                                any(locale["code"] == required for locale in supported_locales)
                                if isinstance(supported_locales, list)
                                else required in supported_locales
                            )
                            if not found:
                        ...
                        if not missing_locales:
                            logger.info(
                                f"✅ Locale endpoint working, {len(supported_locales) if isinstance(supported_locales, list) else len(supported_locales)} locales supported"
                            )
            ...
            for test in currency_tests:
                try:
                    ...
                    if test["expected_symbol"] in formatted_currency:
                        ...
                        logger.warning(
                            f"⚠️ Currency formatting issue {test['locale']}: missing {test['expected_symbol']}"
                        )
        ...
        try:
            ...
            self.test_results.append(
                I18nTestResult(
                    ...
                    success=success,
                    duration_ms=int((time.time() - start_time) * 1000),
                    details={"successful_formats": successful_formats, "total_tests": len(date_tests)},
                )
            )
    ...
    def print_test_summary(self):
        ...
        print(f"\n{'='*60}")
        print("🌍 I18n Integration Test Summary")
        print(f"{'='*60}")
