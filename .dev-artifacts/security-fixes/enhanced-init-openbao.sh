...
# 🆕 Configure ClickHouse (NEW - CRITICAL FIX)
echo "Configuring ClickHouse dynamic secrets..."
bao write database/config/clickhouse \
    plugin_name=mysql-database-plugin \
    connection_url="{{username}}:{{password}}@tcp(clickhouse:9000)/" \
    allowed_roles="signoz,collector,query" \
    username="${CLICKHOUSE_ADMIN_USER:-default}" \
    password="${CLICKHOUSE_ADMIN_PASSWORD:-}"

# Create ClickHouse roles for observability services
echo "Creating ClickHouse service roles..."
bao write database/roles/signoz-query \
    db_name=clickhouse \
    creation_statements="CREATE USER '{{name}}'@'%' IDENTIFIED BY '{{password}}'; GRANT SELECT, INSERT, UPDATE, DELETE ON signoz_traces.* TO '{{name}}'@'%'; GRANT SELECT, INSERT, UPDATE, DELETE ON signoz_metrics.* TO '{{name}}'@'%'; GRANT SELECT, INSERT, UPDATE, DELETE ON signoz_logs.* TO '{{name}}'@'%';" \
    default_ttl="2h" \
    max_ttl="24h"

bao write database/roles/signoz-collector \
    db_name=clickhouse \
    creation_statements="CREATE USER '{{name}}'@'%' IDENTIFIED BY '{{password}}'; GRANT INSERT ON signoz_traces.* TO '{{name}}'@'%'; GRANT INSERT ON signoz_metrics.* TO '{{name}}'@'%'; GRANT INSERT ON signoz_logs.* TO '{{name}}'@'%';" \
    default_ttl="4h" \
    max_ttl="48h"

# Store ClickHouse static configuration
echo "Storing ClickHouse configuration..."
bao kv put dotmac/clickhouse \
    host="clickhouse" \
    port="9000" \
    http_port="8123" \
    databases="signoz_traces,signoz_metrics,signoz_logs" \
    cluster_name="signoz_cluster" \
...
# 🆕 ClickHouse-specific policies
cat > /tmp/clickhouse-query-policy.hcl <<EOF
# Policy for SigNoz Query Service - ClickHouse access
path "database/creds/signoz-query" {
  capabilities = ["read"]
}
...
# Policy for OTEL Collector - ClickHouse write access
path "database/creds/signoz-collector" {
  capabilities = ["read"]
}
path "dotmac/data/clickhouse" {
  capabilities = ["read"]
...
echo "    ✅ ClickHouse dynamic credentials (NEW)"
