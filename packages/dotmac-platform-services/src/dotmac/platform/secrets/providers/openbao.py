...
class OpenBaoProvider(BaseProvider, HTTPProviderMixin):
    ...
    async def _get_http_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None or self._client.is_closed:
            ...
            headers = self._get_headers(
                token=self.token, additional_headers={"X-Vault-Request": "true"}
            )
    ...
    async def _detect_kv_version(self, mount_path: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                detected_version = "v2" if engine_type == "kv" and version == "2" else "v1"
            ...
            logger.warning(
                f"KV version detection failed for {mount_path}: {e}, using configured version"
            )
            return self.api_version
    ...
    async def get_secret(self, path: str) -> SecretData:
        ...
        try:
            ...
            raise ProviderConnectionError(f"Failed to connect to OpenBao: {e}") from e
        except httpx.TimeoutException as e:
            raise ProviderConnectionError(f"Request timeout for secret {path}: {e}") from e
        except json.JSONDecodeError as e:
            raise SecretsProviderError(f"Invalid JSON response for secret {path}: {e}") from e
        except Exception as e:
            raise SecretsProviderError(f"Unexpected error retrieving secret {path}: {e}") from e
    ...
    async def put_secret(self, path: str, data: SecretData) -> bool:
        ...
        payload = {"data": data} if kv_version == "v2" else data
        ...
        try:
            ...
            raise SecretsProviderError(f"Failed to store secret {path}: {e}") from e
    ...
    async def delete_secret(self, path: str) -> bool:
        ...
        try:
            response = await self._retry_with_backoff(
                f"delete_secret:{path}", _delete_secret_data()
            )
            ...
            raise SecretsProviderError(f"Failed to delete secret {path}: {e}") from e
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            async def _health_check() -> httpx.Response:
                return await client.get(url, headers=health_headers)
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"OpenBaoProvider(url={self.url}, mount={self.mount_path}, version={self.api_version})"
        )
