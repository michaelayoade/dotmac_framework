    ...
    try:
        ...
        if metric_count > 0:
            if prometheus_available and details.get("prometheus_metrics_available", False):
                ...
                status = HealthStatus.DEGRADED
                message = "Metrics registry partially operational (Prometheus issues)"
            else:
                status = HealthStatus.HEALTHY
                message = (
                    f"Metrics registry operational with {metric_count} metrics (no Prometheus)"
                )
        else:
            status = HealthStatus.DEGRADED
