...
class EdgeJWTValidator:
    ...
    def __init__(
        ...
        tenant_resolver: Optional[Callable[[Request], Optional[str]]] = None,
    ...
    def configure_sensitivity_patterns(
        self, 
    ...
    def extract_service_token(self, request: Request) -> Optional[str]:
        """
    ...
    async def _validate_user_request(
        ...
        # Verify token
        try:
            claims = self.jwt_service.verify_token(
                token,
...
def create_edge_validator(
    jwt_service: JWTService,
    patterns: Optional[Dict[Tuple[str, str], str]] = None,
