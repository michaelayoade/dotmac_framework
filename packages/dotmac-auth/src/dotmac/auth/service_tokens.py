...
class ServiceTokenManager:
    ...
    def get_service(self, service_name: str) -> Optional[ServiceIdentity]:
    ...
    def issue_service_token(
        ...
        try:
            ...
            raise InvalidServiceToken(f"Failed to encode service token: {e}")
    ...
    def verify_service_token(
        ...
        try:
            ...
            if expected_target and claims.get("target_service") != expected_target:
                raise UnauthorizedService(
                    f"Token not valid for service {expected_target}",
                    target_service=expected_target
                )
            ...
            if service_name not in self.services:
                raise UnauthorizedService(
                    f"Service {service_name} no longer registered",
                    service_name=service_name
                )
            ...
            raise InvalidServiceToken(f"Service token validation failed: {str(e)}")
        
        except Exception as e:
            if isinstance(e, (UnauthorizedService, InvalidServiceToken, TokenExpired)):
                raise
            raise InvalidServiceToken(f"Unexpected service token validation error: {str(e)}")
...
class ServiceAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def _default_error_handler(self, request: Request, error: AuthError) -> Response:
        ...
        return JSONResponse(
            ...
            headers={
                "X-Service-Auth-Error": error.error_code
            }
        )
