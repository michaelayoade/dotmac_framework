...
class SecretsManager:
        ...
        try:
            ...
            if self.cache:
                ...
                if cached_value is not None:
                    ...
                    if isinstance(cached_value.value, dict) and cached_value.value.get("__negative_cache"):
            ...
            if self.validator and self.validate_secrets:
                try:
                    if not self.validator.validate(secret_data, kind):
                        ...
                        if self.observability_hook:
                            self.observability_hook.record_validation_failure(
                                kind, error_msg, path
                            )
    ...
    async def get_symmetric_secret(
        self,
        name: str,
        min_length: int = 32
    ) -> str:
    ...
    async def invalidate_cache(self, path: Optional[str] = None) -> bool:
        """
