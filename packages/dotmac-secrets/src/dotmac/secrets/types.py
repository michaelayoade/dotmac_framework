...
class OpenBaoConfig(ProviderConfig):
    ...
    provider_type: str = Field(default="openbao", const=True)
    url: str
    token: str
...
class EnvConfig(ProviderConfig):
    ...
    provider_type: str = Field(default="env", const=True)
...
class FileConfig(ProviderConfig):
    ...
    provider_type: str = Field(default="file", const=True)
    base_path: str
...
class SecretPaths:
    ...
    @staticmethod
    def jwt_keypair(app: str, kid: Optional[str] = None) -> str:
...
SecretResult = Union[str, bytes, Dict[str, Any], JWTKeypair, DatabaseCredentials]