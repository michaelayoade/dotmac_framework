...
from ..types import DatabaseCredentials, Environment, SecretData
...
class EnvironmentProvider(BaseProvider):
    ...
    async def _get_jwt_secret(self, path: str) -> SecretData:
        ...
        private_key = (
            self._get_env_var(f"JWT_PRIVATE_KEY_{app.upper()}") or
            self._get_env_var(f"JWT_PRIVATE_KEY_{app.upper()}_{kid.upper()}") or
            self._get_env_var("JWT_PRIVATE_KEY")
        )
        
        public_key = (
            self._get_env_var(f"JWT_PUBLIC_KEY_{app.upper()}") or
            self._get_env_var(f"JWT_PUBLIC_KEY_{app.upper()}_{kid.upper()}") or
            self._get_env_var("JWT_PUBLIC_KEY")
        )
        
        algorithm = (
            self._get_env_var(f"JWT_ALGORITHM_{app.upper()}") or
            self._get_env_var("JWT_ALGORITHM") or
            "RS256"
        )
    ...
    async def _get_database_secret(self, path: str) -> SecretData:
        ...
        database_url = (
            self._get_env_var(f"DATABASE_URL_{db_name.upper()}") or
            self._get_env_var("DATABASE_URL")
        )
        ...
        host = self._get_env_var(f"DATABASE_HOST_{db_name.upper()}") or self._get_env_var("DATABASE_HOST")
        port_str = self._get_env_var(f"DATABASE_PORT_{db_name.upper()}") or self._get_env_var("DATABASE_PORT")
        username = self._get_env_var(f"DATABASE_USER_{db_name.upper()}") or self._get_env_var("DATABASE_USER")
        password = self._get_env_var(f"DATABASE_PASSWORD_{db_name.upper()}") or self._get_env_var("DATABASE_PASSWORD")
        database = self._get_env_var(f"DATABASE_NAME_{db_name.upper()}") or self._get_env_var("DATABASE_NAME") or db_name
        driver = self._get_env_var(f"DATABASE_DRIVER_{db_name.upper()}") or self._get_env_var("DATABASE_DRIVER") or "postgresql"
    ...
    def _parse_database_url(self, url: str) -> SecretData:
        ...
        if not parsed.hostname or not parsed.username:
            raise SecretNotFoundError(f"Invalid database URL format")
    ...
    async def _get_service_signing_secret(self, path: str) -> SecretData:
        ...
        secret = (
            self._get_env_var(f"SERVICE_SIGNING_SECRET_{service.upper()}") or
            self._get_env_var("SERVICE_SIGNING_SECRET")
        )
    ...
    async def _get_encryption_key_secret(self, path: str) -> SecretData:
        ...
        key = (
            self._get_env_var(f"ENCRYPTION_KEY_{key_name.upper()}") or
            self._get_env_var("ENCRYPTION_KEY")
        )
    ...
    async def _get_webhook_secret(self, path: str) -> SecretData:
        ...
        secret = (
            self._get_env_var(f"WEBHOOK_SECRET_{webhook_id.upper()}") or
            self._get_env_var("WEBHOOK_SECRET")
        )
    ...
    async def _get_symmetric_secret(self, path: str) -> SecretData:
        ...
        secret = (
            self._get_env_var(f"SYMMETRIC_SECRET_{secret_name.upper()}") or
            self._get_env_var(f"SECRET_{secret_name.upper()}")
        )
