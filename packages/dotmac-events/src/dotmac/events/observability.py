...
class ObservabilityHooks:
    ...
    async def _no_op_retry(self, event: Event, retry_count: int, error: Exception, adapter_name: str) -> None:
        pass
    
    async def _no_op_dlq(self, event: Event, error: Exception, adapter_name: str) -> None:
        pass
    
    async def _no_op_error(self, event: Event, error: Exception, operation: str, adapter_name: str) -> None:
        pass
...
class MetricsCollector:
    ...
    async def on_retry(self, event: Event, retry_count: int, error: Exception, adapter_name: str) -> None:
        """Handle retry event."""
    ...
    async def on_error(self, event: Event, error: Exception, operation: str, adapter_name: str) -> None:
        """Handle error event."""
...
def create_dotmac_observability_hooks(
    ...
    Args:
        metrics_registry: MetricsRegistry from dotmac.observability
        tenant_metrics: TenantMetrics from dotmac.observability
    ...
    try:
        ...
        from dotmac.observability import TenantContext
    except ImportError:
        # Fall back to default hooks if not available
        logger.warning(
            "dotmac.observability not available, using default hooks. "
    ...
    class DotMacObservabilityCollector:
        ...
        async def on_publish(self, event: Event, adapter_name: str, duration: float) -> None:
            ...
            if self.metrics_registry:
                ...
                self.metrics_registry.increment_counter("events_published_total", 1, {
                    "adapter": adapter_name,
                    "topic": event.topic,
                })
                
                self.metrics_registry.observe_histogram("event_publish_duration_seconds", duration, {
                    "adapter": adapter_name,
                    "topic": event.topic,
                })
        ...
        async def on_consume(self, event: Event, adapter_name: str, duration: float) -> None:
            ...
            if self.metrics_registry:
                # Record system metrics
                self.metrics_registry.increment_counter("events_consumed_total", 1, {
                    "adapter": adapter_name,
                    "topic": event.topic,
                })
                
                self.metrics_registry.observe_histogram("event_consume_duration_seconds", duration, {
                    "adapter": adapter_name,
                    "topic": event.topic,
                })
        ...
        async def on_retry(self, event: Event, retry_count: int, error: Exception, adapter_name: str) -> None:
            ...
            if self.metrics_registry:
                self.metrics_registry.increment_counter("events_retried_total", 1, {
                    "adapter": adapter_name,
                    "topic": event.topic,
        ...
        async def on_dlq(self, event: Event, error: Exception, adapter_name: str) -> None:
            """Record DLQ metrics."""
            if self.metrics_registry:
                self.metrics_registry.increment_counter("events_dlq_total", 1, {
                    "adapter": adapter_name,
                    "topic": event.topic,
                    "error_type": type(error).__name__,
                })
        ...
        async def on_error(self, event: Event, error: Exception, operation: str, adapter_name: str) -> None:
            ...
            if self.metrics_registry:
                self.metrics_registry.increment_counter("events_error_total", 1, {
                    "adapter": adapter_name,
                    "topic": event.topic,
                    "operation": operation,
                    "error_type": type(error).__name__,
