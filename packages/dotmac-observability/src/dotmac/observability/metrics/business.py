...
from .registry import MetricsRegistry, MetricDefinition, MetricType
...
class BusinessMetricSpec:
    ...
    def __post_init__(self) -> None:
        ...
        is_lower_better = self.metric_type in (BusinessMetricType.LATENCY, BusinessMetricType.ERROR_RATE)
...
class TenantMetrics:
        ...
        for metric_name, spec in self._business_metrics.items():
            try:
                ...
                if evaluation:
                    ...
                    self._slo_history[metric_name] = [
                        e for e in self._slo_history[metric_name]
                        if e.evaluation_time > cutoff
                    ]
    ...
    def _evaluate_single_slo(
        ...
        is_lower_better = spec.metric_type in (BusinessMetricType.LATENCY, BusinessMetricType.ERROR_RATE)
        ...
        if is_lower_better:
            ...
            if spec.slo_target > 0:
                error_budget_consumed = max(0.0, (current_value - spec.slo_target) / spec.slo_target)
            else:
            ...
            if spec.slo_target > 0:
                error_budget_consumed = max(0.0, (spec.slo_target - current_value) / spec.slo_target)
            else:
                error_budget_consumed = 0.0
