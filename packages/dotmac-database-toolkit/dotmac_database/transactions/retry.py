    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs) -> T:
                return await _async_retry_operation(func, policy, *args, **kwargs)
            ...
            def sync_wrapper(*args, **kwargs) -> T:
                return _sync_retry_operation(func, policy, *args, **kwargs)
    ...
    for attempt in range(policy.max_attempts):
        try:
            ...
            if attempt == policy.max_attempts - 1:
                logger.error(
                    f"Operation failed after {policy.max_attempts} attempts: {e}"
                )
                raise
    ...
    **kwargs,
    ...
    for attempt in range(policy.max_attempts):
        try:
            ...
            if not policy.should_retry(e, attempt):
                logger.error(
                    f"Async operation failed with non-retryable exception: {e}"
                )
                raise

            if attempt == policy.max_attempts - 1:
                logger.error(
                    f"Async operation failed after {policy.max_attempts} attempts: {e}"
                )
                raise
...
class CircuitBreaker:
    ...
    async def _async_call(self, func: Callable[..., T], *args, **kwargs) -> T:
