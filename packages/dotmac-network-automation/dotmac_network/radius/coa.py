...
class CoAManager:
    ...
    async def disconnect_session(self, session: RADIUSSession, reason: str = "Administrative disconnect") -> bool:
        """
        ...
        try:
            ...
            if session.session_id:
                packet.add_attribute(RADIUSAttributeType.ACCT_SESSION_ID, session.session_id)

            if session.nas_ip:
                packet.add_attribute(RADIUSAttributeType.NAS_IP_ADDRESS,
                                   socket.inet_aton(session.nas_ip))

            if session.nas_port is not None:
                packet.add_attribute(RADIUSAttributeType.NAS_PORT,
                                   struct.pack("!I", session.nas_port))

            if session.framed_ip:
                packet.add_attribute(RADIUSAttributeType.FRAMED_IP_ADDRESS,
    ...
    async def change_session_attributes(
        ...
        try:
            ...
            if response and response.packet_type == RADIUSPacketType.COA_ACK:
                logger.info(f"Session {session.session_id} attributes changed successfully")
                return True
            else:
    ...
    async def _send_coa_request(self, nas_ip: str, packet: RADIUSPacket) -> Optional[RADIUSPacket]:
        """
