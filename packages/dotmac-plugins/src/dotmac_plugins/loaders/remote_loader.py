...
class RemotePluginLoader:
    ...
    async def load_plugins_from_repository(
        ...
        try:
            ...
            if plugin_config_path:
                ...
                plugins = await self._python_loader.discover_plugins_in_directory(repo_path, recursive=True)
    ...
    async def _clone_repository(self, repo_url: str, branch: str = "main", force_refresh: bool = False) -> Path:
            ...
            async with self._session.get(query_url) as response:
                ...
                if missing_fields:
                    raise PluginLoadError(
                        ...
                        plugin_path=registry_url,
                        loader_type="remote_registry",
                        original_error=ValueError(f"Registry response missing fields: {missing_fields}"),
                    )
    ...
    async def _load_plugin_from_path(
        ...
        if path.is_file():
            # Single Python file
            if path.suffix == ".py":
                return await self._python_loader.load_plugin_from_file(path, plugin_class_name, plugin_config)
            else:
                raise PluginLoadError(
                    plugin_class_name,
            ...
            if yaml_files:
                ...
                raise PluginLoadError(
                    ...
                    original_error=ValueError(f"Plugin class '{plugin_class_name}' not found in YAML config"),
                )
                ...
                raise PluginLoadError(
                    ...
                    plugin_path=str(path),
                    loader_type="remote_directory",
                    original_error=ValueError(f"Plugin class '{plugin_class_name}' not found in directory"),
                )
