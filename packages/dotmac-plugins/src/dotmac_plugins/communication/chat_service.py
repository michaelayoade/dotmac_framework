...
class ChatService:
    ...
    async def get_session(self, db: AsyncSession, tenant_id: str, session_id: str) -> Optional[ChatSessionResponse]:
        """Get chat session by ID."""
        query = (
            select(ChatSession)
            .where(and_(ChatSession.session_id == session_id, ChatSession.tenant_id == tenant_id))
    ...
    async def get_session_messages(
        ...
        try:
            ...
            session_query = select(ChatSession).where(
                and_(ChatSession.session_id == session_id, ChatSession.tenant_id == tenant_id)
            )
    ...
    async def assign_agent(
        self, db: AsyncSession, session_id: str, agent_id: str, agent_name: Optional[str] = None
    ...
    async def transfer_session(
        self, db: AsyncSession, session_id: str, from_agent_id: str, to_agent_id: str, reason: Optional[str] = None
    ) -> Optional[ChatSessionResponse]:
    ...
    async def end_session(
        self, db: AsyncSession, session_id: str, agent_id: Optional[str] = None, summary: Optional[str] = None, create_ticket: bool = False
    ) -> Optional[ChatSessionResponse]:
        """End a chat session."""
        try:
            session_query = select(ChatSession).where(ChatSession.session_id == session_id)
    ...
    async def rate_session(
        self, db: AsyncSession, tenant_id: str, session_id: str, rating: int, feedback: Optional[str] = None
    ) -> bool:
        """Rate a chat session."""
        try:
            session_query = select(ChatSession).where(
    ...
    async def get_agent_status(self, db: AsyncSession, agent_id: str, tenant_id: str) -> dict[str, Any]:
        """Get agent status information."""
        try:
            query = select(AgentStatus).where(
                and_(AgentStatus.agent_id == agent_id, AgentStatus.tenant_id == tenant_id)
            )
            ...
            if not agent_status:
                ...
                agent_status = AgentStatus(agent_id=agent_id, tenant_id=tenant_id, status=AgentStatus.OFFLINE)
                db.add(agent_status)
                await db.commit()
            ...
            return {
                ...
                "is_available": (
                    agent_status.status == AgentStatus.ONLINE
                    and agent_status.current_chat_count < agent_status.max_concurrent_chats
                ),
            }
    ...
    async def update_agent_status(
        self, db: AsyncSession, agent_id: str, tenant_id: str, status_data: AgentStatusUpdate
    ) -> dict[str, Any]:
        """Update agent status."""
        try:
            query = select(AgentStatus).where(
            ...
            if not agent_status:
                agent_status = AgentStatus(agent_id=agent_id, tenant_id=tenant_id)
                db.add(agent_status)
    ...
    async def get_agent_sessions(
        self, db: AsyncSession, agent_id: str, tenant_id: str, status_filter: Optional[list[str]] = None
    ) -> list[ChatSessionResponse]:
        """Get chat sessions assigned to an agent."""
        try:
            query = select(ChatSession).where(
                and_(ChatSession.assigned_agent_id == agent_id, ChatSession.tenant_id == tenant_id)
            )
    ...
    async def list_sessions(
        self, db: AsyncSession, tenant_id: str, filters: Optional[dict[str, Any]] = None, page: int = 1, page_size: int = 20
    ...
    async def get_analytics_overview(self, db: AsyncSession, tenant_id: str, days: int = 30) -> dict[str, Any]:
        ...
        try:
            ...
            total_sessions_query = select(func.count(ChatSession.id)).where(
                and_(ChatSession.tenant_id == tenant_id, ChatSession.created_at >= cutoff_date)
            )
            ...
            status_query = (
                ...
                .where(and_(ChatSession.tenant_id == tenant_id, ChatSession.created_at >= cutoff_date))
            ...
            # Average wait time
            avg_wait_query = select(func.avg(ChatSession.wait_time_seconds)).where(
                and_(
            ...
            avg_duration_query = select(func.avg(ChatSession.session_duration_seconds)).where(
                and_(
                    ChatSession.tenant_id == tenant_id,
                    ChatSession.created_at >= cutoff_date,
            ...
            avg_rating_query = select(func.avg(ChatSession.customer_rating)).where(
                and_(
                    ChatSession.tenant_id == tenant_id,
                    ChatSession.created_at >= cutoff_date,
    ...
    async def get_agent_performance_analytics(
        self, db: AsyncSession, tenant_id: str, days: int = 30
    ) -> list[dict[str, Any]]:
    ...
    async def get_queue_status(self, db: AsyncSession, tenant_id: str) -> dict[str, Any]:
        """Get current queue status."""
        try:
            # Waiting sessions
            waiting_query = select(func.count(ChatSession.id)).where(
                and_(ChatSession.tenant_id == tenant_id, ChatSession.status == ChatStatus.WAITING)
            )
            ...
            active_query = select(func.count(ChatSession.id)).where(
                and_(ChatSession.tenant_id == tenant_id, ChatSession.status == ChatStatus.ACTIVE)
            )
            active_result = await db.execute(active_query)
            active_count = active_result.scalar()

            # Available agents
            available_agents_query = select(func.count(AgentStatus.agent_id)).where(
                and_(
    ...
    async def _update_agent_chat_count(self, db: AsyncSession, agent_id: str, tenant_id: str, delta: int):
        ...
        try:
            stmt = (
                update(AgentStatus)
                .where(and_(AgentStatus.agent_id == agent_id, AgentStatus.tenant_id == tenant_id))
    ...
    async def _create_ticket_from_chat(self, db: AsyncSession, session: ChatSession, summary: Optional[str] = None):
