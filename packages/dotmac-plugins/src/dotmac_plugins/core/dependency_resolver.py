...
class DependencyResolver:
    ...
    def add_plugin(self, plugin_key: str, metadata: PluginMetadata) -> None:
        ...
        self._logger.debug(f"Added plugin {plugin_key} with {len(metadata.dependencies)} dependencies")
    ...
    def validate_dependencies(self, available_plugins: set[str]) -> dict[str, Any]:
        ...
        for plugin_key in available_plugins:
            ...
            missing_optional = [dep for dep in node.optional_dependencies if dep not in available_plugins]
    ...
    def can_unload_plugin(self, plugin_key: str, active_plugins: set[str]) -> tuple[bool, list[str]]:
        """
