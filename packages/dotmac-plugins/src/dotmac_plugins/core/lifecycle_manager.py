...
class LifecycleMetrics:
    ...
    def record_initialization(self, success: bool, duration: float) -> None:
        ...
        self.avg_initialization_time = sum(self._initialization_times) / len(self._initialization_times)
...
class LifecycleManager:
    ...
    async def initialize_plugins(
        ...
        try:
            if parallel:
                ...
                for group_index, group in enumerate(groups):
                    self._logger.info(f"Initializing group {group_index + 1}/{len(groups)}: {group}")
                    ...
                    if failed_plugins:
                        self._logger.warning(f"Failed to initialize plugins in group: {failed_plugins}")
        ...
        self._logger.info(f"Plugin initialization complete: {successful_count}/{len(plugin_keys)} successful")

        return results

    async def _initialize_plugin_group(self, plugin_keys: list[str], timeout: float) -> dict[str, bool]:
    ...
    async def shutdown_plugin(self, plugin_key: str, timeout: Optional[float] = None, force: bool = False) -> bool:
        """
        ...
        if not force:
            ...
            can_shutdown, blocking_dependents = self.dependency_resolver.can_unload_plugin(plugin_key, active_plugins)
    ...
    async def shutdown_all_plugins(self, timeout: Optional[float] = None, force: bool = False) -> dict[str, bool]:
        """
        ...
        successful_count = sum(1 for success in results.values() if success)
        self._logger.info(f"Plugin shutdown complete: {successful_count}/{len(active_plugin_keys)} successful")
    ...
    async def _emit_event(self, event: LifecycleEvent, plugin: BasePlugin, data: Any = None) -> None:
    ...
    def get_system_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_initialization_time": sum(m.avg_initialization_time for m in all_metrics) / len(all_metrics),
            "avg_shutdown_time": sum(m.avg_shutdown_time for m in all_metrics) / len(all_metrics),
            "avg_health_check_time": sum(m.avg_health_check_time for m in all_metrics) / len(all_metrics),
            "health_monitoring_active": self._health_monitoring_active,
            "shutdown_requested": self._shutdown_requested,
            "active_tasks": {
