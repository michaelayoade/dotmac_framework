    ...
    async def batch_scan_directory(self, directory: Path, pattern: str = "*.py") -> list[PluginSecurityReport]:
        ...
        for file_path in plugin_files:
            try:
                ...
                logger.info(f"Scanned {file_path.name}: {report.security_level} risk ({report.risk_score} score)")

            except Exception as e:
                logger.error(f"Failed to scan {file_path.name}: {e}")
                # Create error report
                error_report = PluginSecurityReport(plugin_id=file_path.stem, security_level="unknown", risk_score=100)
...
__all__ = ["SecurityThreat", "PluginSecurityReport", "AdvancedPluginSecurityScanner", "create_advanced_plugin_scanner"]
