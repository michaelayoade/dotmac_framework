...
from .storage.base import Storage, LockAcquisitionError
...
class BackgroundOperationsManager:
    ...
    async def complete_idempotent_operation(
        ...
        try:
            ...
            current_data["status"] = (
                OperationStatus.FAILED.value if error 
    ...
    async def _execute_saga_steps_forward(self, saga: SagaWorkflow) -> bool:
        ...
        while saga.current_step < len(saga.steps):
            ...
            if success:
                ...
                await self._record_saga_history(
                    saga.saga_id, step.step_id, step.name, 
                    SagaStepStatus.COMPLETED, None, step.retry_count
                )
                ...
                await self._record_saga_history(
                    saga.saga_id, step.step_id, step.name, 
                    SagaStepStatus.FAILED, step.error, step.retry_count
                )
    ...
    async def _execute_saga_step(self, saga: SagaWorkflow, step: SagaStep) -> bool:
        ...
        for attempt in range(step.max_retries + 1):
            try:
                ...
                if not handler:
                    step.error = f"No handler registered for operation: {step.operation}"
    ...
    async def get_operation_status(self, operation_id: str) -> Optional[BackgroundOperation]:
        """
