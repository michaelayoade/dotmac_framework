...
class BackgroundOperationsMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            if existing_key:
                ...
                return await self._handle_new_operation(idempotency_key, request, call_next)
    ...
    async def _handle_new_operation(
        ...
        if hasattr(request.state, 'operation_result') and request.state.operation_result:
            # Complete the idempotent operation
            await self.manager.complete_idempotent_operation(
                idempotency_key,
                request.state.operation_result
            )
...
def is_idempotent_request(request: Request) -> bool:
    ...
    """
    return hasattr(request.state, 'idempotency_key') and request.state.idempotency_key is not None