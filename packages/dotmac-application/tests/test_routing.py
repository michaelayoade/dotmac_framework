...
class TestRouterRegistration:
    ...
    def test_explicit_router_registration(self):
        ...
        mock_router.add_api_route("/test", endpoint=lambda: {"message": "test"}, methods=["GET"])
        ...
        with patch('importlib.import_module') as mock_import:
            ...
            assert "/api/v1/auth/test" in routes or any("/test" in route for route in routes)
    ...
    def test_auto_discovery_router_registration(self):
        ...
        mock_auth_router.add_api_route("/login", endpoint=lambda: {"message": "login"}, methods=["POST"])
        
        mock_users_router = APIRouter()  
        mock_users_router.add_api_route("/profile", endpoint=lambda: {"message": "profile"}, methods=["GET"])
        ...
        with patch('os.listdir') as mock_listdir, \
            ...
            assert any("profile" in path for path in routes) or "profile" in route_paths_str
    ...
    def test_router_config_validation(self):
        ...
        valid_config = RouterConfig(
            module_path="dotmac.valid.module",
            prefix="/api/v1",
            required=True
        )
    ...
    def test_router_tags_and_prefix_application(self):
        ...
        mock_router = APIRouter()
        mock_router.add_api_route("/endpoint", endpoint=lambda: {"message": "test"}, methods=["GET"])
