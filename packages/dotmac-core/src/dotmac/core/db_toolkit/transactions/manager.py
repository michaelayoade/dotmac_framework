...
class TransactionManager:
    ...
    def transaction(
        ...
        """
        if retry_on_failure:
            yield from self._transaction_with_retry(session, auto_commit, is_async=False)
        else:
    ...
    async def async_transaction(
        ...
        """
        if retry_on_failure:
            async for db_session in self._async_transaction_with_retry(session, auto_commit):
                yield db_session
        else:
    ...
    def execute_with_retry(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                with self.transaction(session=session, retry_on_failure=False) as db_session:
