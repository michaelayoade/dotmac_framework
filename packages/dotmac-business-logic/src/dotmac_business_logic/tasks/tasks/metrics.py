...
class MetricsHooks:
    ...
    def enable_logging_metrics(self, log_level: int = logging.INFO) -> None:
        ...
        def log_operation_enqueued(
            tenant_id: str, operation_type: str, metadata: dict[str, Any]
        ) -> None:
            logger.log(
                log_level,
                f"Operation enqueued: {operation_type} for tenant {tenant_id}",
            )
        ...
        def log_operation_completed(
            ...
            logger.log(
                log_level,
                f"Operation completed: {operation_type} for tenant {tenant_id} - {status}",
            )
        ...
        def log_operation_duration(
            ...
            logger.log(
                log_level,
                f"Operation duration: {operation_type} for tenant {tenant_id} took {duration_seconds:.3f}s",
            )

        def log_saga_created(
            tenant_id: str, workflow_type: str, step_count: int
        ) -> None:
            logger.log(
                log_level,
                f"Saga created: {workflow_type} for tenant {tenant_id} with {step_count} steps",
            )

        def log_saga_status_change(
            tenant_id: str, saga_id: str, new_status: str
        ) -> None:
            logger.log(
                log_level,
                f"Saga status change: {saga_id} for tenant {tenant_id} -> {new_status}",
            )

        def log_saga_step_executed(
            tenant_id: str,
            saga_id: str,
            step_name: str,
            success: bool,
            ...
            logger.log(
                log_level,
                f"Saga step: {step_name} in {saga_id} for tenant {tenant_id} - {status}{retry_info}",
            )

        def log_idempotency_hit(tenant_id: str, operation_type: str) -> None:
            logger.log(
                log_level, f"Idempotency hit: {operation_type} for tenant {tenant_id}"
            )

        def log_idempotency_miss(tenant_id: str, operation_type: str) -> None:
            logger.log(
                log_level, f"Idempotency miss: {operation_type} for tenant {tenant_id}"
            )

        def log_storage_operation(
            operation: str, backend: str, duration_seconds: float
        ) -> None:
            logger.log(
                log_level,
                f"Storage {operation} on {backend} took {duration_seconds:.3f}s",
            )

        def log_lock_acquired(lock_key: str, duration_seconds: float) -> None:
            logger.log(
                log_level, f"Lock acquired: {lock_key} (took {duration_seconds:.3f}s)"
            )
...
def record_operation_enqueued(
    tenant_id: str, operation_type: str, metadata: Optional[dict[str, Any]] = None
) -> None:
...
def record_operation_completed(
    tenant_id: str, operation_type: str, success: bool, error: Optional[str] = None
) -> None:
...
def record_operation_duration(
    ...
    _metrics_hooks.record_operation_duration(
        tenant_id, operation_type, duration_seconds
    )
...
def record_saga_step_executed(
    tenant_id: str, saga_id: str, step_name: str, success: bool, retry_count: int = 0
) -> None:
    """Record saga step execution."""
    _metrics_hooks.record_saga_step_executed(
        tenant_id, saga_id, step_name, success, retry_count
    )
