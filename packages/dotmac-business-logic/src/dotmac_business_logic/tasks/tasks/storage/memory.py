...
class MemoryStorage(Storage):
        ...
        async with self._lock:
            ...
            if expires_at:
                ...
                try:
                    ...
                    if (
                        datetime.now(
                            exp_time.tzinfo or datetime.now().astimezone().tzinfo
                        )
                        > exp_time
                    ):
                        # Remove expired data
    ...
    async def list_operations_by_tenant(
        ...
        async with self._lock:
            ...
            tenant_ops.sort(key=lambda x: x.get("created_at", ""), reverse=True)
    ...
    async def list_sagas_by_tenant(
        ...
        async with self._lock:
            ...
            # Sort by created_at (newest first)
            tenant_sagas.sort(key=lambda x: x.get("created_at", ""), reverse=True)
    ...
    async def acquire_lock(self, lock_key: str, timeout_seconds: int = 30) -> bool:
        ...
        async with self._lock:
            ...
            expired_keys = [key for key, expiry in self._locks.items() if now > expiry]
    ...
    async def cleanup_expired_data(self) -> int:
        ...
        async with self._lock:
            ...
            # Clean up expired locks
            expired_locks = [key for key, expiry in self._locks.items() if now > expiry]
