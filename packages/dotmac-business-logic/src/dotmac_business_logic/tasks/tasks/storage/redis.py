...
from .base import Storage, StorageConnectionError, StorageTimeoutError
...
class RedisStorage(Storage):
        ...
        try:
            ...
            if self.retry_on_timeout:
                ...
                try:
                    ...
                    raise StorageTimeoutError(f"Redis operation timed out: {e}") from e
            else:
                raise StorageTimeoutError(f"Redis operation timed out: {e}") from e
    ...
    async def set_idempotency(
        self, key: str, mapping: dict[str, Any], ttl_seconds: int
    ) -> None:
    ...
    async def index_idempotency(self, key: str, timestamp: float) -> None:
        ...
        index_key = self._key("idempo", "index")
        await self._execute_with_retry(self.redis.zadd, index_key, {key: timestamp})
    ...
    async def get_expired_idempotency_keys(self, before_timestamp: float) -> list[str]:
        ...
        expired_keys = await self._execute_with_retry(
            self.redis.zrangebyscore, index_key, "-inf", before_timestamp
        )
    ...
    async def append_saga_history(self, saga_id: str, entry: dict[str, Any]) -> None:
    ...
    async def get_saga_history(
        self, saga_id: str, limit: Optional[int] = None
    ) -> list[dict[str, Any]]:
    ...
    async def set_operation(self, operation_id: str, mapping: dict[str, Any]) -> None:
    ...
    async def list_sagas_by_tenant(
        ...
        # Sort by created_at (newest first)
        sagas.sort(key=lambda x: x.get("created_at", ""), reverse=True)
    ...
    async def acquire_lock(self, lock_key: str, timeout_seconds: int = 30) -> bool:
        ...
        result = await self._execute_with_retry(
            self.redis.set, redis_key, lock_value, nx=True, ex=timeout_seconds
        )
