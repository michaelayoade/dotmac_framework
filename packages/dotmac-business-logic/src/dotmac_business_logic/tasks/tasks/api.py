...
class TasksAPI:
    ...
    async def get_tenant_operations(
        ...
        """
        return await self.manager.storage.list_operations_by_tenant(tenant_id, limit)
...
def create_tasks_api(storage_backend: str = "memory", **config_kwargs) -> TasksAPI:
    """
...
def create_redis_tasks_api(
    redis_url: str = "redis://localhost:6379/0", **kwargs
) -> TasksAPI:
    """
    ...
    """
    storage_config = {"backend": "redis", "redis_url": redis_url, **kwargs}
    return TasksAPI(storage_config=storage_config)
