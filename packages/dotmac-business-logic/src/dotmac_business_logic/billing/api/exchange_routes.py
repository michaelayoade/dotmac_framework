...
from dotmac.core import get_db
...
router_factory = RouterFactory(
    prefix="/exchange", tags=["Multi-Currency Exchange"], dependencies=[]
)
...
async def add_customer_currency(
    ...
    try:
        customer_currency = await service.add_customer_currency(
            customer_id, currency_data
        )
...
async def set_base_currency(
    ...
    try:
        ...
        customer_currency = await service.set_base_currency(
            customer_id, target_currency.currency_code
        )
        return CustomerCurrencyResponse.model_validate(customer_currency)
    except ValueError as e:
        raise HTTPException(
...
async def get_customer_exchange_rates(
    ...
    return ExchangeRateListResponse(exchange_rates=[], total_count=0)
...
async def get_exchange_rate(
    ...
    try:
        exchange_rate = await service.get_exchange_rate(
            customer_id, from_currency.upper(), to_currency.upper()
        )
...
async def get_multi_currency_payments(
    ...
    # For now, return empty list
    return MultiCurrencyPaymentListResponse(payments=[], total_count=0)
...
async def convert_currency(
    ...
    try:
        ...
        return CurrencyConversionResponse(
            ...
            if exchange_rate
            else conversion_request.exchange_rate,
