...
from .models import Base, BillingModelMixin
...
class CustomerCurrency(BillingModelMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "customer_id", "currency_code", name="unique_customer_currency"
        ),
    )
...
class ManualExchangeRate(BillingModelMixin, Base):
    ...
    customer_currency_id = Column(
        PGUUID(as_uuid=True), ForeignKey("customer_currencies.id"), nullable=False
    )
    ...
    status = Column(String(20), default=ExchangeRateStatus.ACTIVE.value, nullable=False)
    ...
    reference_payment_id = Column(
        PGUUID(as_uuid=True), ForeignKey("billing_payments.id"), nullable=True
    )
    ...
    # Relationships
    customer_currency = relationship(
        "CustomerCurrency", back_populates="exchange_rates"
    )
...
class MultiCurrencyPayment(BillingModelMixin, Base):
    ...
    payment_id = Column(
        PGUUID(as_uuid=True), ForeignKey("billing_payments.id"), nullable=False
    )

    # Exchange rate used
    exchange_rate_id = Column(
        PGUUID(as_uuid=True), ForeignKey("manual_exchange_rates.id"), nullable=False
    )
...
def calculate_converted_amount(
    amount: Decimal, exchange_rate: Decimal, from_currency: str, to_currency: str
) -> Decimal:
    """
...
def calculate_variance(
    expected_amount: Decimal, actual_amount: Decimal
) -> tuple[Decimal, str]:
    """
