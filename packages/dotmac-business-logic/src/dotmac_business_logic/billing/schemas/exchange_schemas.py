...
def validate_currency_code(code: str) -> bool:
    return code in get_supported_currencies()
...
class CustomerCurrencyBase(BillingBaseSchema):
    ...
    is_base_currency: bool = Field(
        default=False, description="Whether this is the customer's base currency"
    )
    is_active: bool = Field(default=True, description="Currency is active for use")
    display_name: Optional[str] = Field(
        None, max_length=100, description="Display name for currency"
    )
...
class ManualExchangeRateBase(BillingBaseSchema):
    ...
    from_currency: str = Field(
        ..., min_length=3, max_length=3, description="Currency being converted from"
    )
    to_currency: str = Field(
        ..., min_length=3, max_length=3, description="Currency being converted to"
    )
    ...
    rate_date: datetime = Field(
        default_factory=datetime.utcnow, description="Date when rate was set"
    )
    valid_until: Optional[datetime] = Field(
        None, description="Optional expiry date for rate"
    )
    source: Optional[str] = Field(
        None, max_length=100, description="Source of exchange rate"
    )
    notes: Optional[str] = Field(None, description="Additional notes")
...
class ManualExchangeRateCreate(ManualExchangeRateBase):
    ...
    customer_currency_id: UUID = Field(
        ..., description="Customer currency configuration ID"
    )
...
class ManualExchangeRateUpdate(BillingBaseSchema):
    ...
    exchange_rate: Optional[Decimal] = Field(
        None, gt=0, decimal_places=6, description="New exchange rate"
    )
...
class MultiCurrencyPaymentBase(BillingBaseSchema):
    ...
    original_amount: Decimal = Field(
        ..., gt=0, description="Amount paid in original currency"
    )
    original_currency: str = Field(
        ..., min_length=3, max_length=3, description="Payment currency"
    )
    converted_amount: Decimal = Field(..., gt=0, description="Amount in base currency")
    converted_currency: str = Field(
    ...
    expected_amount: Optional[Decimal] = Field(
        None, description="Expected amount in base currency"
    )
...
class PaymentWithExchangeRateCreate(BillingBaseSchema):
    ...
    base_currency: str = Field(
        ..., min_length=3, max_length=3, description="Customer's base currency"
    )
    exchange_rate: Decimal = Field(
        ..., gt=0, decimal_places=6, description="Manual exchange rate"
    )
...
class CurrencyConversionRequest(BillingBaseSchema):
    ...
    from_currency: str = Field(
        ..., min_length=3, max_length=3, description="Source currency"
    )
    to_currency: str = Field(
...
class CustomerCurrencyListResponse(BillingBaseSchema):
    ...
    currencies: list[CustomerCurrencyResponse] = Field(
        default_factory=list, description="List of customer currencies"
    )
...
class ExchangeRateListResponse(BillingBaseSchema):
    ...
    exchange_rates: list[ManualExchangeRateResponse] = Field(
        default_factory=list, description="List of exchange rates"
    )
...
class MultiCurrencyPaymentListResponse(BillingBaseSchema):
    ...
    payments: list[MultiCurrencyPaymentResponse] = Field(
        default_factory=list, description="List of multi-currency payments"
    )
