...
class RateLimitMiddleware:
    ...
    async def check_message_rate(self, session) -> bool:
        ...
        if session.user_id:
            ...
            if not user_state.consume_burst_token():
                if not user_state.add_message(current_time, window_seconds, self.config.messages_per_minute):
    ...
    def _cleanup_expired_state(self):
        ...
        # Clean IP state
        expired_ips = [
            ip for ip, state in self._ip_state.items()
            if (state.connections == 0 and 
                state.first_connection and 
                current_time - state.first_connection > cleanup_threshold)
        ]
        ...
        # Clean user state
        expired_users = [
            user_id for user_id, state in self._user_state.items()
        ...
        # Clean tenant state
        expired_tenants = [
            tenant_id for tenant_id, state in self._tenant_state.items()
            if state.connections == 0 and len(state.message_times) == 0
        ]
        ...
        expired_sessions = [
            session_id for session_id in self._session_state.keys()
