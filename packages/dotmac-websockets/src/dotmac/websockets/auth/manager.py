...
class AuthManager:
    ...
    def __init__(self, config):
        ...
        self._token_cache: Dict[str, tuple[AuthResult, float]] = {}  # token -> (result, expire_time)
        ...
        try:
            ...
            user_info = UserInfo(
                ...
                extra_data={
                    ...
                    if k not in ["sub", "user_id", "username", "email", "tenant_id", "roles", "permissions", "exp", "iat", "iss"]
                }
            )
            ...
            if self.config.require_permissions:
                missing_perms = [
                    perm for perm in self.config.require_permissions
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        logger.warning(f"User resolver returned status {response.status} for user {user_id}")
    ...
    async def validate_websocket_auth(self, websocket, path: str) -> AuthResult:
        ...
        if not self.config.enabled:
            return AuthResult.success_result(
                UserInfo(user_id="anonymous", tenant_id="default")
            )
    ...
    def cleanup_cache(self):
        ...
        # Clean token cache
        expired_tokens = [
            token for token, (_, expire_time) in self._token_cache.items()
        ...
        # Clean user cache
        expired_users = [
            user_id for user_id, (_, expire_time) in self._user_cache.items()
            if expire_time < current_time
        ]
        ...
        if expired_tokens or expired_users:
            logger.debug(f"Cleaned up {len(expired_tokens)} expired tokens and {len(expired_users)} expired users")
