...
class HealthStatus(enum.Enum, timezone):
...
class TenantHealthCheck(TenantModel):
    ...
    check_timestamp = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc), index=True)
...
class MonitoringAlert(TenantModel):
    ...
    # Alert timing
    first_occurred = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
    last_occurred = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
    ...
    def should_escalate(self, escalation_threshold_minutes: int = 30) -> bool:
        ...
        minutes_active = (datetime.now(timezone.utc) - self.first_occurred).total_seconds() / 60
...
class TenantMetricsSnapshot(TenantModel):
    ...
    snapshot_timestamp = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc), index=True)
