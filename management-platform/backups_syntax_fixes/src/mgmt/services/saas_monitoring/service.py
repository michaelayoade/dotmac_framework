...
class SaaSMonitoringService:
    ...
    async def perform_tenant_health_check(self, tenant_id: str)
        ...
        try:
            ...
            # Generate check ID
            check_id = f"hc-{tenant_id}-{int(datetime.now(timezone.utc).timestamp()}"
    ...
    async def _correlate_health_status(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if mgmt_health and external_reports:
                ...
                mgmt_age = (datetime.now(timezone.utc) - mgmt_health.checked_at).total_seconds()
                external_ages = [(datetime.now(timezone.utc) - report["timestamp"]).total_seconds() 
    ...
    async def _generate_health_alert(self, tenant_id: str, health_check: TenantHealthCheck)
        ...
        try:
            ...
            # Create alert
            alert_id = f"health-{tenant_id}-{int(datetime.now(timezone.utc).timestamp()}"
    ...
    async def _handle_sla_violations(self, tenant_id: str, violations: List[Dict])
        ...
        try:
            for violation in violations:
                alert_id = f"sla-{tenant_id}-{violation['type']}-{int(datetime.now(timezone.utc).timestamp()}"
