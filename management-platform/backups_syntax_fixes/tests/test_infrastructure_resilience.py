...
class TestDatabaseResilience:
    ...
    async def test_database_connection_retry(self, db_session, timezone):
    ...
    async def test_circuit_breaker_pattern(self):
        ...
        class CircuitBreakerService:
            ...
            async def external_call(self):
                ...
                if self.is_open:
                    if (datetime.now(timezone.utc) - self.last_failure_time).seconds > self.recovery_timeout:
                        self.is_open = False
                        self.failure_count = 0
                    else:
...
class TestDisasterRecovery:
    ...
    async def test_data_recovery_process(self):
        ...
        age_hours = (datetime.now(timezone.utc).replace(tzinfo=backup_time.tzinfo) - backup_time).total_seconds() / 3600
