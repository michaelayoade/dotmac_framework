...
class SecurityValidator:
    ...
    def validate_production_config() -> Dict[str, Any]:
        ...
        if settings.is_production:
            ...
            # CORS checks
            localhost_origins = [origin for origin in settings.cors_origins 
                               if 'localhost' in origin or '127.0.0.1' in origin]
            if localhost_origins:
                warnings.append(f"CORS includes localhost origins: {localhost_origins}")
            
            # SSL/TLS checks
            if any('http://' in url for url in [settings.database_url, settings.redis_url] 
                   if url and not url.startswith('redis://')):  # Redis uses redis:// scheme
        ...
        for service, key in external_services.items():
            ...
            elif key and ('placeholder' in key.lower() or 'example' in key.lower() or 
                         'test' in key.lower() or len(key) < 10):
