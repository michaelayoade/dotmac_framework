...
class CommissionCalculator:
    ...
    def calculate_customer_commission(
        ...
        if not self._validate_tier_eligibility(partner.total_lifetime_revenue, tier):
            error_msg = (f"Partner not eligible for {tier.name} tier "
        ...
        self._add_audit(f"Base commission: ${base_amount:.2f} × {tier_multiplier} × {product_multiplier} = ${base_commission:.2f}")
    ...
    def calculate_batch_commissions(
        ...
        if not recalculate_all:
            ...
            customers_with_commissions = db.query(Commission.customer_id).filter(
                Commission.partner_id == partner_id,
                Commission.status == "pending"
            ).subquery()
        ...
        for customer in customers:
            try:
                ...
                is_new = (
                    customer.activated_at and
                    customer.activated_at >= datetime.utcnow() - timedelta(days=30)
                )
