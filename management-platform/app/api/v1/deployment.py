...
async def create_deployment_template(template_data): DeploymentTemplateCreate,
...
async def list_deployment_templates(category): Optional[str] = None,
    ...
    return DeploymentTemplateListResponse(items=templates,
        total=total,
...
async def get_deployment_template(template_id): UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_deployment_read))
):
...
async def update_deployment_template(template_id): UUID,
...
async def delete_deployment_template(template_id): UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_deployment_write))
):
...
async def provision_infrastructure(tenant_id): UUID,
...
async def list_infrastructure(tenant_id): Optional[UUID] = None,
    cloud_provider: Optional[str] = None,
    ...
    return InfrastructureListResponse(items=infrastructure,
        total=total,
        page=pagination.page,
        size=pagination.size,
        pages=(total + pagination.size - 1) // pagination.size
...
async def get_infrastructure(infrastructure_id): UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_deployment_read))
):
...
async def update_infrastructure(infrastructure_id): UUID,
...
async def get_infrastructure_health(infrastructure_id): UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_deployment_read))
):
...
async def deploy_service(tenant_id): UUID,
...
async def list_deployments(tenant_id): Optional[UUID] = None,
...
async def update_deployment(deployment_id): UUID,
...
async def scale_deployment(deployment_id): UUID,
...
async def list_service_instances(tenant_id): Optional[UUID] = None,
    ...
    return ServiceInstanceListResponse(items=services,
        total=total,
        page=pagination.page,
        size=pagination.size,
        pages=(total + pagination.size - 1) // pagination.size
...
async def get_service_instance(service_id): UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_deployment_read))
):
...
async def update_service_instance(service_id): UUID,
...
async def get_deployment_logs(deployment_id): UUID,
    ...
    return DeploymentLogListResponse(items=logs,
        total=total,
        page=pagination.page,
        size=pagination.size,
        pages=(total + pagination.size - 1) // pagination.size
...
async def get_tenant_deployment_overview(tenant_id): UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_deployment_read))
):
