...
async def create_billing_plan(plan_data): BillingPlanCreate,
...
async def get_billing_plan(plan_id): UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_billing_read))
):
...
async def update_billing_plan(plan_id): UUID,
...
async def delete_billing_plan(plan_id): UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_billing_write))
):
...
async def create_subscription(tenant_id): UUID,
    plan_id: UUID,
...
async def list_subscriptions(tenant_id): Optional[UUID] = None,
    ...
    return SubscriptionListResponse(items=subscriptions,
        total=total,
        page=pagination.page,
        size=pagination.size,
        pages=(total + pagination.size - 1) // pagination.size
...
async def list_invoices(tenant_id): Optional[UUID] = None,
    status: Optional[str] = None,
    pagination: PaginationParams = Depends(),
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_billing_read))
...
async def update_invoice(invoice_id): UUID,
...
async def list_payments(tenant_id): Optional[UUID] = None,
...
async def record_usage(usage_data): UsageRecordCreate,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_billing_write))
):
...
async def list_usage_records(tenant_id): Optional[UUID] = None,
    ...
    return UsageRecordListResponse(items=usage_records,
        total=total,
        page=pagination.page,
        size=pagination.size,
        pages=(total + pagination.size - 1) // pagination.size
...
async def get_billing_analytics(start_date): date = Query(..., description="Start date for analytics"),
...
async def get_tenant_billing_overview(tenant_id): UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_billing_read))
):
