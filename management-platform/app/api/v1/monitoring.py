...
async def health_check(
    include_details: bool = Query(default=True, description="Include detailed check information"),
    check_name: Optional[str] = Query(default=None, description="Run specific health check only")
):
    """
    ...
    try:
        ...
        if not metrics:
            ...
            return {
                ...
                "samples_count": 1,
                "cpu_usage_percent": {"current": current.cpu_usage_percent, "average": current.cpu_usage_percent},
...
async def comprehensive_status():
    ...
    try:
        ...
        logger.error("Comprehensive status endpoint failed", error=str(e), exc_info=True)
        raise HTTPException(
            status_code=500,
