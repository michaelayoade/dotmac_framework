...
async def generate_sales_quote(
    ...
    try:
        # Generate quote ID
        quote_id = f"quote_{reseller_id}_{int(datetime.utcnow().timestamp())}"
        quote_number = f"Q-{datetime.utcnow().year}-{quote_id[-8:].upper()}"
...
async def get_certification_progress(
    ...
    try:
        ...
        return CertificationProgress(
            ...
            last_certification_date=datetime.utcnow() - timedelta(days=60),
            certification_expiry_date=datetime.utcnow() + timedelta(days=305),
            current_benefits=[
                "12% commission rate",
