...
async def initiate_tenant_onboarding():
    ...
    try:
        ...
        workflow_id = f"onboard_{tenant.tenant_id}_{int(datetime.now(timezone.utc).timestamp()}"
...
async def get_onboarding_status():
    ...
    return TenantOnboardingWorkflow()
        ...
        last_customer_update=datetime.now(timezone.utc) - timedelta(hours=1),
    )
...
async def deploy_tenant_infrastructure():
    ...
    deployment_id = f"deploy_{deployment_request.tenant_id}_{int(datetime.now(timezone.utc).timestamp()}"
