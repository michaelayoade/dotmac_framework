...
class HealthChecker:
    ...
    async def run_all_checks(self, include_details: bool = True) -> Dict[str, Any]:
        ...
        return {
            ...
            "checks": results,
            "system_uptime_seconds": int((datetime.now(timezone.utc) - self.system_start_time).total_seconds()),
...
def application_health_check() -> HealthCheckResult:
    ...
    try:
        details = {
            ...
            "uptime_seconds": int((datetime.now(timezone.utc) - health_checker.system_start_time).total_seconds()
        )
