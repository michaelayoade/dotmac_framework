    ...
    async def _deploy_service():
        async with async_session() as db:
            try:
                ...
                service_instance = await service.service_repo.create(service_data, user_id, timezone)
...
def cleanup_old_deployments(self, retention_days: int = 30):
    ...
    async def _cleanup_deployments(, timezone):
        async with async_session() as db:
            try:
                service = DeploymentService(db)
...
def backup_deployment_configs(self, tenant_id: str = None):
    ...
    async def _backup_configs(, timezone):
        async with async_session() as db:
            try:
                service = DeploymentService(db)
...
async def _perform_deployment_backup(deployment, deployment_service) -> bool:
    ...
    try:
        ...
        backup_data = await _prepare_backup_data(deployment, deployment_service, timezone)
