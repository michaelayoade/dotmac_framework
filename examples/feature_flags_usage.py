...
from dotmac_shared.feature_flags.decorators import fastapi_context_extractor, set_global_manager
...
async def ab_testing_example():
    ...
    async with FeatureFlagClient(storage_type="memory") as client:
        ...
        await client.create_ab_test_flag(
            ...
            variants=[
                {"name": "control", "percentage": 50.0, "payload": {"flow_type": "original", "steps": 3}},
                {"name": "simplified", "percentage": 30.0, "payload": {"flow_type": "simplified", "steps": 2}},
                {
                    "name": "enhanced",
                    "percentage": 20.0,
...
def create_fastapi_app():
    ...
    async def startup():
        nonlocal client
        client = await FeatureFlagClient(storage_type="redis", environment="development").__aenter__()

        # Set global manager for decorators
        set_global_manager(client.manager)

        # Add middleware
        app.add_middleware(FeatureFlagMiddleware, manager=client.manager, context_extractor=fastapi_context_extractor)
...
async def manual_checks_example():
    ...
    async with FeatureFlagClient(storage_type="memory") as client:
        ...
        await client.create_user_list_flag(
            "beta_features", "Beta Features", user_list=["beta_user_1", "beta_user_2", "power_user_123"]
        )
