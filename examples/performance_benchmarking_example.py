...
async def example_2_system_metrics():
    ...
    for i in range(10):
        ...
        total = sum(x * x for x in range(work_amount))
...
async def example_4_database_benchmarking():
    ...
    try:
        ...
        test_queries = [
            ...
            ("insert_query", "INSERT INTO test_table (name, value) VALUES ('temp', 999); DELETE FROM test_table WHERE name = 'temp'", {})
        ]
...
async def example_5_regression_detection():
    ...
    for i in range(20):
        ...
        detector.add_benchmark_result(
            "api_test", "response_time", api_time, timestamp, "production"
        )
        detector.add_benchmark_result(
            "database_test", "query_time", database_time, timestamp, "production"
        )
...
async def example_6_comparison_analysis():
    ...
    # Perform comparison
    comparison = comparator.compare_benchmark_sets(
        baseline_results,
        current_results,
        "v1.0_baseline",
        "v1.1_current"
    ...
    if comparison.significant_changes:
        ...
        for change in comparison.significant_changes:
            ...
            print(f"   {change.metric_name}: {change.old_value:.1f} → {change.new_value:.1f} "
