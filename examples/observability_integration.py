...
from dotmac_shared.observability import setup_observability, get_logger, traced_db_operation
from dotmac_shared.database.session import get_async_db, create_async_database_engine
...
async def get_partner_customers(
    ...
    async def fetch_partner_customers(session: AsyncSession, pid: str):
        ...
        result = await session.execute(
            select(PartnerCustomer)
            .where(PartnerCustomer.partner_id == pid)
        )
...
async def record_business_metrics(
    ...
    with tracer.start_as_current_span("record_business_metrics") as span:
        ...
        if metrics_data.get("type") == "commission":
            commission_calculation_counter.add(
                1,
                {"partner_id": partner_id, "type": "manual_entry"}
            )
            ...
            partner_customer_counter.add(
                ...
                {
                    "partner_id": partner_id,
                    "event_type": metrics_data.get("event", "unknown")
                }
            )
