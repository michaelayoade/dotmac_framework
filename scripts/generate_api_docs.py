...
class APIDocumentationGenerator:
    def __init__(self, output_dir: str = None):
        ...
        self.output_dir = Path(output_dir) if output_dir else self.project_root / "docs" / "api"
    ...
    def fetch_openapi_spec(self, service_url: str, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                spec = response.json()
                self.logger.info(f"Successfully fetched OpenAPI spec for {service_name}")
                return spec
            else:
                self.logger.error(f"Failed to fetch OpenAPI spec: {response.status_code}")
                return {}
...
"""
            ...
            for endpoint in endpoints:
                ...
                if request_body:
                    ...
                    for content_type, schema_info in content.items():
                        ...
                        if schema:
                            markdown += "```json\n"
                            markdown += self.format_schema_example(schema, spec.get("components", {}))
                            markdown += "\n```\n\n"
                    ...
                    for status_code, response_info in responses.items():
                        ...
                        for content_type, schema_info in content.items():
                            ...
                            if schema:
                                ...
                                markdown += "  ```json\n"
                                markdown += "  " + self.format_schema_example(schema, spec.get("components", {})).replace("\n", "\n  ")
                                markdown += "\n  ```\n"
    ...
    def format_schema_example(self, schema: Dict[str, Any], components: Dict[str, Any]) -> str:
        ...
        try:
            if "$ref" in schema:
                ...
                if ref_path in components.get("schemas", {}):
                    return self.format_schema_example(components["schemas"][ref_path], components)
            ...
            if schema_type == "object":
                ...
                for prop_name, prop_def in properties.items():
                    if prop_def.get("type") == "string":
                        ...
                        example[prop_name] = [self.format_schema_example(items, components)]
                    else:
                ...
                return json.dumps([self.format_schema_example(items, components)], indent=2)
    ...
    def generate_postman_collection(self, spec: Dict[str, Any], service_name: str) -> Dict[str, Any]:
                    ...
                    if method.upper() in ["POST", "PUT", "PATCH"]:
                        ...
                        if request_body:
                            ...
                            if "application/json" in content:
                                ...
                                example_body = self.format_schema_example(schema, spec.get("components", {})
    ...
    def generate_documentation(self) -> bool:
        ...
        self.logger.info(f"✅ Generated: {successful_generations}/{len(services)} services")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate API documentation for DotMac Framework")
    parser.add_argument("--output-dir", help="Output directory for documentation", default=None)
