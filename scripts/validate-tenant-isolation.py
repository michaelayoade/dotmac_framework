...
class TenantIsolationValidator:
        ...
        try:
            ...
            tenant_info = {
                ...
                "containers": {
                    ...
                    "redis": {
                        "id": redis_container.id,
                        "name": redis_container.name
                    },
                    "isp_framework": {
                        "id": isp_container.id,
    ...
    async def _wait_for_postgres(self, container, tenant_id: str, max_attempts: int = 30) -> None:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                result = container.exec_run(
                    f"pg_isready -U tenant_{tenant_id} -d dotmac_{tenant_id}",
                    timeout=5
                )
    ...
    async def validate_network_isolation(self) -> Dict[str, Any]:
        ...
        if len(self.test_tenants) < 2:
            ...
            results["issues"].append("Need at least 2 tenants to test network isolation")
            return results
        ...
        try:
            ...
            container1 = self.docker_client.containers.get(tenant1["containers"]["isp_framework"]["id"])
            ...
            if result.exit_code == 0:
                ...
                results["issues"].append(f"Tenant {tenant1['tenant_id']} can access {tenant2['tenant_id']}'s database")
            else:
                results["tests"].append(f"✓ Tenant {tenant1['tenant_id']} cannot access {tenant2['tenant_id']}'s database")
            ...
            if result.exit_code == 0:
                results["status"] = "failed" 
                results["issues"].append(f"Tenant {tenant2['tenant_id']} can access {tenant1['tenant_id']}'s cache")
            else:
                results["tests"].append(f"✓ Tenant {tenant2['tenant_id']} cannot access {tenant1['tenant_id']}'s cache")
        ...
        try:
            for tenant_info in self.test_tenants:
                ...
                result = postgres_container.exec_run([
                    "psql",
                    f"-U", f"tenant_{tenant_id}",
                    f"-d", f"dotmac_{tenant_id}",
                    "-c", f"CREATE TABLE test_isolation (data TEXT); INSERT INTO test_isolation VALUES ('{test_data}');"
                ])
                
                if result.exit_code == 0:
                    results["tests"].append(f"✓ Created isolated test data for tenant {tenant_id}")
                    ...
                    result = postgres_container.exec_run([
                        ...
                        f"-U", f"tenant_{tenant_id}",
                        f"-d", f"dotmac_{tenant_id}",
                        "-t", "-c", "SELECT data FROM test_isolation;"
                    ])
                    
                    if test_data in result.output.decode():
                        results["tests"].append(f"✓ Verified data isolation for tenant {tenant_id}")
                    else:
                        results["status"] = "failed"
                        results["issues"].append(f"Data verification failed for tenant {tenant_id}")
                else:
                    results["status"] = "failed"
                    results["issues"].append(f"Failed to create test data for tenant {tenant_id}")
                ...
                result = redis_container.exec_run([
                    "redis-cli",
                    "-a", f"password_{tenant_id}",
                    "SET", redis_test_key, redis_test_value
                ])
                ...
                if result.exit_code == 0:
                    ...
                    if redis_test_value in result.output.decode():
                        results["tests"].append(f"✓ Verified Redis data isolation for tenant {tenant_id}")
                    else:
                        results["status"] = "failed"
                        results["issues"].append(f"Redis data verification failed for tenant {tenant_id}")
                else:
                    results["status"] = "failed"
                    results["issues"].append(f"Failed to set Redis test data for tenant {tenant_id}")
            ...
            for tenant_info in self.test_tenants:
                ...
                for service, container_info in tenant_info["containers"].items():
                    ...
                    if hasattr(container.attrs['HostConfig'], 'Memory'):
                        ...
                        if memory_limit > 0:
                            results["tests"].append(f"✓ Memory limit set for {service} in tenant {tenant_id}: {memory_limit} bytes")
                        else:
                            results["issues"].append(f"No memory limit set for {service} in tenant {tenant_id}")
                    ...
                    if expected_network in networks and len(networks) == 1:
                        results["tests"].append(f"✓ Network isolation verified for {service} in tenant {tenant_id}")
                    else:
                        results["status"] = "failed"
                        results["issues"].append(f"Network isolation failed for {service} in tenant {tenant_id}: {networks}")
            ...
            for tenant_info in self.test_tenants:
                ...
                for service, container_info in tenant_info["containers"].items():
                    ...
                    if result.exit_code == 0 and "root" in result.output.decode():
                        ...
                        results["tests"].append(f"✓ Container {service} in tenant {tenant_id} security checked")
                    
                    # Check security options
                    security_opt = container.attrs['HostConfig'].get('SecurityOpt', [])
                    if 'no-new-privileges:true' in security_opt:
                        results["tests"].append(f"✓ No-new-privileges enabled for {service} in tenant {tenant_id}")
                    ...
                    if service == "isp_framework" and not readonly_rootfs:
                        results["issues"].append(f"Read-only root filesystem not enabled for {service} in tenant {tenant_id}")
                    
                    # Check if privileged
                    privileged = container.attrs['HostConfig'].get('Privileged', False)
                    if privileged:
                        results["status"] = "failed"
                        results["issues"].append(f"Container {service} in tenant {tenant_id} is running in privileged mode")
                    else:
                        results["tests"].append(f"✓ Container {service} in tenant {tenant_id} is not privileged")
    ...
    async def cleanup_test_resources(self) -> None:
        ...
        logger.info("Note: Tenant volumes preserved for inspection. Clean manually if needed.")
    ...
    async def run_full_validation(self, num_tenants: int = 2) -> Dict[str, Any]:
        ...
        logger.info(f"Starting tenant isolation validation with {num_tenants} test tenants")
        ...
        try:
            ...
            if network_results["status"] != "passed":
                ...
                validation_results["summary"]["issues"].extend(network_results["issues"])
            else:
            ...
            if resource_results["status"] != "passed":
                validation_results["validation_status"] = "failed"
                validation_results["summary"]["failed"] += 1
                validation_results["summary"]["issues"].extend(resource_results["issues"])
            else:
                validation_results["summary"]["passed"] += 1
            ...
            if security_results["status"] != "passed":
                validation_results["validation_status"] = "failed"
                validation_results["summary"]["failed"] += 1
                validation_results["summary"]["issues"].extend(security_results["issues"])
            else:
                validation_results["summary"]["passed"] += 1
...
async def main():
    ...
    parser.add_argument("--tenants", type=int, default=2, help="Number of test tenants to create")
    parser.add_argument("--output", help="Output file for results (JSON)")
    parser.add_argument("--cleanup-only", action="store_true", help="Only cleanup existing test resources")
    ...
    for test_name, test_results in results["tests"].items():
        print(f"{test_name.replace('_', ' ').title()}: {test_results['status'].upper()}")
