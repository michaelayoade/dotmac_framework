...
def check_environment_variables() -> dict[str, Any]:
    ...
    results = {"status": "passed", "checks": [], "errors": []}
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                if data.get("data", {}).get("bootstrap_credentials_present", True):
                    return {"status": "failed", "message": "❌ Bootstrap credentials still present - security risk!"}
                else:
                    return {"status": "passed", "message": "✅ Bootstrap credentials properly removed"}
            else:
                return {"status": "unknown", "message": "Could not check bootstrap status (authentication required)"}

    except Exception as e:
        return {"status": "error", "message": f"Bootstrap check failed: {e}"}
...
async def check_coolify_connectivity() -> dict[str, Any]:
    ...
    try:
        ...
        if not coolify_url or not coolify_token:
            return {"status": "failed", "message": "❌ Coolify configuration missing"}

        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{coolify_url}/api/v1/servers", headers={"Authorization": f"Bearer {coolify_token}"}, timeout=10
            )

            if response.status_code == 200:
                servers = response.json()
                return {"status": "passed", "message": f"✅ Coolify connected ({len(servers)} servers available)"}
            else:
                return {"status": "failed", "message": f"❌ Coolify API error: {response.status_code}"}

    except Exception as e:
        return {"status": "error", "message": f"Coolify check failed: {e}"}


def check_security_configuration() -> dict[str, Any]:
    """Check security configuration"""

    results = {"status": "passed", "checks": [], "warnings": []}
