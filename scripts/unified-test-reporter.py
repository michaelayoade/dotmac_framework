...
<body>
    ...
    def print_summary(self, report: UnifiedTestReport):
        ...
        if report.coverage_percentage is not None:
            coverage_emoji = (
                "🎯" if report.coverage_percentage >= 80 else "⚠️" if report.coverage_percentage >= 70 else "🔴"
            )
        ...
        for suite in report.suites:
            ...
            print(
                f"{status_emoji} {suite.name}: {suite.passed}/{suite.total} ({suite_rate:.1f}%) in {suite.duration:.1f}s"
            )
