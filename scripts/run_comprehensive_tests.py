...
class ComprehensiveTestRunner:
    ...
    def __init__(self, args):
        ...
        self.test_suites = {
            ...
            "security": TestSuiteConfig(
                name="Security Tests",
                command=["python", "-m", "pytest", "tests/unit/security", "-v", "--tb=short", "-m", "security"],
                timeout=300,
            ...
            "e2e": TestSuiteConfig(
                ...
                command=["python", "-m", "pytest", "tests/e2e", "-v", "--tb=long", "-m", "e2e"],
            ...
            "performance": TestSuiteConfig(
                ...
                command=["python", "-m", "pytest", "tests/performance", "-v", "--benchmark-json=test-reports/benchmark.json"],
    ...
    def _filter_test_suites(self) -> List[str]:
        ...
        if self.args.quick:
            # Quick test mode - only unit and integration
    ...
    def _print_suite_summary(self, result: TestResult):
        ...
        print(f"  {status}")
        print(f"  Tests: {result.passed} passed, {result.failed} failed, {result.errors} errors, {result.skipped} skipped")
    ...
    def _generate_final_report(self):
        ...
        for suite_name, result in self.results.items():
            ...
            print(f"    Coverage: {coverage_status} {result.coverage_percent:.1f}% (target: {config.coverage_target:.1f}%)")
            print(f"    Time: {result.execution_time:.1f}s")
    ...
    def _run_coverage_analysis(self):
        ...
        try:
            # Import and run our comprehensive coverage system
            sys.path.append("tests/coverage")
            from test_coverage_config import CoverageIntegration
...
def main():
    ...
    parser.add_argument(
        "--suites", 
        help="Comma-separated list of test suites to run (unit,integration,security,e2e,performance)"
    )
    parser.add_argument(
        "--quick", 
        action="store_true", 
    ...
    parser.add_argument(
        "--no-capture", "-s", 
        action="store_true", 
        help="Don't capture output (for debugging)"
    )
    parser.add_argument(
        "--no-parallel", 
        action="store_true", 
