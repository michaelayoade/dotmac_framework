...
class VPSValidator:
    ...
    def __init__(self, vps_ip: str, ssh_port: int = 22, ssh_user: str = "root", ssh_key: Optional[str] = None):
        self.vps_ip = vps_ip
    ...
    def log_result(self, result: ValidationResult):
        ...
        # Color output
        colors = {"pass": "\033[92m✅", "fail": "\033[91m❌", "warning": "\033[93m⚠️", "skip": "\033[94mℹ️"}
        reset = "\033[0m"
    ...
    async def validate_ssh_connectivity(self) -> ValidationResult:
        ...
        try:
            ...
            connect_kwargs = {"hostname": self.vps_ip, "port": self.ssh_port, "username": self.ssh_user, "timeout": 10}
            ...
            return ValidationResult(
                name="SSH Connectivity", status="fail", message="SSH connection failed", details={"error": str(e)}
            )
    ...
    async def validate_system_requirements(self) -> list[ValidationResult]:
        ...
        try:
            ...
            connect_kwargs = {"hostname": self.vps_ip, "port": self.ssh_port, "username": self.ssh_user, "timeout": 30}
            ...
            if any(os_name in os_info for os_name in supported_os):
                ...
                results.append(
                    ValidationResult(
                        ...
                        details={"detected_os": specs.get("os_info"), "supported_os": supported_os},
                    )
                )
    ...
    async def validate_docker_installation(self) -> ValidationResult:
        ...
        try:
            ...
            connect_kwargs = {"hostname": self.vps_ip, "port": self.ssh_port, "username": self.ssh_user, "timeout": 15}
...
async def main():
    ...
    # Create validator
    validator = VPSValidator(vps_ip=args.vps_ip, ssh_port=args.ssh_port, ssh_user=args.ssh_user, ssh_key=args.ssh_key)
