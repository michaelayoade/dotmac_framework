...
import { Award, Bell, ChevronDown, HelpCircle, LogOut, Settings, User } from 'lucide-react';
        ...
        <div className='relative'>
          ...
          >
            ...
            <div className='hidden text-left sm:block'>
              <div className='font-medium text-gray-900 text-sm'>{user?.name || 'Partner'}</div>
              <div className='text-gray-500 text-xs'>{user?.company || 'Partner Account'}</div>
            </div>
          ...
          {showUserMenu ? (
            <div className='absolute right-0 z-50 mt-2 w-56 rounded-md border border-gray-200 bg-white py-1 shadow-lg'>
              <div className='border-gray-100 border-b px-4 py-2'>
                <p className='font-medium text-gray-900 text-sm'>{user?.name || 'Partner'}</p>
                <p className='text-gray-500 text-xs'>{user?.email || 'partner@example.com'}</p>
                <p className='font-medium text-green-600 text-xs'>Gold Partner • TS001</p>
              </div>
