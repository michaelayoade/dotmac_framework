 ...
 * Centralizes common UI and application state patterns with real data persistence
 */
...
import type { NetworkDevice, NetworkIncident } from '../hooks/useNetworkOperations';
import type { Invoice, BillingAnalytics } from '../hooks/useBillingOperations';
import type { SupportTicket } from '../hooks/useISPOperations';
...
// Enhanced data cache interfaces
...
interface AppState {
  ...
  // Enhanced data caching with versioning
  cache: {
    customers: DataCache<Customer>;
    devices: DataCache<NetworkDevice>;
    invoices: DataCache<Invoice>;
    tickets: DataCache<SupportTicket>;
    incidents: DataCache<NetworkIncident>;
  };
  ...
  // Enhanced app-level preferences
  preferences: {
