...
class NetworkDeviceFactory(BaseFactory, TenantMixin, TimestampMixin, AuditMixin):
    ...
    last_seen = factory.LazyFunction(lambda: datetime.now(timezone.utc) - timedelta(minutes=fake.random_int(1, 60)
    ...
    def create_offline_device(cls, **kwargs):
        ...
        device_data.update({
            'status': 'offline',
            'last_seen': datetime.now(timezone.utc) - timedelta(hours=fake.random_int(1, 48),
            'cpu_utilization': 0,
...
class NetworkMonitoringEventFactory(BaseFactory, TenantMixin, TimestampMixin):
    ...
    # Event timing
    event_time = factory.LazyFunction(lambda: datetime.now(timezone.utc) - timedelta(minutes=fake.random_int(1, 1440)
    acknowledged = False
    ...
    def create_resolved_event(cls, **kwargs):
        ...
        event_data = kwargs.copy()
        event_time = datetime.now(timezone.utc) - timedelta(hours=fake.random_int(1, 24)
        event_data.update({
...
class NetworkTopologyFactory(BaseFactory, TenantMixin, TimestampMixin):
    ...
    discovery_date = factory.LazyFunction(lambda: datetime.now(timezone.utc) - timedelta(days=fake.random_int(1, 365)
    last_verified = factory.LazyFunction(lambda: datetime.now(timezone.utc) - timedelta(days=fake.random_int(1, 30)
