...
class MemoryAdapter(EventAdapter):
    ...
    def __init__(self, config: MemoryConfig):
        ...
        # Storage
        self._topics: Dict[str, Dict[int, deque]] = defaultdict(
    ...
    async def publish(
        ...
        if topic in self._subscribers:
            consumer_record = ConsumerRecord(
                event=event, offset=message_id, partition=partition, topic=topic
            )
                    ...
                    for i, message in enumerate(messages):
                        if int(message["message_id"]) > offset:
                            ...
                            try:
                                ...
                                logger.warning(
                                    "Subscriber queue full during historical replay"
                                )
                                break

        logger.info(
            "Started memory consumer", topics=topics, consumer_group=consumer_group
        )
    ...
    async def get_topic_info(self, topic: str) -> Dict[str, Any]:
        ...
        if topic in self._topics:
            message_count = sum(
                len(partition) for partition in self._topics[topic].values()
            )
    ...
    async def seek_to_end(
        ...
        if partition is not None:
            ...
            self._consumer_offsets[consumer_group][topic][partition] = int(
                latest_offset
            )
        # Seek all partitions
        elif topic in self._topic_configs:
