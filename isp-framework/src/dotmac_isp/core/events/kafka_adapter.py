...
class KafkaAdapter(EventAdapter):
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Connected to Kafka", bootstrap_servers=self.config.bootstrap_servers
            )
    ...
    async def publish(
        ...
        try:
            ...
            record_metadata = await self._producer.send_and_wait(
                topic, value=event_data, key=key_bytes
            )
    ...
    async def list_consumer_groups(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for group in groups:
                try:
                    group_info = await self._admin_client.describe_consumer_groups(
                        [group.group_id]
                    )
    ...
    async def delete_consumer_group(self, group_id: str) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to delete Kafka consumer group", group_id=group_id, error=str(e)
            )
            raise
