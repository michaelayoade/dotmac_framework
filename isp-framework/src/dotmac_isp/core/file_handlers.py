...
class PDFGenerator:
    ...
    def generate_invoice_pdf(
        ...
        """
        if not output_path:
            output_path = f"/tmp/invoice_{invoice_data['invoice_number']}_{uuid.uuid4()}.pdf"
...
async def export_data_to_csv(data: List[Dict[str, Any]], columns: List[str] = None) -> str:
    """Export data to CSV and return file path."""
    csv_exporter = CSVExporter()
    return csv_exporter.export_to_csv(data, columns=columns)


async def export_data_to_excel(data: List[Dict[str, Any]], columns: List[str] = None) -> str:
