...
class BaseService(Generic[ModelType, CreateSchemaType, UpdateSchemaType, ResponseSchemaType], ABC):
    ...
    async def create(self, data: CreateSchemaType, commit: bool = True) -> ResponseSchemaType:
        ...
        try:
            ...
            self._logger.error(f"Validation error creating {self.model_class.__name__}: {e}")
    ...
    async def update(
        ...
        try:
            ...
            self._logger.error(f"Validation error updating {self.model_class.__name__}: {e}")
            raise ValidationError(f"Invalid input data: {e}")
    ...
    async def list(
        ...
        try:
            ...
            raise ServiceError(f"Failed to list entities: {e}")
...
class BaseTenantService(BaseService[ModelType, CreateSchemaType, UpdateSchemaType, ResponseSchemaType]):
    ...
    async def get_tenant_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ServiceError(f"Failed to get tenant statistics: {e}")
