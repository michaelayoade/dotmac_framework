...
from ..contracts.common_schemas import Priority, ErrorInfo
...
class JobMessage:
    ...
    def calculate_next_retry_delay(self) -> float:
        ...
        delay = self.initial_delay * (
            self.backoff_multiplier ** (self.attempt_count - 1)
        )
    ...
    def make_invisible(self, visibility_timeout_seconds: int):
        ...
        self.visibility_timeout = datetime.now(timezone.utc) + timedelta(
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "visibility_timeout": (
                self.visibility_timeout.isoformat() if self.visibility_timeout else None
            ),
...
class PriorityQueue:
    ...
    def enqueue(self, job: JobMessage):
        ...
        if job.ordering_key:
            ...
            heap_item = (-job.scheduled_at.timestamp(), job.job_id, job)  # Tie breaker
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "jobs_by_priority": {
                p.value: count for p, count in self.jobs_by_priority.items()
            },
            "ordering_queues": len(self.ordering_queues),
            "priority_counters": {
                p.value: count for p, count in self.priority_counters.items()
            },
        }
...
class EnhancedJobQueue:
    ...
    async def _job_processor(self):
        ...
        while self.running:
            try:
                ...
                logger.error("Job processor error", queue_name=self.name, error=str(e))
        ...
        try:
            ...
            if self.storage_adapter:
                await self.storage_adapter.update_job_status(
                    job.job_id, JobStatus.COMPLETED, result
                )
        ...
        while self.running:
            try:
                ...
                for job_id, job in self.processing_jobs.items():
                    if (
                        job.visibility_timeout
                        and current_time >= job.visibility_timeout
                    ):
                ...
                for job in expired_jobs:
                    logger.warning(
                        ...
                        queue_name=self.name,
                        processing_time=(
                            current_time - job.visibility_timeout
                        ).total_seconds(),
                    )
                ...
                logger.error(
                    "Visibility monitor error", queue_name=self.name, error=str(e)
                )
    ...
    def register_handler(self, handler_name: str, handler: Callable):
        ...
        logger.info(
            "Job handler registered", handler_name=handler_name, queue_name=self.name
        )
