...
class TariffService:
    ...
    async def calculate_pricing(
        ...
        calculation["total_usage_charges"] = float(
            sum(
                Decimal(str(charge["amount"]))
                for charge in calculation["usage_charges"]
            )
        )
        ...
        calculation["total_discounts"] = sum(
    ...
    async def _apply_pricing_rule(
        self, rule: Dict[str, Any], usage_data: Dict[str, Any]
    ...
    def _calculate_tiered_pricing(
        self, rule: Dict[str, Any], usage_amount: float
    ) -> Decimal:
    ...
    def _calculate_volume_discount_pricing(
        self, rule: Dict[str, Any], usage_amount: float
    ) -> Decimal:
        ...
        for volume_break in sorted(
            volume_breaks, key=lambda x: x["threshold"], reverse=True
        ):
    ...
    def _calculate_time_based_pricing(
        ...
        for period in time_periods:
            period_usage = usage_data.get(
                f"{rule['unit_type'].lower()}_{period['period']}", 0
            )
            if period_usage > 0:
    ...
    async def _calculate_discounts(
        self, plan: Dict[str, Any], subtotal: Decimal, usage_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
                    ...
                    discounts.append(
                        {
                            ...
                            "percentage": (
                                float((discount_amount / subtotal) * 100)
                                if subtotal > 0
                                else 0
                            ),
                        }
    ...
    async def _calculate_taxes(
        self, plan: Dict[str, Any], taxable_amount: Decimal
    ) -> List[Dict[str, Any]]:
            ...
            if template_id in self._policy_templates:
                ...
                intent = {
                    ...
                    "parameters": self._resolve_policy_parameters(
                        template["parameters"], plan, customer_profile
                    ),
        ...
        for param_name, param_config in template_parameters.items():
            if isinstance(param_config, dict) and "source" in param_config:
                ...
                if source == "plan":
                    ...
                    resolved_params[param_name] = customer_profile.get(
                        param_config["field"]
                    )
                elif source == "static":
