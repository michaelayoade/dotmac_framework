    ...
    async def create_organization(self, **kwargs) -> Organization:
        ...
        if organization.name in self._name_index:
            raise OrganizationError(
                f"Organization name already exists: {organization.name}"
            )
...
class OrganizationsSDK:
    ...
    async def create_organization(
        ...
        **kwargs,
        ...
        return {
            ...
            "billing_owner_id": (
                str(organization.billing_owner_id)
                if organization.billing_owner_id
                else None
            ),
    ...
    async def get_organization(self, organization_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "primary_contact_id": (
                str(organization.primary_contact_id)
                if organization.primary_contact_id
                else None
            ),
            "billing_contact_id": (
                str(organization.billing_contact_id)
                if organization.billing_contact_id
                else None
            ),
            "billing_owner_id": (
                str(organization.billing_owner_id)
