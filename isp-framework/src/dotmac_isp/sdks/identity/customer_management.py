...
from ..events import EventBusSDK, EventMetadata
# CustomerService imported lazily to avoid circular imports
...
class CustomerManagementSDK:
    ...
    def _get_service(self):
        """Lazy-load the customer service to avoid circular imports."""
        if self._service is None:
            from ...modules.identity.services.customer_service import CustomerService
    ...
    async def create_customer(
        ...
        **kwargs,
        ...
        customer = await self._get_service().create_customer(
    ...
    async def get_customer(self, customer_id: str) -> Optional[Dict[str, Any]]:
        """Get customer by ID."""
        customer = await self._get_service().get_customer(UUID(customer_id))
        ...
        return {
            ...
            "organization_id": (
                str(customer.organization_id) if customer.organization_id else None
            ),
            "primary_contact_id": (
                str(customer.primary_contact_id)
                if customer.primary_contact_id
                else None
            ),
            "billing_contact_id": (
                str(customer.billing_contact_id)
                if customer.billing_contact_id
                else None
            ),
            "prospect_date": (
                customer.prospect_date.isoformat() if customer.prospect_date else None
            ),
            "activation_date": (
                customer.activation_date.isoformat()
                if customer.activation_date
                else None
            ),
            "churn_date": (
                customer.churn_date.isoformat() if customer.churn_date else None
            ),
    ...
    async def get_customer_by_number(
        ...
        customer = await self._get_service().get_customer_by_number(customer_number)
        if not customer or customer.tenant_id != self.tenant_id:
            return None
    ...
    async def update_customer(self, customer_id: str, **updates) -> Dict[str, Any]:
        """Update customer."""
        customer = await self._get_service().update_customer(UUID(customer_id), **updates)
    ...
    async def transition_to_prospect(
        self, customer_id: str, changed_by: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def list_customers(
        ...
        customers = await self._get_service().list_customers(
            tenant_id=self.tenant_id,
    ...
    async def _transition_customer_state(
        ...
        customer = await self._get_service().transition_customer_state(
            UUID(customer_id), new_state, UUID(changed_by) if changed_by else None
        )
    ...
    async def _emit_customer_event(self, event_name: str, customer: Customer) -> None:
        """Emit customer lifecycle event using DotMac Core Events."""
        event_data = {
            "event": event_name,
            "customer_id": str(customer.id),
        ...
        # Create event metadata for tracing and correlation
        metadata = EventMetadata(
            source="identity.customer_management",
            correlation_id=None,  # Could be set from request context
            user_id=None,  # Could be set from authentication context
        )

        try:
            # Publish event to the event bus
            stream = f"customer.{event_name}"
            result = await self._event_bus.publish_event(
                stream=stream,
                event_type=f"customer.{event_name}",
                data=event_data,
                metadata=metadata
            )
            
            self._logger.info(
                "Customer event published successfully",
            ...
            # Log error but don't fail the operation - events are async
            self._logger.error(
            ...
            # Fallback: at least log the event data
            self._logger.info("Customer Event (fallback)", **event_data)
