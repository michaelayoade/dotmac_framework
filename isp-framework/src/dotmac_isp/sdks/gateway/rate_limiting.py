...
class RateLimitingService:
    ...
    async def _check_token_bucket(
        self, policy: Dict[str, Any], identifier: str
    ) -> Dict[str, Any]:
    ...
    async def _check_sliding_window(
        ...
        if key not in self._rate_limit_counters:
            self._rate_limit_counters[key] = {"requests": [], "requests_count": 0}
        ...
        if len(counter["requests"]) < requests_per_minute:
            ...
            return {
                ...
                "requests_count": counter["requests_count"],
                "reset_time": (
                    int(counter["requests"][0] + window_size)
                    if counter["requests"]
                    else None
                ),
            }
            ...
            raise RateLimitError(
                f"Rate limit exceeded for {identifier}. Reset at {reset_time}"
            )
