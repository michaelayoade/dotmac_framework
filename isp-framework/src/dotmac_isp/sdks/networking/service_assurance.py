...
class ServiceAssuranceService:
    ...
    async def execute_dns_probe(self, probe_id: str) -> Dict[str, Any]:
        ...
        response_time = (
            (secrets.randbelow(9500) + 500) / 100 if success else None
        )  # 5-100 range
    ...
    async def execute_http_probe(self, probe_id: str) -> Dict[str, Any]:
        ...
        success = secrets.randbelow(4) < 3  # 75% success rate
        response_time = (
            (secrets.randbelow(4500) + 500) / 10 if success else None
        )  # 50-500 range
        status_codes = [200, 200, 200, 404, 500]
        status_code = (
            status_codes[secrets.randbelow(len(status_codes))] if success else None
        )
    ...
    async def check_sla_compliance(self, probe_id: str) -> Dict[str, Any]:
        ...
        if successful_results:
            latency_values = [
                r.get("rtt_ms") or r.get("response_time_ms", 0)
        ...
        compliance_status = (
            "compliant"
            if (availability_compliant and latency_compliant)
            else "violation"
        )
...
class ServiceAssuranceSDK:
    ...
    async def execute_probe(self, probe_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                if k not in ["result_id", "probe_id", "timestamp", "success", "error"]
            },
            "error": result["error"],
        }
    ...
    async def check_sla_compliance(self, probe_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "availability": {
                ...
                "compliant": (
                    compliance.get("availability_actual", 0)
                    >= compliance.get("availability_threshold", 100)
                    if compliance.get("availability_actual") is not None
                    else None
                ),
            },
            "latency": {
                ...
                "compliant": (
                    compliance.get("latency_actual", float("inf"))
                    <= compliance.get("latency_threshold_ms", 0)
                    if compliance.get("latency_actual") is not None
                    else None
                ),
            },
    ...
    async def get_sla_violations(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        recent_violations = [
            violation
            for violation in self._service._sla_violations
        ...
        return [
            ...
            for violation in sorted(
                recent_violations, key=lambda v: v["detected_at"], reverse=True
            )
        ]
