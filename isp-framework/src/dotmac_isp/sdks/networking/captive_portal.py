...
class CaptivePortalService:
    ...
    async def create_hotspot(self, **kwargs) -> Dict[str, Any]:
        ...
        hotspot = {
            ...
            "auth_method": kwargs.get(
                "auth_method", "radius"
            ),  # radius, voucher, free, social
            "radius_server": kwargs.get("radius_server", ""),
            "captive_portal_url": kwargs.get(
                "captive_portal_url", f"http://portal.local/{hotspot_id}"
            ),
    ...
    async def create_portal_user(self, **kwargs) -> Dict[str, Any]:
        ...
        user = {
            ...
            "auth_method": kwargs.get(
                "auth_method", "email"
            ),  # email, sms, social, voucher
            "social_provider": kwargs.get(
                "social_provider", ""
    ...
    async def authenticate_portal_user(self, **kwargs) -> Dict[str, Any]:
        ...
        for portal_user in self._portal_users.values():
            if (
                (username and portal_user.get("username") == username)
                or (email and portal_user.get("email") == email)
                or (phone_number and portal_user.get("phone_number") == phone_number)
            ):
                user = portal_user
                break
        ...
        if user.get("valid_until"):
            valid_until = datetime.fromisoformat(
                user["valid_until"].replace("Z", "+00:00")
            )
...
class CaptivePortalSDK:
        ...
        **kwargs,
        ...
        if auth_result["authenticated"]:
            session = await self._service.create_user_session(
                user_id=auth_result["user_id"], hotspot_id=hotspot_id, **kwargs
            )
    ...
    async def get_active_sessions(
        self, hotspot_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """Get active user sessions."""
        sessions = [
            session
            for session in self._service._user_sessions.values()
            if session["status"] == "active"
            and (not hotspot_id or session["hotspot_id"] == hotspot_id)
        ]
