...
class AlarmEventsService:
        ...
        for rule in self._alarm_rules.values():
            ...
            for key, pattern in match_criteria.items():
                ...
                if (
                    ...
                    and not re.search(pattern, str(event[key]))
                    or not isinstance(pattern, str)
                    and event[key] != pattern
                ):
                    matches = False
                    break
    ...
    async def _create_alarm_from_event(
    ...
    def _get_recent_alarms_for_device(
        self, device_id: str, minutes: int = 5
    ) -> List[Dict[str, Any]]:
    ...
    def _is_alarm_suppressed(self, device_id: str, alarm_type: str) -> bool:
        ...
        for suppression in self._suppressions.values():
            ...
            if (
                suppression.get("device_id") == device_id
                or suppression.get("device_id") == "*"
            ):
                if (
                    suppression.get("alarm_type") == alarm_type
                    or suppression.get("alarm_type") == "*"
                ):
...
class AlarmEventsSDK:
    ...
    async def get_active_alarms(
        ...
        alarms = [
            alarm
            for alarm in self._service._alarms.values()
    ...
    async def get_alarm_statistics(
        ...
        acknowledged_alarms = [
            alarm for alarm in active_alarms if alarm["acknowledged"]
        ]
