...
class RADIUSSDK:
    ...
    async def start_session(
        self, username: str, session_id: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
        """Start RADIUS accounting session."""
        record = await self._service.start_accounting(
            username=username, session_id=session_id, **kwargs
        )
    ...
    async def disconnect_user(
        self, username: str, reason: str = "Admin-Disconnect"
    ) -> Dict[str, Any]:
        ...
        # Send CoA disconnect
        result = await self._service.send_coa(
            username=username, session_id=session_id, coa_type="Disconnect-Request"
        )

        # Stop accounting
        await self._service.stop_accounting(
            username=username, session_id=session_id, terminate_cause=reason
        )
