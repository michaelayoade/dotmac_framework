...
class DeviceProvisioningService:
    ...
    async def execute_activity(
        self, workflow_id: str, activity_name: str, **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            if activity_name == "allocate_ip":
                activity["output"] = await self._allocate_ip_activity(
                    **kwargs.get("parameters", {})
                )
            elif activity_name == "assign_vlan":
                activity["output"] = await self._assign_vlan_activity(
                    **kwargs.get("parameters", {})
                )
            elif activity_name == "configure_device":
                activity["output"] = await self._configure_device_activity(
                    **kwargs.get("parameters", {})
                )
            elif activity_name == "update_radius":
                activity["output"] = await self._update_radius_activity(
                    **kwargs.get("parameters", {})
                )
            elif activity_name == "validate_connectivity":
                activity["output"] = await self._validate_connectivity_activity(
                    **kwargs.get("parameters", {})
                )
            else:
        ...
        try:
            for activity_config in workflow["activities"]:
                ...
                if activity_result["status"] == "failed":
                    ...
                    workflow["error"] = (
                        f"Activity {activity_config['name']} failed: {activity_result['error']}"
                    )
                    break
...
class DeviceProvisioningSDK:
        ...
        default_activities = [
            {
                "name": "allocate_ip",
                "type": "ipam",
                "parameters": {"network_id": "customer_network"},
            },
            ...
            },
            {
                "name": "update_radius",
                "type": "radius",
                "parameters": {"username": customer_id},
            },
            {
    ...
    async def retry_failed_workflow(self, workflow_id: str) -> Dict[str, Any]:
        ...
        if workflow["retry_count"] >= workflow["max_retries"]:
            raise NetworkingError(
                f"Maximum retries exceeded for workflow: {workflow_id}"
            )
    ...
    async def list_workflows(
        ...
        return [
            ...
            for workflow in sorted(
                workflows, key=lambda w: w["created_at"], reverse=True
            )
        ]
        ...
        activities = [
            {
                ...
                "type": "ipam",
                "parameters": {
                    "network_id": service_parameters.get(
                        "network_id", "customer_network"
                    ),
                    "customer_id": customer_id,
                },
            },
