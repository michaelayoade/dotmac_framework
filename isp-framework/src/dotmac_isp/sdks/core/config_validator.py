...
from pydantic import BaseSettings, Field, field_validator, ValidationError
...
class BaseServiceConfig(BaseSettings):
    ...
    service_name: str = Field(
        ..., description="Service name (required)", min_length=1, max_length=50
    )
    ...
    port: int = Field(default=8000, description="Port to bind", ge=1, le=65535)

    # Database
    database_url: str = Field(
        ..., description="PostgreSQL connection URL (required)", min_length=1
    )
    ...
    # Redis
    redis_url: str = Field(
        ..., description="Redis connection URL (required)", min_length=1
    )
    redis_max_connections: int = Field(
        default=50, description="Redis max connections", ge=1, le=500
    )

    # Security
    secret_key: str = Field(
        ..., description="Secret key for JWT/encryption (required)", min_length=32
    )
    ...
    signoz_endpoint: Optional[str] = Field(
        default=None, description="SignOz/OTLP endpoint"
    )
    log_level: str = Field(
        default="INFO",
    ...
    workers: Optional[int] = Field(
        default=None, description="Number of worker processes", ge=1, le=32
    )
    timeout_keep_alive: int = Field(
        default=5, description="Keep-alive timeout in seconds", ge=0, le=300
    )
    graceful_timeout: int = Field(
        default=30, description="Graceful shutdown timeout", ge=0, le=300
    )
    ...
    def validate_connectivity(self) -> ValidationResult:
        ...
        try:
            ...
            if result == 0:
                ...
                errors.append(
                    f"Cannot reach database at {db_url.hostname}:{db_url.port or 5432}"
                )
        ...
        try:
            ...
            if result == 0:
                ...
                errors.append(
                    f"Cannot reach Redis at {redis_url.hostname}:{redis_url.port or 6379}"
                )
...
def validate_service_config(
    ...
    try:
        ...
        # Validate
        result = config.validate_all(level=level, check_connectivity=check_connectivity)
...
if __name__ == "__main__":
    ...
    config = validate_service_config(
        service_name=args.service, check_connectivity=not args.no_connectivity
    )
