...
from dotmac_identity import CustomerService
...
async def update_profile(
    ...
    try:
        updated_customer = await customer_service.update_customer(
            customer_id, profile_data.model_dump(exclude_none=True)
        )

        return {"status": "updated", "customer": updated_customer}

    except Exception as e:
        raise HTTPException(500, f"Failed to update profile: {e!s}") from e
...
async def billing_invoices(
    ...
    return templates.TemplateResponse(
        "billing.html", {"request": request, "invoices": invoices, "customer_id": customer_id}
    )
...
async def get_usage_details(
    ...
    usage_data = await usage_service.get_customer_usage(customer_id, service_id, start_date, end_date)
...
async def make_payment(
    ...
    try:
        ...
        raise HTTPException(500, f"Payment failed: {e!s}") from e
...
async def setup_autopay(
    ...
    try:
        autopay_result = await payment_service.setup_autopay(customer_id, payment_method_id)

        return {"status": "enabled", "autopay_id": autopay_result["autopay_id"]}

    except Exception as e:
        raise HTTPException(500, f"Autopay setup failed: {e!s}") from e
...
async def customer_services(
    ...
    return templates.TemplateResponse(
        "services.html", {"request": request, "services": services, "available_upgrades": available_upgrades}
    )
...
async def request_service_upgrade(
    ...
    try:
        ...
        eligibility = await services_portal.check_upgrade_eligibility(
            customer_id, upgrade_request.service_id, upgrade_request.new_plan
        )
        ...
        raise HTTPException(500, f"Upgrade request failed: {e!s}") from e
...
async def request_service_downgrade(
    ...
    try:
        ...
        raise HTTPException(500, f"Downgrade request failed: {e!s}") from e
...
async def suspend_service(
    ...
    try:
        ...
        raise HTTPException(500, f"Service suspension failed: {e!s}") from e
...
async def get_service_usage(
    ...
    usage_data = await usage_service.get_service_usage(service_id, start_date, end_date)

    return {"service_id": service_id, "usage": usage_data, "period_days": days}
...
async def support_tickets(
    ...
    return templates.TemplateResponse(
        "support.html", {"request": request, "tickets": tickets, "customer_id": customer_id}
    )
...
async def create_support_ticket(
    ...
    try:
        ticket = await support_svc.create_ticket(
            {**ticket_data.model_dump(), "customer_id": customer_id, "source": "customer_portal"}
        )

        return {"ticket_id": ticket.id, "ticket_number": ticket.number, "status": "open"}

    except Exception as e:
        raise HTTPException(500, f"Failed to create ticket: {e!s}") from e
...
async def get_ticket_details(
    ...
    return {"ticket": ticket, "messages": await support_svc.get_ticket_messages(ticket_id)}
...
async def reply_to_ticket(
    ...
    try:
        ...
        reply = await support_svc.add_ticket_message(
            {"ticket_id": ticket_id, "message": message, "sender_type": "customer", "sender_id": customer_id}
        )

        return {"message_id": reply.id, "status": "sent"}

    except Exception as e:
        raise HTTPException(500, f"Failed to send reply: {e!s}") from e
...
async def run_speed_test(
    ...
    try:
        ...
        return {"test_id": speedtest_result["test_id"], "status": "running", "estimated_duration": "60 seconds"}

    except Exception as e:
        raise HTTPException(500, f"Speed test failed: {e!s}") from e
...
async def get_known_outages(customer_id: str = Depends(get_current_customer)):
    ...
    return {"outages": outages, "affected_services": await outage_service.get_affected_services(customer_id)}
...
async def run_diagnostics(
    ...
    try:
        ...
        diagnostic_result = await diagnostics_service.run_diagnostics(
            {"service_id": service_id, "issue_type": issue_type, "initiated_by": "customer"}
        )
        ...
        raise HTTPException(500, f"Diagnostics failed: {e!s}") from e
...
async def get_notifications(
    ...
    return {"notifications": notifications, "unread_count": await customer_svc.get_unread_count(customer_id)}
...
async def update_notification_preferences(
    ...
    try:
        updated_prefs = await customer_svc.update_notification_preferences(customer_id, preferences.model_dump())

        return {"status": "updated", "preferences": updated_prefs}

    except Exception as e:
        raise HTTPException(500, f"Failed to update preferences: {e!s}") from e
