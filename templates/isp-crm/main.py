...
async def create_lead(lead_data: LeadCreate):
    ...
    try:
        # Create lead in CRM
        lead = await lead_svc.create_lead(lead_data.model_dump())
        ...
        await analytics_client.track_event(
            "lead_created", {"lead_id": lead.id, "source": lead_data.source, "interest_level": lead_data.interest_level}
        )

        # Emit event for other services
        await event_bus.emit(
            "crm.lead.created",
            {"lead_id": lead.id, "email": lead_data.email, "phone": lead_data.phone, "source": lead_data.source},
        )

        return {"lead_id": lead.id, "status": "created"}

    except Exception as e:
        raise HTTPException(500, f"Failed to create lead: {e!s}") from e


@app.get("/leads")
async def list_leads(status: Optional[str] = None, source: Optional[str] = None, limit: int = 50, offset: int = 0):
...
async def qualify_lead(lead_id: str, qualification_notes: Optional[str] = None):
    ...
    try:
        ...
        # Track conversion
        await analytics_client.track_event("lead_qualified", {"lead_id": lead_id, "opportunity_id": opportunity.id})

        return {"opportunity_id": opportunity.id, "stage": "qualified"}

    except Exception as e:
        raise HTTPException(500, f"Failed to qualify lead: {e!s}") from e
...
async def convert_opportunity(lead_id: str, customer_data: dict[str, Any]):
    ...
    try:
        ...
        await analytics_client.track_event(
            ...
            {"opportunity_id": opportunity.id, "customer_id": customer.id, "value": opportunity.value},
        )

        return {"customer_id": customer.id, "status": "converted"}

    except Exception as e:
        raise HTTPException(500, f"Failed to convert opportunity: {e!s}") from e
...
async def move_opportunity_stage(opportunity_id: str, update_data: OpportunityUpdate):
    ...
    try:
        # Update opportunity
        opportunity = await opportunity_svc.update_stage(opportunity_id, update_data.model_dump())
        ...
        if update_data.stage == "proposal":
            ...
            quote = await quote_service.create_quote(
                {"customer_id": opportunity.customer_id, "opportunity_id": opportunity_id, "value": update_data.value}
            )
        ...
        raise HTTPException(500, f"Failed to update opportunity: {e!s}") from e
...
async def generate_quote(opportunity_id: str, quote_data: dict[str, Any]):
    ...
    try:
        ...
        quote = await quote_service.create_quote(
            {**quote_data, "opportunity_id": opportunity_id, "customer_id": opportunity.customer_id}
        )
        ...
        await communications_client.send_notification(
            {
                ...
                "data": {"quote_id": quote.id, "amount": quote.total_amount, "quote_url": f"/quotes/{quote.id}"},
            }
        )
        ...
        raise HTTPException(500, f"Failed to generate quote: {e!s}") from e
...
async def create_campaign(campaign_data: CampaignCreate):
    ...
    try:
        # Create campaign
        campaign = await campaign_svc.create_campaign(campaign_data.model_dump())
        ...
        await analytics_client.track_event(
            "campaign_launched",
            {"campaign_id": campaign.id, "type": campaign_data.type, "audience_size": len(audience)},
        )

        return {"campaign_id": campaign.id, "audience_size": len(audience)}

    except Exception as e:
        raise HTTPException(500, f"Failed to create campaign: {e!s}") from e
...
# Unified Inbox
@app.get("/inbox")
async def get_inbox(channel: Optional[str] = None, status: str = "unread", limit: int = 50):
    """Get unified inbox messages"""
    messages = await communications_client.get_inbox_messages({"channel": channel, "status": status, "limit": limit})
...
async def reply_to_message(message_id: str, reply_text: str):
    ...
    try:
        ...
        response = await communications_client.send_reply(
            {"message_id": message_id, "channel": message["channel"], "to": message["from"], "text": reply_text}
        )
        ...
        raise HTTPException(500, f"Failed to reply: {e!s}") from e


@app.post("/customers/{customer_id}/message")
async def send_proactive_message(customer_id: str, channel: str, message: str, template: Optional[str] = None):
    ...
    try:
        ...
        await analytics_client.track_event(
            "proactive_message_sent", {"customer_id": customer_id, "channel": channel, "template": template}
        )

        return {"status": "sent", "message_id": response["id"]}

    except Exception as e:
        raise HTTPException(500, f"Failed to send message: {e!s}") from e
