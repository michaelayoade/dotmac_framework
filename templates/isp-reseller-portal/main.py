...
async def create_reseller(reseller_data: ResellerCreate):
    ...
    try:
        ...
        # Track analytics
        await analytics_client.track_event("reseller_onboarded", {
            "reseller_id": reseller.id,
            "tier": reseller_data.tier,
            "white_label": reseller_data.white_label_enabled
        })

        return {
            "reseller_id": reseller.id,
            "portal_url": f"/portal/{reseller.id}",
            "status": "created"
        }

    except Exception as e:
        raise HTTPException(500, f"Failed to create reseller: {e!s}")
...
async def update_white_label_config(
    ...
    try:
        updated_config = await white_label.update_branding(reseller_id, config.dict())

        # Track branding update
        await analytics_client.track_event("white_label_updated", {
            "reseller_id": reseller_id,
            "has_custom_domain": bool(config.domain)
        })

        return {"status": "updated", "config": updated_config}

    except Exception as e:
        raise HTTPException(500, f"Failed to update branding: {e!s}")
...
async def create_customer_for_reseller(
    ...
    try:
        ...
        await commission_service.setup_customer_commission(
            customer.id,
            reseller_id
        )
        ...
        if customer_data.service_plan:
            provision_result = await provisioning_svc.provision_service({
                ...
                "reseller_id": reseller_id,
                "service_plan": customer_data.service_plan
            })

        # Track customer creation
        await analytics_client.track_event("reseller_customer_created", {
            "reseller_id": reseller_id,
            "customer_id": customer.id,
            "service_plan": customer_data.service_plan
        })
        ...
        raise HTTPException(500, f"Failed to create customer: {e!s}")
...
async def provision_service_for_customer(
    ...
    try:
        ...
        # Track provisioning
        await analytics_client.track_event("reseller_service_provisioned", {
            "reseller_id": reseller_id,
        ...
        raise HTTPException(500, f"Failed to provision service: {e!s}")
...
async def get_customer_billing_for_reseller(
    ...
    billing_data = await commission_service.get_customer_billing_summary(
        customer_id,
        reseller_id
    )
...
async def get_commission_history(
    ...
    commissions = await commission_service.get_reseller_commissions(
        reseller_id,
        start_date,
        end_date
    )
...
async def get_performance_report(
    ...
    return {
        "period": period,
        "metrics": metrics,
        "benchmarks": await analytics_client.get_tier_benchmarks(reseller_id)
    }
...
async def submit_lead_for_commission(
    ...
    try:
        ...
        await analytics_client.track_event("reseller_lead_submitted", {
            "reseller_id": reseller_id,
            "lead_id": lead.id,
        ...
        raise HTTPException(500, f"Failed to submit lead: {e!s}")
...
async def self_service_provisioning(
    ...
    try:
        ...
        can_provision = await reseller_svc.check_provisioning_rights(
            reseller_id,
            provision_request.service_type
        )
