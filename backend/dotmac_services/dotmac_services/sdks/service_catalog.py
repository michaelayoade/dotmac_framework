...
class ServiceCatalogService:
    ...
    def _validate_service_definition(self, definition: Dict[str, Any]):
        ...
        if definition["service_type"] not in valid_types:
            raise ServiceDefinitionError(f"Invalid service type. Must be one of: {valid_types}")
    ...
    async def create_service_plan(self, **kwargs) -> Dict[str, Any]:
        ...
        plan = {
            ...
            "name": kwargs["name"],
            "description": kwargs.get("description", ""),
            "plan_type": kwargs.get("plan_type", "standard"),  # standard, premium, enterprise
            "billing_cycle": kwargs.get("billing_cycle", "monthly"),  # monthly, yearly, one-time
    ...
    def _validate_service_plan(self, plan: Dict[str, Any]):
        ...
        if plan["definition_id"] not in self._service_definitions:
            raise ServicePlanError(f"Service definition not found: {plan['definition_id']}")
        ...
        if plan["billing_cycle"] not in valid_cycles:
            raise ServicePlanError(f"Invalid billing cycle. Must be one of: {valid_cycles}")
    ...
    async def create_bundle(self, **kwargs) -> Dict[str, Any]:
        ...
        bundle = {
            ...
            "name": kwargs["name"],
            "description": kwargs.get("description", ""),
            "bundle_type": kwargs.get("bundle_type", "fixed"),  # fixed, flexible, custom
            ...
            "discount_type": kwargs.get("discount_type", "percentage"),  # percentage, fixed
    ...
    def _validate_bundle(self, bundle: Dict[str, Any]):
        ...
        if bundle.get("maximum_services") and bundle["maximum_services"] < bundle["minimum_services"]:
    ...
    async def create_addon(self, **kwargs) -> Dict[str, Any]:
        ...
        addon = {
            ...
            "name": kwargs["name"],
            "description": kwargs.get("description", ""),
            "addon_type": kwargs.get("addon_type", "feature"),  # feature, capacity, support
    ...
    async def calculate_bundle_price(self, bundle_id: str, selected_services: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "discount_percentage": float((discount_amount / individual_total) * 100) if individual_total > 0 else 0,
        }
...
class ServiceCatalogSDK:
    ...
    async def get_service_catalog(
        ...
        # Filter by criteria
        if category_id:
            definitions = [d for d in definitions if d.get("category_id") == category_id]
        ...
        for definition in definitions:
            plans = [
                plan for plan in self._service._service_plans.values()
                if plan["definition_id"] == definition["definition_id"] and plan["status"] == "active"
            ]
    ...
    async def calculate_bundle_pricing(
        ...
        """Calculate bundle pricing."""
        pricing = await self._service.calculate_bundle_price(bundle_id, selected_services)
    ...
    async def get_compatible_addons(
        ...
        for addon in self._service._addons.values():
            ...
            if (definition_id in addon["compatible_services"] or
                service_plan_id in addon["compatible_plans"]):
        ...
        if addons:
            for addon_id in addons:
                if addon_id not in self._service._addons:
                    ...
                    if not compatible:
                        validation_result["valid"] = False
                        validation_result["errors"].append(f"Add-on {addon_id} not compatible with selected services")
