...
class ResellerPortalSDK:
    ...
    async def grant_reseller_access(
        ...
        **kwargs
        ...
        access = await self._service.create_access(
            ...
            reseller_contact_id=UUID(reseller_contact_id) if reseller_contact_id else None,
            reseller_account_id=UUID(reseller_account_id) if reseller_account_id else None,
        ...
        return {
            ...
            "reseller_contact_id": str(access.reseller_contact_id) if access.reseller_contact_id else None,
    ...
    async def get_reseller_access(self, access_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "reseller_organization_id": str(access.reseller_organization_id),
            "reseller_contact_id": str(access.reseller_contact_id) if access.reseller_contact_id else None,
            "reseller_account_id": str(access.reseller_account_id) if access.reseller_account_id else None,
            "status": access.status.value,
            "access_level": access.access_level.value,
            "permissions": access.permissions,
            "accessible_customers": [str(cid) for cid in access.accessible_customers],
            "accessible_organizations": [str(oid) for oid in access.accessible_organizations],
            ...
            "last_access_at": access.last_access_at.isoformat() if access.last_access_at else None,
            "created_at": access.created_at.isoformat(),
    ...
    async def set_organization_access_scope(
        self,
        access_id: str,
        accessible_organizations: List[str]
    ) -> Dict[str, Any]:
