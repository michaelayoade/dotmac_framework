...
class ConsentPreferencesService:
    ...
    async def update_preference(self, preference_id: UUID, **updates) -> ConsentPreference:
    ...
    async def _create_audit(self, preference: ConsentPreference, old_status: ConsentStatus, new_status: ConsentStatus) -> ConsentAudit:
...
class ConsentPreferencesSDK:
    ...
    async def grant_consent(
        ...
        **kwargs
        ...
        preference = await self._service.create_preference(
            ...
            compliance_regions=[ComplianceRegion(r) for r in (compliance_regions or [])],
            granted_at=preference.granted_at.__class__.utcnow() if hasattr(preference, "granted_at") else None,
            **kwargs
        )
        ...
        return {
            ...
            "granted_at": preference.granted_at.isoformat() if preference.granted_at else None,
    ...
    async def withdraw_consent(self, preference_id: str) -> Dict[str, Any]:
        ...
        preference = await self._service.update_preference(
            ...
            withdrawn_at=preference.withdrawn_at.__class__.utcnow() if hasattr(preference, "withdrawn_at") else None
        )
        ...
        return {
            ...
            "consent_type": preference.consent_type.value,
            "status": preference.status.value,
            "withdrawn_at": preference.withdrawn_at.isoformat() if preference.withdrawn_at else None,
            "updated_at": preference.updated_at.isoformat(),
        }

    async def get_consent_preference(self, preference_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def set_email_preferences(
        ...
        marketing_pref = await self.grant_consent(
            consent_type="marketing_email",
            contact_id=contact_id
        ) if marketing_email else await self._deny_consent("marketing_email", contact_id=contact_id)
        preferences.append(marketing_pref)

        # Transactional email preference
        transactional_pref = await self.grant_consent(
            consent_type="transactional_email",
            contact_id=contact_id
        ) if transactional_email else await self._deny_consent("transactional_email", contact_id=contact_id)
    ...
    async def set_sms_preferences(
        ...
        marketing_pref = await self.grant_consent(
            ...
            contact_id=contact_id
        ) if marketing_sms else await self._deny_consent("marketing_sms", contact_id=contact_id)
        preferences.append(marketing_pref)
        ...
        transactional_pref = await self.grant_consent(
            ...
            contact_id=contact_id
        ) if transactional_sms else await self._deny_consent("transactional_sms", contact_id=contact_id)
        preferences.append(transactional_pref)
    ...
    async def set_gdpr_compliance(
        ...
        data_pref = await self.grant_consent(
            consent_type="data_processing",
            contact_id=contact_id,
            compliance_regions=["gdpr"]
        ) if data_processing else await self._deny_consent("data_processing", contact_id=contact_id, compliance_regions=["gdpr"])
        preferences.append(data_pref)

        # Third party sharing consent
        sharing_pref = await self.grant_consent(
            consent_type="third_party_sharing",
            contact_id=contact_id,
            compliance_regions=["gdpr"]
        ) if third_party_sharing else await self._deny_consent("third_party_sharing", contact_id=contact_id, compliance_regions=["gdpr"])
    ...
    async def _deny_consent(self, consent_type: str, **kwargs) -> Dict[str, Any]:
        ...
        preference = await self._service.create_preference(
            ...
            compliance_regions=[ComplianceRegion(r) for r in kwargs.get("compliance_regions", [])],
            **{k: UUID(v) if k.endswith("_id") and v else v for k, v in kwargs.items() if k != "compliance_regions"}
        )
