...
class CustomerPortalService:
    ...
    async def get_binding_by_username(self, portal_username: str) -> Optional[CustomerPortalBinding]:
    ...
    async def update_binding(self, binding_id: UUID, **updates) -> CustomerPortalBinding:
        """Update binding."""
    ...
    async def list_bindings_by_portal(self, portal_id: UUID) -> List[CustomerPortalBinding]:
...
class CustomerPortalSDK:
    ...
    async def get_customer_portal_binding(self, binding_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "last_login_at": binding.last_login_at.isoformat() if binding.last_login_at else None,
            "created_at": binding.created_at.isoformat(),
            "updated_at": binding.updated_at.isoformat(),
        }

    async def get_binding_by_username(self, portal_username: str) -> Optional[Dict[str, Any]]:
        """Get binding by portal username."""
    ...
    async def publish_credentials_for_networking(
        ...
        binding = await self._service.update_binding(
            UUID(binding_id),
            published_credentials=credentials
        )
