...
class AutomationAction(BaseModel):
    ...
    input_mapping: Dict[str, str] = Field(default_factory=dict, description="Input mapping")
    output_mapping: Dict[str, str] = Field(default_factory=dict, description="Output mapping")
    ...
    condition: Optional[AutomationCondition] = Field(None, description="Execution condition")
...
class AutomationRule(BaseModel):
    ...
    conditions: List[AutomationCondition] = Field(default_factory=list, description="Rule conditions")
    ...
    # Execution limits
    max_executions_per_hour: Optional[int] = Field(None, description="Max executions per hour")
    max_executions_per_day: Optional[int] = Field(None, description="Max executions per day")
...
class ConditionEvaluator:
    ...
    def evaluate_condition(self, condition: AutomationCondition, data: Dict[str, Any]) -> bool:  # noqa: C901
    ...
    def evaluate_conditions(self, conditions: List[AutomationCondition], data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if timeout:
                result = await asyncio.wait_for(
                    handler(action, input_data, context),
                    timeout=timeout
                )
            else:
...
class AutomationEngine:
    ...
    async def process_trigger(
        ...
        try:
            ...
            if rule.conditions:
                if not self.condition_evaluator.evaluate_conditions(rule.conditions, trigger_data):
            ...
            for action in rule.actions:
                ...
                if action_result.get("status") == "completed" and "result" in action_result:
...
class AutomationSDK:
    ...
    def __init__(self, tenant_id: str, storage_adapter=None):
        ...
        self.execution_counters: Dict[str, Dict[str, int]] = {}  # rule_id -> {hour: count, day: count}
    ...
    async def create_rule(self, rule: AutomationRule) -> str:
        ...
        if rule.trigger.trigger_type == TriggerType.EVENT and rule.trigger.event_pattern:
    ...
    async def update_rule(self, rule: AutomationRule) -> bool:
        ...
        if (old_rule.trigger.trigger_type == TriggerType.EVENT and
            old_rule.trigger.event_pattern != rule.trigger.event_pattern):

            if old_rule.trigger.event_pattern:
                self.engine.unregister_event_listener(old_rule.trigger.event_pattern, rule.id)
