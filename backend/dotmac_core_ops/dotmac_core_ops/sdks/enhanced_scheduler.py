...
class ScheduleDriftMetrics:
    ...
    def get_p95_drift(self) -> float:
        ...
        return sorted_measurements[p95_index] if p95_index < len(sorted_measurements) else sorted_measurements[-1]
...
class EnhancedScheduleDefinition(BaseModel):
    ...
    rrule_expression: Optional[str] = Field(None, description="RRULE expression for complex recurrence")
    cron_expression: Optional[str] = Field(None, description="Cron expression (legacy support)")
    interval_seconds: Optional[int] = Field(None, ge=1, description="Simple interval in seconds")
    ...
    jitter_seconds: Optional[int] = Field(None, ge=0, description="Maximum jitter in seconds")
    catch_up_missed: bool = Field(True, description="Catch up missed executions on startup")
    max_catch_up_executions: int = Field(5, ge=0, description="Maximum missed executions to catch up")
...
class ScheduleExecution:
    ...
    def calculate_drift(self):
        ...
        if self.actual_start_time:
            self.drift_seconds = abs((self.actual_start_time - self.scheduled_time).total_seconds())
...
class EnhancedCronScheduler:
    ...
    def add_schedule(self, schedule: EnhancedScheduleDefinition):
        ...
        logger.info("Schedule added", schedule_id=schedule.id, timezone=schedule.timezone)
    ...
    def get_due_schedules(self, current_time: datetime) -> List[EnhancedScheduleDefinition]:
    ...
    def get_catch_up_executions(
        ...
        try:
            if schedule.rrule_expression:
                ...
                while next_time < current_time and len(catch_up_times) < schedule.max_catch_up_executions:
                    catch_up_times.append(next_time)
                    next_time += timedelta(seconds=schedule.interval_seconds)

        except Exception as e:
            logger.error("Error calculating catch-up executions", schedule_id=schedule.id, error=str(e))
...
class EnhancedSchedulerSDK:
    ...
    async def start_scheduler(self):
        ...
        try:
            ...
            logger.error("Enhanced scheduler error", error=str(e), tenant_id=self.tenant_id)
        finally:
            self.running = False
    ...
    async def _perform_startup_catchup(self):
        ...
        for schedule in self.schedules.values():
            ...
            last_run = None
            if self.storage_adapter:
                last_run = await self.storage_adapter.get_last_execution_time(schedule.id)
            ...
            try:
                ...
                if timeout:
                    result = await asyncio.wait_for(
                        handler(input_data, execution.context),
                        timeout=timeout
                    )
                else:
    ...
    def _should_retry(self, schedule: EnhancedScheduleDefinition, execution: ScheduleExecution) -> bool:
