...
class SagaStep(BaseModel):
    ...
    compensation_handler: Optional[str] = Field(None, description="Compensation handler")
    ...
    compensation_timeout: Optional[float] = Field(None, description="Compensation timeout")
...
class SagaEngine:
    ...
    async def execute_transaction(  # noqa: C901
        ...
        try:
            ...
            if step.step_type == SagaStepType.TRANSACTION:
                ...
                if not handler:
                    raise ValueError(f"Transaction handler {step.transaction_handler} not registered")
    ...
    async def execute_compensation(
        ...
        try:
            ...
            if not handler:
                raise ValueError(f"Compensation handler {step.compensation_handler} not registered")
            ...
            result = await asyncio.wait_for(
                handler(compensation_data, context),
                timeout=timeout
            )
    ...
    def _prepare_input_data(self, step: SagaStep, context: ExecutionContext) -> Dict[str, Any]:
...
class SagaSDK:
        ...
        try:
            ...
            while remaining_steps and saga_execution.status == SagaStatus.RUNNING:
                ...
                for step in remaining_steps:
                    if all(dep in saga_execution.completed_steps for dep in step.depends_on):
            ...
            if saga_execution.status == SagaStatus.RUNNING:
                ...
                await self._execute_compensation(saga_def, saga_execution, executed_steps)
    ...
    async def _execute_step_with_retry(
        ...
        for attempt in range(max_retries + 1):
            ...
            success = await self.engine.execute_transaction(step, step_execution, context)
        ...
        if saga_def.compensation_strategy == CompensationStrategy.REVERSE_ORDER:
            ...
            if compensation_tasks:
                results = await asyncio.gather(
                    *[task for _, task in compensation_tasks],
