...
class StateTransition(BaseModel):
    ...
    transition_type: TransitionType = Field(TransitionType.EVENT_DRIVEN, description="Transition type")
    timeout_seconds: Optional[float] = Field(None, description="Timeout for automatic transitions")
...
class StateDefinition(BaseModel):
    ...
    substates: List["StateDefinition"] = Field(default_factory=list, description="Sub-states")
    initial_substate: Optional[str] = Field(None, description="Initial sub-state")

    # State data
    state_data: Dict[str, Any] = Field(default_factory=dict, description="State-specific data")

    # Timeout configuration
    timeout_seconds: Optional[float] = Field(None, description="State timeout")
    timeout_transition: Optional[str] = Field(None, description="Timeout transition target")
...
class StateMachineDefinition(BaseModel):
    ...
    # Configuration
    context_schema: Dict[str, Any] = Field(default_factory=dict, description="Context schema")
    ...
    def validate_states(cls, v, values):
        ...
        final_states = values.get('final_states', [])
    ...
    def validate_transitions(cls, v, values):
        ...
        for transition in v:
            if transition.from_state not in state_ids:
                raise ValueError(f"Transition from unknown state: {transition.from_state}")
...
class StateMachineEngine:
    ...
    async def evaluate_condition(
        ...
        try:
            ...
            from dotmac_platform.security.ast_evaluator import SafeEvaluator
    ...
    async def _execute_transition(
        ...
        try:
            ...
            if transition.action:
                await self.engine.execute_action(transition.action, execution, event_data)
...
class StateMachineSDK:
    ...
    async def get_state_machine_status(self, state_machine_id: str) -> Dict[str, Any]:
        ...
        executions = [e for e in self.executions.values() if e.state_machine_id == state_machine_id]
        status_counts = {}
