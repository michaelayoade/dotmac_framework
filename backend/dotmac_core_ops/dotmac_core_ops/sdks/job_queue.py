...
class JobQueue:
    ...
    def __init__(self, name: str, queue_type: QueueType = QueueType.PRIORITY, max_size: int = 10000):
        self.name = name
    ...
    async def enqueue(self, job_execution: JobExecution) -> bool:
        ...
        async with self._lock:
            ...
            if job_execution.scheduled_at and job_execution.scheduled_at > datetime.now(timezone.utc):
...
class JobQueueSDK:
    ...
    async def create_job_definition(self, job_def: JobDefinition) -> str:
        ...
        if job_def.queue_name not in self.queues:
            self.queues[job_def.queue_name] = JobQueue(job_def.queue_name, QueueType.PRIORITY)
    ...
    async def _worker_loop(self, worker: JobWorker):
        ...
        while self.running:
            try:
                ...
                if not job_def:
                    await self._handle_job_failure(job_execution, "Job definition not found")
                    continue
        ...
        for attempt in range(max_retries + 1):
            ...
            if success:
                ...
                if attempt < max_retries and self._should_retry(job_def, job_execution):
                    ...
                    await self._handle_job_failure(job_execution, "Max retries exceeded")
                    break
    ...
    def _is_job_expired(self, job_def: JobDefinition, job_execution: JobExecution) -> bool:
    ...
    def _should_retry(self, job_def: JobDefinition, job_execution: JobExecution) -> bool:
