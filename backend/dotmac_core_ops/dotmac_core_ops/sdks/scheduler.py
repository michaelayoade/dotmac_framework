...
class ScheduleDefinition(BaseModel):
    ...
    def validate_cron_expression(cls, v, values):
        if v and values.get('schedule_type') == ScheduleType.CRON:
            try:
                croniter(v)
    ...
    def _calculate_next_run(self, schedule: ScheduleDefinition):
        ...
        if schedule.schedule_type == ScheduleType.CRON:
            ...
            if schedule.start_time and schedule.start_time > now:
                self.next_runs[schedule.id] = schedule.start_time
            else:
                # One-time job in the past or no start time, remove it
                self.next_runs.pop(schedule.id, None)

        elif schedule.schedule_type == ScheduleType.RECURRING:
            if schedule.start_time and schedule.interval_seconds:
...
class JobQueue:
    ...
    async def enqueue(self, job_execution: JobExecution):
        ...
        for i, existing_job in enumerate(self.pending_jobs):
            if (self._compare_priority(job_execution, existing_job) > 0 or
                (self._compare_priority(job_execution, existing_job) == 0 and
                 job_execution.scheduled_time < existing_job.scheduled_time)):
    ...
    def _compare_priority(self, job1: JobExecution, job2: JobExecution) -> int:
        ...
        p1 = getattr(job1.context, 'priority', Priority.NORMAL) if job1.context else Priority.NORMAL
        p2 = getattr(job2.context, 'priority', Priority.NORMAL) if job2.context else Priority.NORMAL
...
class JobScheduler:
        ...
        try:
            ...
            if timeout:
                result = await asyncio.wait_for(
                    handler(input_data, job_execution.context),
                    timeout=timeout
                )
            else:
    ...
    def _can_start_instance(self, schedule: ScheduleDefinition, execution_id: str) -> bool:
...
class SchedulerSDK:
    ...
    async def _job_processor(self):
        ...
        while self.running:
            try:
                ...
                asyncio.create_task(self._execute_job_with_retry(schedule, job_execution))
    ...
    async def _execute_job_with_retry(
        ...
        for attempt in range(max_retries + 1):
            ...
            if success:
                ...
                if attempt < max_retries and self._should_retry(schedule, job_execution):
                    # Calculate delay
    ...
    def _should_retry(self, schedule: ScheduleDefinition, job_execution: JobExecution) -> bool:
    ...
    async def get_schedule_status(self, schedule_id: str) -> Dict[str, Any]:
        ...
        running_instances = len(self.job_scheduler.running_instances.get(schedule_id, set()))
