...
class IPAMService:
    ...
    async def allocate_ip(self, **kwargs) -> Dict[str, Any]:
        ...
        if requested_ip:
            ...
            if ip_addr not in network:
                raise IPAMError(f"IP {requested_ip} not in network {network_data['cidr']}")
        ...
        allocation = {
            ...
            "status": "allocated",
            "created_at": utc_now().isoformat(),
            "expires_at": (utc_now() + timedelta(seconds=kwargs.get("lease_time", 86400))).isoformat(),
        }
    ...
    async def reserve_ip(self, **kwargs) -> Dict[str, Any]:
        ...
        reservation = {
            ...
            "status": "reserved",
            "created_at": utc_now().isoformat(),
            "expires_at": (utc_now() + timedelta(seconds=kwargs.get("reservation_time", 3600))).isoformat(),
        }
...
class IPAMSDK:
    ...
    async def get_network_utilization(self, network_id: str) -> Dict[str, Any]:
        ...
        allocated_ips = sum(1 for alloc in self._service._allocations.values()
                          if alloc["network_id"] == network_id and alloc["status"] == "allocated")
        reserved_ips = sum(1 for res in self._service._reservations.values()
                         if res["network_id"] == network_id and res["status"] == "reserved")

        available_ips = total_ips - allocated_ips - reserved_ips
        utilization_percent = ((allocated_ips + reserved_ips) / total_ips) * 100 if total_ips > 0 else 0
    ...
    async def get_allocations_by_network(self, network_id: str) -> List[Dict[str, Any]]:
        ...
        allocations = [
            alloc for alloc in self._service._allocations.values()
    ...
    async def check_ip_availability(self, network_id: str, ip_address: str) -> Dict[str, Any]:
