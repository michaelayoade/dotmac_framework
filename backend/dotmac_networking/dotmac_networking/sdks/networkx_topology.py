...
class NetworkXTopologyManager:
    ...
    async def add_network_device(self, device_id: str, device_type: str, **attributes) -> Dict[str, Any]:
    ...
    async def add_network_link(self, device1: str, device2: str, **link_attributes) -> Dict[str, Any]:
        ...
        self._log_topology_change("link_added", source=device1, target=device2, **link_data)
    ...
    async def remove_network_device(self, device_id: str) -> Dict[str, Any]:
        ...
        self._log_topology_change("device_removed", device_id=device_id, 
    ...
    async def find_shortest_path(self, source: str, target: str, weight: Optional[str] = None) -> List[str]:
    ...
    async def find_all_simple_paths(self, source: str, target: str, max_length: int = 10) -> List[List[str]]:
    ...
    async def calculate_network_metrics(self) -> Dict[str, Any]:
        ...
        if self.graph.number_of_nodes() <= 100:  # Limit for performance
            metrics["centrality"] = {
                ...
                "eigenvector": dict(nx.eigenvector_centrality(self.graph, max_iter=1000))
            }
        ...
        if degrees:
            metrics["degree_stats"] = {
                ...
                "degree_distribution": {str(degree): degrees.count(degree) for degree in set(degrees)}
            }
    ...
    async def identify_critical_nodes(self) -> List[Dict[str, Any]]:
        ...
        for node_id in self.graph.nodes():
            ...
            if criticality_score > 0.1:  # Only include nodes with significant criticality
    ...
    async def identify_critical_edges(self) -> List[Dict[str, Any]]:
        ...
        for edge in bridges:
            ...
            critical_edges.append({
                ...
                "is_bridge": True,
                "edge_betweenness": self._calculate_edge_betweenness(source, target)
            })
    ...
    async def simulate_node_failure(self, node_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "largest_partition_size": max(len(comp) for comp in components) if components else 0,
    ...
    async def find_redundant_paths(self, source: str, target: str, k: int = 3) -> List[List[str]]:
    ...
    async def analyze_network_resilience(self) -> Dict[str, Any]:
        ...
        if nx.is_connected(self.graph):
            try:
                ...
                resilience_analysis["connectivity_resilience"] = {
                    ...
                    "connectivity_score": min(node_connectivity / 3.0, 1.0) * 0.3 + min(edge_connectivity / 3.0, 1.0) * 0.2
                }
            except:
                resilience_analysis["connectivity_resilience"] = {
                    "node_connectivity": 0,
        ...
        connectivity_score = resilience_analysis["connectivity_resilience"].get("connectivity_score", 0.0)
        critical_ratio = (len(critical_nodes) + len(critical_edges)) / (self.graph.number_of_nodes() + self.graph.number_of_edges())
        ...
        if overall_score < 0.5:
            resilience_analysis["recommendations"].append("Network resilience is poor - consider adding redundant links")
        if len(critical_nodes) > self.graph.number_of_nodes() * 0.2:
            resilience_analysis["recommendations"].append("Too many critical nodes - distribute network functions")
        if not nx.is_connected(self.graph):
            resilience_analysis["recommendations"].append("Network is not fully connected - add bridging links")
    ...
    def _calculate_connectivity_impact(self, temp_graph) -> Dict[str, Any]:
        ...
        original_connected = nx.is_connected(self.graph) if not self.graph.is_directed() else nx.is_weakly_connected(self.graph)
        new_connected = nx.is_connected(temp_graph) if not temp_graph.is_directed() else nx.is_weakly_connected(temp_graph)
        
        impact = {
            "connectivity_lost": original_connected and not new_connected,
            "components_before": 1 if original_connected else nx.number_connected_components(self.graph),
            "components_after": nx.number_connected_components(temp_graph) if not temp_graph.is_directed() else nx.number_weakly_connected_components(temp_graph)
        }
...
class NetworkXTopologySDK:
    ...
    async def add_device(self, device_id: str, device_type: str, **attributes) -> Dict[str, Any]:
        """Add network device to topology"""
        return await self.topology.add_network_device(device_id, device_type, tenant_id=self.tenant_id, **attributes)
    
    async def add_link(self, device1: str, device2: str, **attributes) -> Dict[str, Any]:
        """Add network link"""
