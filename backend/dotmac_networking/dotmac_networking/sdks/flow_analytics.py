...
class FlowAnalyticsService:
    ...
    async def ingest_flow_record(self, **kwargs) -> Dict[str, Any]:
        ...
        if kwargs.get("collector_id") and kwargs.get("collector_id") in self._collectors:
    ...
    async def aggregate_flows(self, **kwargs) -> Dict[str, Any]:
        ...
        start_time = datetime.fromisoformat(kwargs.get("start_time", (utc_now() - timedelta(hours=1)).isoformat()))
        end_time = datetime.fromisoformat(kwargs.get("end_time", utc_now().isoformat()))

        flows_in_window = [
            flow for flow in self._flow_records
    ...
    async def get_top_talkers(self, **kwargs) -> List[Dict[str, Any]]:
        ...
        sorted_talkers = sorted(
            talkers.values(),
            key=lambda t: t[f"total_{metric}"],
            reverse=True
        )
...
class FlowAnalyticsSDK:
    ...
    async def get_traffic_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        recent_flows = [
            flow for flow in self._service._flow_records
            if datetime.fromisoformat(flow["ingested_at"]) >= cutoff_time
        ]
        ...
        unique_destinations = len(set(flow.get("dst_addr", "") for flow in recent_flows))
        ...
        return {
            ...
            "average_flow_size_bytes": total_bytes // total_flows if total_flows > 0 else 0,
        ...
        return [
            {
                ...
                "percentage": (talker[f"total_{metric}"] / sum(t[f"total_{metric}"] for t in top_talkers)) * 100 if top_talkers else 0,
            }
    ...
    async def get_protocol_statistics(self, hours: int = 1) -> List[Dict[str, Any]]:
        ...
        for proto_data in protocols.values():
            proto_data["percentage"] = (proto_data["total_bytes"] / total_bytes * 100) if total_bytes > 0 else 0
