...
class NetworkTopologyService:
    ...
    async def add_link(self, **kwargs) -> Dict[str, Any]:
        ...
        link = {
            ...
            "link_type": kwargs.get("link_type", "physical"),  # physical, logical, virtual
...
class NetworkTopologySDK:
    ...
    async def get_site_topology(self, site_id: str) -> Dict[str, Any]:
        ...
        site_nodes = [
            node for node in self._service._nodes.values()
        ...
        site_links = []
        for link in self._service._links.values():
            if (link["source_node"] in site_node_ids and
                link["target_node"] in site_node_ids):
