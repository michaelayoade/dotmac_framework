...
class TR069CWMPService:
    ...
    async def set_parameter_values(self, device_id: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def get_parameter_values(self, device_id: str, parameter_names: List[str]) -> Dict[str, Any]:
...
class TR069CWMPSDK:
    ...
    async def get_device_tasks(self, device_id: str) -> List[Dict[str, Any]]:
        ...
        tasks = [
            task for task in self._service._tasks.values()
    ...
    async def configure_wan_settings(
        ...
        if connection_type == "Static" and ip_address:
            wan_params.update({
                ...
                "Device.IP.Interface.1.IPv4Address.1.SubnetMask": subnet_mask or "255.255.255.0",
            })

            if gateway:
                wan_params["Device.Routing.Router.1.IPv4Forwarding.1.GatewayIPAddress"] = gateway
