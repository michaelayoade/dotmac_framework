    ...
    async def store_event(
        ...
        try:
            ...
            outbox_event = OutboxEvent(
                ...
                event_metadata=json.dumps(event_metadata or {}) if event_metadata else None,
            ...
            async with self.session_factory() as session:
                ...
                for event in events:
                    ...
                    try:
                        ...
                        metadata_dict = (
                            json.loads(event.event_metadata) if event.event_metadata else {}
                        )

                        # Convert to EventMetadata if available
                        event_metadata = None
                        if hasattr(self.event_bus_sdk, "EventMetadata"):
                            event_metadata = self.event_bus_sdk.EventMetadata.from_dict(metadata_dict)
    ...
    async def get_pending_events(
        ...
        try:
            async with self.session_factory() as session:
                ...
                ).order_by(OutboxEvent.created_at).offset(offset).limit(limit)
    ...
    async def get_dead_letter_events(
        ...
        try:
            async with self.session_factory() as session:
                ...
                ).order_by(OutboxEvent.updated_at.desc()).offset(offset).limit(limit)
