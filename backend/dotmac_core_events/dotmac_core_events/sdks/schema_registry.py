...
class SchemaStore(ABC):
    ...
    @abstractmethod
    async def get_schema(self, event_type: str, version: Optional[str] = None) -> Optional[SchemaVersion]:
    ...
    @abstractmethod
    async def delete_schema(self, event_type: str, version: Optional[str] = None) -> None:
...
class SchemaCache:
    ...
    def get(self, event_type: str, version: Optional[str] = None) -> Optional[SchemaVersion]:
    ...
    def put(self, schema_version: SchemaVersion, version: Optional[str] = None) -> None:
        """Put schema in cache."""
        key = self._make_key(schema_version.event_type, version or schema_version.version)
...
class SchemaRegistrySDK:
    ...
    async def register_schema(
        ...
        try:
            ...
            await self._check_compatibility(tenant_event_type, schema, compatibility_level)
    ...
    async def get_schema(
        ...
        if schema_version:
            ...
            schema_version.event_type = self._remove_tenant_prefix(schema_version.event_type)
        ...
        try:
            ...
            if not schema_version:
                return ValidationResult(
                    valid=False,
                    errors=[{"message": f"Schema not found for event type: {event_type}"}],
                )
